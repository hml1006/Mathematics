...
> **一句话大白话**：Itô 公式是随机世界的"换元法"：光滑函数套上随机过程时，除了一阶项，还要补上 Brown 波动的"二阶振动项" $\frac12 f''...$——随机微积分的基本定理，照此对随机过程做微积计算不出错。
>
> **小例子**：对 $X_t=B_t$：$df(B_t)=f'(B_t)dB_t+\tfrac12 f''(B_t)dt$。取 $f(x)=x^2$ 得 $d(B_t^2)=2B_tdB_t+dt$，即 $\int B\,dB=\tfrac12B^2-\tfrac t2$。
