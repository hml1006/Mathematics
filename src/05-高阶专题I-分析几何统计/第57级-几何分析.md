...
8. 掌握共形几何与保角变化公式，理解 Weyl 张量等共形不变量。
9. 掌握曲率比较定理（Bonnet-Myers、Bishop-Gromov、Toponogov）及其对尺寸与拓扑的约束。
10. 掌握 Bochner 技巧与消失定理、第一特征值的估计（Lichnerowicz-Obata）。
11. 了解 Ricci 流的短时存在性、DeTurck 正规化与规范化版本，以及标量曲率流的性质。
...
> **从……说起**：在平直的欧氏空间里，拉普拉斯算子 $\Delta = -\sum_i \partial_i^2$ 是"测量起伏"的老朋友——物理课上解热传导、波动方程全靠它。可一旦走到弯曲的流形上（曲面、球面、引力场中的时空），"二阶偏导"这个式子就没了坐标意义：切向量方向变了、坐标系扭曲了，$\partial_i\partial_i$ 不再自洽。Laplace-Beltrami 算子正是微分几何对"曲流形上的拉普拉斯"给出的唯一规范答案。它的历史与位势论、热方程相伴相生；到 20 世纪，它成为连接**几何（度量、曲率）**与**分析（谱、特征值）**的第一座桥——"听鼓辨形"（Kac, 1966）那类问题问的就是如何从 Laplace 谱反推流形形状。它解决的核心问题是：**如何在弯曲空间里给"二阶导数 + 求平均"一个坐标无关（coordinate-free）的定义**。没有它，曲率信息无法量化成方程，整门几何分析都无法开张。
>
> 直观上，$\Delta$ 量化的是函数每一点的"凹凸弹性"。先用平直直觉铺路，再把它翻译成带度量系数的局部坐标公式——先体会"为什么要有它"，再接受那一堆 $g^{ij}$ 与 $\sqrt{\det g}$ 的繁琐。
...
> **从……说起**：房间里放一个热点，热量会随时间向四周扩散——"热"本质上是一个在流形上（而非坐标网格上）传播的过程。你想追踪这滴热在任意时刻、任意点的分布，就需要一个既满足扩散律、又携带"初始是个点源"信息的对象，这就是**热核**（heat kernel）。它对纯数学的意义远超物理：Poincaré 与前苏格兰学派用它把微分方程、概率（随机游走、布朗运动）串起来；20 世纪前期开始，热核的渐近展开逐渐成为"从谱读几何"的显微镜。要没有它，$L^2$ 谱、Hodge 分解、Atiyah-Singer 指标定理这些重武器都无从下手。直观上先记住：热核 $p_t(x,y)$ 就是"在 $x$ 滴一滴热，$t$ 秒后在 $y$ 点还剩多少"的实况记录——它的三条基本性质（对称/半群/守恒）恰好编码了**可逆、可分、守恒**三个物理直觉。
...
> **从……说起**：de Rham 上同调是流形拓扑的"代数影子"——可它的代表元（闭形式）长得千奇百怪，同一个上同调类里闭形式多如牛毛，不好挑出"规范代表"。Hodge 理论的出发点朴素而高明：既然流形上有度量能谈"大小"和"正交"，那能不能要求代表元与所有平凡方向（形如 $d\alpha$ 与 $\delta\beta$）都垂直？这就引出了**调和形式**（harmonic form）。它回答了"闭形式里哪个最光滑、最'物理'、最能在同调类里当代表"，把上同调从纯代数变成带几何的分析对象。没有它，Betti 数只活在代数拓扑的课本里；有了它，Betti 数等于调和形式的维数，成为可以直接算的谱量。直觉先行：调和形式就是"既不开（$d\omega=0$）也不缩（$\delta\omega=0$）"的平衡态——处于完全均衡、拧不动的"稳定态"。
...
> **从……说起**：把铁丝弯成一个圈、蘸上肥皂水——揭起来的那层皂膜会自动收缩到"面积最小"的形状。**极小曲面**（minimal surface）正是"面积泛函临界点"的严格化。它的谱系古老而光荣：Lagrange 1780 年提出第一个变分问题，Euler 的悬链面（catenoid）是最早的非平凡解，随后在两百年里成为变分法、几何测度论、以及平均曲率流（MCF）的共同起点。它解决的核心问题是：**在给定边界（或拓扑限制）下，什么样的曲面最省面积**？又因为表面张力恰与面积成正比，"极小"往往自动出现在物理现实里。先直觉再形式：所谓"平均曲率向量 $\vec H=\operatorname{tr}(II)$ 处处为 $0$"，就是把"任何方向都扯不动"翻译成一句微分方程——而这句话正是"肥皂泡为什么存在"的深层几何理由。
...
> **从……说起**（起跳台）：如果说热核熨的是"函数"、曲率流熨的是"曲线"，那么先直观铺一层**现实类比**再进入定义：想象一个表面凹凸不平的橡皮球，Ricci 流让曲率像热量一样从高处（曲率大）流向低处（曲率小），随时间推移球面越来越光滑均匀。下面这个嵌套的**思考历程**会详细展开它蜿蜒的历史（Hamilton 1982 定方程、Perelman 2002–03 证 Poincaré 猜想）；这里先点一句最关键的方向：$\partial_t g=-2\,\operatorname{Ric}(g)$ 让度量朝"看来更圆"的方向变形——凸处收缩、凹处舒展。
>
> **⚠️ 常见坑**：$\partial_t g=-2\operatorname{Ric}$ 少一个负号、少一个因子 $2$ 都会彻底改变几何方向（变号后会让曲率发散而非熨平）；而且 Ricci 流不是梯度流、也不是唯一"熨平"的选择（下文还有规范化版本与标量曲率流）。
...
> **从……说起**：Yamabe（1960）想知道：给一个流形，能不能只靠**共形伸缩**（只改大小、不改角度）就把曲率调成常数？这背后是**共形几何**（§2.9 专门展开）——拓扑往往给不了你"圆得好看"的度量，但也许同一个共形类里就藏着一个"均匀"的代表。Yamabe 提出猜想后几经波折（Trudinger、Aubin、Schoen，1984 年才闭环），它解决的核心问题是**度量如何在共形类里被"均匀化"**，对正质量定理、球面与 Ricci 流都有深远回响。直觉：像不改变衣服版型、只局部放缩尺寸，直到把整件衣服的"皱褶能量"（数量曲率）处处抹平。
...
> **从……说起**：解几何里的 PDE（Yamabe 方程、热方程、极小子流形）时，你手上往往只有函数能量的一个估计（梯度平方有限），却必须推出函数"大小、峰值"被控制。流形上的 **Sobolev 不等式** 就是把"梯度能量"翻译成"范数/身高"的词典；**等周不等式**（isoperimetric）则是它最几何的化身——周长固定、圆围面积最大。二者共同回答：**在弯曲空间里，控制导数到底能换回多少关于函数本身的控制**。它们是变分法、正则性理论与谱估计的共同地基。先直觉：绳子圈的周长受限，围出的面积才可能被控住。
...
> **从……说起**：20 世纪后期，人们想同时驾驭"曲率"（几何）与"热扩散、马尔可夫过程"（概率与分析）。Bakry 与 Émery（1985）做出关键观察：与其只谈几何上的 $\operatorname{Ric}\ge K$，不如把它改写成一整条对一切光滑函数都成立的 Bochner 型不等式，于是有了 $\operatorname{BE}(K,N)$ 条件。它把"曲率有下界"变成可计算的解析条件，还能照顾"带权重维数 $N$"，是现代几何分析、最优传输、测度度量空间（CD 条件）的统一语言。直觉：把"引力够强"翻译成"任何函数在顶端的信息都受该引力控制"；当 $N=\infty$ 时它就退回熟悉的 $\operatorname{Ric}\ge Kg$。
...
> **从……说起**：很多几何问题问的是"形状有没有本质不同"，而不是"测量数值是多少"。共形几何研究**只保角、不保长**的变换：像放大镜、地图投影（Mercator）那样，局部角度不变、距离与面积却可随意缩放。它源于复分析与复结构——"能否把一块区域共形地拉正成标准形状"（Riemann 映照定理）——随后被 Yamabe 问题、正质量定理带入现代几何分析。要回答的核心问题是：**哪些量在"纯角度"意义下是真不变量（不随共形伸缩改变）**？答案指向曲率的某种组合（Weyl 张量）以及"共形类"本身。本节先给出共形变换的代数语言与曲率在这些变换下的变化公式——它们正是 §2.6 Yamabe 问题的地基。
>
> 直觉：把一块弹性布局部均匀拉大，角度不变，但表面的"皱褶能量"（曲率）会按规律重新分布；我们关心的是哪些组合逃过了这种重分布。

**定义 2.13**（共形变换与共形类）设 $(M^n,g)$ 是 $n$ 维 Riemann 流形，$u\in C^\infty(M)$ 为正值函数，称新度量
$$
\tilde g=u^{\frac{4}{n-2}}g\quad(n\ge3),\qquad \tilde g=e^{2u}g\quad(n=2)
$$
为 $g$ 的一个**共形变换**（conformal change）。所有与 $g$ 只差一个正值光滑因子的度量之集 $[g]=\{\tilde g=u^{\frac4{n-2}}g:u>0\}$ 称为 $g$ 的**共形类**（conformal class）。共形变换保持任意两点处的夹角不变（仅按点缩放体积元），即满足 $\tilde g=\lambda\,g$（$\lambda>0$）。

指数 $4/(n-2)$（二维取 $e^{2u}$）并非随意：它保证下面的数量曲率变换公式中出现"共形 Laplace 算子"这一在共形类下同协变（conformally covariant）的量——这正是 Yamabe 问题的理由。

**定理 2.14**（数量曲率的保角变化公式）设 $n\ge3$，$\tilde g=u^{\frac4{n-2}}g$，则数量曲率满足
$$
R_{\tilde g}=u^{-\frac{n+2}{n-2}}\Big(-\frac{4(n-1)}{n-2}\,\Delta u+R\,u\Big),
$$
其中 $\Delta=-\operatorname{div}\nabla$ 是 $g$ 的 Laplace-Beltrami 算子。等价地，**共形 Laplace 算子** $L=\frac{4(n-1)}{n-2}\Delta-R$ 在共形类下同协变：
$$
L_{\tilde g}\,\varphi=u^{-\frac{n+2}{n-2}}L\big(u^{-1}\varphi\big),
$$
且 $R_{\tilde g}\equiv\lambda$ 为常数时就得到 $(2.6)$ 的 Yamabe 方程。

> **💡 直观理解（类比）**：共形变换像"只换衣服尺寸、不改版型"。Weyl 张量是版型里那些"怎么缩放都不变的褶皱特征"，数量曲率则是会随尺寸重新分配的"总量"；共形 Laplace 算子扮演了"缩放下自动换算"的角色，因而 Yamabe 问题的解 $u$ 本质是在共形类里找最均匀的"版型"。
>
> **⚠️ 常见坑**：共形类里的度量形状差异可以很大，但**并非所有量**在共形下都谈得上意义——长度、体积、曲率张量的分量都会变化；"共形不变量"必须选那些在变换下保持的组合（如 $n=2$ 的 $K\,dA$、$n\ge3$ 的 Weyl 张量）。

**定义 2.15**（Weyl 张量）在 $n\ge3$ 的流形上，把 Riemann 曲率张量分解为数量曲率部分、Ricci 部分与完全无迹的 **Weyl 张量**（Weyl tensor）$W$：
$$
R_{ijkl}=W_{ijkl}+\big(SA\big)_{ijkl},
$$
其中 $(SA)_{ijkl}$ 由 Ricci 与数量曲率张成（Schouten 张量部分）。关键性质：在 $n\ge4$ 时 $W$ 在共形变换下不变（$W_{\tilde g}{}^{i}{}_{jkl}=W^{i}{}_{jkl}$），故它是"纯共形的几何量"。$n=3$ 时 $W\equiv0$；$W\equiv0$（$n\ge4$）刻画**局部共形平坦**（可共形拉平为欧氏度量）。

> **💡 直观理解（类比）**：Weyl 张量是"形状的纯角度骨架"。把度量按共形自由度任取，它都不变——很像把一张有弹性的图的"构图比例"与"放大倍率"拆开：倍率归共形因子，比例归 $W$。

### 2.10 曲率与比较定理

> **从……说起**：正曲率让流形"变小变圆"，负曲率让它"膨胀变散"——但要把这句话变成定量纲领，是 20 世纪**比较几何**（comparison geometry）的伟大成就。核心思想（Rauch 比较、Toponogov、Bishop-Gromov）是：**同维流形中，谁的截面曲率（或 Ricci）更大，谁就"挤得更紧"**——测地更短、体积更小、直径更短。Bonnet（1855）已用 Gauss-Bonnet 直觉指出正 Gauss 曲率曲面直径有界；Myers（1941）把测地偏离（Jacobi 场）的方法带到高维正 Ricci 情形；Bishop-Gromov（1963/1981）再用 Jacobi 场控制体积增长。它们共同回答：**曲率下界如何限制流形的整体尺寸**，是所有刚性、谱与几何流结论的公共地基。先给直觉：橡皮球（正曲率）怎么吹都有限大，马鞍面（负曲率）却能无限摊开。

**定理 2.16**（Bonnet-Myers）若紧 Riemann 流形 $(M^n,g)$ 满足 $\operatorname{Ric}\ge(n-1)k>0$，则其直径
$$
\operatorname{diam}(M,g)\le\frac{\pi}{\sqrt k},
$$
且基本群 $\pi_1(M)$ 有限。等号情形对应标准球面（见 §3.6 与 Obata/Cheng 刚性）。

**定理 2.17**（Toponogov 比较）若截面曲率 $\sec\ge K$，则 $M$ 中测地三角形"比常曲率 $K$ 的模型空间更窄"：对应边长、夹角被模型空间控制。它把"曲率下界"翻译成"测地三角形的整体比较"。

**定理 2.18**（Bishop-Gromov 体积比较）若 $\operatorname{Ric}\ge(n-1)K$，则对任意的 $0<r<R$，
$$
\frac{\operatorname{Vol}(B_R)}{\operatorname{Vol}(B_r)}\ \le\ \frac{\operatorname{Vol}_{\mathbb M_K^n}(B_R)}{\operatorname{Vol}_{\mathbb M_K^n}(B_r)},
$$
其中 $\mathbb M_K^n$ 是常曲率 $K$ 的模型空间（球面/欧氏/双曲）。特别地，半径 $r\mapsto\operatorname{Vol}(B_r)/\operatorname{Vol}_{\mathbb M_K^n}(B_r)$ 单调不增，故 $\operatorname{Vol}(B_r)\le\operatorname{Vol}_{\mathbb M_K^n}(B_r)$，体积增长率与直径均受控。

> **💡 直观理解（类比）**：把"曲率"想成空间的"引力"：曲率越大引力越强，就越把沿途的测地线往一起"拉拢"、把空间"箍紧"。Bonnet-Myers 说箍得够紧就必然有限大小且处处封闭（$\pi_1$ 有限）；Toponogov 说曲率越大三角形越"收腰"；Bishop-Gromov 说球的容量不超同曲率模型。三者是同一物理直觉的三种测量，而公共引擎是 **Jacobi 场**（测地的二阶变分）。

### 2.11 Bochner 技巧与消失定理

> **从……说起**：20 世纪 40 年代，Bochner 发现一条"用曲率换取拓扑信息"的高产公式：把调和形式的 $|\omega|^2$ 取两阶 Laplace，会冒出一项 Ricci（或更一般的曲率项）。这条公式本身是恒等式，但一旦曲率带正性，就能**逼出调和形式必须为零**——于是 **Bochner 消失定理**（vanishing theorem）：正 Ricci 下调和 $1$-形式为零，故 $b_1(M)=0$，Hodge/上同调与基本群的信息被正曲率"压平"。它回答的核心问题：**正曲率为什么倾向于让流形"无洞"（上的同调与基本群平凡）**。直觉：正曲率像强大的引力，把所有"洞"（调和类）都挤没了。Bochner 技巧是 Kodaira、Lichnerowicz、Yau、Schoen 一切刚性定理的祖传引擎。

**定理 2.19**（Bochner 公式）对 Riemann 流形 $(M,g)$ 与光滑函数 $f$，
$$
\frac12\Delta|\nabla f|^2=|\nabla^2 f|^2+\langle\nabla f,\nabla(\Delta f)\rangle+\operatorname{Ric}(\nabla f,\nabla f).
$$
对调和 $1$-形式 $\omega$（$d\omega=0$、$\delta\omega=0$），结合 Weitzenböck 公式
$$
...
> **💡 直观理解（类比）**：Bochner 公式是一架"守恒天平"：左边是 $\Delta$-二阶总量，右边拆成"平方项（非负）$+$ 曲率项"（还有交叉项）。把调和形式的积分版本放上去，若曲率严格正，曲率项被迫为正、平方项只能为零——于是调和 $1$-形式平行乃至为零，即"正曲率吃掉调和类"。

**定理 2.20**（Bochner 消失定理）设 $(M^n,g)$ 紧且 $\operatorname{Ric}>0$（处处正），则不存在非零调和 $1$-形式，故 $b_1(M)=0$。若 $\operatorname{Ric}\ge0$，则每个调和 $1$-形式平行；紧致情形下若还有一点 $\operatorname{Ric}>0$ 则仍可推出为零。

> **⚠️ 常见坑**：Bochner 公式里的曲率项是 $\operatorname{Ric}(\nabla f,\nabla f)$ 而非数量曲率——只靠"数量曲率正"给不出消失结论；符号约定（$\Delta=-\operatorname{div}\nabla$）直接影响公式中正负号与积分结论，务必与 §2.1 保持一致，否则会得到错误的正曲率结论。

### 2.12 第一特征值的估计

> **从……说起**：紧流形 Laplace 谱里最重要的一格是**第一非零特征值** $\lambda_1$（谱隙）。它度量流形"抵抗低频振动"的能力，常由 Rayleigh 商刻画。问题是：给一个曲率下界，能把 $\lambda_1$ 逼得多紧？Lichnerowicz（1958）证明经典下界——$\operatorname{Ric}\ge(n-1)g$ 时 $\lambda_1\ge n$（用 Bochner 公式加 Poincaré 不等式）；Obata 随后给出等号刚性：$\lambda_1=n$ 当且仅当 $M$ 等距标准球面。它回答的核心问题：**正曲率不仅限制几何尺寸（§2.10），还精确限制振动频率**，是从曲率到谱的最美桥梁之一。

**定理 2.21**（Lichnerowicz 下界）设 $(M^n,g)$ 紧、$\operatorname{Ric}\ge(n-1)g$，则 $\lambda_1\ge n$。
**定理 2.22**（Obata 刚性）在相同假设下，$\lambda_1=n$ 当且仅当 $(M,g)$ 等距于单位球面 $S^n$。

> **💡 直观理解（类比）**：曲率越大，空间越"紧"，低频振动波长被迫变短，第一频率被顶高（$\lambda_1\ge n$）。就像皮筋绷得越紧，拨动时音调越高。
>
> **⚠️ 常见坑**：$\lambda_1\ge n$ 中"球"的归一化是 $\operatorname{Ric}\ge(n-1)g$（即单位球）；若换作半径 $r$ 或截面曲率 $k$ 的表述，下界要相应换成 $\lambda_1\ge n/r^2$ 等，套错归一化会得到错误的常数。

### 2.13 Ricci 流的短时存在性与正规化

> **从……说起**：§2.5 说 Ricci 流是"熨平几何"的引擎，但引擎要真正点火，得先回答最基础的 PDE 问题：**给一族光滑初值，短时间解存在且唯一吗**？对普通抛物方程这通常平凡，可 Ricci 流有个致命缺陷——**它不是严格抛物型**：因为如果单纯缩放/微分同胚移动也能让方程"跑起来"，那么 $\partial_t g$ 的两阶导数在这个方向上是退化/缺缩的。Hamilton 早期用 Nash-Moser 隐函数定理拿到存在性；DeTurck（1983）给出更优雅的**热修正技巧**（gauge fixing / DeTurck trick）：给方程加一项由"热量规范"向量场 $X$ 产生的微分同胚项，强制成严格抛物，先解修正方程，再用 $X$ 生成的微分同胚回旋回拉——由于回拉不改变几何意义，得到的确实是真 Ricci 流解。正规化的意义：原始 Ricci 流会整体缩放体积，引入**规范化 Ricci 流**（normalized）固定或控制体积，便于长期行为分析。回答的核心问题：**几何流如何被"驯服"成可以放心解、并且规范变换不污染解的几何意义**。

**定理 2.23**（短时存在性与唯一性）对紧 $C^\infty$ 初值 $(M,g_0)$，Ricci 流在某个 $T>0$ 上存在唯一光滑解 $g(t)$（Hamilton 1982；DeTurck 1983 给出严格抛物化证明）。解在最大存在时间 $T_{\max}$ 内光滑，且若 $T_{\max}<\infty$，则 $\limsup_{t\to T_{\max}}\|\operatorname{Rm}(g(t))\|_{L^\infty}=\infty$（即曲率爆破——§2.5 所说的奇点）。

**定义 2.24**（DeTurck 热修正与规范化 Ricci 流）。**DeTurck 技巧**：先解带规范向量场 $X$ 的严格抛物方程
$$
\partial_t\tilde g=-2\operatorname{Ric}_{\tilde g}+\mathcal L_X\tilde g,\qquad
\partial_t X=\Delta_g X\qquad\text{（热量规范）},
$$
（形式上把"坐标误动"也纳入 $\mathcal L_X\tilde g$ 吸收掉），解出 $\tilde g$ 后再按 $X$ 生成的微分同胚回旋回拉，得到 Ricci 流真解 $g(t)$。**规范化 Ricci 流**取
$$
\partial_t g=-2\operatorname{Ric}+\frac{2\bar r}{n}\,g,\qquad \bar r=\frac{\int_M R\,dV}{\int_M dV},
$$
其中 $\bar r$ 是平均数量曲率，此项使体积守恒（$\frac{d}{dt}\operatorname{Vol}(M,g)=0$）。

> **💡 直观理解（类比）**：原始 Ricci 流像一台没装"拉直基准"的熨斗——布料在运动时同时被坐标的随意平移拖着跑；DeTurck 就是给熨斗加一根标尺（规范向量场 $X$），先熨直、再按标尺把坐标回旋正，几何结果不变。规范化版本则像"熨完后保持布幅不变大不变小"，方便长期比较。

### 2.14 标量曲率流

> **从……说起**：Ricci 流熨的是整张度量，代价高昂（抛物困难、奇点复杂）。如果只关心最简单的曲率量——**数量曲率** $R$——可以写下更朴素的热型流。最著名的其实是"Ricci 流下数量曲率的演化"
$$
\partial_t R=\Delta R+2|\operatorname{Ric}|^2
$$
（Hamilton 早期的曲率演化恒等式，用 Bianchi 得到）。它和真正的"标量曲率流"（如 $\partial_t R=\Delta R$ 或整体共形收缩 $\partial_t g=-\varepsilon\,\bar R\,g$）共同回答一个问题：**数量曲率如何像热一样扩散、并在何种条件下得到先验下界估计**。这类流比 Ricci 流简单，但因丢掉度量的方向信息，用途远为有限——比较它们正好能彰明 Ricci 流"保形状信息"的可贵。

**定理 2.25**（数量曲率沿 Ricci 流的演化）沿未规范的 Ricci 流 $\partial_t g=-2\operatorname{Ric}$，
$$
\partial_t R=\Delta R+2|\operatorname{Ric}|^2.
$$
由极大值原理，若初值 $R_g\ge c$，则该下界沿流动保持（先验估计）——这是 §2.13 正规化分析的基础。

**说明**（标量曲率流）为几何问题独立引入的标量型流，例如
$$
\partial_t R=\Delta R,\qquad\text{或}\qquad\partial_t g=-\varepsilon\,\bar R\,g,
$$
前者把数量曲率当作热方程扩散，后者是"整体共形收缩"（把平均数量曲率压回去）。二维时 $ \operatorname{Ric}=\frac{R}{2}g$ 完全由标量决定，故标量曲率流与 Ricci 流几乎等价；高维则丢失了 Ricci 的无迹部分，故远不如 Ricci 流强大。

> **💡 直观理解（类比）**：把数量曲率想成"流形上各处的紧度"，标量曲率流让它像温度一样自行扩散、均匀——但因为它只熨"温度"、不熨"螺纹"（形状方向），所以简单却远不如此 Ricci 流强大：它丢掉了形状的方向信息。
...
### 3.6 比较定理：Bonnet-Myers 与 Bishop-Gromov

**定理 3.7**（Bonnet-Myers）若 $(M^n,g)$ 紧、$\operatorname{Ric}\ge(n-1)k>0$，则 $\operatorname{diam}(M,g)\le\pi/\sqrt k$，且 $\pi_1(M)$ 有限。

**证明**：设 $L$ 是一条长为 $L$ 的**单位速度极小测地** $\gamma:[0,L]\to M$。沿 $\gamma$ 选取平行正标架 $e_1,\dots,e_{n-1}\perp\dot\gamma$，并令变分场
$$
V_i(s)=\sin\Big(\frac{\pi s}{L}\Big)e_i(s),\qquad i=1,\dots,n-1,
$$
它们在端点处消失。极小测地的第二变分必须非负 $(I(V_i,V_i)\ge0)$。对 $i$ 求和并利用 $\nabla_{\dot\gamma}V_i=(\pi/L)\cos(\pi s/L)e_i$（平行性）与 Ricci 恒等式 $\sum_i\langle R(V_i,\dot\gamma)\dot\gamma,V_i\rangle=\operatorname{Ric}(\dot\gamma,\dot\gamma)$，得
$$
\sum_{i=1}^{n-1}I(V_i,V_i)=\int_0^L\Big((n-1)\Big(\frac{\pi}{L}\Big)^2\cos^2\frac{\pi s}{L}-\operatorname{Ric}(\dot\gamma,\dot\gamma)\Big)ds .
$$
以 $\operatorname{Ric}(\dot\gamma,\dot\gamma)\ge(n-1)k$ 代入，并注意 $\int_0^L\cos^2(\pi s/L)\,ds=L/2$：
$$
\sum_i I(V_i,V_i)\le (n-1)\Big(\frac{\pi^2}{2L}-kL\Big).
$$
若 $L>\pi/\sqrt k$，则 $L^2>\pi^2/k\ge \pi^2/(2k)$，于是 $\pi^2/(2L)-kL<0$，总第二变分为负——与 $\gamma$ 是极小测地矛盾。故任意极小测地长 $\le\pi/\sqrt k$，直径被控。对 $\pi_1$ 有限：把 $k$ 约束的度量拉回万有覆盖 $\widetilde M$，其仍满足相同 Ricci 下界，于是 $\widetilde M $ 紧，覆盖层数有限——$\pi_1$ 有限。$\square$

**定理 3.8**（Bishop-Gromov 体积比较）若 $\operatorname{Ric}\ge(n-1)K$，则 $r\mapsto\operatorname{Vol}(B_r)/\operatorname{Vol}_{\mathbb M_K^n}(B_r)$ 单调不增；特别地 $\operatorname{Vol}(B_r)\le\operatorname{Vol}_{\mathbb M_K^n}(B_r)$。

**证明思路**：在法坐标里用极半径 $r$，体积元化为
$$
\sqrt{\det g}\,dV=\prod_{\alpha=1}^{n-1}J_\alpha(\theta,r)\,dr\,d\theta,
$$
其中 $J_\alpha$ 是沿 $r$ 方向的线性 Jacobi 场（$\partial_r$ 正交于 $\frac{\partial}{\partial r}$ 的副场），满足 Jacobi 方程与初值 $J_\alpha(0)=0,\ J_\alpha'(0)=1$。由曲率比较（Jacobi 方程的解对曲率单调）且 $\operatorname{Ric}\ge(n-1)K$，在半径取平均意义下方根行列式 $\Theta(r)=\operatorname{Vol}(\partial B_r)/\operatorname{Vol}_{\mathbb M_K^n}(\partial B_r)$ 满足 $\Theta'\le0$（Bishop 的 volume ratio 单调性）。积分两次即得对数凸/单调体积比，进而 $\operatorname{Vol}(B_R)/\operatorname{Vol}(B_r)\le\operatorname{Vol}_{\mathbb M_K^n}(B_R)/\operatorname{Vol}_{\mathbb M_K^n}(B_r)$。$\square$

**推论 3.8.1**（联合）正 Ricci 下直径与体积都被同维球面控制：$\operatorname{diam}\le\pi/\sqrt k$、$\operatorname{Vol}(M)\le\operatorname{Vol}(S^n(1/\sqrt k))$。二者是 §3.7 谱估计中"几何先验"的来源。

> **💡 直观理解（类比）**：证明里的 $V_i=\sin(\pi s/L)e_i$ 像"在测地上撑开一根根弧弦"：如果太长，把这些弦放在半径更小（更紧）的正曲率空间里，总弦的弹性能其实被要求为负，物理上必然塌缩——于是测地被判"不可能那么长"。Jacobi 场就是这个"弦的弹性能量"的严格化身。

### 3.7 Lichnerowicz 第一特征值

**定理 3.9**（Lichnerowicz 下界 + Obata 刚性）设 $(M^n,g)$ 紧、$\operatorname{Ric}\ge(n-1)g$，则第一非零特征值 $\lambda_1\ge n$；且 $\lambda_1=n$ 当且仅当 $(M,g)\cong(S^n,g_{\text{std}})$。

**证明思路**：取 $\Delta f=\lambda_1 f$ 的特征函数。对 Bochner 公式（定理 2.19）分部积分（$\int_M\Delta|\nabla f|^2=0$），并利用 Green 恒等式 $\int_M\langle\nabla f,\nabla(\Delta f)\rangle=\int_M f\,\Delta(\Delta f)=\lambda_1^2\int_M f^2=\lambda_1\int_M|\nabla f|^2$，得到
$$
\lambda_1\int_M|\nabla f|^2=\int_M|\nabla^2 f|^2+\int_M\operatorname{Ric}(\nabla f,\nabla f).
$$
另一方面由 $\operatorname{Ric}\ge(n-1)g$ 与点式不等式 $|\nabla^2 f|^2\ge\frac1n(\Delta f)^2$（迹平方小于等于 Frobenius 平方），并注意 $\int(\Delta f)^2=\lambda_1\int|\nabla f|^2$，取积分得
$$
\lambda_1\ge \frac1n\lambda_1+(n-1)\ \Longrightarrow\ \lambda_1\ge n.
$$
**刚性（Obata）**：等号成立要求处处 $\operatorname{Ric}(\nabla f,\nabla f)=(n-1)|\nabla f|^2$ 且 $\nabla^2 f=\frac1n(\Delta f)g$，即 Ricci 处处饱和、$\nabla f$ 构成共形雅可比场。结合 $\int(\Delta f)^2=\lambda_1\int|\nabla f|^2$ 与 $\lambda_1=n$，可证 $M\to\mathbb R^{n+1}$ 的嵌入 $x\mapsto(f(x),|\nabla f|^{-1}\nabla f)$ 把 $M$ 映成单位球，故 $M\cong S^n$。$\square$

> **💡 直观理解（类比）**：Lichnerowicz 把"所有低频都被顶高"这个结论拆成三个证料：Bochner 列出能量账、迹不等式挤掉 $1/n$ 的损耗、Poincaré 收敛出下界。Obata 则说"等号"恰恰表明六个方向全饱和——你无处可弯，只能是球。
...
**习题 301**（共形几何）写出共形变换 $\tilde g=u^{4/(n-2)}g$（$n\ge3$）与 $\tilde g=e^{2u}g$（$n=2$），说明共形变换保持什么、不保持什么。

**习题 302**（共形几何）写出共形类 $[g]$ 的定义，并说明哪些量是共形不变量（如 $n=2$ 的 $K\,dA$、$n\ge3$ 的 Weyl 张量）。

**习题 303**（比较定理）陈述 Bonnet-Myers 定理：$\operatorname{Ric}\ge(n-1)k>0$ 时关于直径与基本群的结论。

**习题 304**（比较定理）陈述 Bishop-Gromov 体积比较：正 Ricci 下测地球体积至多等于对应模型空间球体积。

**习题 305**（Bochner）写出 Bochner 公式 $\frac12\Delta|\nabla f|^2=|\nabla^2 f|^2+\langle\nabla f,\nabla\Delta f\rangle+\operatorname{Ric}(\nabla f,\nabla f)$，并指出其中的曲率项。

**习题 306**（Bochner）陈述 Bochner 消失定理：$\operatorname{Ric}>0$ 时紧流形上调和 $1$-形式为零，进而 $b_1=0$。

**习题 307**（谱估计）写出 Lichnerowicz 下界 $\lambda_1\ge n$ 所需的曲率假设（归一化）。

**习题 308**（谱估计）用 Rayleigh 商写出 $\lambda_1=\inf_{\int f=0,f\ne0}\frac{\int|\nabla f|^2}{\int f^2}$。

**习题 309**（Ricci 流）陈述 Ricci 流短时存在性与唯一性定理（Hamilton/DeTurck）。

**习题 310**（Ricci 流）写出规范化 Ricci 流 $\partial_t g=-2\operatorname{Ric}+\frac{2\bar r}{n}g$，说明其作用。

**习题 311**（标量曲率流）写出数量曲率沿（未规范）Ricci 流的演化方程。

**习题 312**（标量曲率流）说明为什么标量曲率流不如 Ricci 流强大（丢失了什么信息）。
...
**习题 313**（共形几何）证明二维流形局部共形平坦（存在等温坐标 $g=e^{2u}\delta$）。

**习题 314**（共形几何）写出共形 Laplace 算子 $L=\frac{4(n-1)}{n-2}\Delta-R$ 的共形协变性，并说明其与 Yamabe 方程的关系。

**习题 315**（比较定理）用 Jacobi 场（第二变分）证明 Bonnet-Myers 的直径上界 $\operatorname{diam}\le\pi/\sqrt k$。

**习题 316**（比较定理）说明如何由 Bishop-Gromov 体积比较给出直径或体积上界。

**习题 317**（Bochner）用 Bochner 公式证明 $\operatorname{Ric}>0$ 时紧流形上 $b_1=0$。

**习题 318**（Bochner）证明 $\operatorname{Ric}\ge0$ 时调和 $1$-形式平行（Bochner 刚性）。

**习题 319**（谱估计）证明 Lichnerowicz 下界 $\lambda_1\ge n$。

**习题 320**（Ricci 流）简述 DeTurck 技巧：把 Ricci 流化为严格抛物方程并回旋回拉得到原解。

**习题 321**（标量曲率流）用 Bianchi 恒等式推导数量曲率沿 Ricci 流的演化 $\partial_t R=\Delta R+2|\operatorname{Ric}|^2$。

**习题 322**（Ricci 流）计算规范化 Ricci 流的体积变化，说明 $\frac{d}{dt}\operatorname{Vol}=0$。

**习题 323**（标量曲率流）说明二维情形下标量曲率流与 Ricci 流的等价性（$\operatorname{Ric}=\frac R2 g$）。

**习题 324**（谱估计）简述 Obata 刚性：$\lambda_1=n$ 等号当且仅当标准球面。
...
**习题 325**（共形几何）画出证明骨架：$n\ge4$ 时 Weyl 张量为零等价于局部共形平坦。

**习题 326**（比较定理）证明 Myers 等号情形对应标准球面（刚性 / 体积为球体积的关键步）。

**习题 327**（Bochner/消失）综合证明：正 Ricci 紧流形的 $\pi_1$ 有限（Bochner $1$-形式消失 + 覆盖论证）。

**习题 328**（谱估计）给出 Obata 定理的完整证明线索（等号条件逐点取等 + 比较刚性）。

**习题 329**（Ricci 流）论述为何退化抛物需要 DeTurck/Nash-Moser 正则化，以及短时存在性的要点。

**习题 330**（标量曲率流）讨论标量曲率流的布雷/规范化渐近行为，并与 Ricci 流比较（开放层面）。
...
101. 共形变换 $\tilde g=u^{4/(n-2)}g$（$n\ge3$）、$e^{2u}g$（$n=2$）；保持每点夹角与共形结构，不保持长度、面积、体积。

102. $[g]=\{\tilde g=u^{4/(n-2)}g:u>0\}$；共形不变量如 $n=2$ 的 $K\,dA$、$n\ge3$ 的 Weyl 张量 $W$（$W^{g}$ 与 $W^{\tilde g}$ 的对应分量相同）。

103. $\operatorname{Ric}\ge(n-1)k>0$ 时 $\operatorname{diam}(M,g)\le\pi/\sqrt k$ 且 $\pi_1(M)$ 有限。

104. $\operatorname{Ric}\ge(n-1)K$ 时 $r\mapsto\operatorname{Vol}(B_r)/\operatorname{Vol}_{\mathbb M_K^n}(B_r)$ 单调不增，特别 $\operatorname{Vol}(B_r)\le\operatorname{Vol}_{\mathbb M_K^n}(B_r)$。

105. Bochner 公式中曲率项为 $\operatorname{Ric}(\nabla f,\nabla f)$。

106. $\operatorname{Ric}>0$（紧）⇒ 调和 $1$-形式恒为零 ⇒ $b_1(M)=0$。

107. 归一化假设 $\operatorname{Ric}\ge(n-1)g$，则 $\lambda_1\ge n$。

108. $\lambda_1=\inf_{\int f=0,\,f\ne0}\frac{\int|\nabla f|^2}{\int f^2}$。

109. 对紧光滑初值，在某个 $T>0$ 上存在唯一光滑解；$T_{\max}<\infty$ 时曲率无界（$\limsup\|\operatorname{Rm}\|_{L^\infty}\to\infty$）。

110. $\partial_t g=-2\operatorname{Ric}+\frac{2\bar r}{n}g$，其中 $\bar r$ 为平均数量曲率；此式保持体积常数，便于长期行为分析。

111. 未规范下 $\partial_t R=\Delta R+2|\operatorname{Ric}|^2$。

112. 标量曲率流只熨标量 $R$，丢失度量方向（无迹 Ricci / 形状）信息，高维几乎无法由标量恢复几何。
...
91. 设局部坐标使 $g=e^{2u}\delta$，则度量即共形平坦（二维各向伸缩一下夹角不变）；由 Beltrami/等温方程局部可解 $u$，故二维处处存在等温坐标，证得局部共形平坦。

92. $L\varphi=\frac{4(n-1)}{n-2}\Delta\varphi-R\varphi$；在 $g\mapsto\tilde g=u^{4/(n-2)}g$ 下 $L_{\tilde g}\varphi=u^{-\frac{n+2}{n-2}}L(u^{-1}\varphi)$；令 $\varphi=u$ 且 $R_{\tilde g}=\lambda$（常数），即得 Yamabe 方程 $-\frac{4(n-1)}{n-2}\Delta u+Ru=\lambda u^{(n+2)/(n-2)}$。

93. 见定理 3.7：取极小测地与 $V_i=\sin(\pi s/L)e_i$，第二变分求和 $\le(n-1)(\pi^2/(2L)-kL)<0$ 当 $L>\pi/\sqrt k$，矛盾；覆盖空间仍满足假设故 $\pi_1$ 有限。

94. 由 $\operatorname{Vol}(B_R)/\operatorname{Vol}(B_r)\le$ 模型比值，取 $r\to0$（局部欧氏）并令 $R\to\operatorname{diam}$，得整体体积 $ \operatorname{Vol}(M)\le\operatorname{Vol}$（同曲率球）且半径/直径一致有上界。

95. 对调和 $1$-形式积分 Bochner：$0=\int|\nabla\omega|^2+\int\operatorname{Ric}(\omega^\sharp,\omega^\sharp)$；$\operatorname{Ric}>0$ 迫使 $\omega\equiv0$，故 $b_1=\dim\mathcal H^1=0$。

96. 同上，$\operatorname{Ric}\ge0$ 时平方项非负故 $\int|\nabla\omega|^2=0$，即 $\nabla\omega\equiv0$，$\omega$ 平行（Bochner 刚性）。

97. 由 $\lambda_1\int|\nabla f|^2=\int|\nabla^2f|^2+\int\operatorname{Ric}\ge\frac1n\int(\Delta f)^2+(n-1)\int|\nabla f|^2$，又 $\int(\Delta f)^2=\lambda_1\int|\nabla f|^2$，得 $\lambda_1\ge\frac1n\lambda_1+(n-1)$，即 $\lambda_1\ge n$。

98. 解严格抛物方程 $\partial_t\tilde g=-2\operatorname{Ric}_{\tilde g}+\mathcal L_X\tilde g$（$X$ 为热量规范向量场，吸收退化方向），得 $\tilde g$；再由 $X$ 生成微分同胚回旋回拉，回拉不改变几何意义，得到唯一 Ricci 流解。

99. 对 $R=g^{ij}R_{ij}$ 求 $t$ 导，用 $\partial_t g=-2\operatorname{Ric}$ 与 Bianchi $\operatorname{div}\operatorname{Ric}=\frac12\nabla R$ 及度量演化整理即得 $\partial_t R=\Delta R+2|\operatorname{Ric}|^2$。

100. $\frac{d}{dt}\sqrt{\det g}=\frac12\operatorname{tr}(g^{-1}\partial_t g)\sqrt{\det g}=(-R+\bar r)\sqrt{\det g}$，积分后 $\frac{d}{dt}\operatorname{Vol}=\int(\bar r-R)\,dV=0$，故规范化流保体积。

101. 二维 $\operatorname{Ric}=\frac R2 g$，Ricci 完全由标量 $R$ 决定：$\partial_t g=-2\operatorname{Ric}=-R g$ 即共形（标量）流，故二维下标量曲率流与 Ricci 流等价；高维才有独立于 $R$ 的无迹部分而分离。

102. Obata：$\lambda_1=n$ 等号时推导中的 $|\nabla^2 f|^2=\frac1n(\Delta f)^2$ 与 $\operatorname{Ric}(\nabla f,\nabla f)=(n-1)|\nabla f|^2$ 处处取等 → $\nabla f$ 为共形雅可比场且 Ricci 处处饱和，配合比较刚性（Cheng）得 $M\cong(S^n,g_{\text{std}})$。
...
71. 在 $n\ge4$，$W=0$ 时共形群和保角变换允许通过共形法坐标/局部共形约化把度量拉成平坦（由一个共形 Laplace 零解 $u$ 给出 $g'=u^{-2}g$ 平坦）；反方向：平直度量显然 $W=0$，而 $W$ 共形不变使 $W_{\tilde g}=0$ 也成立，故等价。

72. 若 $\operatorname{diam}=\pi/\sqrt k$，取达到直径的测地；定理 3.7 证明中的第二变分处处取等，或等价的 Bishop-Gromov 体积比等于球体积（等号），结合 Myers 共同断言 $M$ 等距于半径 $1/\sqrt k$ 的球面。

73. $\operatorname{Ric}>0$ 由 Bochner 得 $b_1=0$；由 Hurewicz $H_1(M;\mathbb R)=0$ 与覆盖理论，再加 Bonnet-Myers 使万有覆盖紧（有限覆盖），综合得 $\pi_1(M)$ 有限。

74. Obata 完整证明：特征函数代入 Bochner 并令所有不等号取等，得 $\nabla^2 f$ 纯迹（$\nabla^2 f=\frac1n(\Delta f)g$）与 Ricci 饱和；由此构造等距嵌入 $M\to\mathbb R^{n+1}$，$x\mapsto(f(x),c\nabla f(x))$ 映为单位球且拉回度量即 $g$，故 $M\cong S^n$。

75. 退化抛物来自 Ricci 流在微分同胚/共形退化方向缺缩：需把度量分量投影出这些方向；DeTurck 用热量规范向量场 $X$ 加 $\mathcal L_X$ 把方程强制成严格抛物（经典反函数定理给出解），Nash-Moser 用于早期无此规范时；唯一性由抛物性、能量与最大原理保证。

76. 规范化标量曲率流 $\partial_t R=\Delta R$ 平滑化并保持积分布（$\int R\,dV$ 守恒）、$R_{\min}$ 单调不降（最大原理），在受控几何下向常数均匀化；但因丢失 Ricci 方向（无迹部分）信息，高维全局收敛不可控——与 Ricci 流"熨整张度量"相比是退化弱版本，高维行为与耦合问题属开放层面。
...
> 几何分析的主线是"用偏微分方程研究几何、用几何激发分析"：几何结构（度量、曲率、子流形）被编码为微分方程的解，而解的性质反过来揭示几何与拓扑。它的源流可追溯到位势论、变分法与黎曼几何的早期，二十世纪中由谱几何、极小子流形与比较定理逐渐成形，最终在 1980 年代由 Ricci 流、正质量定理等冲破拓扑老大难而成为变革性工具。本章沿"调和结构→变分结构→刚性结构→流结构"的线索，追述每个工具与定理的前因后果。

### 7.1 从位势论到谱几何：分析的第一支先声

- **调和函数的孕育**：十九世纪的位势论（拉普拉斯方程 $\Delta u=0$、调和函数、格林函数）是几何分析最初的样板——物理驱动的偏微分方程在几何对象上求解，正是这门学科的原始形态（第 55 级将展开）。
- **黎曼度量与拉普拉斯-贝特拉米算子**：黎曼引入黎曼度量后，贝特拉米把标量拉普拉斯算子推广到曲面上，得到 Laplace-Beltrami 算子 $\Delta_g$；克里斯托费尔为算子的张量分析奠定框架——曲率与算子由此挂钩。
- **霍奇理论**：1930—40 年代，霍奇在紧黎曼流形上把微分形式分解为调和形式与精确形式之直和，用调和形式之维数恢复贝蒂数——拓扑量首次被分析量（调和方程解）完全刻画，这是几何分析与代数拓扑合流的里程碑。
- **鼓能否听到**：1960 年代由卡茨提出的著名问题"能否听出鼓的形状"（是否由谱决定几何）把谱几何推向台前，把"算子决定几何"这一主题变成数学追问（第 33 级谱理论将展开）。

### 7.2 变分法与极小子流形：肥皂膜的数学化

- **柏拉图式的低谷**：面积泛函的临界点（极小子流形）源于"肥皂膜"这一直觉命题：浸入的膜在边界固定的情形下取极小面积。欧拉与拉格朗日早已给出变分方程，但"是否存在极小曲面"一度悬而未决。
- **普拉托问题**：1930 年代道格拉斯与拉多在变分框架下严格解出普拉托问题（给定边界圈，求极小曲面），并获 1936 年菲尔兹奖——变分法由此成为几何分析的第二支柱。
- **调和映射**：把"能量泛函"而非面积泛函的临界点视为调和映射，同一变分思想被推广到"流形之间最平缓的映射"，成为研究度量、曲率与紧性的重要工具（第 46 级流形与张量分析将展开）。

### 7.3 曲率比较与布氏限制：刚性的地理学

- **邦内-迈尔斯定理**：若 Ricci 曲率有正下界，则流形直径有界、必致紧致且基本群有限——曲率下界第一次被翻译成"尺度"与"拓扑"的刚性约束。
- **毕晓普-格罗莫夫体积比较**：把曲率下界记为与球对称模型的体积比较，成为后续等周、谱、Gromov-Hausdorff 收敛的基本脚手架（第 47 级黎曼几何将展开）。
- **Bochner 技巧**：李奇内罗维奇用 Bochner 恒等式（对微分形式求 $L^2$ 模 + 曲率项）证明正 Ricci 曲率下第一 Betti 数为零、给出 Laplacian 第一非零本征值的下界——"把一个恒等式读作刚性来源"从此成为几何分析最通用的杠杆。
- **意义**：比较定理与 Bochner 技巧为"曲率真地支配拓扑"提供了定量与定性的双重证据，几何分析的刚性路线由此成形。

### 7.4 等周与 Sobolev：几何逼出来的分析

- **等周不等式**：$n$ 维等周不等式（给定体积，球面面积最小）在流形上的推广（Levy-Gromov 等）不断把几何的"最优"翻译为分析上的"最优范数"。
- **Sobolev 嵌入**：索博列夫在研究变分问题时建立的嵌入定理 $W^{k,p}\hookrightarrow L^q$ 在流形上成为把"弱导数"推向"连续性"的标准通道——分析所需的一切范数工具由此齐备。
- **意义**：这些"对函数空间下界"的估计是偏微分方程解的紧性、正则性的地基；没有它们，变分问题与流动都无从谈起。

### 7.5 Yamabe 问题与正质量定理：共形与引力的交错

- **Yamabe 问题**：1960 年山边英彦提出"每个共形类里是否存在常标量曲率度量"，其变分论证有缺陷；随后的特鲁丁格修正，并由舍恩（1984）在借助正质量定理后彻底解决——这是共形几何与偏微分方程最经典的联姻。
- **正质量定理**：正质量定理断言（适当边界条件下）ADM 质量非负且仅在平坦时空取零。舍恩与召峻用极小子流形/逆散射方法证明，维滕则以旋量（Dirac 算子）方法给出更优雅的证明——它把"标量曲率非负"与引力能量联系起来，也攻克了广义相对论中的质量问题。
- **意义**：Yamabe 问题与正质量定理横向打通了几何、物理与非线性分析三界，使"由偏微分方程解几何刚性问题"成为公认的方法论。

### 7.6 邓雅克与调和映射理论：由调和推广而来的刚性

- **能量泛函的全局理论**：调和映射的紧性、正则性（由格罗莫夫、倪里朗等）与邓雅克小能量正则性理论，为"弱解如何升级为光滑"提供了标准范式。
- **刚性与拓扑**：由调和映射做 Poincaré 圆周、北极/南极型目标，可推得流形之间正/负曲率情形的刚性与刚性消失（如 Siu 刚性、比安基不等）——"调和映射作为结构探测"成为几何分析的高端工具。
- **意义**：调和映射理论既承接变分法又展望镜对称、广义相对论的真空方程（第 135 级生物数学等将应用），是几何分析承前启后的枢纽。

### 7.7 Ricci 流的引入：用流去光滑化

- **哈密顿的 Ricci 流**：1982 年哈密顿引入 Ricci 流
$$\frac{\partial}{\partial t}g=-2\,\mathrm{Ric},$$
目标是让流把任意黎曼度量"光滑化"成常曲率度量，以期为庞加莱猜想等提供分析路径。
- **先期的成果**：哈密顿本人证明三维正 Ricci 曲率情形的解最终收敛到常曲率（三维塌缩的极大原则证明）；杜邦与等提出用极小曲面与凸性技巧处理奇点。
- **短时间与奇点理论**：流的短时间存在性由游形的局部正则性给出；发展的纳维尔-斯图存在奇点时的古典刻画与后续的雷格曲率估计，为佩雷尔曼的突破备下舞台。

### 7.8 佩雷尔曼与几何化猜想：庞加莱的收官

- **佩雷尔曼的三篇论文**：2002—2003 年，佩雷尔曼用熵单调公式、$\kappa$-非塌缩与奇点分析证明积分收敛，并借 Cheeger-格罗莫夫紧致性完成 Ricci 流奇点分析，最终证明庞加莱猜想及更一般的瑟斯顿几何化猜想。
- **几何化的意义**：历史上五个几何化（球面、欧氏、双曲……）由 Ricci 流统一——"一流的分析解决一段百年的拓扑"。瑟斯顿早先对几何化的猜想因佩雷尔曼而被彻底证明。
- **影响**：Ricci 流方法不仅解决庞加莱猜想，更催生了空无穷流形、标量曲率刚性（Schoen）与卡拉比流等一批新工具，几何分析因此几乎重新定义了现代亚拓扑的疆界（第 68 级低维拓扑将展开）。

### 7.9 标量曲率与近距质量：从极小子流形到 Witten 旋量

- **稳定性与平均曲率流**：由极小曲面同受控的稳定性（局部最小）出发，胡胜、Simon 等发展的平均曲率流把"流"的思想贯彻到浸入——这是几何分析面向构造的武器。
- **Witten 的旋量方法**：维滕用 Dirac 算子（旋量）证明正质量定理，把调和/旋量技巧引入广义相对论与弦论背景下的几何。
- **意义**：极小子流形、平均曲率流与旋量/Dirac 算子的多路合流，使几何分析的工具箱同时容纳变分、抛物方程与谱（第 49 级偏微分方程将展开）。

### 7.10 现代几何分析的图景：收敛、刚性与流

- **Gromov-Hausdorff 收敛与极限**：几何分析的现代语言用 Gromov-Hausdorff 距离研究流形族的极限，配合曲率下界导出极限度量空间的分类（第 47 级黎曼几何将展开）。
- **Ricci 流与卡拉比-Yau**：在复几何中，Kähler-Ricci 流与卡拉比-丘流成为理解第一 Chen 类与流形紧性的核心抓手（第 67 级复几何将展开）。
- **意义**：从古典位势论到佩雷尔曼，几何分析的图景始终围绕"几何量（曲率、体积、谱）与方程的解（调和、极小、流）相互决定"展开——它是分析、几何、拓扑三者的交汇舞台，也深刻改变着低维拓扑、复几何与广义相对论的走向。

### 7.11 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 19 世纪 | 拉普拉斯、格林、黎曼 | 位势论与黎曼度量：偏微分方程进入几何 |
| 19 世纪末 | 贝特拉米、克里斯托费尔 | Laplace-Beltrami 算子与张量分析 |
| 1920—30 年代 | 道格拉斯、拉多 | 变分法解普拉托问题（1936 菲尔兹奖） |
| 1930—40 年代 | 霍奇 | Hodge 理论：调和形式刻画拓扑 |
| 1950—60 年代 | 索博列夫、Lévy、Gromov | 等周与 Sobolev 嵌入在流形上的推广 |
| 1960 年 | 山边 | Yamabe 问题：存在常标量曲率度量 |
| 1960 年代 | 卡茨 | "能否听到鼓的形状"：谱几何 |
| 1970—72 年 | 舍恩、召峻；维滕 | 正质量定理：极小子流形与旋量证法 |
| 1982 年 | 哈密顿 | 引入 Ricci 流 |
| 1984 年 | 舍恩 | 借助正质量定理彻底解 Yamabe 问题 |
| 1990 年代 | 聂、桑评委员会等 | 调和映射与邓雅克正则性理论 |
| 2002—03 年 | 佩雷尔曼 | 证明庞加莱猜想与几何化猜想 |

### 7.12 小结

> 几何分析的历史，是"偏微分方程与几何相互导演"的历史。它的第一支柱来自位势论与霍奇理论——调和对象竟能编码拓扑；第二支柱来自变分法——极小曲面与调和映射把"面积/能量"的临界点变成理解刚性、曲率与拓扑的窗口；第三支柱则是 Bochner 技巧与技术比较定理，它们把"曲率下界"翻译成"拓扑限制"。当这一切汇入流——哈密顿的 Ricci 流——并经佩雷尔曼之手撬动庞加莱猜想与几何化猜想时，几何分析便从一门工具学科升格为能改变拓扑纪元的变革力量。与此同时，Yamabe 问题、正质量定理与调和映射理论则不断证明：共形、引力与偏微分方程本是同一件事的不同面。正是这份"分析改几何、几何改拓扑"的闭环，使几何分析在 20—21 世纪成为数学深度交叉最富成果的方向之一。
...
8. **共形几何与保角变化公式**：Weyl 张量等共形不变量把"只保角"的几何从长度几何中分离出来，是 Yamabe 问题与正质量定理的几何语言。
9. **曲率比较定理（Bonnet-Myers、Toponogov、Bishop-Gromov）**：曲率下界约束直径、体积与基本群，是一切刚性、谱与几何流结论的公共地基。
10. **Bochner 技巧与消失定理**：正曲率迫使调和形式消失、$b_1=0$、$\pi_1$ 有限，是 Kodaira、Lichnerowicz、Yau、Schoen 刚性定理的祖传引擎。
11. **第一特征值估计（Lichnerowicz-Obata）**：$\operatorname{Ric}\ge(n-1)g\Rightarrow\lambda_1\ge n$，等号恰为标准球面，构成"曲率→谱→刚性"的典范链条。
12. **Ricci 流的短时存在性与正规化**：DeTurck 热修正把退化抛物方程严格抛物化，并引入保体积的规范化流以便长期分析。
13. **标量曲率流**：数量曲率的热演化 $\partial_t R=\Delta R+2|\operatorname{Ric}|^2$ 给出先验估计；与 Ricci 流对照，反衬出后者保有度量方向（形状）信息的可贵。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对几何分析核心概念（Laplace 算子、热核、梯度估计与曲率流）的肌肉记忆。小节间相互独立，可按需跳读。

### 8.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到几何分析语境的追问上。

| 步骤 | 行动 | 几何分析中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 流形 $(M,g)$ 是几维、紧否？对象是函数、度量还是形式？问特征值、热核、梯度估计还是曲率流？ |
| ② 计划 | 选择可行工具、预估方向 | 用 $\Delta=-\operatorname{div}\nabla$ 还是 Hodge Laplacian？用谱分解、热核渐近、Bochner 公式还是比较定理？用 Ricci 流方程？先想清哪条路最省力。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 逐项保留符号，$\Delta$ 作用对象、特征指标 $\lambda_k$、几何量 $\operatorname{Ric}$ 与数量曲率 $R$ 别张冠李戴。 |
| ④ 回看 | 核对结果、检验合理性 | 特征值是否满足已知界（Lichnerowicz 等）？结果是否坐标无关？对 $S^n$、$S^1$ 等标准流形是否吻合？ |

> 💡 **策略演示**：求圆 $S^1$（半径 $1$）上 $\Delta=-\partial_\theta^2$ 的最小非零特征值。
> ① 理解：在一维紧流形上问谱。② 计划：解 $-\partial_\theta^2\psi=\lambda\psi$ 的周期本征函数。③ 执行：$\psi_k(\theta)=e^{ik\theta}$ 给出 $\lambda_k=k^2$，最小非零为 $\lambda_1=1$。④ 回看：$\lambda_0=0$ 为常值、$\lambda_1=1$，与 $S^1$ 的标准谱吻合，合理。
...
1. 标准球面 $S^n$ 上 Laplace 算子的特征值为 $\lambda_k=k(k+n-1)$，对应 $k$ 阶球谐函数。（　）
2. 本文取 $\Delta=-\operatorname{div}\nabla$，使 $\Delta$ 成为正算子。（　）
3. 热核 $p_t(x,y)$ 关于 $x$ 与 $y$ 不对称。（　）
4. Ricci 流方程写作 $\partial_t g=+2\operatorname{Ric}(g)$。（　）
5. Hodge 调和 $k$-形式 $\omega$ 满足 $\Delta\omega=0$，等价于 $d\omega=0$ 且 $\delta\omega=0$。（　）
6. 若 $\operatorname{Ric}\ge(n-1)g$，则 Lichnerowicz 估计给出 $\lambda_1\ge n$，等号恰为标准球。（　）

**选择**：

7. 标准球面 $S^2$ 上 Laplace 的最小非零特征值为（　）。
   - A. $2$　B. $0$　C. $1$　D. $3$
8. 热核的半群性质是（　）。
   - A. $p_{t+s}=p_t+p_s$　B. $p_{t+s}(x,y)=\int p_t(x,z)p_s(z,y)\,dV_g(z)$　C. $p_{ts}=p_t\circ p_s$　D. $p$ 与 $t$ 无关
9. Yamabe 问题问的是（　）。
   - A. 共形类内是否存在常数量曲率度量　B. Laplace 是否正定　C. 热核是否对称　D. 调和形式维数

### 8.3 分步骤工作示例（Worked Examples）

> 每道示例严格套用四步流程，并在「回看」中给出一条易错点或检验法。

**示例 S1：圆环上的 Laplace 特征值**

> **问题**：求 $S^1=\mathbb{R}/2\pi\mathbb{Z}$ 上 $\Delta=-\partial_\theta^2$ 的全部特征值。

- ① 理解：一维紧流形，问谱；本征函数须满足 $2\pi$-周期性。
- ② 计划：解 $-\psi''=\lambda\psi$ 并强制周期边界，指数基是正交完备系。
- ③ 执行：

$$
\psi_k(\theta)=e^{ik\theta},\quad -\partial_\theta^2\psi_k=k^2\psi_k,\quad \lambda_k=k^2\quad(k\in\mathbb{Z}).
$$

  故谱为 $\{k^2:k\in\mathbb{Z}\}=\{0,1,4,9,\dots\}$。
- ④ 回看：**检验法**：$\lambda_0=0$ 对应常值函数、$\lambda_1=1$ 是首位非零值，符合"紧流形第一非零特征值 $>0$"。**易错点**：$e^{ik\theta}$ 与 $e^{-ik\theta}$ 同谱，别漏 $k\in\mathbb{Z}$ 的负向。

**示例 S2：球面族在 Ricci 流下的半径演化**

> **问题**：半径为 $r_0$ 的 $n$ 维圆球 $S^n(r_0)$ 作为 Ricci 流初值，求 $r(t)^2$ 的演化与收缩时刻。

- ① 理解：对象是定曲率球面度量，$\operatorname{Ric}=(n-1)r^{-2}g$，Ricci 流保圆球形状。
- ② 计划：把 $g$ 写成 $r^2 g_{\mathrm{unit}}$，代入 $\partial_t g=-2\operatorname{Ric}$ 化为关于 $r^2$ 的常微分方程。
- ③ 执行：

$$
\frac{d}{dt}r(t)^2=-2(n-1),\qquad r(t)^2=r_0^2-2(n-1)t .
$$

  验证：$\partial_t(r^2 g_{\mathrm{unit}})=-2(n-1)g_{\mathrm{unit}}=-2[(n-1)r^{-2}g]=\ -2\operatorname{Ric}$。
- ④ 回看：**检验法**：球在 $T=r_0^2/[2(n-1)]$ 收缩成点；对 $S^2$（$n=2$）$T=r_0^2/2$。**易错点**：负号或因子 $2$ 错一个，球面就会由收缩变膨胀。

**示例 S3：调和函数恒为常数的梯度检验**

> **问题**：设 $(M,g)$ 是紧连通 Riemann 流形，$f\in C^\infty(M)$ 且 $\Delta f=0$。证明 $f$ 恒为常数。

- ① 理解：$\Delta$ 正定，调和即零"能量"；紧连通用积分配对。
- ② 计划：用 $\int_M f\Delta f\,dV_g=\int_M|\nabla f|^2\,dV_g$（正算子 + Green 恒等式）。
- ③ 执行：

$$
\int_M |\nabla f|^2\,dV_g=\int_M f\,\Delta f\,dV_g=0\;\Longrightarrow\;\nabla f\equiv0 .
$$

  紧连通流形上梯度处处为零推出 $f$ 为常数。
- ④ 回看：**检验法**：改用强极值/均值原理也能得到同一结论。**易错点**：该积分恒等式依赖"紧、无边界（或法向导数为零）"；边界情形需要加条件。

**示例 S4：调和函数的平方梯度估计**

> **问题**：设 $f$ 在 $\mathbb{R}^n$ 上调和（$\Delta f=0$），计算 $\Delta(f^2)$。

- ① 理解：目标是用乘积法则把"调和"转成对 $f^2$ 的方程。
- ② 计划：展开 $\Delta(f^2)=2f\Delta f+2|\nabla f|^2$，再代入 $\Delta f=0$。
- ③ 执行：
...
- ④ 回看：**检验法**：取 $f(x)=x_1$（调和），$\Delta(x_1^2)=2$，右边 $2|\nabla x_1|^2=2$，一致。**易错点**：交叉项 $2f\Delta f$ 只有在调和时才消失，别提前丢弃。

### 8.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：用谱方法数值计算圆环 $[0,2\pi)$ 上 $\Delta=-\partial_\theta^2$ 的前若干特征值，与 $\lambda_k=k^2$ 对照，并画出 $k=1,2,3$ 的特征函数。
- **聚焦 P2（跨章节联系）**：把 $S^2$ 上球谐特征值 $\lambda_k=k(k+1)$ 与第 54 级「数学物理方法」中的球面调和函数交叉引用，验证 $k=1$ 时 $\lambda_1=2$，恰为 Lichnerowicz 等式的等号情形。
- **聚焦 P3（探索项目）**：对圆球族 Ricci 流做显式时间切片，画出 $r(t)^2$ 的线性下降曲线，观察 $t\to T$ 处收缩点的速率与维度关系。
...
- **自测 1**：用一句话说明为何取 $\Delta=-\operatorname{div}\nabla$ 使 $\Delta$ 成为正算子，从而谱非负。
- **自测 2**：热核的对称性、半群性、守恒性分别编码了哪三个物理直觉。

**$\bullet$ 技能题**

- **自测 3**：求 $S^1$ 上 $\Delta$ 的第二个非零特征值（$k=2$）及对应特征函数。
- **自测 4**：对 $n=3$ 的圆球 Ricci 流，写出 $r(t)^2$ 与收缩时刻 $T$。
- **自测 5**：用 $\int_M f\Delta f\,dV_g=\int_M|\nabla f|^2\,dV_g$ 证明紧连通流形上调和函数恒定，并给出完整四步过程。

**$\blacklozenge$ 综合题**

- **自测 6**：在 $S^2$ 上验证 Lichnerowicz 估计 $\lambda_1\ge2$，并说明等号由 $k=1$ 的球谐达到。
- **自测 7**：写一段含「表格 + 一句话结论」的分析，比较 Ricci 流 $\partial_t g=-2\operatorname{Ric}$ 与标量曲率流 $\partial_t R=\Delta R+2|\operatorname{Ric}|^2$ 各自保有的信息。

### 8.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（例 2.2）。2. **正确**（定义 2.1 后注）。3. **错误**（对称性 $p_t(x,y)=p_t(y,x)$）。4. **错误**（应是 $-2\operatorname{Ric}$）。
> 5. **正确**（定义 2.5）。6. **正确**（Lichnerowicz-Obata）。
>
> **选择**：7. A　8. B　9. A。

> **自测**：
> - 自测 1：因 $\int_M f\Delta f\,dV_g=\int_M|\nabla f|^2\,dV_g\ge0$，故 $\Delta$ 正算子、谱非负。
> - 自测 2：可逆对称、时间可分、总热量守恒。
> - 自测 3：$k=2$ 时 $\lambda_2=4$，特征函数为 $e^{\pm2i\theta}$。
> - 自测 4：$r(t)^2=r_0^2-4t$，$T=r_0^2/4$（$n=3$）。
> - 自测 5：积分配对给 $\|\nabla f\|_2^2=0$，紧连通推出 $f$ 恒为常数。
> - 自测 6：$n=2$ 时 $\lambda_1=k(k+n-1)=2$，恰好等于下界 $n=2$，由 $k=1$ 球谐（坐标函数）达到。
> - 自测 7：表格列"度量方向/形状信息：保 / 不保"等维度；一句话：Ricci 流保有度量（形状）信息，而标量曲率流只追踪标量 $R$，故前者信息更全。