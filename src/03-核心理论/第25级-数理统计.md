...
Stewart 式教学的核心，"先给可背诵的解题策略，再用完整示例演练、辅以应用与直觉"。本节针对本章两大主干——**极大似然估计（MLE）** 与 **估计量的性质 / 区间估计**，给出两条"策略框"，随后用三个完整示例 S1–S3 逐行演示，最后落到一个真实应用。

> **🧭 解题策略：求极大似然估计（MLE）的固定流程**
>
> 遇到"求某参数 MLE"，别在一个个公式里打转，按下面五步按部就班即可：
>
> 1. **写似然**：样本 i.i.d. 时，似然是各点密度的乘积 $L(\theta)=\prod_{i=1}^{n}f(x_i;\theta)$；
> 2. **取对数**：算对数似然 $\ell(\theta)=\ln L(\theta)=\sum_{i=1}^{n}\ln f(x_i;\theta)$，乘积变和为求导开路；
> 3. **对参数求导置零**：解似然方程 $\frac{\partial\ell}{\partial\theta}=0$，得候选 $\hat\theta$；
> 4. **检查**：验证二阶导 $\frac{\partial^2\ell}{\partial\theta^2}<0$ 表明确为极大；若参数支撑含边界，还需把端点与内部驻点一并比较、选较大者；
> 5. **翻译成显式统计量**：把 $\hat\theta$ 写成样本函数（常含 $\bar X$、$\sum_i X_i$）。
>
> **提示**：MLE 常与你"直觉中该取均值处"只差一个单调变换（如 $1/\bar X$、$\bar X$、$1-\bar X$），可先猜再用流程锁定。

> **🧭 解题策略：判定估计量的无偏性 / 有效性 / 一致性**
>
> "性质三连问"要逐条独立核对，别混在一起：
>
> 1. **无偏**：算 $\operatorname{Bias}(\hat\theta)=\mathbb{E}[\hat\theta]-\theta$，看是否为 $0$。常靠线性期望、以及两条"工厂装配线"：$\mathbb{E}[S^2]=\sigma^2$，$\frac{(n-1)S^2}{\sigma^2}\sim\chi^2(n-1)$。
> 2. **比较有效性**：只有两估计量**都无偏**时才能比谁的方差小；只要有偏，就用公平标尺 $\operatorname{MSE}=\operatorname{Var}+(\operatorname{Bias})^2$，综合"准不准＋抖不抖"。
> 3. **一致性**：看 $n\to\infty$ 时 $\hat\theta_n\xrightarrow{p}\theta$。便捷充分条件：若 $\lim_{n}\operatorname{Bias}=0$ 且 $\lim_{n}\operatorname{Var}=0$（即 $\operatorname{MSE}\to0$），则一致。
>
> **提示**：$\operatorname{MSE}\to0\Rightarrow$ 一致，是"无偏 + 方差收缩"的一键判定；用它常常最快。

### Stewart 示例 S1（极大似然估计的完整流程）：指数分布的 MLE

**题目**：已知某电子元件寿命 $X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}\operatorname{Exp}(\lambda)$，密度为 $f(x;\lambda)=\lambda e^{-\lambda x}$（$x>0,\lambda>0$）。试用最大似然法求 $\lambda$ 的 MLE，并由此写出平均寿命 $\mathbb{E}[X]=\frac{1}{\lambda}$ 的 MLE。

**解**（严格按策略框五步）：

**第 1 步——写似然**。由 i.i.d.，似然为各密度之积：
...
恒为负，故 $\hat{\lambda}=\frac{1}{\bar X}$ 确为唯一极大值点。

**第 5 步——翻译结论**。由 MLE 的变换不变性，平均寿命的 MLE 为

$$
\frac{1}{\hat{\lambda}}=\bar{X}
$$

**结论**：$\lambda$ 的 MLE 为 $\hat{\lambda}=\dfrac{1}{\bar X}$；相应地，平均寿命 $\dfrac{1}{\lambda}$ 的 MLE 就是样本均值 $\bar X$。

**现实类比/应用**：灯泡或电子元件"多久会坏"常近似指数分布（无记忆性：用旧了危险率不变）。有了这组寿命数据，把样本平均寿命再取倒数作 $\hat{\lambda}$，制造商即可据此标定"平均无故障时间"，也不必信任某一次单独观测。

### Stewart 示例 S2（无偏 / 有效 / 一致三连判）：$S^2$ 与 MLE 的 MSE 之争

**题目**：设 $X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}N(\mu,\sigma^2)$。考虑 $\sigma^2$ 的两个估计量：样本方差 $S^2=\frac{1}{n-1}\sum_{i=1}^{n}(X_i-\bar X)^2$，以及正态 MLE $\hat\sigma_M^2=\frac{1}{n}\sum_{i=1}^{n}(X_i-\bar X)^2=\frac{n-1}{n}S^2$。分别判定两者的**无偏性**、**以 MSE 衡量的优劣**与**一致性**。

**解**：

**无偏性**。比值 $\frac{(n-1)S^2}{\sigma^2}\sim\chi^2(n-1)$，而 $\mathbb{E}[\chi^2(n-1)]=n-1$，故

$$
\mathbb{E}[S^2]=\frac{\sigma^2}{n-1}\mathbb{E}\Big[\frac{(n-1)S^2}{\sigma^2}\Big]=\frac{\sigma^2}{n-1}\cdot(n-1)=\sigma^2
$$

即 $S^2$ **无偏**。而

$$
\mathbb{E}[\hat\sigma_M^2]=\frac{n-1}{n}\sigma^2,\qquad \operatorname{Bias}(\hat\sigma_M^2)=-\frac{\sigma^2}{n}\ne0
$$

即 MLE 是**有偏**的（系统性低估）。

**有效性 / MSE**。利用 $\operatorname{Var}[\chi^2(n-1)]=2(n-1)$：

$$
\operatorname{Var}(S^2)=\Big(\frac{\sigma^2}{n-1}\Big)^{2}\cdot2(n-1)=\frac{2\sigma^4}{n-1},\qquad \operatorname{Var}(\hat\sigma_M^2)=\Big(\frac{n-1}{n}\Big)^{2}\cdot\frac{2\sigma^4}{n-1}=\frac{2(n-1)}{n^2}\sigma^4
$$

两者方差一比较，$\frac{2(n-1)}{n^2}<\frac{2}{n-1}$，故 $\operatorname{Var}(\hat\sigma_M^2)<\operatorname{Var}(S^2)$（MLE 更"聚拢"）。但 $S^2$ 无偏、MLE 有偏，不能单比方差，必须比 MSE：
...
$$
\operatorname{MSE}(\hat\sigma_M^2)=\operatorname{Var}(\hat\sigma_M^2)+[\operatorname{Bias}(\hat\sigma_M^2)]^{2}=\frac{2(n-1)}{n^2}\sigma^4+\frac{\sigma^4}{n^2}=\frac{2n-1}{n^2}\sigma^4
$$

对任意 $n\ge2$，$(2n-1)(n-1)<2n^2$（即 $-3n+1<0$），故 $\frac{2n-1}{n^2}<\frac{2}{n-1}$，从而 $\operatorname{MSE}(\hat\sigma_M^2)<\operatorname{MSE}(S^2)$：**虽有轻微负偏，MLE 的总体误差反而更小**。

**一致性**。$\operatorname{Bias}(\hat\sigma_M^2)=-\sigma^2/n\to0$ 且 $\operatorname{Var}(\hat\sigma_M^2)\to0$，故 $\operatorname{MSE}(\hat\sigma_M^2)\to0$；由 $\operatorname{MSE}\to0\Rightarrow$ 一致，$\hat\sigma_M^2\xrightarrow{p}\sigma^2$。对 $S^2$ 同理，$\operatorname{Var}(S^2)=\frac{2\sigma^4}{n-1}\to0$ 且无偏，故 $S^2\xrightarrow{p}\sigma^2$。二者都**一致**。

**结论**：$S^2$ 无偏、一致；$\hat\sigma_M^2$ 有轻微负偏差（$-\sigma^2/n$）、也一致，但因为方差足够小，整体 MSE 更小——两把"尺子"一把更准（无偏）却不如另一把"稳"。

**现实类比/应用**：无偏性像"射手瞄准不偏"，方差像"手抖不抖"，MSE 是综合打靶分。MLE 略微打低了一点点（无偏性吃亏），但每次分布更集中，总得分反而更高；且样本多了它一样逼近真相（一致）。这解释了统计里为何常"愿意用一丁点偏差换更多精确"。

### Stewart 示例 S3（抽样分布 + 充分统计量 + 置信区间）：绕未知方差 $\mu$ 的区间估计

**题目**：设 $X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}N(\mu,\sigma^2)$（$\mu,\sigma^2$ 均未知）。(a) 写出样本均值枢轴量的抽样分布；(b) 用因式分解定理判断 $\bar X$ 与 $S^2$ 是否对 $(\mu,\sigma^2)$ 充分；(c) 已知 $n=9,\ \bar x=50,\ s=3$，构造 $\mu$ 的 95% 置信区间（$t_{0.025}(8)=2.306$）。
...
其分布不含未知参数，是反解 $\mu$ 的理想"枢纽"。

**(b) 充分统计量**。写出联合密度并配方：

$$
f(\mathbf x;\mu,\sigma^2)=\prod_{i=1}^{n}\frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{(x_i-\mu)^2}{2\sigma^2}}=(2\pi\sigma^2)^{-\frac{n}{2}}\,e^{-\frac{1}{2\sigma^2}\sum_{i=1}^{n}(x_i-\mu)^2}
$$

又 $\sum_i(x_i-\mu)^2=\sum_i x_i^2-2\mu\sum_i x_i+n\mu^2$，故

$$
f(\mathbf x;\mu,\sigma^2)=\underbrace{(2\pi\sigma^2)^{-\frac{n}{2}}e^{-\frac{n\mu^2}{2\sigma^2}}e^{\frac{\mu}{\sigma^2}\sum_i x_i}e^{-\frac{1}{2\sigma^2}\sum_i x_i^2}}_{g(\sum_i x_i,\ \sum_i x_i^2;\ \mu,\sigma^2)}\cdot\underbrace{1}_{h(\mathbf x)}
$$

按因式分解定理，$(\sum_i X_i,\sum_i X_i^2)$ 是 $(\mu,\sigma^2)$ 的充分统计量；它又是 $\bar X$ 与 $S^2$ 的一一函数，因此 $(\bar X,S^2)$ 也充分——把 $n$ 个数压缩成"位置 + 离散"两个量，不丢任何关于 $(\mu,\sigma^2)$ 的信息。

**(c) 构造置信区间**。由 (a) 的枢轴量反解：

$$
\bar X\pm t_{0.025}(n-1)\frac{S}{\sqrt n}=50\pm 2.306\times\frac{3}{\sqrt9}=50\pm 2.306\times\frac{3}{3}=50\pm 2.306
$$

得到区间 $(47.694,\ 52.306)$。

**结论**：抽样分布 $\frac{\bar X-\mu}{S/\sqrt n}\sim t(n-1)$ 是构造 $\mu$ 区间的依据；$\bar X$、$S^2$ 为充分统计量（数据压缩不丢信息）；代入数据得到 $\mu$ 的 95% 置信区间 $(47.694,\ 52.306)$。

**现实类比/应用**：抽样分布回答"$\bar X$ 自己会怎么抖"，充分统计量回答"摊牌够不够"，置信区间回答"真值大概藏在哪一段"。工程上"这台机器平均误差是 $50\pm2.3$"就是 $(47.7,52.3)$ 的口语版本——比单报一个 $50$ 更诚实。

### 应用：用 MLE 标定指数寿命参数 $\lambda$ 并对均值寿命做置信评估（含 Bootstrap）

现在把 S1 的 MLE 放到真实流程里。某批次电子元件的寿命近似 $\operatorname{Exp}(\lambda)$，抽 $n=100$ 只测得其平均寿命 $\bar x=500$（小时）。

1. **MLE 标定**：$\hat\lambda=\frac{1}{\bar x}=\frac{1}{500}=0.002$（每小时故障率）。

2. **用抽样分布给不确定度**：由 8.1 的渐近正态性，$\hat\lambda$ 的近似标准误为 $\operatorname{SE}(\hat\lambda)\approx\frac{\hat\lambda}{\sqrt n}=\frac{0.002}{10}=0.0002$，95% 近似置信区间

$$
\hat\lambda\pm z_{0.025}\frac{\hat\lambda}{\sqrt n}=0.002\pm1.96\times0.0002=(0.001608,\ 0.002392)
$$

倒过来，平均寿命 $\frac{1}{\lambda}$ 的区间约为 $\big(\frac{1}{0.002392},\frac{1}{0.001608}\big)=(418,\ 622)$ 小时。

3. **Bootstrap 兜底检验**（对应 11.4.9）：不敢直接信正态近似时，对样本**有放回重抽样** $B=1000$ 次，每次都算 $\hat\lambda^{*}=\frac{1}{\bar x^{*}}$，得到 $\hat\lambda^{*}$ 的经验分布；取 2.5% 与 97.5% 分位作为区间，绕开"必须正态"的假设。这一步只依赖"重抽样 = 用经验分布近似总体"这一直觉，是大样本假设不可靠时的兜底方案。

4. **质量结论**：若产品承诺"平均寿命 $\ge450$ 小时"，因 $418<450$，样本证据并不能把下限抬到承诺值之上，需要增加样本或改进工艺——这就是"用点估计 + 区间估计做质检"的完整闭环。

> **💡 概念直觉与类比**：整章的核心其实是一条轻松的心智链——**点估计**是"猜一个数字"（MLE 是"哪个参数最可能造就眼前数据"）；**抽样分布 / 充分统计量**告诉你"这个猜测本身会怎么抖、哪些数值得保留"；**区间估计 / Bootstrap** 把"猜测 ± 抖动"打包成"真值大概在这段里"。类比 Stewart 微分里的切线：$f'(a)$ 是"局部直线"，$\hat\theta$ 是"数据直线的斜率"，而置信区间是给这条切线"包一个允差带"——精不精不是看切点，而看保留下来的不确定带窄不窄。
...
> 数理统计回答一个古老又现代的难题："从有限的数据，怎么有根据地推断背后看不见的总体？"它的前身在 9 世纪阿拉伯的密码破译与 17 世纪英国的生命表里就出现了，真正成形却在 19—20 世纪——勒让德与高斯的误差理论、皮尔逊与戈塞特挖出的抽样分布、费希尔的高似然与充分统计、内曼的置信区间，一棒接一棒地把"统计"从记账式的描述变成一门有严谨方法的推断科学。本章沿着"政治算术→误差理论→抽样分布→估计与信息→置信区间"的主线，追溯数理统计每个核心概念的来龙去脉。

### 12.1 政治算术与最早的数据：从密码破译到生命表

- **最古老的统计实践**：统计手法最早可追溯至公元前 5 世纪的普查与记账；公元 9 世纪，阿拉伯学者肯迪在《密码破译》里用"字母出现频率"的统计方法破译密文——统计与密码分析竟同一天诞生。
- **政治算术的桂冠**：1663 年，英国商人约翰·格兰特出版《自然与政治观察》，依据伦敦死亡记录编制生命表，推断人口结构，通常被看作专业统计学的诞生；其后哈雷 1693 年又用之构造生命表与年金计算。统计最初是"国家的算术"，服务于征税、征兵与人数管理。
- **名字从政客来**：statistics 源自拉丁语 statista（政治家），德语 Statistik 于 1749 年由阿亨瓦尔用来指"研究国家的学问"，再由约翰·辛克莱引荐进英语——这门学科的血脉里始终带着"治国与决策"的基因。

### 12.2 从概率到统计：两兄弟的分道扬镳

- **概率论的赠礼**：17 世纪帕斯卡、费马、伯努利发展起来的大数定律与期望，第一次给"从部分推断整体"提供了数学依据——概率论与统计学自此结成连体。
- **描述与推断的分野**：早期统计偏重"描述"（平均、比例、生命表），而要"用样本推断总体"，还差一套处理误差与不确定性的语言——这套语言由天文学强制需求催生。
- **天文学逼出来的问题**：观测行星与测量经线时，同一量重复测量总有小偏差，"如何把多次观测'最佳地'组合成一个值"成为迫在眉睫的工程问题，也孕育出现代估计理论。

### 12.3 误差分析与最小二乘法：勒让德与高斯

- **最小二乘的法签**：1805 年法国人勒让德正式发表最小二乘法（把误差平方和最小化求最佳值）；而高斯在 1795 年前后已经在实践中使用该方法，并于 1809 年把"误差服从正态分布"与"最小二乘给出最佳估计"联系起来。
- **谁先谁后的公案**：围绕优先权两人曾有争议，但事实是勒让德先发表、高斯先使用并使它在理论上站稳——最小二乘法就此成为回归分析与估计理论的核心工具。
- **从天文到统计**：最小二乘法让"用多个观测估计一个参数"有了标准答案，现代统计推断的估计思想（点估计、最优点）从这里迈出第一步。

### 12.4 正态分布与误差的"钟形"神话

- **钟形曲线的问世**：正态分布最早由棣莫弗在二项分布近似里窥见，拉普拉斯与高斯又在误差理论中把它推到中心；高斯 1809 年将其与最小二乘绑定，使它成为"误差服从的分布"的标准形象。
- **为何"处处正态"**：中心极限定理说明大量独立小误差之和趋于正态，这让"误差别本就是正态的"在心理学与物理测量中几乎成了默认前提——尽管后人逐渐明白并非一切数据都正态，它的枢纽地位仍无可撼动。
- **通向抽样分布**：有了正态与最小二乘，下一步是弄清"样本的统计量（均值、方差）服从什么样的分布"——这就是抽样分布理论的起点。

### 12.5 描述与学院化：统计学会与生物计量学派

- **统计的行业化**：19 世纪，统计学会（如 1834 年成立的伦敦统计学会）与各国统计机构把数据收集、制表推向职业化，"政府统计"成为现代国家的基础设施。
- **生物计量学**：高尔顿研究遗传与身高的"回归"现象，牵线认识"相关性"（relatedness）这个概念；卡尔·皮尔逊将该思想量化，创立生物计量学派，把统计从料账推向"描述变量间关系"的科学。
- **从描述走向推断的前奏**：描述统计（均值、方差、相关、回归）在 19 世纪末已经够用，但它缺"误差多大、可信几何"的语言——抽样分布理论正是在这一缺口上拔地而起。

### 12.6 抽样分布的革命：χ²、t 与 F 分布

- **皮尔逊与 χ²**：1900 年，卡尔·皮尔逊提出 χ² 分布与拟合优度检验，第一次给出"理论与实测差异有多大"的分布依据，抽样分布理论由此开创。
- **戈塞特与小样本之谜**：酿酒厂统计师戈塞特（笔名 Student）在 1908 年研究小样本均值的分布，得到 t 分布——因为厂方数据量总是太小，他被迫问"只有几个样本时该信什么"，这反而缔造了 t 检验的雏形。
- **费希尔与 F 分布**：费希尔在研究方差分析时推动 F 分布（斯内德科后正式命名为 F，以纪念费希尔）的运用，使"比较组间差异""比较方差大小"成为可检验的命题。
- **三大基石的意义**：χ²、t、F 三者的引入，把统计推断从"靠大样本正态近似"推进到"小样本也有精确分布"，本教程的三大抽样分布正由此登台。

### 12.7 费希尔与极大似然：估计理论的里程碑

- **1922 年的一篇**：费希尔在 1922 年系统提出极大似然估计，主张"选使观测数据出现的概率（似然）最大的参数值"——把点估计建立在统一的优化原则上，取代了五花八门的直观估计。
- **性质的理论化**：费希尔还证明极大似然估计在正则条件下具有渐近正态、渐近有效的优良性质，并引入"似然比"思想，为统计推断提供了统一而漂亮的数学框架。
- **估计理论的定格**：从"猜一个数"到"有一整套估计与检验理论"，点估计在费希尔手中第一次跻身严格科学。

### 12.8 充分统计与信息：数据压缩的哲学

- **充分统计量**：费希尔提出"充分统计量"：一个统计量若已浓缩样本中关于参数的全部信息，就可放心丢掉其余数据——它为"怎样把数据压得最小又不丢信息"立下标准。
- **费希尔信息**：他还定义了费希尔信息（刻画数据携带参数信息的多少），使"信息"成为可定量比较的量，估计与检验的效率因此都能被精确评价。
- **Rao-Blackwell 定理与后世**：基于充分统计量，Rao-Blackwell 定理说明"对无偏估计取条件期望只会更好"，把压缩与改善完美结合——这一思想后来在贝叶斯与决策论中被一再绽放。

### 12.9 区间估计与置信区间：内曼的语言

- **点估计的局限**：点估计只给一个数，缺"这个数有多准"的判断；统计学家迫切需要一个既显示位置又显示不确定性的表达。
- **1937 年内曼的置信区间**：内曼在 1937 年提出"置信区间"：在重复抽样中，区间以给定概率（置信水平）覆盖真实参数的构造方法——用频率学派语言把"不确定有多大"严格化。
- **与假设检验的姊妹**：置信区间与假设检验在内曼-皮尔逊框架下互为镜像，成为现代推断的两大输出形式；本教程对置信区间与假设检验的并列安排正来自这一历史分工。

### 12.10 回归、方差分析与实验设计：费希尔的体系

- **回归的近代化**：从高尔顿的"回归到均值"，到皮尔逊的相关与最小二乘回归，回归分析在 19 世纪末成形；费希尔又把它们纳入统一的线性模型框架。
- **方差分析与实验设计**：1920 年代费希尔在洛桑农业试验站发展方差分析，把"组间差异/组内误差"拆解开，并创立随机化、重复与区组等实验设计原则——"怎么安排试验"第一次成为科学方法的一部分。
- **现代统计的骨架**：回归、ANOVA、卡方、t、F 与非参数方法一起，构成了本教程应用于实证研究的标准工具箱。

### 12.11 数理统计的公理化与内曼的严谨化

- **频率学派的成熟**：20 世纪中叶，数理统计以频率学派（频率即概率的长期意义）为纲领走向公理论证，内曼、威尔克斯等把估计、检验、区间统一到严格的概率语言之下。
- **与概率的公理化呼应**：柯尔莫哥洛夫 1933 年公理化概率（见第 24 级）之后，统计推断有了更坚固的分析地基，数理统计作为一门数学分支正式成熟。
- **摇篮到学科的句点**：从《自然与政治观察》的人口表，到费希尔、内曼的推断理论，数理统计完成了从"记账与算命"到"有据可依的推断科学"的蜕变，也正是本教程所覆盖全部内容的历史定位。

### 12.12 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 9 世纪 | 肯迪 | 用字母频率统计破译密文（统计与密码学同源） |
| 1663 年 | 格兰特《自然与政治观察》 | 生命表与人口统计：专业统计学的诞生 |
| 1749 年 | 阿亨瓦尔 | 用 Statistik 指"研究国家的学问" |
| 1794—1805 年 | 高斯、勒让德 | 最小二乘法使用与发表；估计理论先声 |
| 1809 年 | 高斯 | 把误差正态与最小二乘联系，确立正态分布地位 |
| 1900 年 | 卡尔·皮尔逊 | χ² 分布与拟合优度检验 |
| 1908 年 | 戈塞特（Student） | t 分布与小样本检验 |
| 1922 年 | 费希尔 | 极大似然估计、充分统计量、费希尔信息 |
| 1920 年代 | 费希尔 | 方差分析（F 分布）、实验设计原则 |
| 1937 年 | 内曼 | 置信区间与检验的严谨化（Neyman-Pearson 框架） |
| 20 世纪中叶 | 内曼、威尔克斯等 | 数理统计的公理体系建立与学科成熟 |

### 12.13 小结

> 数理统计的历史，是一部"把猜变成算"的重建史。它在 9 世纪密码破译与 17 世纪生命表里起步，最初只是"政治算术"式的记账与描述；天文学家对误差求解的渴求逼出勒让德与高斯的最小二乘与正态分布，把"从数据估参数"变成可计算的问题。20 世纪是它的成人礼：皮尔逊的 χ²、戈塞特的 t、费希尔的 F 把抽样分布铸成三大基石，费希尔又用极大似然、充分统计量与方差分析搭起估计和实验设计的体系，内曼再用置信区间把"不确定有多大"严格化。纵观三百年，数理统计始终在回答同一个问题——数据有限，其背后的真相却无限，我们如何在两者之间架起一座有理由、有误差、有置信的桥？从人口表到大数据时代，这一桥不断被加固，从未真正被架尽。
