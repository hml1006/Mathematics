...
李群（Lie group）理论与李代数的根源，要追溯到挪威数学家索福斯·李（Sophus Lie，1842—1899）。李在研究微分方程的解时观察到，许多方程的解集在自然的"连续变换"下保持某种对称性。与有限群描述的离散对称不同，这些对称通常是连续的，例如平移、旋转与缩放。李的工作以 1870 年代在莱比锡（Leipzig）的讲座与论文为核心，发展出一套刻画"连续变换群"（continuous groups of transformations）的完整框架。他意识到，连续变换群在其单位元附近的"无穷小"信息几乎完全决定了整个群的结构，这种无穷小生成元张成的线性空间后来被抽象为李代数。一般认为，李代数概念及其基本理论正是由李在 1870 年代为研究连续变换群而引入的。

### 7.2 指数映射与李定理

李理论的核心思想之一，是把复杂的群结构线性化：将连续变换用其无穷小生成元（即李代数中的元素）来描述，两者之间由指数映射 $X \mapsto e^{X}$ 相联系。对应群上单参数子群、从而连接李代数元素与群上元素的做法，直接来自李当初对变换群的"无穷小方法"。李给出的一系列基本结果——包括"第三定理"（每个实李代数在局部上都对应一个李群）以及有关可解李代数的 Lie 定理等——构筑了李群与李代数之间的桥梁。这些结果后来在有限维与无限维、解析与光滑等多种背景下被后人推广和严格化。

### 7.3 结构理论：Cartan 子代数、根系与 Dynkin 图

李群的结构理论主要归功于 Wilhelm Killing（威廉·基灵，1847—1923）与 Élie Cartan（埃利·嘉当，1869—1951）。Killing 在 1880 年代末按李代数的结构对复半单李代数进行分类，并首先引入根、Cartan 子代数（公理性地刻画其性质）等概念。Cartan 在其 1894 年的博士论文中严格证明了复单李代数的完全分类，系统确立了根系理论，并进而研究实李代数与各种类型李群的分类。此后，Eugene Dynkin（尤金·邓金）与（E.B. Dynkin 之外）Coxeter 等人将根系用图表示出来，演变为今天教科书中的 Dynkin 图；配以最高权理论、Weyl 群（以赫曼·韦伊 Hermann Weyl 命名）及 Weyl 特征标公式，半个多世纪里逐步形成了描述半单李代数表示的标准语言。Weyl 在 1920 年代进一步发展了紧李群及其表示理论，并给出著名的 Peter–Weyl 定理（与数学家 F. Peter 合作），说明紧群上的函数可由其有限维表示矩阵系数逼近。

### 7.4 表示论与量子力学

李群表示论的大发展与其在物理学中的应用密不可分。20 世纪 20 至 30 年代，Weyl、von Neumann（冯·诺依曼）等人将群表示方法与量子力学中的对称性分析结合起来，wigner（维格纳）等人利用旋转群表示研究原子与粒子的能级结构。李代数的表示——特别是最高权表示、伴随表示 $\mathrm{Ad}$ 等——成为统一描述角动量、自旋与对称性的核心工具。教材中反复出现的 $SU(2)$ 与 $\mathfrak{sl}(2)$ 的表示理论，本质上是对量子力学角动量代数 $\mathfrak{su}(2)$ 的抽象；Weyl 特征标公式则给出了紧李群不可约表示在其极大环面上的精确刻画。一般认为，正是"对称性驱动物理分类"这一理念，使李群与李代数理论从纯数学走向了现代物理的基础地位。
