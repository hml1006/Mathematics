...
贝叶斯统计以条件概率的 **Bayes 定理**为核心，其基础公式最早见诸 **托马斯·贝叶斯（Thomas Bayes）**。他生前未发表，相关论文《论求解一个机遇问题的学说》由其友人 Richard Price 整理，于 1763 年由皇家学会刊登。贝叶斯定理给出了依据先验信息与新证据更新信念的规则，尽管当时它更多被视为概率论的一个条件概率公式。真正把贝叶斯思想确立为一套推断体系的是法国学者 **Pierre-Simon Laplace**，他在 18 世纪末至 19 世纪初大量运用后验概率解决天文与司法问题，并独立识别并运用了逆概与先验，与贝叶斯分别奠定了该路径的数学基础。

### 7.2 经典学派与贝叶斯学派的争论

19 世纪至 20 世纪中叶，**Ronald Fisher** 建立的频率学派与 **Karl Pearson、Jerzy Neyman** 的假设检验框架主导统计学，他们认为先验带有主观任意性，因而贝叶斯方法一度边缘化。**Harold Jeffreys（1939）**在《概率论》中为贝叶斯推断的正当性作了系统辩护，并与 Neyman–Pearson 思路展开争论，为贝叶斯方法的复兴保留薪火。

### 7.3 共轭先验与计算突破

贝叶斯的实用化长期受限于后验的解析计算。为便于计算，研究者发展出**共轭先验**（conjugate prior）——选择与似然函数形式配套的先验，使后验保持同类分布（如 Beta–二项、Gamma–泊松）。但更根本的突破来自 **MCMC（马尔可夫链蒙特卡洛）**方法：**Metropolis 等（1953）**提出 Metropolis 算法，**Hastings（1970）**推广为 Metropolis–Hastings，**Geman 兄弟（1984）**与 **Gelfand–Smith（1990）**引入 Gibbs 抽样与贝叶斯计算，使高维后验抽样成为可能。此系统引擎让贝叶斯正式进入大规模统计与机器学习应用。

### 7.4 贝叶斯方法的当代回归

此后贝叶斯统计在机器学习时常以"在证据基础上更新先验信念"的范式出现，与频率学派并立为现代统计两大支柱。先验的选取、马尔可夫链收敛诊断、变量选择（如 spike-and-slab 先验）与层次贝叶斯模型等课题持续丰富着该领域，使其在自然语言处理、计算机视觉、决策分析与工程推断中发挥着不可替代的作用。课文中用贝叶斯定理、先验/后验、共轭先验、MCMC 抽样等工具刻画"用数据修正假设"的思想，正承袭自这条跨越两百多年的学术脉络。
