...
> **一句话大白话**：Sobolev 空间里的函数虽然"只能弱导数可积"，但只要积分阶次足够高，就能被"捧"成真正的连续、可导函数——弱条件足够强，照样逼出好性质。
>
> **小例子**：在一维情形，$H^1$（弱导数平方可积）中的函数必能被连续代表元替代（$H^1\hookrightarrow C^{0,1/2}$ 嵌入连续函数空间）；在 $n$ 维中当 $pk>n$ 时 $W^{k,p}\hookrightarrow C^{0,\alpha}$。
