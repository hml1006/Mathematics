...
> **一句话大白话**：把偏微分方程化成"双线性泛函"后，只要双线性形式在函数空间上"适中且有界下方"（椭圆性），就能凭空断言弱解存在且唯一——用泛函极值一锤定音，不用真的动手解。
>
> **小例子**：Dirichlet 问题 $-\Delta u=f$，$u|_{\partial\Omega}=0$ 对应 $a(u,v)=\int_\Omega \nabla u\cdot\nabla v$，它在 $H^1_0$ 上强制，故存在唯一 $u\in H^1_0$ 使 $a(u,v)=(f,v)$ 对所有 $v$ 成立。
