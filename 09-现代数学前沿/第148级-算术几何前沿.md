...
![算术几何前沿：左侧Weil猜想通过Zeta函数$Z(X,t)=\exp(\sum|X(\mathbb{F}_{q^n})|t^n/n)$连接计数问题与上同调；中间p-adic Hodge理论通过比较同构$H_{\text{ét}}\otimes B_{\text{dR}}\cong H_{\text{dR}}\otimes B_{\text{dR}}$连接不同上同调；右侧Perfectoid空间通过Tilting对应$X_{\text{ét}}\cong X^{\flat}_{\text{ét}}$在特征0与特征p之间建立桥梁](images/148_arithmetic_geometry.svg)
