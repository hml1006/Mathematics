...
> **一句话大白话**：把一个多项式方程组的零点集合当"形状"（仿射簇），并给它一种很"稀"的拓扑：只有"整条代数方程"才能算开的——闭合集是能靠多项式方程组一笔画出的。
>
> **小例子**：$\mathbb{R}^2$ 中 $x^2+y^2=1$ 决定的单位圆是仿射簇；其开集几乎都巨大到是"补掉某条代数方程零点"的结构——小块开邻域在 Zariski 下"抠不出来"。
