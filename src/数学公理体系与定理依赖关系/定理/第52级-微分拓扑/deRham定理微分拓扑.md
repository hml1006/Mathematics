...
> **一句话大白话**：用微积分（微分形式求导）数出来的"洞"，和用几何三角形硬拼出来的"洞"，个数总是一模一样——分析的眼睛和拓扑的眼睛看到的是同一幅形状地图。
>
> **小例子**：环面 $T^2$ 上 de Rham 上同调 $H^1_{dR}(T^2)\cong\mathbb{R}^2$，奇异上同调 $H^1(T^2)\cong\mathbb{R}^2$，两者同构：一个度量绕的圈、一个度量经的圈。

## 一、定理介绍

> **前置依赖**：微分形式与外导数、Stokes 定理、奇异上同调、Mayer-Vietoris 序列与五引理、Poincaré 引理、单位分解
