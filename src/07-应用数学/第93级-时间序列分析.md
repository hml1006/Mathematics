...
时间序列分析处理按时间先后排布、彼此相关的观测数据。其源头可追溯到 19 世纪末经济与天文学中的"周期"研究，但作为独立学科大致定型于 20 世纪。**G. Udny Yule**（1927）与 **Eugen Slutsky** 在研究太阳黑子与经济周期时提出自回归（AR）模型，主张当前值可由过去若干期值的线性组合加随机扰动来解释；**G. T. Walker**（1931）随后引入滑动平均（MA）项，并命名"自回归滑动平均（ARMA）"模型。至此时间序列被系统地形成为一个随机过程，平稳性、自相关函数等核心概念随之确立。

### 7.2 平稳性与随机游走

**平稳性**（stationarity）概念源于对随机过程在平移下统计性质不变的要求，为模型识别与推断提供了前提。**随机游走**（random walk）的思想最早可追溯至 Pearson（1905）提出的醉汉行走随机模型与 Bachelier（1900）对巴黎证券价格的研究；1950 年代 **Samuelson** 与 **Fama（1970）**在有效市场假说中将其与金融价格行为联系起来，指出随机游走意味着历史价格无助于预测未来。这一概念既是对 ARIMA 单位根（integrated）情形的重要特例，也是现代计量金融的出发点之一。

### 7.3 ARIMA 与 Box–Jenkins 方法论

1970 年 **George Box** 与 **Gwilym Jenkins** 出版《时间序列分析：预测与控制》，系统提出 **ARIMA**（自回归单整滑动平均）模型与三阶段方法论——识别（identification）、估计（estimation）、诊断检验（diagnostic checking），并以 ACF/PACF 作为识别工具。E. Parzen 等人发展了谱分析与自回归谱估计，用频域视角刻画周期性；当序列存在长期相关性时，又会引出分数阶单整与长记忆模型（如 Hurst 指数）。Box–Jenkins 范式至今仍是单变量时间序列建模的标准流程。

### 7.4 矢量、非线性与现代发展

随着多变量经济数据增多，**Sims（1980）**提出向量自回归（VAR）以刻画多个序列的交互；1987 年 **Engle–Granger** 提出协整（cointegration）与误差修正模型，为存在长期均衡关系的非平稳序列打通了建模路径；**Akaike（1974）**的 AIC、系数检验与单位根检验（Dickey–Fuller）进一步完善了模型选择与诊断体系。现代机器学习、状态空间模型与深度学习方法则在处理大维度、非线性时间序列上不断拓展，但其底层平稳性、自相关与可预测性的基本思考仍与百年前一脉相承。
