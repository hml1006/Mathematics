...
极值概率论研究随机变量的最大值（或最小值）的极限分布。经典中心极限定理处理的是"和"的极限，而极值定理关心的对象是"最大值"。早期工作见于 **Bortkiewicz、Gumbel** 等对洪水、旱灾等极端现象的统计观测；**R. A. Fisher 与 L. H. C. Tippett（1928）**首次严格分类了标准差化的最大值的极限分布只能是三种基本类型之一（Gumbel、Fréchet、Weibull），这为极值理论奠定了雏形。**Maurice Fréchet** 同年给出 II 型分布，而极值分布的完整刻画来自 **B. V. Gnedenko（1943）**。

### 7.2 Fisher–Tippett–Gnedenko 定理

**Fisher–Tippett–Gnedenko 定理**是极值理论的基石：它等价于中心极限定理之于求和，断言：在适当标准化后，独立同分布随机变量的样本最大值（若存在非退化极限分布）必属于 Gumbel、Fréchet、Weibull 三种分布族之一，其类型由原分布的**尾行为**决定。Gnedenko（1943）给出了三者成立的充要条件，von Mises（1936）与 Jenkinson（1955）将之统一为**广义极值分布（GEV）**的形式，用位置、尺度、形状三个参数同时涵盖三种类型。

### 7.3 阈值法与 POT 方法

在实践中，分块最大值法常浪费样本信息，因此研究者转向**阈值方法**。**Pickands（1975）**提出广义帕累托分布（GPD）作为超过高阈值之上"超额量"极限分布的模型；**Balkema 与 de Haan（1974）**证明了阈值超额的极限定理。由此形成 **POT（Peaks-Over-Threshold）方法**：选取足够高的阈值，以 GPD 拟合超过该阈值的观测，再据此外推极值分位数（如重现水平、重现期）。Davidson 与 Smith 等进一步完善了这种建模与估计流程。

### 7.4 现代极值理论的应用

随着20世纪末以来金融风险、气候变化与保险精算对"极端事件"的高度关注，极值理论被大量应用于 VaR（Value-at-Risk）、极端天气重现期估计、结构性可靠度分析等领域（如 Embrechts、Klüppelberg、Mikosch 的经典著作）。多层次结构、点过程视角（如用泊松过程刻画超阈值到达）也使极值分析与随机点过程、分数布朗运动等现代概率工具接轨。本文件关于极值分布、Fisher–Tippett–Gnedenko 三型定理、广义极值分布与 POT 方法的内容，正对应着这门以"避免被'百年一遇'的意外击溃"为初衷的学科的发展主线。
