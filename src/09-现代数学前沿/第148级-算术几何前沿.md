...
### 七.1 Weil猜想与现代算术几何的开端

算术几何是数论与代数几何交汇的现代分支。其现代形态常以"有限域上的代数簇的点数"为出发点：对有限域$\mathbb{F}_q$上定义的代数簇$X$，问其$\mathbb{F}_{q^n}$有理点的个数。1949年，安德烈·韦尔（André Weil）在《有限域上方程的解数》中提出一组天才猜想（Weil猜想）——有限代数簇的Zeta函数应有理性、满足函数方程与Riemann假设的类比、其Betti数与几何相符。韦尔敏锐地意识到，要给出他提出的"对合/傅里叶对偶"式证明，需要一种类比于经典奇异上同调的上同调理论。Weil猜想的提出直接催生了现代算术几何，成为该领域的奠基性纲领。

### 七.2 Grothendieck与étale上同调、Deligne的证明

为实现Weil猜想，格罗滕迪克（Grothendieck）在1960年代发展了étale上同调与ℓ-adic上同调，给有限域上的代数簇"重新戴上拓扑的帽子"，使Frobenius作用与Betti数成为上同调中的可观察量。1974年，德利涅（Pierre Deligne）在格罗滕迪克的框架（以及Riemann假设部分）上完成了Weil猜想的证明，并因此获1978年菲尔兹奖。这一成就不仅解决了一个核心开放问题，也使"上同调、基本群、层论"成为算术几何的通用语言；Weil猜想的思想（如"点数=上同调的迹"）深深影响了后续的L-函数、表示论与算术对偶。本教程的Weil猜想、étale/ℓ-adic上同调、平展基本群等章节正是这一主线。

### 七.3 p-adic Hodge与Fontaine的周期环

20世纪80年代后，研究由$\ell\neq p$转向$\ell=p$（p-adic）情形。法国数学家芳丹（Jean-Marc Fontaine）发展出p-adic Hodge理论，构造了一系列"周期环"（$B_{\mathrm{dR}}$、$B_{\mathrm{HT}}$、$B_{\mathrm{cris}}$、$B_{\mathrm{st}}$），用"张量周期环"的比较同构把p-adic étale上同调与de Rham/晶体上同调统一起来，从而理解p-adic Galois表示的结构（分解、滤过、Frobenius）。这一理论为p-adic Langlands与算术几何的p-adic侧提供了关键工具。本教程的p-adic Hodge理论、周期环、比较定理等章节正源于此。

### 七.4 Scholze的Perfectoid与Fargues-Fontaine曲线

2010年代是算术几何的又一次飞跃。德国数学家舒尔茨（Peter Scholze, 2018菲尔兹奖）创立perfectoid空间理论，通过"tilting对应"在特征$0$（p-adic）与特征$p$（有限域）之间建立等价的平展拓扑，使人能在特征$p$世界中完成计算后平移回特征$0$世界。舒尔茨又与法尔格斯（Laurent Fargues）构造了Fargues-Fontaine曲线（用晶体周期环的Frobenius特征部分拼成一条光滑完备曲线，酷似$\mathbb{P}^1$），并为p-adic Langlands提供几何舞台；Fargues-Scholze对应（把$\mathrm{GL}_n$的p-adic表示翻译为Fargues-Fontaine曲线上向量丛的分类）成为近年最深刻的构造之一。本教程的perfectoid、钻石空间、Fargues-Fontaine曲线与p-adic Langlands对应章节即属于这一前沿。

### 七.5 从Shimura簇到算术对偶的丰饶图景

算术几何还延伸出一整片研究与工具：Shimura簇（志村五郎，参数化Hodge结构变形的模空间）为自守形式与Galois表示的配对提供几何舞台，谷山-志村-韦尔猜想（模性定理）与费马大定理的证明正轴心于此；Shtuka（Drinfeld，俄语"裤子"）与p-adic自守形式、以及"算术对偶/横向对偶"（延伸自p-adic Hodge理论）等概念不断丰富这一图景。在Diophantine几何侧，莫代尔–韦尔定理与法尔廷斯定理（1983）把"几何复杂度与有理点个数"联系起来。算术几何的历史，是"点数—上同调—p-adic几何—模空间"层层递进、把数论的离散世界与几何的连续世界连通起来的历程，本教程的Shimura簇、Shtuka、Diophantine几何等章节即是这一丰饶图景的代表。
