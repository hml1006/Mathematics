...
> **一句话大白话**：对于那些"把开头随便改改都不变"的遥远事件（尾部事件），概率要么铁定是0、要么铁定是1，绝没有中间地带——因为前端不管怎么闹腾都动摇不了它的结局。
>
> **小例子**：事件"$\sum_n X_n$ 收敛"只依赖无穷远处的项；把前面 1000 项任意乱改，这个事件到底发生与否完全不变，所以它的概率只能是非0即1。
