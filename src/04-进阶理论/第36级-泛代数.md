...
泛代数（Universal Algebra，又译通用代数）的目标是研究一切"用运算符号定义"的代数结构之共同规律。其思想源头可上溯到 19 世纪**布尔（George Boole, 1815–1864）**创立的布尔代数以及**德·摩根（De Morgan）**、**皮尔斯（C. S. Peirce）**对抽象代数（如格、代数关系）的思考；**怀特海（A. N. Whitehead）**在《A Treatise on Universal Algebra》中也试图统一代数系统。真正把"通用代数"系统化为独立学科的是**伯克霍夫（Garrett Birkhoff, 1911–1996）**：他在 1935 年论文及与 Mac Lane 合著的《A Survey of Modern Algebra》中，系统梳理了抽象代数（群、环、域、格）的公共结构。

### 12.2 伯克霍夫与簇理论（HSP 定理）

泛代数的里程碑是**伯克霍夫（Garrett Birkhoff）**在 1935 年证明的 **HSP 定理**（即 Birkhoff 簇定理）：一个代数簇（等值类）恰好是若干代数在同态像（H）、子代数（S）与积（P）运算下封闭的类；反过来，HSP 封闭类必由等式公理决定。这一结果把"由等式定义的结构族"与"保持代数运算的构造"联系起来，建立了泛代数中"簇"与"等式逻辑"之间的桥梁。自由代数（由生成元自由生成）与直积、次直积理论也在同一时期系统发展，**伯克霍夫表示定理**（有限分配格同构于某集合格）亦出自其手。本章的簇、等式、HSP 定理、次直积与 Jónsson 引理，均是伯克霍夫范式体系的后继。

### 12.3 格论与数学结构的"道路"

格论是泛代数中最有特色的分支之一。**戴德金（Richard Dedekind, 1831–1916）**在 1890 年代研究整环分解时提出了模格与格的概念；**伯克霍夫**在 1930–40 年代把格论系统化，给出格的同态与表示定理。**Jónsson**、**McKinsey**等对格的分配性与模性展开研究。**马采夫（Anatoly Maltsev/Mal'cev, 1909–1967）**提出的 **Mal'cev 条件**（如结合律、可置换性）开启了"用运算条件刻画一类代数的同余格性质"的大门，而**Loś（耶日·沃什，Jerzy Łoś, 1920–1998）**关于**超积**与基本模型论的定理（Łoś 定理）则把泛代数与一阶逻辑的紧致性、完备性（哥德尔、马利泽夫）联系在一起，为模型论奠定基础。本章的格论、同余关系、直积与超积小节，正是这一交汇的产物。

### 12.4 同态、自由代数与代数本体论

同态与商代数（同余类）是贯穿所有代数结构的骨架，其思想在抽象代数（群、环）中早已存在，泛代数则把它们抽象为"同余格＋同态"的统一语言。自由代数的概念源出群论中**自由群**的构造（**Dyck** 与 **Nielsen** 早期研究、**Schreier** 奠基），泛代数将其拓展到任意签名与任意簇上，形成"由生成元生成、由等式确定"的泛性质表述。**Mal'cev** 对代数的可置换同余与算术簇的工作、**Taylor** 与 **Hobby–McKenzie** 关于 Mal'cev 条件与有限代数可识别性的深入研究，使泛代数在计算机代数、自动推理与逻辑编程中找到了应用。
