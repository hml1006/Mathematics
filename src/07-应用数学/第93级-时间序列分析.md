...
## 从"猜下个月的销量"说起：时间序列分析要解决的根问题

> 在摆弄 $\phi(B)X_t=\theta(B)\varepsilon_t$ 之前，先想清楚一个问题：**人为什么需要时间序列分析？**

**一个真实的生活场景（第一步：直觉）**

想象你是超市的进货经理，手里的电子表格记着过去四年每月某款牛奶的销量。老板对你的要求只有一句话：**把下个月的备货量定准。** 你能用的工具，仅仅是这一串按时间先后排好的数字 $X_1,X_2,\dots,X_n$。你敏锐地感觉到：这些数字并非彼此孤立——昨天卖得好不好，多半会牵连今天；而且似乎每年夏天都卖得更俏。要是把每个数字都当成独立的数来对待，你就白白浪费了"先后顺序"里藏着的规律。"顺着时间这条线，读懂'今天和昨天、去年今天'之间的牵连，再用它去预报还没到来的那一格"，这正是时间序列分析要干的活。

**它和"普通回归"差在哪（第二步：要解决的问题）**

在第 92 级，回归分析假设观测彼此独立合理——这是它能工作的大前提。可一旦数据天生按时间排列（股票价格、气温、心电、销售额），"今天 $"与"昨天"几乎必然相关，独立性立刻破产。早在 1927 年，统计学家尤尔（G. U. Yule）为了解释困扰多年的**太阳黑子 11 年周期**，第一次用"上一期的值去回归本期值"写出形如 $X_t=\phi X_{t-1}+\varepsilon_t$ 的递推式，这成了今天自回归（AR）模型的鼻祖（详见第 2.5 节）。此后沃克、沃尔德、博克斯与詹金斯一步步把它从天文带进工程与金融，最终把"序列之间有记忆"这个朴素直觉，磨成了 ARMA/ARIMA 整座建模体系。时间序列分析的**核心问题**浓缩成一句话：**给定一串按时间排序、彼此牵连、还常常带趋势和季节的观测，如何把这种"时间上的依赖"抓住、并有把握地预报未来？**

**看待本章的三种眼光（第三步：正式定义前的准备）**

- **看"脾气"的眼光**：先搞清楚序列是否平稳、记忆力多长——用平稳性、ACF、PACF 给序列"把脉"（第 2.1–2.4 节）。
- **写"规律"的眼光**：把时间上的依赖写成模型——AR/MA/ARMA 记线性记忆，差分与 SARIMA 对付趋势和季节，GARCH 赴东波动本身（第 2.5–2.14 节）。
- **在线推断的眼光**：状态空间 + Kalman 滤波让你"边收新数据边更新判断"，而不是等全部到齐再算（第 2.13 节）。

读完这一节，请记住的不是公式，而是：**时间序列分析的灵魂，在于把"先后顺序"当成信息源——解读序列的记忆、趋势与节律，并据此预报未来。** 后面的每一步，都围绕这一句展开。

> **🧠 一句话记住时间序列分析的"出生证"**
>
> 时间序列分析（time series analysis）源于尤尔对太阳黑子周期的建模，它解决的**根本问题**是：
>
> $$ \boxed{\text{如何把一串按时间排序、彼此牵连且常含趋势/季节的观测，建模成能可靠预报未来的一门学问？}} $$
>
> 它的一切概念，都是在把"序列记得过去"这一直觉精确化。

---

## 开始之前

**本章在讲什么**：本章研究"顺着时间读数据并预报未来"这一任务的**主干**。你会先掌握平稳性、ACF、PACF、白噪声四个"看序列脾气"的基础工具，再依次建立 AR、MA、ARMA 三种经典线性记忆模型，用差分和 SARIMA 收拾非平稳与季节性，用 Box-Jenkins 方法论把"识别-估计-诊断-预测"串成流程，最后看指数平滑、状态空间/Kalman 滤波与 GARCH 如何应对趋势、未知状态与波动聚集。

**为什么要学它**：第 92 级（回归分析）教你"用多个变量预测一个变量"，但它默认观测彼此独立；本章把舞台换成"数据按时间排列、天然相关"的场景，是回归思想在一维"时间轴"上的升级。它既承接第 88/89 级（概率与统计推断，分布、相关、估计是全部建模的语言），又为第 95 级（贝叶斯统计，Kalman 滤波本质是递推贝叶斯滤波）、第 99 级（统计学习理论，预报误差的偏差-方差权衡与模型选择在此复用）供给最经典的应用场景。

**开始前请确认你还记得**：

- 期望、方差、协方差与相关（见第 88 级）：$\text{Cov},\text{Corr}$ 的定义与性质，平稳性、ACF 都建立在其上。
- 白噪声与独立同分布（见第 89 级）：i.i.d.、均值为零、方差恒定的"纯净随机"概念。
- 线性回归与最小二乘（见第 92 级）：AR 模型本质就是"用 $X_{t-1},X_{t-2}$ 回归 $X_t$"，Yule-Walker 估计和条件最小二乘都借它求参数。
- 线性差分方程与特征方程（见第 11 级）：用来判断 AR 过程"要不要收敛到平稳"，如 $1-\phi z=0$ 的根在单位圆外。
- 概率密度与条件期望（见第 88/89 级）：Kalman 滤波与预测分布都要用到。
...
> **🔑 为什么"平稳性"是整座模型的基石**：几乎全部经典建模（ARMA、推断、谱分析）都建立在"均值/方差/协方差不随时间漂移"之上——只有平稳，你才敢用"一个样本观测段"去估计"整条序列的规律"，并把过去规律外推到未来。理解了这一点，就不会把平稳性当成"加分条件"，而会把它看成"模型能工作"的门票：遇见非平稳序列，第一反应不该是硬套 ARMA，而是先差分（(1-B)^d）把它"拧"平稳（见 2.8、2.10）。

> **常见坑**：别把"非平稳"和"有趋势"混为一谈、就草率地忽略它。把两条各带随机趋势（单位根）的时间序列直接做回归，哪怕它们毫无关系，$R^2$ 也会经常虚高——这被称为"伪回归"（spurious regression），是时序建模的头号陷阱。判定到底该不该差分，要靠单位根检验（见 2.10）来回答，而不是凭眼睛看图形。
...
> **常见坑**：定阶不能只凭眼睛。ACF 与 PACF 的"截尾/拖尾"判断只是初筛线索，实际定 $p,q$ 还应结合信息准则（AIC/BIC）与多重候选模型比较，否则容易"差一阶"。更隐蔽的是**过度差分**：把本来就平稳的序列也去差分，会徒增方差、丢失信息、甚至引入原本没有的虚假自相关，让模型越修越糟。稳当做法是：能少差分就少差分，每次差分都用单位根检验确认必要性。
...
> 时间序列的教科书公式，落到现实里都是同一个动作：**顺着时间的先后，把"记忆"抓住，再预报还没有到来的那一格**。本节沿用 Stewart 的"策略 → 例题 → 分步解 → 回读现实"四步法，把指数平滑、AR(1) 预测、MA(1) 识别这三样核心工具放进备货、需求预测、模型识别等真实场景里跑一遍。每道例题先给**解题策略**，让你看清"这道题该往哪个方向想"，再给出**每步都解释"为什么"**的分步解答。

### 🎯 本级解题策略（Principles of Problem Solving）

Stewart 式的时序题，讲究"先看序列脾气，再选模型"。做一道预测或识别题，请按这四步走：

- **第一步 · 看平稳**：先判序列带不带趋势/季节。带趋势就先差分，带周期就考虑季节模型；非平稳硬套 ARMA 等于刻舟求剑。
- **第二步 · 选记忆模型**：若用"新旧数据加权"，选指数平滑（$\alpha$ 越大越听新数据）；若用"过去值回归今天"，选 AR($p$)；若只是"几记冲击的叠加"，选 MA($q$)。
- **第三步 · 递推预测**：把已知值逐期代入递推式：指数平滑 $\hat y_{t+1}=\alpha y_t+(1-\alpha)\hat y_t$；AR(1) 用 $\hat x_{t+1|t}=\phi x_t$。多步预测就反复套。
- **第四步 · 回读并自检**：看预测是否"向均值回归"（记性不足会衰减），残差是否像白噪声；结合 ACF 截尾/拖尾判断模型阶数。

> **一句话带走**：时序的分步解就是"看脾气 → 选模型 → 递推代入 → 回读预测"——"把昨天的权重留几分、新来的数据信几分"，关键是 $\alpha$ 与 $\phi$ 都落在稳定区间内、预测才不发散。

---

### 例题 A · 下月备货：简单指数平滑做预测

> **🎯 建模策略**：进货经理只想知道"下期备多少"。指数平滑 $\hat y_{t+1}=\alpha y_t+(1-\alpha)\hat y_t$ 用常数 $\alpha\in(0,1)$ 把握"新信息占比"。**策略：给初始预测，逐期把公式代一遍。**

**问题**：某牛奶过去 5 个月销量（箱）为 $y=(100,110,105,120,115)$，用 $\alpha=0.5$、初始预测 $\hat y_1=100$ 做超交易指数平滑，递推到第 6 个月的预测 $\hat y_6$。
...
**步骤 2：逐期递推**
$$
\hat y_2=\tfrac12 y_1+\tfrac12\hat y_1=50+50=100,\quad \hat y_3=\tfrac12y_2+\tfrac12\hat y_2=55+50=105
$$
$$
\hat y_4=\tfrac12y_3+\tfrac12\hat y_3=52.5+52.5=105,\quad \hat y_5=\tfrac12y_4+\tfrac12\hat y_4=60+52.5=112.5
$$

**步骤 3：第 6 个月预测**
$$
\hat y_6=\tfrac12 y_5+\tfrac12\hat y_5=57.5+56.25=113.75
$$

*（为什么这么加权：新观测权重 $\alpha$、旧预测权重 $1-\alpha$，越近越吃香；$\alpha=0.5$ 表示新旧各听一半。)*

> **回读现实**：第 6 个月按 113.75 箱备货——它既不追着 120、115 的最新冲，也没落在很旧的 100 上，而是被历史平滑地"拉"到中段。真正每天的订单波动很大，预测只是给备货一个"不慌不忙"的锚。

---

### 例题 B · AR(1) 一步预测：让记忆按比例衰减

> **🎯 建模策略**：AR(1) $X_t=\phi X_{t-1}+\varepsilon_t$ 说"今天=昨天的 $\phi$ 倍再加新冲击"。一步预测是最优条件期望 $\hat X_{t+1|t}=\phi X_t$；多步则反复套 $\hat X_{t+k|t}=\phi^k X_t$。**策略：选好 $\phi$，从当前值逐次乘下去。**

**问题**：收益率序列拟合得 $X_t=0.8X_{t-1}+\varepsilon_t$，当前 $X_{10}=50$。求 $X_{11}$ 与 $X_{12}$ 的一步、两步预测。

**解**：

**步骤 1：一步预测**  $\hat X_{11|10}=0.8\times50=40$。

**步骤 2：两步预测**  $\hat X_{12|10}=0.8\times40=32$（等价于 $0.8^2\times50=32$）。

**步骤 3：看衰减**  预测逐期向均值 $0$ 靠拢，收敛速度由 $|\phi|=0.8$ 决定。

*（为什么向零回：平稳的 AR(1) 记性有限，越往前的值影响力按 $\phi^k$ 指数递减，所以离现在越远越"忘"。）*

> **回读现实**：$X$ 越偏离均值，模型越敢说"下一期会往回拉一些"——这就是"均值回复"的工程版。50 → 40 → 32 的阶梯，正是"今天的量天真地直接外推"与"承认它会被拉回"之间的折中。

---

### 例题 C · 从 ACF"认出"模型：MA(1) 的身份牌

> **🎯 建模策略**：MA(1) $X_t=\varepsilon_t+\theta\varepsilon_{t-1}$ 的 ACF 在滞后 1 之后截尾为零，且 $\rho(1)=\frac{\theta}{1+\theta^2}$。由样本 $\hat\rho(1)$ 反解 $\theta$（取可逆根 $|\theta|<1$）。**策略：看 ACF 在哪截尾 → 定模型 → 用公式反推参数。**

**问题**：某平稳序列的样本自相关为 $\hat\rho(1)=0.4$、$\hat\rho(h)\approx0$（$h\ge2$）。判断模型类型并估计参数 $\theta$（可逆分支）。

**解**：

**步骤 1：识别**  ACF 在滞后 1 后截尾 → 是 MA(1)。

**步骤 2：列方程**  $0.4=\frac{\theta}{1+\theta^2}$，去分母得 $\theta^2-2.5\theta+1=0$。

**步骤 3：求解选根**  $\theta=\frac{2.5\pm\sqrt{6.25-4}}{2}=\frac{2.5\pm1.5}{2}$，得 $\theta=2$ 或 $\theta=0.5$；可逆要求 $|\theta|<1$，取 $\theta=0.5$。

*（为什么舍 $\theta=2$：可逆性要求 MA 多项式根在单位圆外，等价于 $|\theta|<1$；取 0.5 才有唯一的可逆表示。)*

> **回读现实**：ACF"在 1 阶猛地刹住"正是 MA(1) 的身份证——它的"记忆"只直接延续一期，之后全靠中间传话拖尾。由此定出 $\theta=0.5$，下一步就能用残差做一期预测。
...
**应用题 31**（指数平滑备货—时序）：某超市近 3 个月用电量（千度）为 $y=(200,210,220)$，用 $\alpha=0.5$、初始预测 $\hat y_1=200$ 的简单指数平滑，预测第 4 个月的用电量。

**应用题 32**（AR(1) 多步预测—时序）：某温度序列拟合得 $X_t=0.9X_{t-1}+\varepsilon_t$，当前 $X_{20}=100$。求 $X_{21}$、$X_{22}$ 的预测，并说明预测随步长如何变化。

**应用题 33**（MA(1) 的 ACF—识别验证）：设某个 MA(1) 过程 $X_t=\varepsilon_t+\theta\varepsilon_{t-1}$ 的系数 $\theta=0.5$。写出它的 $\rho(1)$，并说明其 ACF 从哪个滞后开始截尾为零。

**应用题 34**（Holt 水平递推—时序）：用 Holt 方法只有水平项时 $L_t=\alpha y_t+(1-\alpha)L_{t-1}$。若 $\alpha=0.4$、$L_0=100$、$y_1=120$，求 $L_1$ 并用它预测 $y_2$。
...
1. **指数平滑备货** $\hat y_1=200$；$\hat y_2=\tfrac12y_1+\tfrac12\hat y_1=100+100=200$；$\hat y_3=\tfrac12y_2+\tfrac12\hat y_2=105+100=205$；第 4 月预测 $\hat y_4=\tfrac12y_3+\tfrac12\hat y_3=110+102.5=212.5$（千度）。备货按 212.5 准备，既不冒进追 220，也不像旧值那样落后。
2. **AR(1) 多步预测** 一步 $\hat X_{21}=0.9\times100=90$；两步 $\hat X_{22}=0.9\times90=81$（即 $0.9^2\times100=81$）。随步长增加，预测按 $\phi^k$ 指数衰减、逐步回落向均值，步数越大越"记不清"当前值。
3. **MA(1) 的 ACF** $\rho(1)=\frac{\theta}{1+\theta^2}=\frac{0.5}{1+0.25}=\frac{0.5}{1.25}=0.4$；对 $h\ge2$ 因 $\varepsilon$ 不相关且无重叠项，$\rho(h)=0$，故 ACF 自滞后 2 起截尾为零。
4. **Holt 水平递推** $L_1=\alpha y_1+(1-\alpha)L_0=0.4\times120+0.6\times100=48+60=108$；下一期预测 $\hat y_2=L_1=108$。

---

## 7. 数学史（History of Mathematics）

> 时间序列分析要回答的问题朴素而实用："如果观察到的数据是按时间先后排布、彼此相关的，我能不能从过去当中读出规律，去预判下一个月、下一年？"它的前身在 19 世纪末的气象与经济"周期"研究中孕育，20 世纪 20 年代的尤尔与斯勒茨基把它写成自回归与滑动平均，沃尔德以分解定理为平稳性立基，巴舍利耶、萨缪尔森与法马用随机游走连通金融，鲍克斯与詹金斯（1970）以三阶段方法论把它铸成标准流程，恩格尔-格兰杰的协整与博克斯-金克的 GARCH 又为其补上非平稳与波动性两款利器。本章沿"从直觉趋势图到概率模型"的主线，追溯时间序列分析从粗糙图表走向严格随机过程的一百年。

### 7.1 火山的影子与"周期"迷思：气象与经济里的前奏

- **太阳黑子的诱惑**：19 世纪人们痴迷于寻找"周期性"——太阳黑子、农作物收成、商品价格都被热心者画成波形，想从中发现上涨下跌的轮回。1880 年代经济学家杰文斯甚至主张太阳黑子驱动商业周期。这套"从图表里找周期"的做法，是时间序列最早的、也是最容易误入歧途的冲动。
- **趋势、季节与噪声**：正式分析尚未独立之前，人们已会把数据粗略地拆成"长期趋势＋季节波动＋随机残余"三块，用移动平均或平滑去"磨光"曲线——这正是后来趋势分解与指数平滑的雏形。
- **从描述到建模的临界**：靠肉眼与周期表始终得不到可检验的规律，学界迫切需要把"序列中的关联"写成解析的形式——这个缺口在 1920 年代由尤尔补上。

### 7.2 尤尔与自回归：让"当前"由"过去"决定

- **自回归的诞生**：英国统计学家尤德尼·尤尔在研究太阳黑子与英国商品价格时，1927 年前后提出自回归（AR）思想：当前观测值 $x_t$ 可由过去若干期的线性组合加上随机扰动来解释，即 $x_t=\phi_{1}x_{t-1}+\cdots+\phi_{p}x_{t-p}+\varepsilon_{t}$。这第一次把"序列内部依赖"写成可估计的方程。
- **为什么是"回归自己"**："自回归（autoregressive）"意为"对自己回归"——用序列自己的滞后值去解释当前值。它与静态的横截面回归根本不同：解释变量与被解释变量来自同一条时间链。
- **雅鲁-尤尔的呼应**：同一时期，俄国统计物理背景的斯勒茨基等人也在讨论"随机输入经线性组合后能否产生周期振荡"，为 AR 提供了理性造境。

### 7.3 斯勒茨基与滑动平均：噪声也能造出"假周期"

- **滑动平均的概念**：把若干期噪声取平均（滑动平均，MA），就得到 $x_{t}=\theta_{1}\varepsilon_{t-1}+\cdots+\theta_{q}\varepsilon_{t-q}+\varepsilon_{t}$ 这类表示。
- **斯勒茨基现象**：斯勒茨基指出一个反直觉的事实——把纯随机噪声反复求和或平均，就能"凭空造出"貌似规则的周期波。这让人们惊醒：图表里看到的"周期"未必真实，可能只是随机过程的幻影，从而把时间序列研究推向严肃的概率建模。
- **AR 与 MA 的合流**：把自回归与滑动平均写进一个模型，就得到 ARMA 模型；1931 年前后沃克明确使用并命名了这一形式。时域里"用过去解释现在"的两条腿就此齐全。

### 7.4 沃尔德分解与平稳性：为一类序列立基

- **平稳性**：平稳序列要求统计性质（均值、自协方差）不随时间平移而改变，是模型可识别的前提。这个概念从严格数学意义上规范了"可以建模的序列"该长什么样。
- **沃尔德分解定理**：1938 年沃尔德证明：任何（弱）平稳序列都能唯一分解为一个"确定性感"部分与一个"可测扰动的线性组合"部分——即一切这样的平稳序列都可表为确定性项加上一个 MA 表示。这给了"ARMA 能覆盖多广"一个漂亮的答案。
- **ACF 与 PACF 的身份牌**：为识别模型阶数，人们用自相关函数与偏自相关函数的记忆形态来"点名"模型类型——平稳序列的自相关随滞后衰减的节奏，正是建模者读出的第一张线索。

### 7.5 随机游走：从醉汉到股票市场

- **巴舍利耶的先知**：1900 年巴舍利耶的博士论文就用布朗运动的数学描述巴黎证券价格，提出价格路径不可预测——比爱因斯坦的物理布朗运动还早，却长期被冷落。
- **萨缪尔森与有效市场**：1950 年代萨缪尔森以数学形式化"股价走势是贴现值过程"的讨论；1970 年法马总结成有效市场假说：若市场有效，历史价格不再携带可用于获利的预测信息——随机游走由此进入金融理论中心。
- **随机游走与单位根**：随机游走恰是 ARMA 的"单位根"情形（特征方程有根在单位圆上），是非平稳的典型；它既有漂亮的极限理论，也解释了为何很多经济序列"毫无记忆地漂移"。这为后面 ARIMA 的"差分取稳"埋下伏笔。

### 7.6 谱分析：从时域听见频率

- **傅里叶与周期性的新视角**：20 世纪中斯基尤、图基、帕尔岑等把傅里叶思想引入时间序列，用功率谱密度在"频率轴"上描述序列的能量分布，周期特征变成谱峰，时域的 ACF 与频域的谱密度互为傅里叶变换对。
- **谱估计工程化**：快速傅里叶变换（1965 年）让谱估计可算，周期图、平滑窗等工具随之成熟，频域分析为天文、气象、声学与工程提供了远眺窗口。
- **与 ARMA 的对话**：谱分析与时域 ARMA 是同一枚硬币的两面——ARMA 在时域写方程、在频域画谱，二者共同构成时间序列的完整画像。

### 7.7 鲍克斯与詹金斯：ARIMA 与三阶段方法论

- **把非平稳也纳入**:为处理带趋势的非平稳序列，把序列做差分（差分一次去掉线性趋势、再差分去掉二次趋势）再套 ARMA，就得到 ARIMA(p,d,q)——"差分整数次后平稳"。这个"先差分取稳再说"的思路把火车拉回正轨。
- **三阶段方法论**：1970 年鲍克斯与詹金斯出版《时间序列分析：预测与控制》，系统提出识别、估计、诊断检验三阶段流程，以 ACF/PACF 判断阶数、用极大似然估计参数、用残差自相关作诊断检验——这套范式至今仍是单变量建模的标准动作。
- **为什么它成了圣经**:它把"建模"从艺术变成可复现的流程，预测也有了一整套区间与准则，ARIMA 因而成为 20 世纪后半叶时间序列方法论的代名词。

### 7.8 单位根、协整与长记忆：给"伪回归"上锁

- **伪回归的警示**：两个各自漂移的随机游走序列往往在回归里"显著相关"，却毫无真实联系——这被称作伪回归。迪基与富勒（1979）提出单位根检验（ADF），先把序列的"非平稳程度"查验清楚再谈建模。
- **协整的救赎**：1987 年恩格尔与格兰杰提出协整：虽然每个序列单独非平稳，但它们的某线性组合却平稳——这意味着存在长期均衡关系，误差修正模型（ECM）随即把短期波动与长期均衡接上。恩格尔与格兰杰以此获 2003 年诺贝尔经济学奖。
- **长记忆**：某些序列的自相关衰减极慢、记忆"超长"，引出分数阶单整与赫斯特指数（长记忆模型），为水文、网络与金融中的"持久依赖"给出刻画。

### 7.9 多元与状态空间：从 VAR 到卡尔曼滤波

- **向量自回归（VAR）**：1980 年西姆斯为回答"多个宏观序列如何相互影响"提出 VAR，把每个变量都用所有变量的滞后去解释，避免人为二分"内生-外生"，成为计量经济学的标配。
- **状态空间与卡尔曼滤波**：把动态系统写成"观测方程＋状态方程"，卡尔曼滤波（1960 年前后）用递归手段从噪声观测中估计隐藏状态——从导弹制导到卫星导航再到经济基本面，都能统一在状态空间框架下。
- **从多元到非线性**：现代方法再往高维、非线性（张量、神经网络、深度学习）推进，但底层"平稳性-自相关-可预测性"的基本思考，仍与百年前一脉相承。

### 7.10 博克斯-金克式波动：ARCH 与 GARCH

- **波动聚集现象**：金融收益常"安静一段、剧烈一段"，方差并非恒定。1982 年恩格尔提出 ARCH 模型让方差跟着过去误差走；1986 年博勒斯莱夫推广为 GARCH，用"过去的波动"去解释"当下的波动"。
- **刻画厚尾与聚集**：GARCH 族把"风险随时间变化"写进模型，为期权定价、风险管理与资产组合提供了必需的波动率语言，恩格尔再以此获诺贝尔经济学奖。
- **如今的地理**：GARCH 与随机波动率、多元 GARCH、杠杆效应等构成金融时间序列核心的一支，与随机微分方程（第 96 级）在连续时间金融中相通。

### 7.11 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1880 年代 | 杰文斯等 | 用太阳黑子等解释周期，追求"周期"的早期尝试 |
| 1900 年 | 巴舍利耶 | 用布朗运动描述证券价格，先知而沉寂 |
| 1927 年前后 | 尤尔 | 提出自回归（AR）模型 |
| 1931 年前后 | 沃克 | 命名并确立自回归滑动平均（ARMA） |
| 1930 年代 | 斯勒茨基 | 指认滑动平均可"造假周期"，推动概率化 |
| 1938 年 | 沃尔德 | 分解定理，为平稳序列建模立基 |
| 1943 年 | Cramér | 频谱表示理论 |
| 1950—1970 年代 | Samuelson、Fama | 随机游走与有效市场假说 |
| 1960 年代 | Kalman 等 | 卡尔曼滤波 |
| 1965 年 | Cooley、Tukey | 快速傅里叶变换，谱分析可算化 |
| 1970 年 | Box、Jenkins | 出版专著，确立 ARIMA 与三阶段方法论 |
| 1980 年 | 西姆斯 | 向量自回归（VAR） |
| 1982／1986 年 | 恩格尔、博勒斯莱夫 | ARCH／GARCH 模型 |
| 1979 年 | Dickey、Fuller | 单位根（ADF）检验 |
| 1987 年 | 恩格尔、格兰杰 | 协整与误差修正模型 |
| 1974 年 | 赤池 | AIC 模型选择准则 |

### 7.12 小结

> 时间序列分析的一百年，是把"看曲线找周期"升华为"写方程论概率"的过程：尤尔与斯勒茨基用 AR 与 MA 为"序列内部依赖"立了方程，沃尔德用分解定理给平稳序列画了边界，随机游走经巴舍利耶、萨缪尔森与法马连通金融，谱分析与 ARIMA 分别从频域和时域为建模提供工具，单位根、协整与 GARCH 则回应了非平稳与波动性这两大真实世界的顽症。贯穿始终的追问只有一个——过去的记忆究竟如何传导到未来？从自回归系数到协整向量、从卡尔曼滤波到 GARCH 方差，答案越写越精密，而那个"从过去预判未来"的初心从未改变。
