...
> **一句话大白话**：用来量化"系统对初值的敏感度"的标尺：初始相差一丁点的两条轨道，间距按 $e^{\lambda t}$ 增长，$\lambda>0$ 就表示指数级别的背道而驰——这正是混沌的指纹。
>
> **小例子**：Logistic 映射 $x_{n+1}=4x_n(1-x_n)$ 的最大 Lyapunov 指数 $\lambda\approx\ln2>0$，说明相邻轨道间距大约每步翻倍。
