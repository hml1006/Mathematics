...
> **一句话大白话**：所有"阿贝尔型"的数域扩张（伽罗瓦群可交换）全都能装进某个单位根域里——即由 $1$ 的 $n$ 次复根拼出来的分圆域，无一例外。
>
> **小例子**：$\mathbb{Q}$ 的每个阿贝尔扩张如 $\mathbb{Q}(\sqrt{5})$、$\mathbb{Q}(\cos\frac{2\pi}{7})$ 都含于某个 $\mathbb{Q}(\zeta_N)$（$N$ 次单位根），所以它们都能用 $n$ 次单位根表达式写出来。
...
- [Kronecker-Weber 定理（第76级-代数数论）](../第76级-代数数论/Kronecker-Weber定理.md)：与本条目为同一定理，另收录于第76级-代数数论，可交叉参考。
