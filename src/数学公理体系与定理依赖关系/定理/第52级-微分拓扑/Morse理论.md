...
> **一句话大白话**：光滑曲面或高维形状上"山峰、山谷、垭口"这些关键点（临界点）的个数，能精确告诉我这个形状有多少个洞、整体长什么样——好像在给地形数关键点来猜它有几个环。
>
> **小例子**：甜甜圈（环面）上选"高度"作函数，只有两个临界点（最高环和最低环），就对应它有一个洞；而气球球面上只有一个峰和谷、个数更少，对应它没有洞。

## 一、定理介绍

> **前置依赖**：临界点与 Hessian 矩阵、Taylor 展开与线性代数中的二次型分类、Sard 定理、向量场的流、CW 复形与奇异同调
