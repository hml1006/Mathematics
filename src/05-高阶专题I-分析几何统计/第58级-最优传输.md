...
> **动机与来龙去脉**：最优传输的种子是 **1781 年 Monge 的“搬土问题”**：给定两堆形状不同的沙土，如何设计一个**一一对应的搬运映射** $T$，把源处每一粒沙送到目标处的唯一位置，使搬运总量（代价常用路程）最小？它的难处有二：其一，要找的不是“一份松散的计划”而是“一个确定的方向对” $x \mapsto T(x)$，还要满足质量守恒 $T_\#\mu=\nu$；其二，如果没有度量“搬得多省”的标尺，问题根本无从谈起。正因“硬搬”太苛刻，这个点对点版本两百多年几乎原地踏步，直到 1940 年代 Kantorovich 从经济规划的角度把它“松绑”（见 2.2）才真正起飞。**这段从“点对点”到“计划”的递进，正是整个最优传输的来龙去脉。**
...
> **动机与来龙去脉**：既然 Monge 的“点对点硬搬”常常无解（比如源的质量挤成一团而目标却是分散的），1942 年 Kantorovich 在经济规划背景下另辟蹊径：与其规定“谁送给谁”，不如承认“到达 $x$ 的质量可以拆散，分别流向不同的 $y$”。于是他把问题提升到“运输计划”（coupling）$\gamma$ 的层面——$\gamma$ 好比一份“交叉发货明细”，读数 $\gamma(A\times B)$ 就是从 $A$ 发往 $B$ 的质量。这个看似退一步的动作，让可行集 $\Pi(\mu,\nu)$ 永远非空（至少含乘积测度 $\mu\otimes\nu$），目标与约束全部线性，从而天然落进**线性规划**与凸对偶的框架。一句话：**Kantorovich 用“计划”替换了“搬法”，把一道极难的组合几何题，换成可对偶化的线性凸优化题。**
...
> **🧠 思考历程："把沙搬到最省力"这么老的问题，怎么成了现代数学的宠儿？**
>
> 最优传输问的不过是"怎样搬沙最省力"，可它从 1781 年的蒙日难题，一路演化成连接概率、几何与机器学习的枢纽理论。关键的一步，是 Kantorovich 把"运沙"变成"规划一份运输计划"。
>
> - **蒙日：一沙不落、遍历最省**（1781）。蒙日最初的问法是找一个**点对点的搬运映射** $T$：从源点 $x$ 到像点 $T(x)$ 一一对应，还要求映射可逆、连续。**这个"硬搬"版本极难解**：映射的存在性、正则性、唯一性都不好办，蒙日自己也只把它当工程师课题。
> - **Kantorovich：从"映射"放松到"计划"**，1940 年代使运输问题与线性规划捆绑，而 Kantorovich 的破局是在**"如何分配"而非"非得一对一"上动刀**：允许"一堆沙可以同时拆给多个目的地"，于是问题变成在耦合 $\Pi(\mu,\nu)$ 里求产生最小费用的 $\gamma$。**一松绑，非空封闭、凸可行域全来了，还顺手带出漂亮的对偶理论（见 2.4/3.1）。**
> - **为什么"计划"好过"搬法"**。检查映射版本，你常要从"增广空间里挑"；计划版本却在"双分布的所有耦合"上做线性优化——**可行性与控制性都大幅提升**。蒙日死磕的"正则唯一解"，往往要在 Brenier 定理（3.2）等强条件下才拿得回来。
> - **它为什么现在爆火**。Wasserstein 距离"在概率分布上给出度量"，正是图像、GAN、域自适应里"两个分布差多远"的理想尺子；熵正则 Sinkhorn（2.7）又让它可上机。**一个做完 200 年的运筹题，成了当前数据科学的日常工具。**
> - **心路**：最优传输教你：**当"完美的解"难求时，别急着硬凑，先思考"能否理性放松成线性可解的计划"**。能把几何直觉翻译成可优化的凸问题，往往是现代应用数学最值钱的招式。

### 2.3 Wasserstein 距离

> **动机与来龙去脉**：有了耦合与最小运输代价，一个自然的念头是：把“最优搬运成本”当作度量两个分布差异的尺子。真正的难点在于，它要成为一把“尺子”就必须满足对称、三角不等式、以及“距离为零当且仅当两分布相等”。逐步成形的 **Wasserstein 距离** 正是定义 $W_p(\mu,\nu)=\bigl(\min_{\gamma\in\Pi(\mu,\nu)}\int|x-y|^p\,d\gamma\bigr)^{1/p}$，把整套距离公理搬进“分布的天地”。相比逐点比较密度，它能同时捕捉“搬多远”和“搬多少”，所以在地球物理里用它衡量震后人口迁移、在地图绘制里用它比较两片人口分布——**缺了它，两个分布离得多近只能靠逐点目测。**
...
> **💡 直观理解（类比）**：$W_p$ 是一条"正八尺"：满足对称性、三角不等式，且距离为零当且仅当两堆分布完全相同。这让我们能像在物理空间里量"两个城市相距多远"那样去量"两个概率分布差多远"——整套距离该有的好性质，由此原封不动地搬进了"分布的天地"。
...
> **动机与来龙去脉**：最小化问题（min）好不好算，往往要看它有没有一个“对偶”（max）——从另一个角度给出同一数字的问题。Kantorovich 问题恰是线性规划，而线性规划最值钱的本领正是强对偶与廉价的可行解判定。**Kantorovich 对偶** 把“找最优运输计划”换成“找一份满足自身估价之和不超过运价的价目表 $(\varphi,\psi)$，再让这些估价的积分尽量抬高”。这个视角的价值在于：**验证对偶可行常比构造最优计划容易得多**，也常能直接读出最优计划的支撑结构。来龙去脉浓缩成一句：**原问题难在计划太多、要挑最优；对偶把选择换成价目表的凸约束，顺手暴露最优势的结构。**
...
> **💡 直观理解（类比）**：Kantorovich 对偶问题把"搬沙最少花费"换成"找一份可行的'价目表'（$\varphi,\psi$），让两端估价函数的积分尽量抬高"。约束 $\varphi(x)+\psi(y)\le c(x,y)$ 像"每个发货—收货组合的折价总和不能超过实际运费"，而抬高的上确界恰与最小运费相等。这就像把"物流成本"和"供应商报价表"两套账在一次优化里对齐到同一个数。

### 2.5 Monge-Ampère 方程

> **动机与来龙去脉**：一旦知道最优映射是凸势的梯度 $T=\nabla\phi$，下一个自然问题就是：**这个 $\phi$（因而 $T$）被哪个方程刻画、从哪里解出来**。对平方代价，把质量守恒 $T_\#\mu=\nu$ 按换元公式写成密度与 Jacobian 的关系，得到的正是 **Monge-Ampère 方程** $\det(D^2\phi(x))=\rho_\mu(x)/\rho_\nu(\nabla\phi(x))$。它把最优传输接到经典偏微分方程与凸几何上：**没有它，映射虽存在却难以显式求解；有了它，就能借助数值 PDE、凹包与 Newton 类方法把 $\phi$ 算出来**——这正是本节要在概念层级单独安放它的原因。
...
> **💡 直观理解（类比）**：当运价是距离平方时，最优搬运变成"爬山寻势"：每个点按某凸函数 $\phi$ 的梯度方向被推走（$T=\nabla\phi$），而密度之比恰好等于"Hessian 行列式"$\det(D^2\phi)$。Monge-Ampère 方程就像一张"体积拉伸对账表"：左边是映射局部的放缩因子，右边是源测度与像测度的密度比——"路怎么偏、沙就怎么密"由此被精确锁定。

### 2.6 Benamou-Brenier 公式

> **动机与来龙去脉**：Wasserstein 距离原本是“静止”的一次最小化：一次算清该搬多重、搬多远。但很多场合（流体、图像配准、显微）需要的是**一段时间内怎么搬**，而恰恰是这种“动态”视角带来巨大便利。**Benamou–Brenier 公式** 观察到：$W_2^2(\mu_0,\mu_1)$ 等于在所有满足质量守恒（连续性方程 $\partial_t\rho+\nabla\cdot(\rho v)=0$）的流场里，对“动能 $\int_0^1\int|v_t|^2\,d\rho_t\,dt$”取最小。它把“搬沙”等价成“最小作用量”的水力学问题，**既为数值计算提供了可迭代求解的框架，也为理解 $W_2$ 是长度空间并具有测地线铺好了几何直觉。**
...
> **💡 直观理解（类比）**：Benamou-Brenier 公式把"搬沙"改写成"一场水流"：$W_2^2$ 等于在所有从 $\mu_0$ 流向 $\mu_1$、且满足质量守恒（连续性方程）的流场里，对"动能 $\int |v_t|^2\,d\rho_t$ 沿时间的积分"取最小。就像在水管网络里找一个"平摊最少动能"的转移过程——几何的"搬运代价"被翻译成流体力学的"最小作用量"。

### 2.7 熵正则化与 Sinkhorn 算法

> **动机与来龙去脉**：理论上的最优计划常把质量“钉”在零测集或对角线上，数值上极不稳定；而真实数据又充满噪声，并不需要那么锐利的最优。于是 2010 年代起，人们给 Kantorovich 问题加一份“平滑费”——相对熵 $\varepsilon\,\mathrm{KL}(\gamma\|\mu\otimes\nu)$，逼着计划既便宜又“软和”。这把问题变成可求导、可并行、甚至有闭式迭代的凸问题，催生了 **Sinkhorn 算法**：靠“行归一、列归一”交替缩放核矩阵 $K_{ij}=e^{-c(x_i,y_j)/\varepsilon}$。它让最优传输从“只能在地图上演示”变成“能在 GPU 上处理百万级数据点”——这是最优传输近十年在机器学习爆发的直接推手。
...
> **💡 直观理解（类比）**：熵正则化在搬运花费之上再加一笔"平滑费"：除原运价 $\int c\,d\gamma$ 外，按相对熵 $\operatorname{KL}(\gamma\|\mu\otimes\nu)$ 罚掉那些"过度集中、几乎一一对应"的尖锐方案，逼着计划变得又软又散。$\varepsilon$ 是"平滑旋钮"：越大越稀糊、越接近 $0$ 越贴近硬最优。Sinkhorn 靠"行归一、列归一"交替对运输矩阵缩放（$a,b$ 迭代），像不断往两侧配平的流水线，高效逼近带平滑费的最优计划。

### 2.8 c-变换 与 c-凹（对偶策略）

> **动机与来龙去脉**：对偶问题里的势 $(\varphi,\psi)$ 只是被约束 $\varphi(x)+\psi(y)\le c(x,y)$ 牵在一起的“标价系统”，自由度很大（各自可整体加常数、也未必最优）。我们需要一套洗礼，把这些任意性收进一个单变量的自洽框架——这就是 **$c$-变换**。它在最优传输中的地位，恰如 Legendre–Fenchel 共轭在凸分析中的地位：**它把“满足约束的双边对偶势”压缩成“单变量势 $(\varphi,\varphi^c)$”，从而把对偶问题写成 $\sup_\varphi(\int\varphi\,d\mu+\int\varphi^c\,d\nu)$**，并顺带推出均衡（互补松弛）与最优支撑条件。没有 $c$-变换，对偶势的任意性会让求解与分析失序；有了它，整套对偶理论得以闭合。

**定义 2.9**（$c$-变换 与 $c$-凹函数）设 $c: X \times Y \to \mathbb{R}\cup\{+\infty\}$ 是代价。对 $\varphi: X\to \mathbb{R}\cup\{-\infty\}$，其 **$c$-变换** 定义为
$$
\varphi^c(y) = \inf_{x\in X}\bigl( c(x,y) - \varphi(x) \bigr), \qquad y\in Y .
$$
对称地定义 $\psi^c(x)=\inf_{y\in Y}(c(x,y)-\psi(y))$。若 $\varphi=\varphi^{cc}$（记 $\varphi^{cc}=(\varphi^c)^c$），则称 $\varphi$ 为 **$c$-凹函数**。

> **💡 直观理解（类比）**：把 $\varphi$ 想成“发货方给每个源点开的报价”，$c$-变换 $\varphi^c(y)$ 就是“在不超过运价的前提下，接收点 $y$ 最多愿意补多少”——它由源点报价与运价共同敲定。$c$-凹则要求这份报价“自洽”：用 $c$-变换还能原样换回来（$\varphi=\varphi^{cc}$）。就像凸分析里 $f^{**}=f$ 的凸闭包，$c$-凹函数正是对偶意义下“已经收拾干净”的势。

> **常见坑/容易混**：不要把 $c$-变换与 Legendre 共轭混为一谈——二者只在二次代价 $c=|x-y|^2/2$ 下相差一个二次项（见 3.2 与进阶习题 309）。一般 $c$-凹函数不保证连续，甚至可取 $-\infty$；本课程总在 $c$ 下半连续、下有界的前提下讨论。

### 2.9 离散最优输运：线性规划 与 Sinkhorn 迭代

> **动机与来龙去脉**：现实中我们手里往往不是连续分布，而是一列样本点（像素、网格、客户）。此时 $\mu=\sum_{i}a_i\delta_{x_i}$、$\nu=\sum_{j}b_j\delta_{y_j}$，运输计划退化成一张**成本矩阵 $C_{ij}=c(x_i,y_j)$** 上的联合分配矩阵 $\Gamma=(\Gamma_{ij})$，最优传输变成标准的**线性规划**——目标 $\min\sum_{ij}C_{ij}\Gamma_{ij}$、行和列约束 $\sum_j\Gamma_{ij}=a_i$、$\sum_i\Gamma_{ij}=b_j$、$\Gamma_{ij}\ge0$。这正是 Kantorovich 1940 年代与经济学的“运输问题”（transportation problem）直接衔接之处；加上 2.7 的熵正则后用 Sinkhorn 矩阵缩放可高效逼近。**不掌握这张“离散＋LP＋Sinkhorn”的版图，你在应用里最常见的“给两组点求对应”就无从下手。**

**定义 2.10**（离散最优输运的线性规划形式）设 $\mu=\sum_{i=1}^m a_i\delta_{x_i}$、$\nu=\sum_{j=1}^n b_j\delta_{y_j}$ 为离散概率测度，成本矩阵 $C_{ij}=c(x_i,y_j)$。离散 Kantorovich 问题为
$$
\min_{\Gamma\in\mathbb{R}^{m\times n}_+} \sum_{i,j} C_{ij}\Gamma_{ij}
\quad\text{s.t.}\quad \sum_j\Gamma_{ij}=a_i,\ \sum_i\Gamma_{ij}=b_j ,
$$
其中 $\Gamma_{ij}$ 表示从 $x_i$ 运往 $y_j$ 的质量。这是严格意义上的线性规划（目标线性、约束线性、$\Gamma\ge0$），其最优解存在性、对偶与求解都有经典理论。

**Sinkhorn 迭代**：加入熵正则后，缩放迭代化为
$$
\Gamma^{(k)}_{ij} = u^{(k)}_i K_{ij} v^{(k)}_j, \qquad
u^{(k+1)}_i = \frac{a_i}{\sum_j K_{ij}v^{(k)}_j}, \qquad
v^{(k+1)}_j = \frac{b_j}{\sum_i K_{ij}u^{(k+1)}_i},
$$
其中 $K_{ij}=e^{-C_{ij}/\varepsilon}$。这套“行—列交替归一”正是 Sinkhorn（矩阵平衡）算法，收敛到熵正则化问题的最优解。

> **💡 直观理解（类比）**：离散版本像一本“发货总账”：横轴是源、纵轴是目的地，$C_{ij}$ 是单价、$\Gamma_{ij}$ 是发货量，行告诉每个源发出去多少、列告诉每个目的地收进来多少，都要和给定预算 $a,b$ 对账。Sinkhorn 的交替归一就像不断“这行超支了就按比例减小、那列不足就整体补点”去配平整张账本，越配越接近最优。

### 2.10 W1 与 Kantorovich–Rubinstein 对偶

> **动机与来龙去脉**：当运价就是直线距离 $c=|x-y|$ 时，最优传输出现一个漂亮的降维：**Wasserstein-$1$ 距离根本不需要显式构造耦合**，而等于
> $$
W_1(\mu,\nu) = \sup_{\|f\|_{\mathrm{Lip}}\le 1}\Bigl( \int f\,d\mu - \int f\,d\nu \Bigr),
$$
> 其中 $\|f\|_{\mathrm{Lip}}=\sup_{x\ne y}|f(x)-f(y)|/|x-y|$。这就是 **Kantorovich–Rubinstein 对偶**（定理 3.6）。它把“搬多远”重新表述成“所有 1-Lipschitz 测试函数 $f$ 能在两类分布上认出多大的积分差”，是分布鲁棒优化、GAN 判别器的 Lipschitz 稳定化（如 WGAN）、以及文本/图像比对（Word Mover's Distance 等）的直接基石。**没有它，$W_1$ 只在抽象耦合层面定义，既难算也难以解释。**

> **💡 直观理解（类比）**：$W_1$ 的 KR 对偶好比“用一块带坡度的地形去量两堆货”：$f$ 是坡度不超过 1（1-Lipschitz）的地形，$\int f\,d\mu-\int f\,d\nu$ 是“两堆货摊在同一块地形上读得的平均海拔差”。上确界在所有合法地形上取——最妙的是它恰好等于“一比一搬运、每段不多绕路”的最小土方量。关于它与分位数公式 $W_1=\int|F_\mu-F_\nu|$ 的等价性，见示例与习题。
...
> **💡 直观理解（类比）**：Kantorovich 对偶性说"最小运费"等于"最大可行的双边估价和"（$\varphi,\psi$ 满足 $\varphi(x)+\psi(y)\le c(x,y)$）。这好比"买家能接受的最低价"与"卖家肯报的最低价"在均衡处恰好相等——Min-Max 交换（Fenchel-Rockafellar）保证这两个"视角"给出同一个数字，是线性规划强对偶在最优传输上的具体化身。
...
> **💡 直观理解（类比）**：Brenier 定理说：只要起点分布是"连续的"（绝对连续），用平方距离搬运时最优方案必然"一对一地推着走"，且推力是一个凸函数 $\phi$ 的梯度 $T=\nabla\phi$。就像一块被均匀施加压力的连续橡胶垫，表面张力决定每个点朝哪个斜度滑过去——"凸势 + 梯度"这一结构既保证唯一，又给出可解的 Monge-Ampère 方程。

**推论 3.3**（最优传输映射的存在唯一性）在 Brenier 定理的条件下，存在唯一的 $W_2$-最优传输映射 $T = \nabla \phi$，其中 $\phi$ 是凸函数。

> **💡 直观理解（类比）**：推论 3.3 把唯一性钉死：只要源分布连续，$W_2$ 最优搬运映射就存在且唯一，且必是某凸函数的梯度。就像两条确定质量的流水线之间，最省的"逐份对搬"方案只有一套——不会出现两个不同的完美搬运法让我们纠结。
...
> **💡 直观理解（类比）**：Benamou-Brenier 公式断言 $W_2^2$ 恰等于"把 $\mu_0$ 变成 $\mu_1$ 的所有水流曲线里动能积分的最小值"。它的证明把最优映射 $T$ 按线性插值铺成一段演化 $(T_t)$，并验证这股流既满足质量守恒、又恰好达到动能下界——相当于亲手把"几何搬运"与"流体最小作用量"两本账对上了号，这正是后面数值求解与理解 Wasserstein 空间几何的钥匙。
...
> **💡 直观理解（类比）**：定理 3.5 说 $W_2$ 空间是一条"会凹下去的滑梯"：连接 $\mu_0$ 与 $\mu_1$ 的测地线 $\mu_t=((1-t)\mathrm{id}+tT)_\#\mu_0$ 之间"插值不走弯路"，且到固定目标 $\nu$ 的平方距离沿测地线保持凸性。就像在凹形轨道上滑行，中途点到两端越来越近、越来越均匀——"平坦 + 凸曲率"正是 $W_2$ 空间能让梯度流"放心下山"的几何保证。

### 3.5 Kantorovich–Rubinstein 定理

**定理 3.6**（Kantorovich–Rubinstein）设 $X=Y=\mathbb{R}^n$，$\mu,\nu\in\mathcal{P}_1(\mathbb{R}^n)$，代价 $c(x,y)=|x-y|$。则
$$
W_1(\mu,\nu) = \sup_{\|f\|_{\mathrm{Lip}}\le 1}\Bigl( \int f\,d\mu - \int f\,d\nu \Bigr),
$$
其中 $\|f\|_{\mathrm{Lip}}=\sup_{x\ne y}\frac{|f(x)-f(y)|}{|x-y|}$。

**证明要点**：对 $c=|x-y|$，$c$-凹势恰为 1-Lipschitz 函数，且 $f^c=-f$（见 2.8、3.2 与进阶习题 308）。把 Kantorovich 对偶（定理 3.1）中的 $\psi$ 记为 $-\varphi=-f$，对偶目标退化为上述上确界。分三步：
1. **化 $c$-凹（可行化）**：由定理 3.1，势可取 $\psi=\varphi^c$，且最优对总可代以 $c$-凹对。对 $c=|\cdot-\cdot|$，$c$-凹的 $\varphi$ 正是 1-Lipschitz，并满足 $\varphi^c=-\varphi$。
2. **代入对偶**：目标 $\int\varphi\,d\mu+\int(-\varphi)\,d\nu=\int\varphi\,d\mu-\int\varphi\,d\nu$，约束化为 $\|\varphi\|_{\mathrm{Lip}}\le1$。
3. **可达性**：用一族 1-Lipschitz 逼近函数（如 $\varphi_\varepsilon(x)=\bigl(|x-x_0|^2+\varepsilon^2\bigr)^{1/2}$ 的规范化组合）让上确界随 $\varepsilon\to0$ 逼近 $W_1$。$\square$

> **💡 直观理解（类比）**：KR 对偶让 $W_1$ 从“算一场搬运”变成“测一份落差”：在两堆分布上铺一块坡度不超过 1 的地形 $f$，比较它们在 $f$ 上读出的平均海拔差；能找到的最陡合法落差正是 $W_1$。这也是 WGAN 中判别器要约束成 1-Lipschitz 的数学根源——限制斜率就是限制“测距的诚实度”。

### 3.6 最优传输映射的次微分表示与均衡（互补）条件

**定理 3.7**（最优支撑、均衡与次微分表示）设 $c$ 下半连续、下有界，$\mu,\nu$ 为概率测度，Kantorovich 问题有解。
1. （**均衡/互补松弛**）存在最优对 $(\varphi,\psi)$ 使 $\psi=\varphi^c$；任一最优计划 $\gamma$ 支撑在互补松弛集
$$
E=\bigl\{(x,y)\in X\times Y : \varphi(x)+\psi(y)=c(x,y)\bigr\}
$$
上，即最优质量只在“价差恰好为零”的配对间流动。
2. （**梯度/次梯度表示**）对二次代价 $c=|x-y|^2/2$ 且 $\mu$ 绝对连续，存在凸势 $\phi$ 使唯一的最优计划为 $\gamma=(\operatorname{id}\times\nabla\phi)_\#\mu$，且 $\gamma$-a.e. 有 $y\in\partial\phi(x)$；$\mu$ 的绝对连续性保证 $\partial\phi(x)=\{\nabla\phi(x)\}$，从而 $T(x)=\nabla\phi(x)$ 是单值映射。

**证明要点**：均衡条件来自最优性——若有 $\varphi(x)+\psi(y)<c(x,y)$ 的支撑质量，微调那里的分配会降低原始目标，矛盾（即互补松弛）。次微分表示部分是 3.2/3.3 结论在次微分语言下的整理：最优对经 $c$-凹闭包后，靠最优条件得到 $y\in\partial\phi(x)$，而 $\mu$ 绝对连续使 $\partial\phi$ 单值化为 $\nabla\phi$，最终 $\gamma=(\operatorname{id}\times\nabla\phi)_\#\mu$。$\square$

> **💡 直观理解（类比）**：定理 3.7 把最优计划理想化成一张“紧贴等势线”的转账单：只有满足 $\varphi(x)+\psi(y)=c(x,y)$ 的“零价差”配对才被允许交易（均衡），其余作废；当代价为平方时，这张转账单自动退化成一张“梯度表” $y=\nabla\phi(x)$——从“计划”收敛到“函数”，正是 Brenier 定理（3.2）在次微分语言下的翻版。
...
> **💡 直观理解（类比）**：一维最优传输把"按比例对号入座"这件小事做绝了：先把两堆点在数轴上各自从小到大排队（分位数 $F^{-1}$），再按同一份"排位第几"一一配对 $T=F_\nu^{-1}\circ F_\mu$。这样任何 $p$ 阶代价都能精确算成"两列队伍同一序号之间的距离差之和"。对高斯而言，这个结果更是清爽——均值差与标准差差分别"平方相加"，搬家距离全由"中心差一项 + 胖瘦差一项"两块拼成，像把两个钟形图案错开又拉伸后量出的总位移。
...
> **💡 直观理解（类比）**：高斯到高斯的最优搬法是"一步仿射齐活"：用线性映射 $T(x)=Ax+b$ 先按矩阵 $A$ 把"胖瘦椭圆"拉伸转正，再平移中心 $b$。$W_2^2$ 拆成"中心差 $|m_1-m_2|^2$"与"椭圆胖瘦差（迹项）"两笔账——前者是搬哪、后者是整不整型，就像把一枚被捏成椭圆的徽章，经旋转拉伸后精确扣到目标椭圆的位置上。
...
> **💡 直观理解（类比）**：当两堆高斯都是"球形"（协方差各向同性 $\sigma^2 I$）时，拉伸只按同一比例缩放每个方向的胖瘦，$W_2^2$ 变成"中心差平方 + $n\times$ 半径差平方"。好比两颗大小不同但都是正圆的气球，搬过去只需沿每个坐标同比例吹大/缩小 $(\sigma_1-\sigma_2)^2$，还乘上维度数 $n$——因为 $n$ 个方向各要单独"吹一次"。
...
> **💡 直观理解（类比）**：一维离散最优传输就像一个"两支队伍按名次配对"的规则：把源点从小到大排好队、目标点也从小到大排好队，然后第 $i$ 小的源点搬到第 $i$ 小的目标点。直觉上"不交叉"最省力——若让两队交叉搬运（大搬小、小搬大）反而多绕路。这条"保序配对"正是 $W_2$ 最小化的钥匙，$W_2^2$ 则是所有配对距离平方和再取平均。
...
**习题 301**（c-变换）定义函数 $\varphi^c(y)$（$c$-变换），并说明它如何把双边对偶问题化成单变量势的形式 $\sup_\varphi(\int\varphi\,d\mu+\int\varphi^c\,d\nu)$。

**习题 302**（c-凹）给出 $c$-凹函数的定义（$\varphi=\varphi^{cc}$），并指出二次代价 $c(x,y)=|x-y|^2/2$ 下 $c$-凹函数与凸函数之间的联系。

**习题 303**（离散最优输运）写出离散 Monge–Kantorovich 问题的线性规划形式：成本矩阵 $C_{ij}$、分配矩阵 $\Gamma_{ij}$、以及行和列的两个边缘约束。

**习题 304**（Sinkhorn 迭代）写出熵正则化离散最优输运中 Sinkhorn 的矩阵缩放迭代式（$u,v$ 的交替更新与 $\Gamma$ 的参数化 $u_iK_{ij}v_j$）。

**习题 305**（Kantorovich–Rubinstein）陈述代价 $c=|x-y|$ 时 $W_1$ 的 Kantorovich–Rubinstein 对偶公式（用 1-Lipschitz 函数的积分差上确界）。
...
**习题 306**（进阶·c-变换）证明对任意 $\varphi$ 有 $\varphi^{cc}\le\varphi$，并证明等号成立当且仅当 $\varphi$ 本身是 $c$-凹的。

**习题 307**（进阶·离散LP对偶）写出离散运输问题的对偶线性规划（引入势变量 $f_i,g_j$），并依据强对偶写出 $\min_\Gamma\sum C_{ij}\Gamma_{ij}=\max_{f_i+g_j\le C_{ij}}\sum f_i a_i+\sum g_j b_j$。

**习题 308**（进阶·KR）对 $c=|x-y|$ 证明任一 1-Lipschitz 函数 $f$ 满足 $f^c=-f$，并由此推导 $W_1(\mu,\nu)=\sup_{\|f\|_{\mathrm{Lip}}\le1}\int f\,d\mu-\int f\,d\nu$。

**习题 309**（进阶·c-变换与凸共轭）设 $c(x,y)=|x-y|^2/2$，令 $\phi(x)=|x|^2/2-\varphi(x)$，建立 $c$-变换与 Legendre–Fenchel 共轭的联系，证明 $\varphi^c(y)=|y|^2/2-\phi^*(y)$。
...
**习题 310**（挑战·均衡与c-凹）证明：对下半连续、下有界的代价 $c$（Polish 空间），存在 $c$-凹最优对 $(\varphi,\psi)=(\varphi^{cc},\varphi^c)$ 使 Kantorovich 对偶的上确界达到，且最优计划支撑在互补松弛集 $c-\varphi-\psi=0$ 上。

**习题 311**（挑战·Sinkhorn 收敛）证明 Sinkhorn 迭代收敛到熵正则化离散最优输运的唯一解，并讨论当 $\varepsilon\to0$ 时正则化解如何逼近原始 Kantorovich 解。

**习题 312**（挑战·KR 严格证明）严格证明 Kantorovich–Rubinstein 定理：由 $c=|x-y|$ 的对偶势（1-Lipschitz）出发，构造最优测试函数族并说明上确界可达。
...
301. $\varphi^c(y)=\inf_x(c(x,y)-\varphi(x))$。它给出与 $\varphi$ 配对的接收点势 $\psi=\varphi^c$，把双边对偶目标化为单变量 $\sup_\varphi(\int\varphi\,d\mu+\int\varphi^c\,d\nu)$，无需对两个函数整体搜索。
302. $c$-凹即 $\varphi=\varphi^{cc}$（$c$-共轭不动点）。对 $c=|x-y|^2/2$，令 $\phi=|x|^2/2-\varphi$，则 $c$-凹等价于 $\phi$ 凸，即 $\varphi=|x|^2/2-\phi$（$\phi$ 凸）——这正是 3.2 的桥梁。
303. 设 $\mu=\sum_ia_i\delta_{x_i}$、$\nu=\sum_jb_j\delta_{y_j}$，$C_{ij}=c(x_i,y_j)$：$\min_{\Gamma\ge0}\sum_{ij}C_{ij}\Gamma_{ij}$，约束 $\sum_j\Gamma_{ij}=a_i$、$\sum_i\Gamma_{ij}=b_j$，$\Gamma_{ij}$ 为从 $x_i$ 到 $y_j$ 的质量。
304. $\Gamma_{ij}^{(k)}=u_i^{(k)}K_{ij}v_j^{(k)}$，$K_{ij}=e^{-C_{ij}/\varepsilon}$；交替更新 $u_i^{(k+1)}=a_i/\sum_jK_{ij}v_j^{(k)}$、$v_j^{(k+1)}=b_j/\sum_iK_{ij}u_i^{(k+1)}$，即“行—列”交替归一。
305. $W_1(\mu,\nu)=\sup_{\|f\|_{\mathrm{Lip}}\le1}(\int f\,d\mu-\int f\,d\nu)$，$\|f\|_{\mathrm{Lip}}=\sup_{x\ne y}|f(x)-f(y)|/|x-y|$。
...
306. 对任意 $y$：$\varphi^{cc}(x_0)=\inf_y(c(x_0,y)-\varphi^c(y))\le c(x_0,y)-\varphi^c(y)\le c(x_0,y)-(c(x_0,y)-\varphi(x_0))=\varphi(x_0)$；等号对所有 $x_0$ 成立当且仅当 $\varphi=\varphi^{cc}$，即 $\varphi$ 自身 $c$-凹。
307. 对偶引入 $f_i,g_j$，$f_i+g_j\le C_{ij}$，目标 $\sum_if_ia_i+\sum_jg_jb_j$；由线性规划强对偶得 $\min_\Gamma\sum C_{ij}\Gamma_{ij}=\max_{f+g\le C}\sum f_ia_i+\sum g_jb_j$，即离散 Kantorovich 对偶。
308. $f$ 为 1-Lipschitz：$f(x)-f(y)\le|x-y|$ $\Rightarrow f^c(y)=\inf_x(|x-y|-f(x))\ge -f(y)$；取 $x=y$ 得反向，故 $f^c=-f$。在对偶 $\sup_{\varphi+\psi\le|x-y|}\int\varphi\,d\mu+\int\psi\,d\nu$ 中取 $\psi=-\varphi=-f$（$f$ 为 1-Lipschitz）即得 KR 公式。
309. $c=|x-y|^2/2=|x|^2/2-\langle x,y\rangle+|y|^2/2$，故 $\varphi^c(y)=\inf_x(|x-y|^2/2-\varphi(x))=|y|^2/2+\inf_x(\phi(x)-\langle x,y\rangle)=|y|^2/2-\sup_x(\langle x,y\rangle-\phi(x))=|y|^2/2-\phi^*(y)$。
...
310. 由 $c$ 下半连续、下有界与耦合集紧性，原始最小化可达；以 $c$-凹闭包 $(\varphi^{cc},\varphi^c)$ 代替不增大目标且满足 $\varphi^{cc}+\varphi^c\le c$，故上确界可达并等于原值。若最优计划在 $c-\varphi-\psi>0$ 处集中会增大目标，故其必集中于互补松弛集 $\{c-\varphi-\psi=0\}$（均衡条件）。
311. 熵正则项严格凸，问题有唯一解；其解具有对角缩放结构 $\Gamma=uKv$（Kuhn–Tucker 型条件导出）。Sinkhorn 的交替归一等价于在 KL 几何上交替做 Bregman 投影，由 Bregman 投影定理收敛到该唯一解。$\varepsilon\to0$ 时正则化解族有界、可取子列，再经定界估计其子列极限即为原始 Kantorovich 解。
312. 由 308 知 $f$ 取 1-Lipschitz 且 $f^c=-f$。用一族规范化 1-Lipschitz 逼近函数（如 $\varphi_\varepsilon(x)=(|x-x_0|^2+\varepsilon^2)^{1/2}$ 的组合）使 $\int f_\varepsilon\,d\mu-\int f_\varepsilon\,d\nu\to W_1(\mu,\nu)$，并结合强对偶即得上确界可达、公式成立。
...
> 最优传输的主线是"用最小的代价，把一堆质量从一处搬运到另一处"：它始于 1781 年蒙日的搬土问题，困顿于"确定性映射"这一过强的约束近两百年；1942 年坎托罗维奇用联合分布的线性规划框架一举解套，1970 年代被瓦瑟斯坦重命名为距离、1980 年代由布雷尼耶把传输出映射还原为凸函数梯度并接入 Monge-Ampère 方程，最终在熵正则化与机器学习中迎来复兴。本章沿"蒙日之问→线性规划之解→几何/分析之身→现代复兴"的线索，追述这个从工程问题长成跨学科枢纽的全程。

### 7.1 蒙日之问：1781 年的搬土问题

- **蒙日的搬土问题**：加斯帕尔·蒙日 1781 年提出经典问题——把一堆土搬入一条沟，若点 $\mathbf{x}$ 处的土运到 $\mathbf{y}$，希望最小化总搬运费用
$$\int c(\mathbf{x},T(\mathbf{x}))\,d\mu(\mathbf{x}),$$
经典情形费用取欧氏距离。他要的是一个确定性传输映射 $T$。
- **"过硬的约束"**：蒙日的表述限定"一一对应、映射推送"，这一确定性约束在测度带有原子时极易不存在解——变分解的可解性迟迟无法确认，问题因而搁置了近两个世纪。
- **意义**：蒙日的问题本质是"最优化"与"测度"的第一次相遇，却因要求"每个点都被明确指派"而在数学上无从下手——它正是最优传输漫长蛰伏期的见证。

### 7.2 坎托罗维奇的线性规划革命：1942 年

- **坎托罗维奇的松弛**：1942 年，莱昂尼德·坎托罗维奇在苏联研究战时资源最优配置时，把问题改写为在联合分布（耦合测度）$\gamma$ 上的线性规划：允许把质量"拆散"再重新分配，只需最小化
$$\int c(\mathbf{x},\mathbf{y})\,d\gamma(\mathbf{x},\mathbf{y}),$$
并约束 $\gamma$ 的边缘分布恰为 $\mu$ 与 $\nu$。
- **对偶的钥匙**：线性规划的对偶理论给出 Kantorovich 对偶性——最小搬运费等于"势函数差"的最大下确界，极大简化了计算并带来可证性。
- **经济学与诺奖**：坎托罗维奇因最优配给与线性规划在经济中的应用获 1975 年诺贝尔经济学奖——最优传输由此从一个几何问题变成经济学与运筹学的共享语言。

### 7.3 Wasserstein 距离：从"费用"到"度量"

- **瓦瑟斯坦的命名**：若费用取距离的 $p$ 次幂，最小代价恰在概率测度空间上定义一个度量——Wasserstein 距离 $W_p$（由瓦瑟斯坦提出并命名，故又译 Vaserstein 距离）。它是"两个分布差多远"的几何化答案，兼有度量性质与凸性。
- **坎托罗维奇-鲁宾斯坦定理**：$W_1$ 可被刻画为有界 Lipschitz 函数对偶空间上的范数——$W_1$ 由此与双重 Lipschitz 结构、概率集中等现代工具接轨（第 51 级随机过程将展开）。
- **意义**：Wasserstein 距离让最优传输从"一个费用最小的搬运"升级为"分布空间上的一种几何"，为梯度流、测地线与机器学习打下基础。

### 7.4 布雷尼耶定理：映射之回归

- **布雷尼耶的主定理**：1980 年代布雷尼耶证明：当费用为平方欧氏距离且原测度绝对连续（并排除共同原子）时，最优传输映射几乎处处存在且唯一，并可写成某个凸函数 $u$ 的梯度 $T=\nabla u$——被松弛掉的"确定性映射"在平方代价情形被并无代价地找回。
- **Monge-Ampère 方程**：若上述映射光滑，其 Jacobian 满足 Monge-Ampère 方程
$$\det(D^2u)=\frac{\mu}{\nu\circ T},$$
最优传输从而与非线性椭圆偏微分方程深度绑定（第 49 级偏微分方程将展开）。
- **意义**：布雷尼耶以"凸函数梯度"给出最优传输的内在几何刻画，使这门学问同时成为凸几何、PDE 与概率论三者的交汇点。

### 7.5 动力学视角：Benamou-Brenier 与 Wasserstein 测地线

- **流体力学表示**：2000 年贝纳穆与布雷尼耶把问题改写为在"密度 + 速度场"上的最小作用量问题，Wasserstein 距离由此等于从一个分布到另一个分布的最省作用量；其最优轨迹即 Wasserstein 测地线。
- **连续性方程**：测地线满足连续性方程 $\partial_t\mu+\nabla\cdot(\mu v)=0$，把最优传输纳入流体力学与变分法共用框架。
- **意义**：动力学视角使"搬土"上升为"流随时间演化"，是后来梯度流（暗物理）与热方程解释的起点。

### 7.6 熵正则化与 Sinkhorn：大规模计算的突围

- **熵正则化的痛点破解**：给耦合分布加熵惩罚、并把传输费用化为带熵的双目标，问题变成可高效求解的矩阵重标定——这正是 Sinkhorn 迭代（1970 年代提出的矩阵比例调整算法）乘势而起的原因，它把大规模离散最优传输变为可并行化的线性代数。
- **从理论到机器学习**：熵正则化让最优传输具备可微、凸、可采样等现代优化所必需的属性，因而成为图像配准、生成模型（如 WGAN、OT-based 聚类）与数据同化的标准部件（第 99 级统计学习、第 100 级机器学习将展开）。
- **意义**：如果说蒙日被"确定性"卡住、坎托罗维奇被"可计算性"卡住，那么熵正则化恰是在可计算性上为最优传输打开了闸门。

### 7.7 最优传输与几何：曲率、测地与梯度流

- **Wasserstein 空间上的几何**：概率测度空间装上 Wasserstein 距离后带有丰富的凸性与测地结构；延莱特等用这一结构研究热方程的 Wasserstein-测地凸性，得到 Fokker-Planck 方程的最优传输解释。
- **曲率视角**：最优传输与测度度量空间上的曲率条件（如曲率维数条件 CD）结合，成为现代度量几何研究 Ricci 曲率下界的工具。
- **意义**：最优传输由此与几何分析、度量测度空间理论互相反哺，并把"搬土"这一直觉永久地写进了现代几何的词典。

### 7.8 从古典到现代：最优传输的领域版图

- **多文明与多学科的再汇聚**：从蒙日的军事工程、坎托罗维奇的计划经济、瓦瑟斯坦/布雷尼耶的概率与几何、到 Sinkhorn 的算法复兴，最优传输将**工程、经济、概率、几何、PDE 与机器学习**熔为一炉，其领域版图横跨 20 世纪各阶段的数学范式。
- **与其他分支交汇**：在东尼·芒契奥、Villani 等人的系统整理下，最优传输被写成标准教科书（如《Topics in Optimal Transportation》），把 Knott-Smith / 配岩定理、梯度流、相对熵等的整体图景固定下来。
- **意义**：如今的最优传输已是"运输问题"与"分布几何"的复合体，也是分析、几何与概率在新时代最有生机的交叉口之一（第 99、100 级将展开具体应用）。

### 7.9 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1781 年 | 蒙日 | 提出搬土问题（确定性传输映射） |
| 1942 年 | 坎托罗维奇 | 联合分布线性规划框架与对偶定理 |
| 1975 年 | 坎托罗维奇 | 因最优配置获诺贝尔经济学奖 |
| 1970 年代 | 瓦瑟斯坦 | Wasserstein 距离之命名与度量性质 |
| 1980 年代 | 布雷尼耶 | 最优传输映射 = 凸函数梯度（Brenier 定理） |
| 1987—90 年 | Knott、Smith 等 | 平方代价的配岩/对偶刻画 |
| 2000 年 | 贝纳穆、布雷尼耶 | 流体力学表示与 Wasserstein 测地线 |
| 1970 年—今 | Sinkhorn 等 | 熵正则化与矩阵重标定算法 |
| 2003—09 年 | Villani、Ambrosio 等 | 系统教科书与测度度量空间理论 |

### 7.10 小结

> 最优传输的历史，是一部"约束过强则无解、松弛有度则重获生机"的辩证法史。蒙日 1781 年以"确定性映射"立下难题，因这个约束太过儒家而蛰伏近两百年；坎托罗维奇 1942 年用联合分布的线性规划给它松绑，引入对偶并赢得诺奖；瓦瑟斯坦把"最优费用"升格为分布空间上的距离，布雷尼耶又在平方代价情形下把"映射"优雅地找回、并把问题接入 Monge-Ampère 方程；最终熵正则化与 Sinkhorn 迭代让大规模计算成为可能，启动机器学习的狂欢。整条路线贯穿着一个不变的母题——在"转移"与"变换"之间寻找最优、在理想解与可计算解之间取舍——而这正是最优传输能够成为分析、几何、概率与工程共通枢纽的根本原因。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对最优传输核心概念（对偶、Wasserstein 距离、凸共轭）的肌肉记忆。小节间相互独立，可按需跳读。

### 8.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到最优传输语境的追问上。

| 步骤 | 行动 | 最优传输中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 源/目标 $\mu,\nu$ 在哪个空间、是否绝对连续？代价是平方还是欧氏距离？求映射 $T$、计划 $\gamma$ 还是距离 $W_p$？ |
| ② 计划 | 选择可行工具、预估方向 | 用 Kantorovich 对偶还是 Monge-Ampère？用凸共轭 $f^*$ / $c$-变换还是 Brenier 势？用 Sinkhorn 还是分位公式？先想清哪条路最省力。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 保留符号：$T_\#\mu=\nu$、$\gamma\in\Pi(\mu,\nu)$、$\det D^2\varphi$、凸共轭公式 $f^*(y)=\sup_x(\langle x,y\rangle-f(x))$ 别出错。 |
| ④ 回看 | 核对结果、检验合理性 | 结果是 min 还是 sup？对偶与原问题数值是否相等？$W_p$ 的对称、三角不等与量纲是否合理？ |

> 💡 **策略演示**：$\mu=\delta_0$、$\nu=\delta_2$（$\mathbb{R}$ 上），求 $W_2(\mu,\nu)$。
> ① 理解：两个 Dirac 点，唯一耦合是 $\gamma=\delta_{(0,2)}$。② 计划：直接用 $W_2^2=\int|x-y|^2\,d\gamma$。③ 执行：$W_2^2=4$，故 $W_2=2$。④ 回看：等于两点欧氏距 $|0-2|=2$，且 $W_2(\delta_0,\delta_0)=0$ 自洽，合理。
...
1. 当 $\mu$ 为 Dirac 而 $\nu$ 非 Dirac 时，Monge 问题可能无可行映射。（　）
2. Kantorovich 问题的可行耦合集 $\Pi(\mu,\nu)$ 恒非空。（　）
3. $W_p(\mu,\nu)=0$ 当且仅当 $\mu=\nu$。（　）
4. 对 $c=|x-y|$，$W_1(\mu,\nu)=\sup_{\|f\|_{Lip}\le1}(\int f\,d\mu-\int f\,d\nu)$ 成立（KR 对偶）。（　）
5. Legendre-Fenchel 共轭定义为 $f^*(y)=\sup_x(\langle x,y\rangle-f(x))$。（　）
6. 平方代价下 Brenier 最优映射为某个凸函数的梯度。（　）

**选择**：

7. Kantorovich 对偶把原始最小问题换成一个（　）问题。
   - A. 最大化势函数积分 $\int\varphi\,d\mu+\int\psi\,d\nu$　B. 最小化势积分　C. 求特征值　D. 热流
8. $W_2$ 在一维的分位公式是（　）。
   - A. $W_2^2=\int_0^1\bigl(F_\mu^{-1}(t)-F_\nu^{-1}(t)\bigr)^2dt$　B. $W_2^2=\int|F_\mu-F_\nu|$　C. $W_2^2=\max|F_\mu-F_\nu|$　D. $W_2^2=0$
9. Brenier 凸势 $\varphi$ 满足的方程是（　）。
   - A. 热方程　B. 波方程　C. $\det D^2\varphi=$ 密度比　D. $\Delta\varphi=0$

### 8.3 分步骤工作示例（Worked Examples）

> 每道示例严格套用四步流程，并在「回看」中给出一条易错点或检验法。

**示例 S1：计算二次型函数的 Legendre-Fenchel 共轭**

> **问题**：求 $f(x)=\tfrac12 x^2$（$x\in\mathbb{R}$）的凸共轭 $f^*(y)$。

- ① 理解：对象是凸函数，目标是按定义做 sup 优化。
- ② 计划：$f^*(y)=\sup_x(xy-f(x))$，对 $x$ 求驻点。
- ③ 执行：

$$
f^*(y)=\sup_{x\in\mathbb{R}}\bigl( xy-\tfrac12 x^2\bigr)=\tfrac12 y^2 .
$$

  驻点条件 $y-x=0$ 即 $x=y$。
- ④ 回看：**检验法**：凸共轭对合 $f^{**}=f$（凸闭包意义）——$(f^*)^*(x)=\tfrac12 x^2=f(x)$。**易错点**：$f^*$ 的定义域与值域别颠倒，这里二次型自对偶。

**示例 S2：一维分位公式求 $W_2$**

> **问题**：$\mu=\tfrac12\delta_0+\tfrac12\delta_1$，$\nu=\delta_{1/2}$（$\mathbb{R}$ 上），求 $W_2(\mu,\nu)$。

- ① 理解：一维，用分位／单调重排公式最省力。
- ② 计划：$F_\mu^{-1}(t)=0$（$t<1/2$）与 $1$（$t\ge1/2$），$F_\nu^{-1}(t)=1/2$ 常数；套 $W_2^2=\int_0^1(F_\mu^{-1}-F_\nu^{-1})^2dt$。
- ③ 执行：

$$
W_2^2=\int_0^{1/2}\bigl(0-\tfrac12\bigr)^2dt+\int_{1/2}^{1}\bigl(1-\tfrac12\bigr)^2dt=\tfrac14\;\Rightarrow\; W_2(\mu,\nu)=\tfrac12 .
$$

  检验：$\mu$ 的两份质量各搬 $1/2$ 到 $1/2$，平方代价 $=\tfrac12(\tfrac12)^2\cdot2=\tfrac14$，一致。
- ④ 回看：**检验法**：直接搬算与分位公式殊途同归。**易错点**：分位公式里平方在差之后、再积分，别把次序搞反。

**示例 S3：用 KR 对偶验证 $W_1$**

> **问题**：对 $\mu=\delta_0$、$\nu=\delta_2$（代价 $c=|x-y|$），求 $W_1$ 并用 KR 对偶复核。

- ① 理解：一维两点，$W_1$ 即一对一搬距。
- ② 计划：$W_1=|0-2|=2$；对偶取 $f(x)=x$（1-Lipschitz）读取同值。
- ③ 执行：

$$
W_1(\delta_0,\delta_2)=\sup_{\|f\|_{Lip}\le1}\bigl(f(0)-f(2)\bigr)=2 .
$$

  虽 $f(x)=x$ 时 $f(0)-f(2)=-2$，但取 $f$ 再适当取符号（如 $-x$）即得 $\sup=2$。
- ④ 回看：**检验法**：等于单原子一笔搬运 $|0-2|=2$，与 KR 对偶一致。**易错点**：KR 中上确界对所有 1-Lipschitz $f$ 取，别只盯某一个线性 $f$。

**示例 S4：均匀区间的最优传输映射与 $W_2$**

> **问题**：$\mu=\mathbf1_{[0,1]}(x)dx$、$\nu=\mathbf1_{[2,3]}(y)dy$，平方代价，求最优映射 $T$ 与 $W_2$。

- ① 理解：两个均匀密度、绝对连续，可用 Brenier 单调映射。
- ② 计划：最优化即单调重排 $T(x)=F_\nu^{-1}(F_\mu(x))$，这里是大平移。
- ③ 执行：

$$
T(x)=F_\nu^{-1}(F_\mu(x))=x+2,\qquad W_2^2=\int_0^1 2^2\,dx=4\;\Rightarrow\; W_2=2 .
$$

  检验 $T_\#\mu$：$T$ 把 $[0,1]$ 平移成 $[2,3]$，质量守恒成立。
- ④ 回看：**检验法**：每点搬距恒为 $2$，$W_2=\sqrt{\int_0^1 2^2\,dx}=2$。**易错点**：$W_2$ 是"先平方积分再开根"，别把积分内直接开根写混。

### 8.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：用 Sinkhorn 迭代（小 $\varepsilon$）对两组离散点算熵正则运输计划，观察 $\varepsilon\to0$ 时逼近硬最优。
- **聚焦 P2（跨章节联系）**：把第 55 级「凸分析」的 Legendre-Fenchel 共轭与 $c$-变换相联系，对 $c=|x-y|^2/2$ 显式写出 $\varphi^c$ 与 $\varphi^*$ 的关系。
- **聚焦 P3（探索项目）**：对两个给定一维分布写排序/分位算法，数值验证 $W_1=\int|F_\mu-F_\nu|$ 与 $W_2^2=\int_0^1(F_\mu^{-1}-F_\nu^{-1})^2$ 两种公式。
...
- **自测 1**：用一句话说明 Kantorovich 松弛相对 Monge 问题的优势。
- **自测 2**：解释 KR 对偶把"搬多远"翻译成了什么样的优化问题。

**$\bullet$ 技能题**

- **自测 3**：求 $\mu=\delta_a$、$\nu=\delta_b$（$\mathbb{R}$ 上）的 $W_2$，写完整四步。
- **自测 4**：计算 $f(x)=\tfrac12|x|^2$ 的共轭，并给出「回看」结论。
- **自测 5**：$\mu=\mathbf1_{[0,1]}dx$、$\nu=\mathbf1_{[3,4]}dy$，平方代价，求最优映射与 $W_2$。
- **自测 6**：对 $\mu=\tfrac12\delta_0+\tfrac12\delta_1$、$\nu=\delta_{1/2}$ 用分位公式求 $W_2$（复核 S2）。

**$\blacklozenge$ 综合题**

- **自测 7**：写一段含「表格 + 一句话结论」的分析，比较 KR 对偶（$W_1$）与 Kantorovich 对偶（一般代价）的适用代价与几何含义。
- **自测 8**：给出 $m=n=2$ 的离散最优传输线性规划的原问题与对偶势，并各写一组可能最优解。

### 8.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（Dirac 到非 Dirac 无一一可行映射）。2. **正确**（含乘积测度 $\mu\otimes\nu$）。3. **正确**（性质 2.4）。
> 4. **正确**（KR 对偶，定理 3.6）。5. **正确**（共轭定义）。6. **正确**（Brenier）。
>
> **选择**：7. A　8. A　9. C。

> **自测**：
> - 自测 1：把"点对点硬搬"换成"计划/耦合"的线性优化，可行集恒非空且凸，便于对偶与 LP 求解。
> - 自测 2：把 $W_1$ 变成在 1-Lipschitz 测试函数上最大化 $\int f\,d\mu-\int f\,d\nu$。
> - 自测 3：$W_2=|a-b|$；唯一耦合 $\delta_{(a,b)}$，平方代价开根。
> - 自测 4：$f^*=\tfrac12 y^2$；回看 $(f^*)^*=f$。
> - 自测 5：$T(x)=x+3$，$W_2=3$（每点搬距 $3$）。
> - 自测 6：与 S2 相同，$W_2=\tfrac12$。
> - 自测 7：表格列"代价 / 对偶形式 / 适用场景"；一句话：KR 对偶是 $c=|x-y|$ 特化的 Kantorovich 对偶，用 1-Lipschitz 势刻画。
> - 自测 8：设成本 $C=\begin{pmatrix}0&2\\2&0\end{pmatrix}$、边际 $a=(1,0)$、$b=(0,1)$；对偶 $\max\sum_i a_i u_i+\sum_j b_j v_j$，约束 $u_i+v_j\le c_{ij}$；最优 $\Gamma_{12}=1$，总代价 $=2$。