...
> **一句话大白话**：函数越光滑，越能用低次多项式逼近得既快又好——光滑程度与收敛速度形成一门交易：导数越多越顺，逼近误差掉得越狠。
>
> **小例子**：若 $f\in C^r$，Jackson 定理给出 $E_n(f)\le \frac{C_r}{n^r}\omega(f^{(r)},1/n)$（$E_n$ 为 $n$ 次最佳逼近误差）；反过来 Bernstein 定理保证逼近过快必然意味着光滑，二者互为"逼近速度—光滑度"黑白配。

## 一、定理介绍

> **前置依赖**：连续模（连续性模）的定义与性质、三角多项式与 Fourier 级数、正卷积核（Jackson 核）的矩估计、Bernstein 三角多项式导数不等式、级数一致收敛与逐项求导。
...
- [Jackson 定理与 Bernstein 定理（第104级-逼近论）](../第104级-逼近论/Jackson定理与Bernstein定理.md)：与本条目为同一定理，另收录于第104级-逼近论，可交叉参考。
