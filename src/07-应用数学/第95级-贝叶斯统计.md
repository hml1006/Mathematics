...
## 从"更新你对一枚硬币的怀疑"说起：贝叶斯统计要解决的根问题

> 在摆弄 $\pi(\theta\mid\boldsymbol{x})$ 之前，先想清楚一个问题：**人为什么需要贝叶斯统计？**

**一个真实的生活场景（第一步：直觉）**

想象你第一次拿到一枚来历不明、可能做过手脚的硬币。你心里先有个大致的怀疑：**它多半还算是公平的**（正反面各约 1/2），但也不敢打包票。你开始抛它，每次落地都是一条新证据——抛了 20 次，正面竟有 16 次。这时你的怀疑会自然地上调："天呐，这枚硬币可能真偏向正面。"注意这件事的奇妙之处：你的判断不是"非黑即白"地跳变，而是**由最初的那句"我估计它是公平的"（先验），一点一点被 20 次抛掷的证据（似然）改写成一个新的判断（后验）**。贝叶斯统计，就是把这个人人都有的"见证据、改信仰"的本能，写成一条严谨的数学规则。

**它和最主流的"频率学派"差在哪（第二步：要解决的问题）**

在第 89 级的经典推断里，"参数 $\theta$"被看成**一个固定的、未知的常数**——估计不过是"把它猜准一点"，推断则反复强调"重复做实验会怎样"。这套框架很严谨，却对"我只有这一次观察，现在就想下结论"的情形无能为力：你不能实实在在把实验重做一千遍。1783 年前后完成的贝叶斯（Thomas Bayes）遗稿先回答了"如何由结果反推原因的概率"，拉普拉斯（Laplace）随后独立重建并发扬光大，甚至用它反推砝码质量的误差；20 世纪中叶 Metropolis–Hastings 与 Gibbs 等 MCMC 算法的出现，又把后验里那些难积分的式子变成了可计算的东西（详见第 2.11–2.13 节）。贝叶斯统计的**核心问题**浓缩成一句：**在只能拿到一次数据的情况下，如何用先验 + 证据，理性地把"关于未知参数的相信程度"更新成后验，并直接给出关于它的概率陈述？**

**看待本章的三种眼光（第三步：正式定义前的准备）**

- **固化"更新"的眼光**：先建立贝叶斯公式与先验/似然/后验三件套，弄清"证据如何改写信仰"（第 2.1–2.4 节）。
- **算得动的眼光**：用共轭先验让更新变成记账，用后验做估计、可信区间、Bayes 因子与预测（第 2.5–2.10 节）。
- **算不动的眼光**：后验复杂到没有解析式时，用 MCMC（MH、Gibbs）采样兜底（第 2.11–2.13 节），并用第 2.14 节清点它与频率学派的异同。

读完这一节，请记住的不是公式，而是：**贝叶斯的灵魂，在于把"相信"写成可更新、可计算的概率分布——证据一来，信仰就按规则调整。** 后面每个概念，都是这一条的展开。

> **🧠 一句话记住贝叶斯统计的"出生证"**
>
> 贝叶斯统计源起于贝叶斯、拉普拉斯"由果推因"的工作，它解决的**根本问题**是：
>
> $$ \boxed{\text{在给定证据后，如何把对未知参数的先验信念理性更新为后验，并直接给出关于它的概率陈述？}} $$
>
> 它把"参数"当随机变量、把不确定性变成概率本身，靠 MCMC 实现可计算。

---

## 开始之前

**本章在讲什么**：本章研究"用先验 + 数据更新对未知参数的相信"这一任务的**一条主线**。你会先吃透贝叶斯公式与先验/似然/后验这三件套，再用共轭先验把更新化简、"算出"后验点估计（均值/中位数/众数）、可信区间与后验预测分布，随后用 Bayes 因子做模型选择、用分层模型处理"借力"，最后当后验没有解析式时，用 Metropolis–Hastings 与 Gibbs 采样构成的 MCMC 就地取材地采样逼近。第 2.14 节再用一张对照表收尾贝叶斯与频率学派之别。

**为什么要学它**：第 88/89 级（概率与统计推断）给了你分布、条件概率与"频率式"推断，但频率预设"参数是固定常数"并依赖重复抽样解释；本章翻转前提、把参数当随机变量，用一种更贴近人脑的"证据更新"方式重做估计、区间与检验。它是第 92/94 级（回归分析、非参数统计）背后的推断哲学的第二极，也是第 93 级（时间序列，Kalman 滤波本质是递推贝叶斯滤波）、第 99 级（统计学习理论，正则化可看作对参数施加高斯先验的 MAP 估计）的深层底座。

**开始前请确认你还记得**：

- 条件概率与全概率公式、贝叶斯公式（见第 88 级）：$p(A\mid B)=\frac{p(B\mid A)p(A)}{p(B)}$，它是本章密度版直接的离散原型。
- 概率密度与归一化（见第 88 级）：$f(x\mid\theta)$ 对 $x$ 积分得 1，但对 $\theta$ 未必——这是"似然不是密度"的关键辨析。
- Beta、正态、Gamma 等常用分布及其共轭搭配（见第 88/89 级）：共轭先验、后验都建立在这些分布族上。
- 最大似然估计与 Fisher 信息（见第 89 级）：理解 Jeffreys 先验 $\pi(\theta)\propto\sqrt{\det I(\theta)}$、以及 MAP 与 MLE 的关系所需。
- 期望与积分、多元积分（见第 11/13 级）：后验均值、预测分布与边际似然本质上都是积分。
...
> **💡 直观理解（类比）**：贝叶斯把参数 $\theta$ 当"嫌疑人"，先验就是"还没审问前的怀疑（猜测 $\theta$ 多大）"；看到证据（数据）后用似然更新怀疑，得到"审问后的后验"。相比频率学派反复强调"重复做实验的概率"，贝叶斯更符合人脑的直觉——证据来了，信仰就调整。
...
> **💡 直观理解（类比）**：先验就像"事前打分的赌注"——在掷出骰子之前，你已经按常识或历史给每种可能性一个信心额度。无信息先验是"一律平等、谁也不偏袒"（均匀），共轭先验是"选了能算到底的便利形态"，Jeffreys 先验则是"换个单位描述也照样公平"（变换不变），主观先验是"专家的偏心但有理"。

### 2.3 似然函数

**似然函数** $f(\boldsymbol{x} \mid \theta)$ 是在给定参数 $\theta$ 下观测到数据 $\boldsymbol{x}$ 的概率密度（或质量）函数。它不是 $\theta$ 的概率密度——它通常不满足归一化条件。

> **💡 直观理解（类比）**：似然函数像"把参数当已知、问数据成不成立"——固定 $\theta$，看"在它之下，我们眼前的这套数据有多凑巧"。它不像密度那样对 $\theta$ 积分得 1，所以只是"像不像"的一把尺子，跟先验相乘（加权）才是完整的证据。
...
> **💡 直观理解（类比）**：三种估计就像"面对一座后验山的三种选点策略"——均值是"整座山的重心"（整体权衡，适合平方损失）、中位数是"把山左右的体积对半分的位置"（抗极端瘤子，适合绝对损失）、众数是"站在山顶"（最可能的单点，适合0-1损失）。你犯哪种错越贵，就越选那种策略来报数。
...
> **💡 直观理解（类比）**：可信区间像"从后验山上圈一片面积为 $1-\alpha$ 的地盘，直接宣告参数大概率在这里"——它是参数随机性下的直接概率陈述，好比"押注坐标落在这片地里的概率是 $1-\alpha$"。普通可信区间像"从左边起圈面积"，而 HPDI 像"先挑最高最陡的峰谷圈，剩下的边缘低洼处才圈"，所以同样面积下手感最短最集中，信息最划算。

> **常见坑**：不要把"可信区间"和"置信区间"混为一谈。置信区间（频率派）说的是"反复抽样本，约 $1-\alpha$ 比例的区间会盖住真值"——是对**抽样过程**的频率陈述；而可信区间直接说"$\theta$ 落在这个区间的概率是 $1-\alpha$"——是对**参数**的概率陈述，二者含义完全不同，绝不能互换解释。另外，与单峰后验相比，多峰后验的"普通可信区间"可能把两个峰都圈进去却把中间的凹部也框了进来，此时应优先用 HPDI 或对每个峰分别报告区间。
...
> **💡 直观理解（类比）**：Bayes因子就像"两个嫌疑人背对背，看眼前证据更指向谁"——它直接给出"数据支持模型 1 是模型 2 的几倍"，是数据自己投给两个模型的得票比。它天然偏爱简单模型：复杂模型要把先验的赌注摊薄到海量参数上，解释同一堆数据时赢面反而变薄，这就是内置的"奥卡姆剃刀"。

> **常见坑**：Bayes 因子分子分母各自都需要 **proper（可归一的）先验**。若某个模型用的是无信息/异常先验（积分发散的 improper prior），其边际似然 $m(\boldsymbol{x})$ 无穷或不唯一，Bayes 因子就会"失去定义"。所以凡涉及 Bayes 因子比较模型，都要先确认每个模型下的先验是 proper 的；反之，若只求后验分布本身，异常先验通常仍可用（因为 $\pi(\theta\mid x)\propto f(x\mid\theta)\pi(\theta)$ 里的常数会被约掉）。这是一个非常容易被忽略的边界情形。
...
> **💡 直观理解（类比）**：后验预测分布像"请一大群按后验权重配票的专家分别给新观测做预测，再把它们的预言加权汇总"——它不押注在某个单一 $\hat{\theta}$ 上，而是把每个 $\theta$ 的预测 $f(\tilde{x}\mid\theta)$ 按后验概率 $\pi(\theta\mid\boldsymbol{x})$ 平均起来，彻底抹平"参数到底是多少"的不确定，只留下一份对下一个观测的完整概率赌注。
...
> **💡 直观理解（类比）**：分层贝叶斯像"全校各班的成绩既能各报各的，又分享一个共同的年级水平"——每个班有自己的参数 $\theta_i$，但这些 $\theta_i$ 又都从同一个超先验"抽"出来。小班数据少、判断不牢时，会顺着超参数被"拉向年级总体水平"（借力），类似差班与好班的差距在共享的基准上被适度拉近；代价是太拼凑时可能磨平真实差异。

### 2.11 马尔可夫链蒙特卡洛

**马尔可夫链蒙特卡洛**（Markov Chain Monte Carlo, MCMC）是一类从后验分布中采样的方法。当后验分布没有解析形式时，MCMC通过构建以目标分布为不变分布的马尔可夫链来生成样本。

> **💡 直观理解（类比）**：MCMC 像"在山里撒一群忘了路的游客，让他们在山上瞎逛，逛久了你数他们站在哪里的次数，就得到山的高低轮廓"——后验复杂到算不出积分时，你不去硬解公式，而是让一条马尔可夫链在这座后验山上随机游走，链走到高点就"容易赖着不走"、走到低点就"容易脚滑溜走"，于是长期停驻的频率恰好描出分布形状。
...
> **💡 直观理解（类比）**：MH 算法像"山上盲人探路"——闭着眼前一步，看脚下这一步是往高处还是低处迈：往上走就欣然接受；往下走则按"落差有多大、随机骰子乐不乐意"来决定是否将就着走，好偶尔能翻过山口到另一座更高的峰。接受率 $\alpha=\min\{1,\text{目标比值}\times\text{步法修正}\}$ 就是在"多爬山（探索）"和"别乱跑（保持正态）"之间找平衡，最终让脚印密度恰好描出分布这个"山形"。
...
> **💡 直观理解（类比）**：Gibbs 采样像"多人轮流照着队友现况顺应调整再走一步"——当多变量联合分布难倒我们时，它一次只更新一个分量，用它"给定其他分量当前的取值"的条件分布来抽新值，依次轮流刷新。单独看每一步都有点像"邻居动了我这格怎么落"，但整轮转下来，各变量交替跟着彼此的最新状态走，轨迹的长期分布正是联合后验，而且不必操心接受的取舍。
...
> **💡 直观理解（类比）**：这个定理是贝叶斯派的"总流水线"——先验是零时的信任档案，似然是新到的目击证词，两者相乘再做一遍"归一化"（除以证据），就刷新成看完证词后的信任档案。它把"经验更新信念"这件事用概率密度精确写死：证据怎么改怀疑，后验就怎么动，其余贝叶斯推断全是在这条流水线上加环节。
...
> **💡 直观理解（类比）**：这条定理相当于给"先验能偷懒到哪种程度"发了张族谱——只要似然属于指数族，就先验必有搭档，且更新"只需把数据的充分统计量打进先验参数里"（像记账一样原地加一行）。以 Bernoulli 为例，抛硬币新得正面就 $+1$ 到 $\alpha$、反面就 $+1$ 到 $\beta$，后验仍是 Beta；省去每轮巨额积分正是共轭家族的好处。
...
> **💡 直观理解（类比）**：Doob 定理像"验钞机越验越准"——只要数据真按某个 $\theta_0$ 生成，随着样本增多，后验这座山的质量就不断往真值 $\theta_0$ 脚下堆，最终几乎全压在那里，别的地方几乎被清空。唯一的例外是"几个参数说明完全一样的分布、根本分不清谁真谁假"这样的不可识别情形，而这种模糊在开局前（按先验）就已经没概率了。它给了贝叶斯学派"数据多了必逼近真相"的安心丸。
...
> **💡 直观理解（类比）**：这条定理说"只要链走得到山上每个角落（不可约）又不原地打转固步自封（非周期），游走轨迹的长期统计就必然逼近目标分布（全变差收敛），样本均值就逼近后验期望（遍历大数定律）"——好比一遍遍在山里转悠，只要哪儿都去得、不困在单一沟壑，脚印的频率最终精确刻画出山的整体形状，采样结果就能安心当作后验的替身来算期望。
...
> **💡 直观理解（类比）**：这个定理把"两个模型谁更配得上数据"翻译成"两个模型各自对数据的总期待值之比"——每个模型的边际似然，就是它在先验标尺下"预料到会看到这堆数据"的平均可能性，需要把参数全部积掉。它同时解释了奥卡姆剃刀：复杂模型的先验赌注被摊到高维参数上，平均下来"料中数据"的能力反而低，所以更简单的模型更容易在这场投票里胜出，而算这笔积分通常难到要动用数值近似。
...
> **💡 直观理解（类比）**：这个例子像"用一杆默认天平校准硬币偏心率"——先验是先放上去的砝码（$\text{Beta}(\alpha,\beta)$ 自带 $\alpha+\beta$ 个"虚拟抛掷"），数据是后补的真实抛掷。抛 7 正 3 反后，天平读数从 0.5 被拨到 0.667（向数据靠拢）；而先验砝码加得越重（如 $\text{Beta}(10,10)$），结果的惯性就越大，读数被拉回 0.5 的倾向越明显。这直观演示了"先验强度 vs 数据量"谁说话算数。
...
> **💡 直观理解（类比）**：这个例子像"两位老师各自报一个估测值，再按各自信任度加权取折中"——样本均值 $\bar x$ 由数据出题、先验均值 $\mu_0$ 由经验给稿，精确度（方差倒数）就是各自的可信权重：数据准或样本多就多听数据的，先验自信（$\tau_0^2$ 小）就多留一份先验的保守。加了信息先验后，95% 可信区间从 $(9.62,10.50)$ 收窄到 $(9.68,10.40)$，正说明先验像给判断加了根"镇尺"，把预测约束得更紧。

---

## 4.S Stewart 微积分风格补充：现实应用、分步例题与解题策略

> 贝叶斯的公式落到现实里，都是从同一个动作出发：**抓住一条新证据，把"我对未知参数的多大把握"按规则改写一遍**。本节沿用 Stewart 的"策略 → 例题 → 分步解 → 回读现实"四步法，把共轭先验更新、后验均值、Bayes 因子这三样核心工具放进抛硬币、实验估计、模型选择等真实场景里跑一遍。每道例题先给**解题策略**，让你看清"这道题该往哪个方向想"，再给出**每步都解释"为什么"**的分步解答。

### 🎯 本级解题策略（Principles of Problem Solving）

Stewart 式的贝叶斯题，讲究"先写对三件套，再谈怎么算"。做一道贝叶斯题，请按这四步走：

- **第一步 · 立三件套**：先写清先验 $\pi(\theta)$、似然 $f(\boldsymbol{x}\mid\theta)$、后验核 $\pi(\theta\mid\boldsymbol{x})\propto f(\boldsymbol{x}\mid\theta)\pi(\theta)$。写清"手里有什么、拿到的证据是什么"，比急着套共轭公式更重要。
- **第二步 · 认共轭**：看似然与先验是不是"一家人"——二项 $\times$ Beta 得 Beta 后验、正态 $\times$ 正态得正态后验、Poisson $\times$ Gamma 得 Gamma 后验。能对上，更新就只是改参数的记账。
- **第三步 · 算并报数**：后验均值 $=\mathbb{E}[\theta\mid\boldsymbol{x}]$ 常是关键点估计；正态情形它等于"先验均值与样本均值的精度加权平均"。要报告"不确定性"就用可信区间，要比较模型就用 Bayes 因子 $BF_{12}=\frac{f(\boldsymbol{x}\mid M_1)}{f(\boldsymbol{x}\mid M_2)}$。
- **第四步 · 回读并审先验**：小样本下后验对先验敏感，要把结论说成"在给定这个先验之下"；比较模型时务必确认每个模型的先验是 proper（可归一）的，否则 Bayes 因子失去定义。

> **一句话带走**：贝叶斯的分步解就是"立三件套 → 认共轭 → 加权算后验 → 回读结论"——"数据信几分、旧信念留几分"，靠的是先验与似然的精度之比，别急着只背公式。

---

### 例题 A · 一枚硬币的"嫌疑指数"：Beta-二项共轭更新

> **🎯 建模策略**：把硬币偏心程度 $p$ 当随机变量，先验取 $\text{Beta}(\alpha,\beta)$。抛 $n$ 次得到 $y$ 次正面后，后验还是 Beta：$\text{Beta}(\alpha+y,\beta+n-y)$。**策略：先认共轭，再把（先验参数 + 证据计数）逐项相加，最后用后验均值 $\frac{\alpha+y}{\alpha+\beta+n}$ 报数。**

**问题**：先验取 $\text{Beta}(2,2)$（不偏不倚、也留余地），连抛 20 次硬币得到 16 次正面。求后验分布 $p\mid y$ 并给出后验均值。

**解**：

**步骤 1：认共轭**  二项似然 $y|p\sim\text{Bin}(n,p)$ 共轭 Beta 先验，后验仍为 Beta。

**步骤 2：更新参数**  $\alpha'=2+16=18$，$\beta'=2+(20-16)=6$，故 $p|y\sim\text{Beta}(18,6)$。

*（为什么只需加计数：Beta 的 $\alpha,\beta$ 正是"成功/失败的虚拟计数"，数据每来一条正面就加给 $\alpha$，反面就加给 $\beta$——共轭让更新变成纯记账。）*

**步骤 3：后验均值**  $\mathbb{E}[p\mid y]=\frac{\alpha'}{\alpha'+\beta'}=\frac{18}{24}=0.75$。

> **回读现实**：一个人一开始认为硬币基本公平（先验峰约 0.5），看到 20 次里 16 次正面后，把"偏心程度期望"改到 0.75——既没死守 0.5，也没完全追着样本比例 0.8，而是被先验往回拉了一小截。这就是贝叶斯"证据更新信仰"的活样板。

---

### 例题 B · 数据与经验如何"分工"：后验均值=精度加权平均

> **🎯 建模策略**：正态-正态情形 $x_i|\mu\sim N(\mu,\sigma^2)$、先验 $\mu\sim N(\mu_0,\tau_0^2)$，后验均值是"先验均值与样本均值的精度加权平均"：$\mu_n=\frac{\tau_0^{-2}}{\tau_0^{-2}+n\sigma^{-2}}\mu_0+\frac{n\sigma^{-2}}{\tau_0^{-2}+n\sigma^{-2}}\bar{x}$。**策略：先算两条精度，再按精度比分配权重。**

**问题**：先验 $\mu\sim N(10,1)$（精度 $\tau_0^{-2}=1$），抽 $n=4$ 个样本、$\sigma^2=16$（即 $\sigma^{-2}=\frac1{16}$）、样本均值 $\bar x=12$。求后验均值 $\mu_n$ 与后验方差。

**解**：

**步骤 1：算两条精度**  先验精度 $W_{\text{prior}}=\tau_0^{-2}=1$；样本精度 $W_{\text{data}}=n\sigma^{-2}=4\times\frac1{16}=0.25$。

**步骤 2：加权平均**
$$
\mu_n=\frac{1}{1+0.25}\times10+\frac{0.25}{1+0.25}\times12=\frac{10+3}{1.25}=10.4
$$

**步骤 3：后验方差**  后验精度 $W_n=1+0.25=1.25$，故 $\text{Var}(\mu_n)=\frac{1}{1.25}=0.8$。

*（为什么数据权重只有 0.25/1.25：先验自信（$\tau_0^2=1$ 很"紧"）＋样本仅 4 个且方差大，所以先验几乎"拽住"了 4/5 的分量，数据只添了 1/5。）*

> **回读现实**：手头证据噪声大（$\sigma^2=16$）又只有 4 次观测，最稳的办法不是纯听 $\bar x=12$，而是让"经验底子"先把结论按在 10.4 附近。这正说明贝叶斯在样本小、数据糙时，把先验当成一根"镇尺"来用——结论更稳，代价是更顺着先验说话。

---

### 例题 C · 硬币是否被做了手脚？用 Bayes 因子比较两个模型

> **🎯 建模策略**：比较 "公平 $H_0:p=0.5$" 与 "可偏 $M_2:p\sim\text{Uniform}(0,1)$" 两个模型，用 Bayes 因子 $BF_{12}=\frac{m(\boldsymbol{x}\mid M_1)}{m(\boldsymbol{x}\mid M_2)}$。**策略：分别算两个模型下的边际似然，再取比值；$BF<1$ 说明数据偏爱模型 2。**

**问题**：抛 20 次硬币得 16 次正面。模型 $M_1$ 固定 $p=0.5$，模型 $M_2$ 给 $p$ 均匀先验 $\text{Beta}(1,1)$。计算 Bayes 因子并解读。

**解**：

**步骤 1：$M_1$ 下的边际似然**  $m(y\mid M_1)=\binom{20}{16}(0.5)^{20}=\frac{4845}{2^{20}}=\frac{4845}{1048576}\approx0.00462$。

**步骤 2：$M_2$ 下的边际似然**  这是 Beta-二项预测：
$$
m(y\mid M_2)=\binom{20}{16}\frac{B(17,5)}{B(1,1)}=\binom{20}{16}\frac{16!\,4!}{21!}=\frac{1}{21}\approx0.0476
$$

**步骤 3：取比值**  $BF_{12}=\frac{0.00462}{0.0476}\approx0.097$，即 $BF_{21}\approx10.3$。

*（为什么 $M_2$ 赢：把 $p$ 视为可变参数后，$p=0.5$ 附近的证据并不特别难得；而固定 $p=0.5$ 时，一下抛出 16/20 正面这种事概率极低。"用一个参数解释这些数据"自然比"硬塞固定值"更省力。）*

> **回读现实**：数据强烈偏向"硬币可能不公"（$BF_{21}\approx10.3$，即约十倍证据）。但要注意：$M_2$ 的均匀先验是 proper 的，Bayes 因子才有意义；若换成发散的 improper 先验，这个比值就会失去定义——这是比较模型时最容易踩的坑。
...
**应用题 31**（Beta-二项后验更新—贝叶斯）：先验取 $\text{Beta}(2,2)$，连抛 10 次硬币得到 7 次正面。写出后验分布与后验均值。

**应用题 32**（正态-正态精度加权—贝叶斯）：先验 $\mu\sim N(10,1)$，抽 $n=9$ 个样本、$\sigma^2=9$、$\bar x=12$。计算后验均值 $\mu_n$ 与后验方差，并说明先验与数据各占约多少比重。

**应用题 33**（Bayes 因子比较—贝叶斯）：抛 20 次硬币得 16 次正面。模型 $M_1$ 固定 $p=0.5$，模型 $M_2$ 给 $p$ 均匀先验。计算 Bayes 因子并说明数据更支持哪个模型。

**应用题 34**（后验预测—贝叶斯）：沿用例题 A 的后验 $p\sim\text{Beta}(18,6)$，求"再抛一次是正面"的后验预测概率，并与后验均值比较。
...
1. **Beta-二项后验更新** 二项似然共轭 Beta 先验，后验 $\text{Beta}(\alpha+y,\ \beta+n-y)=\text{Beta}(2+7,\ 2+10-7)=\text{Beta}(9,5)$；后验均值 $\mathbb{E}[p\mid y]=\frac{9}{9+5}=\frac{9}{14}\approx0.643$。既偏向样本比例 $0.7$，又被先验"我在 0.5"轻轻拉回，正是"旧信念＋新证据"的折中。
2. **正态-正态精度加权** 先验精度 $\tau_0^{-2}=1$，样本精度 $n\sigma^{-2}=9\times\frac19=1$，后验精度 $=1+1=2$，故 $\mu_n=\frac{1}{2}\times10+\frac{1}{2}\times12=11$，后验方差 $\text{Var}=\frac12=0.5$。先验与数据精度相同（都为 1），故各占约 50% 比重——两股信息几乎平分话语权。
3. **Bayes 因子比较** $M_1$ 下 $m(y\mid M_1)=\binom{20}{16}(0.5)^{20}=\frac{4845}{1048576}\approx0.00462$；$M_2$（均匀先验 $\text{Beta}(1,1)$）下 $m(y\mid M_2)=\binom{20}{16}\frac{B(17,5)}{B(1,1)}=\frac{1}{21}\approx0.0476$。故 $BF_{12}=\frac{0.00462}{0.0476}\approx0.097$，$BF_{21}\approx10.3$——数据强烈支持"$p$ 可变"的模型 $M_2$，即硬币很可能不公。
4. **后验预测** 再抛一次为正面的后验预测概率等于后验均值 $=\frac{\alpha'}{\alpha'+\beta'}=\frac{18}{18+6}=\frac{18}{24}=0.75$。它与后验均值完全一致：对单个新观测，Beta 后验的平均成功概率就是后验均值本身，二者指向同一个"信心数"。

---

## 7. 数学史（History of Mathematics）

> 贝叶斯统计回答一个极具实用感的追问："在拿到数据之前我已有一些认识，数据本身又能纠正这种认识——那么我应如何把两者合起来，得到对未知量更聪明的判断？"它的根基是一枚硬币两面的概率规则——贝叶斯定理，由托马斯·贝叶斯提出、理查德·普莱斯刊行，随后被拉普拉斯独立而大力地推进；19 世纪末到 20 世纪中叶，频率学派对"主观先验"的质疑一度把它压到边缘，靠杰弗里斯的辩护留住火种；真正让它复兴的是共轭先验与自 1950 年代起的马尔可夫链蒙特卡洛（MCMC）计算革命，使高维后验"算得出、可收敛"。本章沿"逆概率→先验之争→计算突破→当代复兴"的主线，追溯贝叶斯统计两百多年的沉浮。

### 7.1 逆概率的伏笔：条件概率与乘法法则

- **条件概率先到场**：早在贝叶斯之前，条件概率、乘法法则与"在已知 B 时 A 的似然"已有雏形（棣莫弗的《机遇论》等），为"由结果反推原因"埋下了计算的种子。
- **"逆概率"指什么**：人们关心的是反问题——给定观测的结果，去推断"产生它的原因/参数"的概率。这类"Inverse probability"问题正是贝叶斯定理将要回答的对象。
- **时代背景**：17—18 世纪概率论尚是"赌博算术"与天文误差处理的工具，"由果溯因"的需求随观测科学的发展日益迫切，理论原料已经齐备。

### 7.2 贝叶斯定理的诞生：贝叶斯与普莱斯

- **贝叶斯的论文**：托马斯·贝叶斯生前未发表相关成果，其友人理查德·普莱斯整理成《论求解一个机遇问题中的学说》并于 1763 年由皇家学会刊印——这被视为贝叶斯定理的正式出处。
- **定理的内容与含义**：它把"先验"与"似然"合成"后验"：$P(H\mid D)\propto P(D\mid H)P(H)$，给出了依据新证据更新信念的标准规则。在当时它更多被当作条件概率的一个推论，尚未形成学科。
- **为何留名**：贝叶斯的贡献不在于发现了一条公式（条件概率本就存在），而在于"把先验信念与数据结合起来更新认知"这一哲学立场——它奠定了后世整套贝叶斯推断路线的地基。

### 7.3 拉普拉斯：独立的推手与逆概率的正名

- **拉普拉斯的独立推进**：法国学者拉普拉斯在 18 世纪末至 19 世纪初大量运用"逆概率"解决天文、测量与司法问题，独立识别了先验与后验的结构，并给出"日出问题"等著名算例，把贝叶斯思想从一条孤立公式发展成可操作的推断方法。
- **连续先验与正态联系**：他还把贝叶斯思想与误差、正态分布联系，建立了基于后验的估计与概率陈述——后世称"贝叶斯-拉普拉斯方法"，两人共同构成这条路径的两大源头。
- **一度的主流**：在 19 世纪上半叶，"逆概率"其实相当流行，是许多应用问题的自然语言，直至频率学派崛起才被啁啾。

### 7.4 先验的主观之疑：费舍尔与内曼-皮尔逊的挑战

- **缺陷在哪里**：贝叶斯方法需要一个先验分布，而先验常带主观色彩；不同人给不同先验，结论就不同——这在追求"客观"的统计者眼中是致命伤。
- **频率学派的替代**：费舍尔以极大似然与显著性检验、内曼与皮尔逊以假设检验与置信区间建立另一套"不涉先验、只看频率"的框架，主张长期频率下的错误率控制才是科学判据。
- **贝叶斯被边缘化**：在整个 20 世纪上半叶，频率学派成为统计教学与工业的标准语言，贝叶斯方法从主流退居研究前沿，只在一小批人手中延续。

### 7.5 杰弗里斯的隔离辩护

- **为贝叶斯正名的孤军**：1939 年哈罗德·杰弗里斯出版《概率论》，系统为贝叶斯推断的正当性辩护，回应频率学派关于先验任意性的批评，并提出"杰弗里斯先验"等不变量先验。
- **哲学与数学双线**：他把概率解释为"逻辑地表达信念程度"，主张先验反映合理信念而非随意拍脑袋，为贝叶斯保留了理论上的体面。
- **薪火的延续**：杰弗里斯的工作当时未能翻盘，但为二战后贝叶斯的复兴保存了完整的理论内核与文献，数学家、哲学家与工程师得以在日后重拾。

### 7.6 共轭先验：让后验"算得动"的巧计

- **计算的瓶颈**：后验分布需对先验乘似然再归一化，多数情形积不出闭式，贝叶斯长期受限于"算不出来"这一技术短板。
- **共轭的便利**：选择与似然"配套"的先验（共轭先验），使后验与先验同属一族分布——如 Beta-二项、Gamma-泊松、正态-正态。这样后验解析可求、更新只是参数递推，极大便利了经典教科书与简单应用。
- **限度**：共轭先验只在指数族等特定场合可用，一旦模型复杂（高维、层次、非共轭）便失效——它只是辅助，真正「解锁」贝叶斯还需要更根本的计算工具。

### 7.7 计算革命（上）：Metropolis 与 Hastings

- **蒙特卡洛思想**：二战后计算机兴起，用随机抽样近似积分成为现实，为"算后验"提供了新路径。
- **Metropolis 算法**：1953 年 Metropolis 等在洛斯阿拉莫斯提出以接受-拒绝狩猎构造马尔可夫链、使其收敛到目标分布的 Metropolis 算法，最初用于物理统计力学。
- **Hastings 的推广**：1970 年 Hastings 把它推广为 Metropolis–Hastings，允许非对称提议分布，使其成为普适到几乎任何后验都可抽样的引擎——这是贝叶斯计算史上最关键的支点。

### 7.8 计算革命（下）：Gibbs 抽样与 Gelfand-Smith

- **Gibbs 抽样**：1984 年杰曼兄弟提出 Gibbs 抽样，按坐标轮换从条件分布采样，把高维后验抽样拆成一个个一维条件抽样，极大简化了层次模型与图模型的计算。
- **Gelfand 与 Smith**：1990 年 Gelfand 与 Smith 的经典论文把 MCMC 系统引入贝叶斯统计，宣告"MCMC 时代"降临，高维、非共轭、层次贝叶斯从此普遍可计算。
- **革命的连锁**：成套的收敛诊断（Gelman-Rubin 等）、自适应方法、切片采样与哈密顿蒙特卡洛随之上位，贝叶斯真正从"理念"变成"工程"。

### 7.9 贝叶斯的当代复兴与应用

- **与机器学习的合流**：贝叶斯方法在机器学习时常以"贝叶斯网络、贝叶斯推断、贝叶斯优化"出现，spike-and-slab 先验、层次模型与变分推断（VI）等工具持续丰富工具箱。
- **两大统计范式的并立**：如今贝叶斯与频率学派并列为现代统计两大支柱，先验设计、收敛诊断、变量选择等问题成为前沿课题。
- **跨领域落地**：自然语言处理、计算机视觉、医学决策、工程推断与不确定性量化都大量使用贝叶斯——本文件的贝叶斯定理、先验/后验、共轭先验与 MCMC 正是这套血脉在当代的浓缩。

### 7.10 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 约 17—18 世纪 | 棣莫弗等 | 条件概率、乘法法则，孕育逆概率思想 |
| 1763 年 | 贝叶斯（普莱斯整理） | 《论……学说》刊行，贝叶斯定理诞生 |
| 18 世纪末—19 世纪初 | 拉普拉斯 | 独立推进逆概率，天文与司法应用 |
| 19 世纪上半叶 | 逆概率方法 | 一度成为应用主流的时期 |
| 20 世纪上半叶 | 费舍尔、内曼、皮尔逊 | 频率学派主导，贝叶斯被边缘化 |
| 1939 年 | 杰弗里斯《概率论》 | 系统为贝叶斯推断辩护 |
| 1953 年 | Metropolis 等 | Metropolis 算法 |
| 1970 年 | Hastings | 推广为 Metropolis–Hastings |
| 1984 年 | 杰曼兄弟 | Gibbs 抽样 |
| 1990 年 | Gelfand、Smith | 把 MCMC 引入贝叶斯统计，开启计算时代 |
| 现代 | 变分推断、哈密顿蒙特卡洛等 | 与机器学习深度交融 |

### 7.11 小结

> 贝叶斯统计的历史是一部"理念先行、计算殿后"的复兴史：贝叶斯与拉普拉斯确立了用先验乘似然得到后验的推断路线，费舍尔与内曼-皮尔逊的挑战让"主观先验"沦为被攻击的软肋，杰弗里斯以一己之力守卫其逻辑正当性，共轭先验化解了部分算力危机，而 Metropolis-Hastings 与 Gibbs 抽样经由 Gelfand-Smith 引爆计算革命，终使贝叶斯从理论边缘重回统计与机器学习的中心。纵观全程，贝叶斯的核心价值始终是它敢于面对"不确定性也是可更新的信念"，而在计算终于跟上的今天，这种敢为终于获得了与数学和工程相匹配的回报。
