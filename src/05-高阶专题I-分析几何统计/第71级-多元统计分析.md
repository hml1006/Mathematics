...
10. 掌握样本主成分特征值与特征向量的渐近分布理论（PCA 分布理论）及其在推断中的应用。
...
**动机与来龙去脉**：一元时，我们用一个均值 $\mu$ 和一个方差 $\sigma^2$ 就足以描述随机变量 $X \sim N(\mu,\sigma^2)$。可一到真实问题，一个研究对象几乎总同时冒出好几个测量：一份体检表上的血压、血脂、血糖，一个考生的语文、数学、外语成绩，一套房子的面积、区位、楼龄与总价。**若只逐个去看单变量，就会把"变量之间并肩而动、互相牵连"的结构彻底丢掉**——体育成绩可能与身高正相关，房价的几项属性总是一起涨落。为了把这些"一起动"的关系也装进同一个模型，统计学家把一元的 $\mu$、$\sigma^2$ 升格为 *均值向量* $\boldsymbol{\mu}$ 与 *协方差矩阵* $\boldsymbol{\Sigma}$。多元正态分布 $N_p(\boldsymbol{\mu},\boldsymbol{\Sigma})$ 正是这种考虑的自然产物：对角线编码了每个变量自己的水平与离散度，非对角线则编码了任意两个变量"同频共振"的强弱。**没有它，从高维数据做任何正规的估计、检验、判别都无从下手**——它是整门多元统计的地基。
...
**从 $\chi^2$ 说起**：一元里，为了检验方差、构造 $t$ 与 $F$ 统计量，我们必须先知道"样本平方和 $\sum_i X_i^2$ 服从什么分布"——答案是 $\chi^2$。多元里我们撞上同一道关卡：要做 Hotelling $T^2$、MANOVA、判别分析，都绕不开"样本散布矩阵 $\boldsymbol{W}=\sum_i \boldsymbol{X}_i \boldsymbol{X}_i^\top$ 服从什么分布"这个问题。Wishart（1928）正是为补这块地基而生的：**它是 $\chi^2$ 向随机矩阵的推广**。有了它，样本协方差矩阵的"形状波动"才变得可描述、可推断——一元统计离不开卡方，而几乎一切多元推断都踩着 Wishart 这块基石。
...
#### 从 $t$ 统计量到多维：均值向量的假设检验

**从 $t$ 检验说起**：一元里，检验"均值是否等于目标 $\mu_0$"用 $t=\frac{\bar X-\mu_0}{s/\sqrt n}$，它度量"样本均值离目标多远、按标准差归一"。到了多元，问题是"整个均值向量 $\boldsymbol{\mu}$ 是否等于 $\boldsymbol{\mu}_0$"。一元那种"每个坐标各归各"的 $t$ 检验根本不适用——因为变量彼此相关，必须用协方差矩阵整体归一，而不是逐个坐标单独归一。Hotelling（1931）把 $t$ 统计量"平方化"，并用样本散布矩阵（一个 Wishart）的逆做归一：$T^2 = n(\bar{\boldsymbol{X}}-\boldsymbol{\mu}_0)^\top \boldsymbol{S}^{-1}(\bar{\boldsymbol{X}}-\boldsymbol{\mu}_0)$。它本质上是"按样本椭球归一后的马氏距离平方 × 样本量"。**这是把"均值是否等于某标准"从一维抬升到多维的标配统计量**，也是多元均值推断的起点。
...
#### 从 ANOVA 的 $F$ 到 MANOVA：一次裁决整组向量

**从单因素 ANOVA 说起**：一元 ANOVA 用 $F=\frac{SS_B/df_B}{SS_W/df_W}$ 比较若干组的均值是否相等。可当每组观测是一个 $p$ 维向量（比如同时测身高、体重、肺活量），我们想知道"这 $g$ 个组的均值向量是否全相等"。若偷懒把每个指标分开做一元 ANOVA，则会反复踩中"多重比较"的陷阱，把犯错的概率越叠越高。Wilks（1932）的 Lambda $\Lambda=\frac{|\boldsymbol{W}_W|}{|\boldsymbol{W}_W+\boldsymbol{W}_B|}$ 用行列式（≈体积）把"组内分散"与"组内加组间"的总体积浓缩成一个 $[0,1]$ 的比值，**一次裁决多组向量究竟分没分开**。这是 MANOVA 的开山统计量，也定义了多元假设检验里"体积比对"这一经典范式。
...
#### 从"变量太多"到"看准方向"：降维的第一动机

**动机与来龙去脉**：想象一份问卷同时采了几十上百个高度相关的指标（城市宜居度：房价、通勤、医疗、绿化、安全……）。直接罗列这 $p$ 个数既冗长又臃肿——因为它们彼此纠缠、信息大量冗余。Pearson（1901）与 Hotelling（1933）先后问了一个朴素的问题：**能不能用更少的"综合指标"，最大程度还原数据本身的变异？** PCA 的回答是：数据云总会长成某个偏长的椭球，把"云最拉得开的几个方向"找出来当新坐标，就等于把主要方差都保住了。它把"变量的个数"让位给"结构的维度"，于是成为一切降维、可视化和数据科学 pipeline 的第一步。
...
#### 从"共同源头"说起：相关性背后藏着隐形因子

**动机与来龙去脉**：心理学家很早就注意到，一个学生的语文、数学、外语成绩彼此正相关。为什么会这样？合理的猜测是：这些分数背后藏着一个看不见的"能力"因子，同时推高了各科成绩。Spearman（1904）据此提出**共同因子**的雏形：**观察到的相关，大多来自少数不可观测的公共因子**。因子分析就是把这一直觉形式化——把每个可观测变量 $\boldsymbol{X}$ 拆成少数公共因子 $\boldsymbol{F}$ 的线性组合（载荷 $\boldsymbol{L}$），加上每个变量自己的独特噪声 $\boldsymbol{\varepsilon}$。于是协方差 $\boldsymbol{\Sigma}=\boldsymbol{LL}^\top+\boldsymbol{\Psi}$ 干脆利落地说明："变量间的相关主要来自共享因子，$\boldsymbol{\Psi}$ 是个体独有部分。"它和 PCA 的分工不同：PCA 只管把方差压缩成几个方向，因子分析则想解释相关的来源，并借正交旋转换一个更可读的坐标系。
...
#### 从"怎么分类"说起：Fisher 的投影智慧

**动机与来龙去脉**：现实中常有"根据多项测量把新对象归入某个已知类别"的需求：CT 指标判肿瘤良恶性、芯片参数判生产批次。最省事的做法也许是"每个变量定一个阈值"，但变量一旦相关，这种逐变量判定就会反复出错。Fisher（1936）换了一个角度（也正是 Anderson 教材里经典的一章）：**找一个投影方向，让投影后不同类"分得最开、各自内部又最紧"**。他构造了准则 $J(\boldsymbol{a})=\frac{\boldsymbol{a}^\top\boldsymbol{S}_B\boldsymbol{a}}{\boldsymbol{a}^\top\boldsymbol{S}_W\boldsymbol{a}}$，其最优解 $\boldsymbol{a}\propto\boldsymbol{S}_W^{-1}(\boldsymbol{\mu}_1-\boldsymbol{\mu}_2)$ 把"降维"与"分类"二合一——这就是线性判别分析的肇始，也是"先白化、再朝差异方向看"这一智慧的第一次亮相。
...
**动机与来龙去脉**：与前面几节相反，聚类是**没有类别标签**时让数据自己告诉我们"它们天然分几堆"——比如无监督地把客户、基因表达谱、用户行为分组。这源于人最朴素的直觉"物以类聚"。K-means 把直觉约成最简目标：找 $K$ 个中心，使每个点到归属中心的平方距离之和最小；层次聚类则从最细粒度出发逐级合并最近的两簇（或反方向的逐步分裂），产出完整的树状谱系。两者是无监督探索最常用的两把钥匙，回答的正是 PCA 不涉及、却同样基本的问题："点与点靠得多近？"
...
#### 从"两组变量之间的关系"说起：不止一个相关系数

**动机与来龙去脉**：普通的相关系数只描述"一个 $X$ 和一个 $Y$"的线性关系。可现实里常要问"两组变量"之间的关系：一组课程成绩与一组能力测评是什么关系？一项训练前后两套生理指标是否整体联动？Hotelling（1936）把"两组的关系"升维：**各挑一个线性组合，使两个组合的相关系数最大**，得到第一典型相关系数 $\rho_1$；再在与已取方向正交的剩余空间里继续找第二、第三对……于是"两组变量相关"这种模糊说法被展开成一串逐级递减的典型相关系数。CCA 因此可被看成"相关系数"向"两组向量"的全面推广，也是今天多视图学习(multi-view learning)的数学雏形。
...
#### 从 ANOVA 到 MANOVA：同时检验整组均值向量

**动机与来龙去脉**：一元 ANOVA 解决"若干组均值是否相等"。当每组观测是一整个 $p$ 维向量时，偷懒的做法是把 $p$ 个指标逐个做 ANOVA——但这样既忽略指标间的相关，又因"多重比较"放大了第一类错误。MANOVA 把 $H_0:\boldsymbol{\mu}_1=\cdots=\boldsymbol{\mu}_g$ 这个整体假设一次检验：把组内与组间散布浓缩成 Wilks $\Lambda$、Pillai 迹、Hotelling-Lawley 迹、Roy 最大根等统计量统一裁决。当 $p=1$，它自动退回一元 ANOVA。它解决了"多响应同时比较"在检验层面的一揽子问题，是 Wilks Lambda 小节里那个"体积比对"思路的正式落地。
...
**动机与来龙去脉**：有时候，我们手里的数据不是坐标，而仅仅是"两两之间的距离/相异度"——城市间的里程表、消费者对不同品牌的感知差异问卷。**问题：只有一张距离表，能不能还原出这些对象在一个低维空间里的坐标？** Torgerson（1952）等给出的回答就是**经典多维标度**：对距离平方矩阵做双中心化、再特征分解，用特征向量直接给出坐标。它让"相异性 → 地图"成为可能，是心理感知相似性研究与数据可视化的经典工具；下面会看到，它的代数骨架与 PCA 一脉相承——都是"从某种矩阵的特征分解里取坐标"。
...
#### 从"鸡尾酒会问题"说起：分离混在一起的信号

**动机与来龙去脉**：把几个说话人的声音同时录进一个麦克风里，堪称"盲源分离"的代表场景——鸡尾酒会问题。PCA 靠二阶矩只能"去相关"，却分不开彼此独立、但分布非高斯的声源。ICA（Jutten & Hérault 1980s、Comon 等人）改用更强的"独立性"（依赖高阶统计量）：假设观测 $\boldsymbol{X}$ 是未知独立源 $\boldsymbol{S}$ 经常数矩阵 $\boldsymbol{A}$ 线性混合而成，通过最大化非高斯性或最小化互信息反解出 $\boldsymbol{A}^{-1}$。它与 PCA 的根本差别正在强度：**"去相关"（二阶）与"分离独立"（高阶）不是一回事**，这也是脑电、音频源分离为什么点名要 ICA。
...
**动机与来龙去脉**：一元回归研究一个响应 $Y$ 如何被若干预测变量 $X$ 解释；多元回归则把"多个响应同时被同一堆预测变量解释"放进同一个模型 $\boldsymbol{Y}=\boldsymbol{XB}+\boldsymbol{E}$。**为什么要把多个响应"绑在一起"拟合，而不是各自单独回归？** 因为各响应往往彼此相关——一起拟合能利用它们的协方差结构，得到更高效的估计与更自洽的推断（这正是 Anderson 线性模型一章反复强调的动机）。系数矩阵 $\boldsymbol{B}$ 的每一列，就是某个响应自己的回归系数；误差行 $\sim N_q(\boldsymbol{0},\boldsymbol{\Sigma})$ 则允许响应之间共享相关。
...
#### 从"潜变量 + 因果链"说起：测量与结构的合体

**动机与来龙去脉**：社会科学常想验证"看不见的概念之间有没有因果/路径关系"，比如"教育 → 收入 → 幸福感"。但"幸福感"本身测不准，得先用若干可观测题项把它"量出来"。SEM（Jöreskog、Wiley，融合因子分析与路径分析）把这一整套装进一个模型：**测量模型**说明观测分数是潜在变量的"影子"，**结构模型**说明潜在变量之间彼此指向（带系数与路径）。它让研究者能一次同时核查"测没测准（验证性因子分析）"与"因不因果（路径系数）"，是社会科学建模的标准工具，也是第 2.6 节因子分析的"加因果链"延伸。
...
### 2.15 主成分分布理论（样本主成分的渐近）

#### 从"算法"到"推断"：样本主成分能有多准？

**动机与来龙去脉**：PCA 的最优性（定理 3.3）告诉我们最优方向*应该*往哪儿看，但在真实数据里 $\boldsymbol{\mu}$ 与 $\boldsymbol{\Sigma}$ 是未知的，只能拿*样本*协方差 $\boldsymbol{S}$ 的特征分解当作"样本主成分"。于是自然要问：**样本主成分离总体主成分有多远？标准误多少？能不能做置信区间、假设检验（比如判断某几个总体特征值是不是其实相等）**？要回答这些，就必须建立**样本特征值、特征向量的分布理论**。Girshick（1939）与 Anderson（1963）在多元正态大样本下给出了渐近正态结果：特征根与特征向量的波动都随样本量以 $1/\sqrt{n}$ 的量级衰减。这一层"从样本回到总体"的推断，正是把 PCA 从一套漂亮算法升格为可检验统计方法的关键一步。

**定义 2.16**（样本主成分及其渐近分布）设 $\boldsymbol{X}_1,\ldots,\boldsymbol{X}_n$ 独立同分布于 $N_p(\boldsymbol{\mu},\boldsymbol{\Sigma})$，总体谱分解为 $\boldsymbol{\Sigma}=\boldsymbol{\Gamma}\boldsymbol{\Lambda}\boldsymbol{\Gamma}^\top$，其中 $\boldsymbol{\Lambda}=\operatorname{diag}(\lambda_1,\ldots,\lambda_p)$ 且 $\lambda_1>\cdots>\lambda_p>0$（互不相等），$\boldsymbol{\Gamma}=(\boldsymbol{\gamma}_1,\ldots,\boldsymbol{\gamma}_p)$ 正交。记样本协方差 $\boldsymbol{S}=\frac{1}{n}\sum_{i=1}^n(\boldsymbol{X}_i-\bar{\boldsymbol{X}})(\boldsymbol{X}_i-\bar{\boldsymbol{X}})^\top$ 及其谱分解 $\boldsymbol{S}=\hat{\boldsymbol{\Gamma}}\hat{\boldsymbol{\Lambda}}\hat{\boldsymbol{\Gamma}}^\top$（$\hat\lambda_1\ge\cdots\ge\hat\lambda_p$）。则当 $n\to\infty$，**样本特征值**近似独立地满足：

$$
\sqrt{n}\,(\hat{\lambda}_j-\lambda_j)\ \xrightarrow{d}\ N(0,\;2\lambda_j^2),\qquad j=1,\ldots,p.
$$

而第 $j$ 个**样本特征向量**（取定合适的符号）渐近满足：

$$
\sqrt{n}\,(\hat{\boldsymbol{\gamma}}_j-\boldsymbol{\gamma}_j)\ \xrightarrow{d}\ N_p\!\left(\boldsymbol{0},\;\lambda_j\sum_{k\neq j}\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}\,\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top\right),\qquad j=1,\ldots,p.
$$

> **💡 直观理解（类比）**：第一条公式说，样本特征值 $\hat\lambda_j$ 渐近正态、方差约 $2\lambda_j^2/n$——大意是"从自己的谱里再抽样"的波动随 $n$ 收缩。第二条更有味道：样本特征向量的渐近协方差里出现因子 $\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}$，**分母是谱间隙 $\lambda_j-\lambda_k$**。**两个特征值挨得越近，它们各自的特征向量方向就越难被样本分辨、也就越容易反复"翻跳"**；彼此拉开距离的特征值对应的方向才稳定。这正解释了"近似相等的特征值 → 主轴不唯一 → 实际主成分方向抖动"的现象，也为选择主成分个数、构造置信区间提供了理论依据。这些渐近公式同时也支撑着 Bartlett 关于"后 $p-k$ 个总体特征值是否相等"（球性检验）等方法。
...
**定理 3.6**（样本主成分的渐近分布）设 $\boldsymbol{X}_1,\ldots,\boldsymbol{X}_n$ 独立同分布于 $N_p(\boldsymbol{\mu},\boldsymbol{\Sigma})$，$\boldsymbol{\Sigma}=\boldsymbol{\Gamma}\boldsymbol{\Lambda}\boldsymbol{\Gamma}^\top$，特征值 $\lambda_1>\cdots>\lambda_p>0$ 互不相等，样本协方差 $\boldsymbol{S}=\hat{\boldsymbol{\Gamma}}\hat{\boldsymbol{\Lambda}}\hat{\boldsymbol{\Gamma}}^\top$。则当 $n\to\infty$：

1. 样本特征值渐近独立，且 $\sqrt{n}(\hat\lambda_j-\lambda_j)\xrightarrow{d}N(0,\,2\lambda_j^2)$；
2. 样本特征向量（定好符号）$\sqrt{n}(\hat{\boldsymbol{\gamma}}_j-\boldsymbol{\gamma}_j)\xrightarrow{d}N_p\big(\boldsymbol{0},\ \lambda_j\sum_{k\neq j}\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top\big)$。

> **💡 直观理解（类比）**：定理 3.6 把上一节的两个渐近公式正式定格，并点出两处实用含义：其一，特征值的误差异常简单——只由特征值本身与样本量 $n$ 决定（$2\lambda_j^2/n$），与其它特征值无关；其二，特征向量方向是否"稳"，完全取决于**谱间隙** $\lambda_j-\lambda_k$——间隙越大越稳，间隙趋近 0 时方差发散（正是整段主轴翻跳的数学来源）。理解这两条，是给 PCA 报告标准误、构造置信区间、以及审视重复特征值问题（$n<p$ 时退化子空间）的钥匙。

**证明**：我们给出沿"一阶扰动展开"思路的概要证明。

**步骤 1：把样本特征向量写成总体向量的扰动**

记 $\boldsymbol{S}=\boldsymbol{\Sigma}+\boldsymbol{R}$，其中 $\boldsymbol{R}$ 是"误差矩阵"。由谱 / 特征向量的可微性，把 $\hat{\boldsymbol{\gamma}}_j$ 在 $\boldsymbol{\Sigma}$ 附近一阶 Taylor 展开，得增量主要项 $\hat{\boldsymbol{\gamma}}_j-\boldsymbol{\gamma}_j \approx \sum_{k\neq j}\frac{\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top \boldsymbol{R}\boldsymbol{\gamma}_j}{\lambda_j-\lambda_k}$（矩阵摄动理论中的一阶近似）。

**步骤 2：利用 Wishart 的行分布**

因为 $n\boldsymbol{S}=\sum_i(\boldsymbol{X}_i-\bar{\boldsymbol{X}})(\boldsymbol{X}_i-\bar{\boldsymbol{X}})^\top \sim W_p(n-1,\boldsymbol{\Sigma})$，可用 $\boldsymbol{R}=\boldsymbol{S}-\boldsymbol{\Sigma}$ 的收敛速率与中心极限定理的组合块判断 $\boldsymbol{R}$ 的元素是 $\sqrt{n}$-一致收敛的渐近正态随机量，且不同 $(j,k)$ 的增量互不相关（对应特征值"近似独立"）。

**步骤 3：结合各向同性与正交约束**

由于 $\lambda_1>\cdots>\lambda_p$ 互不相同，$\hat{\boldsymbol{\gamma}}_j$ 的符号与方向在邻域内唯一可辨；再借 $\boldsymbol{\gamma}_j^\top(\hat{\boldsymbol{\gamma}}_j-\boldsymbol{\gamma}_j)$ 的二阶小量可忽略，投影到 $\boldsymbol{\gamma}_k$（$k\neq j$）的分量即由 $\frac{\boldsymbol{\gamma}_k^\top\boldsymbol{R}\boldsymbol{\gamma}_j}{\lambda_j-\lambda_k}$ 主控。综合各分量后得到协方差：

$$
\operatorname{Cov}_{\mathrm{asymp}}[\hat{\boldsymbol{\gamma}}_j]=\frac{1}{n}\lambda_j\sum_{k\neq j}\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top.
$$

特征值部分 $\sqrt{n}(\hat\lambda_j-\lambda_j)\approx\sqrt{n}\,\boldsymbol{\gamma}_j^\top\boldsymbol{R}\boldsymbol{\gamma}_j\xrightarrow{d}N(0,2\lambda_j^2)$。$\square$
...
#### 样本主成分分布理论的补充习题（编号与上文连续）

**基础题**

**习题 301**（样本特征值渐近）写出样本特征值 $\hat\lambda_j$ 满足的渐近分布关系式，并说明其方差随样本量如何变化。

**习题 302**（样本特征向量渐近）写出第 $j$ 个样本特征向量 $\hat{\boldsymbol{\gamma}}_j$ 的渐近协方差矩阵形式，指出其中起关键作用的"谱间隙"因子。

**习题 303**（样本主成分来源）说明实际做 PCA 时，用的"主成分"与"总体主成分"（$\boldsymbol{\Sigma}$ 的谱分解）有何区别，为什么需要渐近分布理论。

**进阶题**

**习题 304**（方差来源）解释样本特征值渐近方差 $\operatorname{Avar}(\hat\lambda_j)=2\lambda_j^2$ 的直观来源（与 Wishart 行分布、$\sqrt{n}$ 中心极限的关系）。

**习题 305**（谱间隙）解释为什么当特征值 $\lambda_j$ 与 $\lambda_k$ 越接近时，对应样本特征向量 $\hat{\boldsymbol{\gamma}}_j$ 的方向越不稳定。

**习题 306**（Bartlett 检验）简述用于检验"后 $p-k$ 个总体特征值相等"的 Bartlett 统计量的思路及其与本节渐近结果的关系。

**挑战题**

**习题 307**（渐近推导框架）简述"一阶扰动展开"推导样本特征向量渐近的主要步骤（$\boldsymbol{S}=\boldsymbol{\Sigma}+\boldsymbol{R}$ 后如何用 $\frac{\boldsymbol{\gamma}_k^\top\boldsymbol{R}\boldsymbol{\gamma}_j}{\lambda_j-\lambda_k}$ 组织各投影分量）。

**习题 308**（若干主成分的选择）说明在已知前 $k$ 个样本特征值渐近独立的条件下，如何据此为"保留 $k$ 个主成分"给出标准误或近似推断。
...
1. $\sqrt{n}(\hat\lambda_j-\lambda_j)\xrightarrow{d}N(0,\,2\lambda_j^2)$。方差为 $2\lambda_j^2/n$，随样本量 $n$ 增大而以 $1/n$ 缩小（与 $\lambda_j$ 本身成正比，特征值越大波动越大）。
2. $\sqrt{n}(\hat{\boldsymbol{\gamma}}_j-\boldsymbol{\gamma}_j)\xrightarrow{d}N_p\big(\boldsymbol{0},\ \lambda_j\sum_{k\neq j}\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top\big)$。关键因子是谱间隙平方的倒数 $\frac{1}{(\lambda_j-\lambda_k)^2}$——间隙越小，该方向的特征向量越不稳定。
3. 总体的"主成分"来自未知的 $\boldsymbol{\Sigma}$ 的特征分解（理论方向），实际只能用样本协方差 $\boldsymbol{S}$ 的谱分解（$\hat{\boldsymbol{\gamma}}_k,\hat\lambda_k$）去近似它。渐近分布理论正是回答"这个近似误差有多大、是否显著"从而做推断的依据。

**进阶题答案**

1. $n\boldsymbol{S}\sim W_p(n-1,\boldsymbol{\Sigma})$，其对角元素平方的展开可看作 $n$ 个独立同分布平方和（近似 $\chi^2_n$，方差 $2$）×$\lambda_j^2$），故 $\operatorname{Cov}(\hat\lambda_j)\approx\frac{1}{n^2}\cdot 2\lambda_j^2 n=2\lambda_j^2/n$，再按 $\sqrt n$ 归一即 $2\lambda_j^2$。
2. 谱间隙 $\lambda_j-\lambda_k$ 越小，分母 $(\lambda_j-\lambda_k)^2$ 越小，渐近协方差系数 $\frac{\lambda_k}{(\lambda_j-\lambda_k)^2}$ 越大，方向 $\hat{\boldsymbol{\gamma}}_j$ 越易受样本扰动而"翻跳"。
3. Bartlett（1951）检验后 $p-k$ 个总体特征值相等的统计量基于 $\sum_{j=k+1}^p\hat\lambda_j$ 与几何均值之差（${n}\left[\sum_{j=k+1}^p\log\hat\lambda_j-(p-k)\log\!\big(\tfrac{1}{p-k}\sum_{j=k+1}^p\hat\lambda_j\big)\right]\approx\chi^2$），其合理性可理解成"相等的特征值彼此难以分辨、方向不稳"，与本节渐近方差发散一致。

**挑战题答案**

1. 记 $\boldsymbol{S}=\boldsymbol{\Sigma}+\boldsymbol{R}$，把 $\hat{\boldsymbol{\gamma}}_j$ 一阶 Taylor 展开，主增量 $\approx\sum_{k\neq j}\frac{\boldsymbol{\gamma}_k\boldsymbol{\gamma}_k^\top\boldsymbol{R}\boldsymbol{\gamma}_j}{\lambda_j-\lambda_k}$；由 Wishart 行分布 + $\sqrt{n}$ 中心极限得到 $\boldsymbol{R}$ 元素的正态性，各投影分量互不相关，再拼合即得渐近协方差。
2. 由于前 $k$ 个样本特征值渐近独立，可对每个 $\hat\lambda_j$ 给出近似标准误 $\sqrt{2}\lambda_j/\sqrt{n}$（或代入 $\hat\lambda_j$ 估计），据此给出累计方差占比、$\hat\lambda_j$ 差的置信区间，从而为"选 $k$ 个主成分是否稳定/是否多留或少留一个"提供数量依据，并与累计方差、碎石图准则互补使用。

---


> 多元统计分析研究"同时看多个变量"时如何从数据里抓住结构。它的主线有二：一是把单变量的正态、卡方、t、F 检验系统推广到向量与矩阵，长成分布理论这棵大树；二是"降维与探索"流派——从高尔顿、皮尔逊到费希尔与霍特林，一心想从高维数据里提炼出少数方向。这两股力量在二十世纪交汇，交出多元正态、Wishart、PCA、因子分析、判别与聚类等一整套既有严密分布又有海量应用的方法体系，并最终通向二十一世纪的数据科学。本章沿"遗传与回归的源头 → 多元分布理论 → 降维与判别 → 现代数据方法"的线索，追述这门应用统计之女王的前因后果。

### 1. 源头：高尔顿的回归与皮尔逊的相关

- **遗传学的召唤**：十九世纪后期，高尔顿（Francis Galton）在研究父代与子代的身高遗传时，第一次在"两个变量的观测对"上操作——他得到直观的椭圆等高线与回归直线，并推动"回归"概念的诞生。
- **皮尔逊的定量化**：卡尔·皮尔逊（Karl Pearson）把这种"两变量一起变的程度"提炼为相关系数与联合正态分布，并系统建立起"相关—回归"大词汇——它正是多元分析要在维度更大时才完全展开的雏形。
- **从二维到一般的伏笔**：单变量统计到向量统计的距离并不远，但真正难点在"矩阵与行列式"——协方差矩阵、逆矩阵、特征分解等工具需由下一代人逐一补齐。

### 2. 多元正态与 Wishart：给"向量样本"一本分布册

- **多元正态分布的成形**：协方差矩阵 $\Sigma$ 把"变量之间怎么联动"编码为对称正定阵；多元正态的密度、似然与极大似然估计在 1930—40 年代被系统化，成为多元推断的支点。
- **Wishart 分布的提出**（1928）：约翰·威沙特把卡方分布推广为矩阵情形——样本协方差矩阵的分布服从 **Wishart 分布**，由此能构造多元正态的似然比检验与相关系数的抽样分布。
- **从分布到检验的推进**：Wishart 及其相伴的广义特征值分布，让"多元正弦问题""多元方差分析"真正落地：分布理论从此是多元方法学的唯一通行证。

### 3. Hotelling T² 与 Wilks Λ：把 t 和 F 抬进向量世界

- **Hotelling T²**（1931）：霍特林把单样本/双样本的 $t$ 检验推广到均值向量，构造 $T^{2}$ 统计量——先得"单变量测单个均值，向量测一簇均值一起猜"的现代语感。
- **Wilks Λ 与 MANOVA**（1932 前后）：威尔克斯把 $F$ 检验推广为向量版的 Wilks $\Lambda$，把"比较多组均值向量"做成多元方差分析（MANOVA）的似然比统计量——多分类问题自此有严谨可算的分野检验。
- **从均值到综合**：T² 与 Λ 联手把"单变量的检验引擎"整体移植进多元,多元推断的地基就此铺平，也让判别与回归有了统计检验的底层账户。

### 4. PCA：皮尔逊的几何与霍特林的代数

- **皮尔逊的"主直线"**（1901）：皮尔逊在《以最小二乘法拟合平面与主组件》中给出几何意义上的"最优化投影方向"——在一个方向上投入最大方差，即主成分方向的雏形。
- **霍特林的特征分解**（1933）：霍特林以协方差矩阵的特征分解独立重演 PCA，把"找方差最大的正交方向"写成代数问题（主成分即特征向量，方差即特征值）。
- **PCA 的现代定型**：样本主成分的渐近分布（特征值/特征向量的极限）由 Anderson、Girshick 等完善，使 PCA 值班到标准误、置信区间与检验——"降维＋推断"二合一由此可操作化。

### 5. 因子分析：斯皮尔曼的"共同因子"之梦

- **智力测验的背景**（1904）：斯皮尔曼在智力测量中发现"一个共同因素＋环境噪声"能解释许多测验的相关——因子分析的初心。
- **共同因子与特殊因子的对立**：把可观察变量的相关结构用少数潜在因子（共同因子）加上各自的特殊因子来求解，形成探索性因子分析；旋转（varimax 等）、最大似然估计把解判据与确定性补齐。
- **从探索到结构**：把"由数据找因子"与后来的"结构方程模型"衔接，因子分析成为心理计量与社会科学里最具定义力的多元工具之一。

### 6. 歧视分析与费希尔的投影

- **Fisher 线性判别**（1936）：费希尔把"组间离散/组内离散之比最大"这一最优投影，化成特征向量问题——判别问题由此变成几何与代数皆成的优化问题。
- **马氏距离与分类**：以协方差阵的逆做尺度校准的 **Mahalanobis 距离** 让分类器对数据旋转不变，成为判别/聚类里最顺手的"分水岭"标准。
- **从两分类到多分类**：Fisher 判别到多元判别分析再到二次判别与贝叶斯分类，判别流派把"在多类之间的边界"做成既能可视化、又可统计检验的整套语系。

### 7. 典型相关（CCA）与降维的第三条路

- **Hotelling 的典型相关**（1936）：霍特林发问"两组变量之间最强的相关方向是什么"，给出典型相关分析（CCA）：把两组高维变量各自投影到一维时相关的最大方向反复取出，得到一组典型相关系数。
- **三线合流**：PCA（在组内找方向）、判别（组间比组内）、CCA（两组间找相关）共同构成"找方向"谱系的三大支柱，多元降维语言自此完整。
- **作为现代工具的复兴**：CCA/监督式 CCA 在机器学习（多视图学习、协变量推断）里再度走红，其"找最大相关投影"的骨架直到今天仍然坚硬。

### 8. 无监督世界：聚类、MDS 与 ICA

- **数值分类与聚类**：草地与土壤样本的自动分群需求，促使 Sneath—Sokal 的数值分类、Ward 层次法与 $k$-均值相继成型——标签缺失时"只凭相似度抱团"的多元方法家族就此扎根。
- **多维标度（MDS）**：托格森（1952）、克鲁斯卡（1964）将"已知对象间距离、反推低维坐标"做成 MDS，让心理测验的相似矩阵在低维平面内呈现——可视化的多元方法由此诞生。
- **独立成分分析（ICA）**：约滕与埃罗（1988）、科蒙（1994）面对"鸡尾酒会问题"把混合观测分离成统计上独立的源，用高阶统计量做"盲分离"，成为信号处理与数据科学里最刺眼的新工具。

### 9. 结构方程模型（SEM）与因果的语言

- **怀特的路径分析**（1921）：遗传学家怀特用路径图表达变量间的直接影响，奠定了"潜变量＋路径"的因果词汇雏形。
- **SEM 的统一**（1970s）：约雷斯库等把测量模型（因子分析）与结构模型（路径/回归）纳入统一的似然估计框架（如 LISREL），构建起现代结构方程模型。
- **从相关到因果**：SEM 的关键是把"相关结构"与"因果结构"分开建模——它让统计不再只是描述相关，还能"谈原因"，是社会科学里影响最深远的方法共同体。

### 10. 现代融合：从经典检验到数据科学

- **高维数据的挑战**：样本数少于变量数的时代（$p\gg n$），经典多元正态分布与 Wishart 的框架需被收缩估计、随机矩阵理论（Marchenko-Pastur 分布）等新语言重写。
- **与机器学习的联姻**：PCA/判别/聚类/ICA 各自演化为机器学习里正则化、核方法、谱聚类与表示学习的地基；多元统计的"找方向、抱团、分群"词汇成为深度学习的抽象语法。
- **分布理论仍作心脏**：无论方法多数据化，多元正态、Wishart、T²、Λ 背后的矩阵分析与极限分布，仍是检验与误差把控的唯一心脏——多元统计分析作为"应用统计之女王"的地位至今未变。

### 11. 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 十九世纪末 | 高尔顿 | 回归与相关/等高线的胚胎 |
| 1901 | 皮尔逊 | PCA 的几何雏形（主直线） |
| 1904 | 斯皮尔曼 | 因子分析（共同因子） |
| 1921 | 怀特 | 路径分析：SEM 之祖 |
| 1928 | 威沙特 | Wishart 分布 |
| 1931—1933 | 霍特林 | Hotelling T²；PCA 的代数定型 |
| 1932 前后 | 威尔克斯 | Wilks Λ 与 MANOVA |
| 1936 | 费希尔；霍特林 | Fisher 判别；典型相关（CCA） |
| 1952—1964 | 托格森、克鲁斯卡 | 多维标度（MDS） |
| 1950s—1960s | 数值分类学派 | 聚类与 k-均值 |
| 1970s | 约雷斯库等 | 结构方程模型（SEM/LISREL） |
| 1988—1994 | 约滕、埃罗、科蒙 | 独立成分分析（ICA） |
| 二十—二十一世纪 | Anderson 等 | 样本主成分/多元分布的渐近理论 |

### 12. 小结

> 多元统计分析的百年是一条"两条线索汇成一江"的旅程：一条是分布理论，从高尔顿的回归与皮尔逊的相关，经 Wishart、Hotelling T² 与 Wilks Λ，把单变量统计系统抬进向量与矩阵；另一条是降维与探索，由皮尔逊的 PCA、斯皮尔曼的因子分析、费希尔的判别到霍特林的典型相关，专为"从高维里找出少数方向"而战。两者被聚类、MDS、ICA 与结构方程模型在应用侧接续，并在 $p\gg n$ 时代与机器学习和随机矩阵理论化合成今天的模样。贯穿全程的主线相当清晰：**先问数据里有哪些"一起变的方向"，再给这些方向一套严格的分布与检验语言，最后把结论交给真实世界的分类、解释与因果**——多元统计分析由此成为连接数学、统计与数据科学最持久的一座桥梁。
...
2. **降维方法**：主成分分析通过方差最大化原则找到数据的主要变异方向，是最常用的无监督降维方法；其样本特征值与特征向量的渐近分布理论（样本主成分的分布）进一步把 PCA 延伸到标准误、置信区间与特征值相等性检验等推断层面。因子分析通过潜在变量解释观测变量间的相关性结构，在心理测量学等领域有广泛应用。多维标度从距离矩阵重建低维坐标，适用于可视化高维数据的相似性结构。独立成分分析则利用高阶统计量分离独立源信号。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对多元统计分析（协方差、主成分特征分解、判别、聚类、降维距离）核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 8.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到多元统计语境的追问上。

| 步骤 | 行动 | 多元统计中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 数据矩阵是 $n\times p$？变量结构是单一协方差阵还是多组？要降维、分类还是聚类？已知哪些分布假设？要估计参数还是做检验？ |
| ② 计划 | 选择可行工具、预估方向 | 用特征分解（PCA）还是 Fisher 投影（判别）？距离用欧氏还是 Mahalanobis？用 Wishart 做样本协方差的推断还是用 $T^2$ 检验均值？先想清楚哪条路最省力且符合假设。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 每步写明依据（定理编号），去中心化、特征值排序、投影方向 $\mathbf a$、协方差阵的对称分解，矩阵的维数与对称半正定别出错。 |
| ④ 回看 | 核对结果、检验合理性 | 结果是否依赖样本而波动、是否违背常识（方差非负、特征向量正交、距离非负）？能否用另一估计或距离量再验证一遍？ |

> 💡 **策略演示**：对两层样本矩阵求第一主成分方向。
> ① 理解：数据已去中心化，$n\times2$，目标是找方差最大的单位方向。② 计划：对样本协方差阵 $\hat\Sigma$ 做特征分解。③ 执行：设 $\hat\Sigma=\lambda_1\mathbf a_1\mathbf a_1^T+\lambda_2\mathbf a_2\mathbf a_2^T$，取最大特征值 $\lambda_1$ 对应的 $\mathbf a_1$。④ 回看：$|\mathbf a_1|=1$、$\lambda_1$ 为总方差最大份额，答案合理。
...
1. 样本协方差阵总是对称的半正定矩阵。（　）
2. PCA 的主成分方向就是协方差阵的特征向量组，并按特征值（方差）从大到小排列。（　）
3. 距离的选择不影响聚类的结果。（　）
4. Fisher 线性判别等价于最大化「类间散度 / 类内散度」之比。（　）
5. K-means 需要事先指定簇数 $K$。（　）
6. Mahalanobis 距离用总体协方差阵的逆对坐标做标准化，对原变量的线性变换不变。（　）

**选择**：

7. PCA 求取的第一主成分方向满足（　）。
   - A. 使投影方差最小　B. 使投影方差最大　C. 使样本均值最大　D. 使协方差为 0
8. Mahalanobis 距离相对欧氏距离关键区别在于（　）。
   - A. 引入了总体协方差阵的逆 $S^{-1}$ 做旋转缩放　B. 只是把坐标加绝对值　C. 完全不依赖数据分布　D. 只适用于一维数据
9. Hotelling $T^2$ 检验适用于（　）。
   - A. 多元均值向量的假设检验　B. 单个标量均值的检验　C. 两变量相关数的估计　D. 因子个数的确定
...
> **问题**：给定样本 $\mathbf x_1=(1,1),\mathbf x_2=(3,3),\mathbf x_3=(5,5)$（$n=3,p=2$），计算去中心化后的样本协方差阵 $S$（以 $n$ 为分母）。

- ① 理解：目标是 $S=\tfrac1n\sum_{i=1}^n(\mathbf x_i-\bar{\mathbf x})(\mathbf x_i-\bar{\mathbf x})^T$，先求均值向量。
- ② 计划：均值 $\bar{\mathbf x}=\tfrac1n\sum_i\mathbf x_i$，再逐个偏差外积累加。
- ③ 执行：$\bar{\mathbf x}=(3,3)$，偏差为 $(-2,-2),(0,0),(2,2)$，于是

$$
S=\frac1n\sum_{i=1}^n(\mathbf x_i-\bar{\mathbf x})(\mathbf x_i-\bar{\mathbf x})^T=
\begin{pmatrix}8/3&8/3\\8/3&8/3\end{pmatrix}.
$$

- ④ 回看：**易错点**——分母用 $n$ 而无偏估计用 $n-1$，两套口径要对齐；**检验**：矩阵半正定（两个特征值为 $16/3$ 和 $0$），相关系数 $\rho=1$，符合完全正相关的原始数据。

**示例 S2：用四步法做主成分的特征分解**

> **问题**：对 $\Sigma=\begin{pmatrix}5&2\\2&5\end{pmatrix}$ 做主成分分解，给出特征值、特征向量并说明第一主成分的方差解释比例。

- ① 理解：目标是把实对称阵对角化并按特征值大小排列。
- ② 计划：解特征方程 $|\Sigma-\lambda I|=0$ 求 $\lambda$，再回代求单位特征向量，最后算解释比例 $\lambda_1/(\lambda_1+\lambda_2)$。
- ③ 执行：

$$
|\Sigma-\lambda I|=(\lambda-5)^2-4=0,\quad \lambda_1=7,\ \lambda_2=3,
$$

$$
\mathbf a_1=\frac1{\sqrt2}(1,1)^T,\qquad \mathbf a_2=\frac1{\sqrt2}(1,-1)^T .
$$

- ④ 回看：**易错点**——特征向量忘归一化或配对顺序颠倒；**检验**：$A\mathbf a_1=7\mathbf a_1$、$A\mathbf a_2=3\mathbf a_2$，且 $A=\lambda_1\mathbf a_1\mathbf a_1^T+\lambda_2\mathbf a_2\mathbf a_2^T$，解释比例 $7/10=70\%$。

**示例 S3：用四步法做 Fisher 线性判别**

> **问题**：两类的样本均值 $\bar{\mathbf x}_1=(4,6)$、$\bar{\mathbf x}_2=(2,2)$，类内协方差为对角阵 $S_W=2I_2$，求 Fisher 最优投影方向 $\mathbf a$。

- ① 理解：目标是最优投影 $\mathbf a\propto S_W^{-1}(\bar{\mathbf x}_1-\bar{\mathbf x}_2)$，可归一化。
- ② 计划：先求 $S_W^{-1}$，再右乘类均值差向量，最后归一化。
- ③ 执行：$S_W^{-1}=\tfrac12 I_2$，类差 $\bar{\mathbf x}_1-\bar{\mathbf x}_2=(2,4)$，故 $\mathbf a\propto \tfrac12(2,4)=(1,2)$；取 $\mathbf a=(1,2)/\sqrt5$；投影后类在该方向的最大可分性由 Fisher 准则保证。
- ④ 回看：**易错点**——把 $S_W$ 与 $S_B$ 混淆，方向会错；**检验**：投影均值 $\mathbf a^T\bar{\mathbf x}_1$、$\mathbf a^T\bar{\mathbf x}_2$ 的差应尽可能大，且类内方差在此方向最紧。

**示例 S4：用四步法计算降维距离（欧氏 vs Mahalanobis）**

> **问题**：两点 $\mathbf x=(1,2)$、$\mathbf y=(3,4)$，协方差 $S=\begin{pmatrix}2&0\\0&3\end{pmatrix}$，分别求欧氏距离与 Mahalanobis 距离。

- ① 理解：$\mathbf d=\mathbf y-\mathbf x=(2,2)$；欧氏用 $\|\mathbf d\|$，Mahalanobis 用 $\sqrt{\mathbf d^T S^{-1}\mathbf d}$。
- ② 计划：欧氏直接算模长；Mahalanobis 先求 $S^{-1}$ 再做内积。
- ③ 执行：$S^{-1}=\begin{pmatrix}1/2&0\\0&1/3\end{pmatrix}$，

$$
d_E=\sqrt{4+4}=2\sqrt2,\qquad d_M=\sqrt{2\cdot\tfrac12\cdot2+2\cdot\tfrac13\cdot2}=\sqrt{2+\tfrac43}=\sqrt{\tfrac{10}{3}} .
$$

- ④ 回看：**易错点**——混合两个坐标时忘记除以各分量方差；**检验**：$d_M\le d_E$ 在此成立（欧氏距离未加权，Mahalanobis 按单位方差重赋权），且两者都是对称非负。

### 8.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的数据分析与计算任务，任选其一完成。

- **聚焦 P1（真实数据集 PCA）**：取鸢尾花数据集（150×4），去中心化后求样本协方差阵并做特征分解，画出碎石图并统计前两个主成分累计解释的方差比例。
- **聚焦 P2（聚类稳定性 / 选 $K$）**：对同一数据集分别用肘部法则与轮廓系数选择 K-means 的簇数 $K$，比较两种判据的一致性并说明各自局限。
- **聚焦 P3（降维可视化与 MDS 距离重建）**：把高维点用 PCA 投影到二维并可视化；再仅由两两距离矩阵用 MDS 重建坐标，检验重建坐标与原始欧氏距离的结构是否一致。
...
- **自测 1**：用一句话解释为什么样本协方差阵总是半正定，并举出它严格正定的一个充分条件。
- **自测 2**：说明 PCA 与因子分析在目标与模型假设上的主要区别。

**$\bullet$ 技能题**

- **自测 3**：对三样本 $\mathbf x_1=(1,2),\mathbf x_2=(3,4),\mathbf x_3=(5,6)$ 计算样本协方差阵 $S$（以 $n$ 为分母）并判断其秩。
- **自测 4**：对 $\Sigma=\begin{pmatrix}4&2\\2&4\end{pmatrix}$ 做主成分分解，给出特征值、特征向量与第一主成分的方差解释比例。
- **自测 5**：给定 $S=\begin{pmatrix}1&0\\0&4\end{pmatrix}$，计算 $\mathbf x=(0,1)$ 与 $\mathbf y=(2,3)$ 的欧氏距离与 Mahalanobis 距离。

**$\blacklozenge$ 综合题**

- **自测 6**：设计一个「两类判别的完整流程」：从数据标准化、估计 $S_W$、求 Fisher 方向，到给出新样本的分类规则，并写出「回看」。
- **自测 7**：写一段含「表格 + 一句话结论」的比较：K-means 与层次聚类在簇数预设、结果稳定性、对离群点敏感度三个维度上的差异。

### 8.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（协方差阵由偏差外积求和构成，为对称半正定）。2. **正确**（特征分解即 PCA 方向，按特征值降序）。3. **错误**（距离度量不同会改变聚类分组）。4. **正确**（Fisher 准则即类间/类内比最大化）。5. **正确**（K-means 需先定 $K$）。6. **正确**（Mahalanobis 经 $S^{-1}$ 旋转缩放，对可逆线性变换不变）。
>
> **选择**：7. B　8. A　9. A。

> **自测**：
> - 自测 1：协方差阵是 $\sum_i \mathbf v_i\mathbf v_i^T$ 的和，乃 Gram 矩阵故半正定；当数据张满 $\mathbb{R}^p$（样本数 $n\ge p$ 且各方向均有变差）时严格正定。
> - 自测 2：PCA 只做正交投影取方差最大的方向、无误差模型；因子分析假设观测由少数潜因子 + 特异性方差线性生成，是可解释的隐变量模型。
> - 自测 3：均值 $(3,4)$，偏差外积累加，$S=\tfrac13\sum \mathbf v_i\mathbf v_i^T$，秩为 1（三偏差共线），故特征值之一为 0。
> - 自测 4：$\lambda_1=6,\lambda_2=2$，$\mathbf a_1=(1,1)/\sqrt2$、$\mathbf a_2=(1,-1)/\sqrt2$；第一主成分解释 $6/8=75\%$。
> - 自测 5：$\mathbf d=(2,2)$，欧氏 $=2\sqrt2$；$S^{-1}$ 对角元 $(1,1/4)$，Mahalanobis $=\sqrt{2\cdot1\cdot2+2\cdot\tfrac14\cdot2}=\sqrt{4+1}=\sqrt5$。
> - 自测 6：标准化→估计 $S_W$→由 $S_W^{-1}(\bar{\mathbf x}_1-\bar{\mathbf x}_2)$ 得 $\mathbf a$→选阈值 $\tfrac12(\mathbf a^T\bar{\mathbf x}_1+\mathbf a^T\bar{\mathbf x}_2)$ 分类；回看：检验投影后类重叠度，必要时改阈值或加正则化。
> - 自测 7：K 预先指定、边界稳定但依赖初值；层次不预设 $K$、输出树状图但大样本开销高、对噪声敏感。结论：数据规模大且 $K$ 可估时偏好 K-means，结构层次重要时偏好层次聚类。
