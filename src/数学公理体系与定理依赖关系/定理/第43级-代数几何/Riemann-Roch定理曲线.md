...
> **一句话大白话**：一条曲线上"允许有多少自由度"来放某个除子（账本式地数函数零点极点），被一个固定的"亏格税"和广义次数所钉死——用一条公式就把曲线上的函数空间维数框出来。
>
> **小例子**：对曲线 $C$ 的除子 $D$，$h^0(D)-h^0(K-D)=\deg D-g+1$；里外项一挪，函数个数跟除子次数贴在一个简单等号上，是代数曲线上的"户口本"。
