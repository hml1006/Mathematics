...
> **🧠 来龙去脉：为什么要"压缩数据"？** 原始样本又长又冗余，但不同地方记录的信息价值并不相等。Fisher 在 1922 年系统引入**充分性（sufficiency）**，正是为回答一个问题：**能不能把一堆样本压缩成一个（或少数几个）数字，却一点都不丢失关于未知参数的信息？** 抛 20 次硬币时，你只需知道"总共 13 次正面"就能估计 $p$，逐次记下 $(1,0,1,1,\dots)$ 不会带来任何额外信息。这份"可压缩而不失真"的性质，是后面 Rao-Blackwell 定理（§2.4）、UMVUE（Lehmann–Scheffé，§2.5）乃至指数族天然充分统计量（§3.1）的共同地基——"降维不丢信息"这个朴素愿望，正是现代数据压缩、表示学习思想的统计源头。
...
> **🧠 来龙去脉：从"会不会显著"到"怎样才算最优"** 第 25 级的假设检验已经给了 $z$、$t$、$F$、$\chi^2$ 一大堆方法，但都停留在"某个检验有效"；统计推断理论要回答更本质的问题：**在固定误判率（$\alpha$）下，这些检验里哪个才是"最好的"？最优检验能否被系统地构造出来？** 这第 4 章正是为此而设——先把"检验"抽象成**检验函数** $\phi(x)\in\{0,1\}$，用显著性水平压住第一类错误，再在所有"守规矩"的检验里挑功效（$\pi(\theta)$）最大的那个（UMP）。当只能在两个具体分布间抉择时，Neyman–Pearson 引理给出显式最优解（似然比）；当参数连续、要做单边假设时，靠**单调似然比（MLR）**直达 UMP；两者都行不通，就用**广义似然比检验（GLRT）**兜底，并借大样本 $\chi^2$ 近似定临界值。于是整套方法从"实践技巧"上升为"最优性理论"——这就是 Casella & Berger《Statistical Inference》假设检验那一章的骨架。
...
**10.5.1**（效益直觉）把你的"信息压缩"直觉用一句话说出：为什么抛 20 次硬币估计 $p$ 时，只需记录"正面总数 $T=\sum X_i$"而不需要记录逐次序列？用条件分布 $P(\mathbf X \mid T=t)$ 是否依赖 $p$ 回答。

**10.5.2**（指数族的充分统计量）对 $\operatorname{Exp}(\lambda)$ 样本 $\mathbf X=(X_1,\dots,X_n)$，把联合密度写成指数族形式，并指出其中的自然参数、充分统计量与对数配分函数。

**进阶题**

**10.5.3**（LM 与 GLRT）说明在正则条件下，广义似然比检验统计量 $-2\log\Lambda$ 的渐近分布，以及它允许检验哪些类型（单点/复合、多维）的假设；它比 NP 的"简单对简单"检验强在何处。

**10.5.4**（功效与样本量）沿用功效公式 $\pi=\Phi(\Delta\sqrt n/\sigma-z_\alpha)$，若 $\sigma=4$、$\Delta=1$、$\alpha=0.05$，要使功效达 $0.8$，所需样本量 $n$ 为多少（$z_{0.05}=1.645$，$z_{0.8}=0.84$）？

**挑战题**

**10.5.5**（后验渐近正态）简述 Bernstein–von Mises 定理的内容与意义：为什么大样本下"贝叶斯后验"与"频率 MLE 的正态近似"趋于一致？这给先验选择带来什么启示？

**10.5.6**（经验似然 vs 非参数）比较"Bootstrap 分位区间"与"基于浓缩似然的区间"在**假设更少**的框架下各自的适用条件，说明哪一个在真实总体非正态、且统计量非平滑时更稳健，并给出理由。
...
**10.5.1**　给定 $T=\sum X_i=t$ 时，样本的条件分布为"$t$ 个正面在 $n$ 个位置上如何排列"，即 $P(\mathbf X\mid T=t)=\binom{n}{t}^{-1}$，与 $p$ 无关。由充分性定义，$T$ 是关于 $p$ 的充分统计量：正面总数已含全部关于 $p$ 的信息，逐次序列不再提供新信息，故估计 $p$ 只写 $T$ 即可。

**10.5.2**　$f(\mathbf x;\lambda)=\lambda^n e^{-\lambda\sum_i x_i}=e^{n\ln\lambda}\,\mathbf 1_{\{x_i\ge0,\forall i\}}\cdot e^{-\lambda\sum_i x_i}$。写成指数族形式：
$$
f(\mathbf x;\lambda)=\underbrace{\mathbf 1_{\{x_i\ge0\}}}_{h(\mathbf x)}\,\exp\Big(\underbrace{-\lambda}_{自然参数 \eta}\cdot \underbrace{\sum_i x_i}_{T(\mathbf x)}-\underbrace{(-\ n\ln\lambda)}_{\psi(\lambda)}\Big).
$$
故自然参数 $\eta=-\lambda$，充分统计量 $T=\sum_i X_i$，对数配分函数 $\psi(\lambda)=-n\ln\lambda$。也可等价取自然参数 $\eta=-\lambda$、$T=\sum x_i$、$\psi(\eta)=n\ln(-\eta)$。

**10.5.3**　在正则条件下 $-2\log\Lambda\xrightarrow{d}\chi^2_\nu$，$\nu=\dim\Theta-\dim\Theta_0$。GLRT 能处理**复合假设**（$H_0$ 内多个参数值、多余参数）与**多维参数**假设，只需在相应子空间取 sup 最大似然；而 NP 只针对"事先给出两个具体分布"的简单对简单假设。GLRT 是 NP 在复合/多维情形下的自然推广，大样本下有近似最优的渐近性质。

**10.5.4**　功效 $0.8$ 时需 $\Phi(\Delta\sqrt n/\sigma-z_\alpha)=0.8$，即 $\frac{\Delta\sqrt n}{\sigma}-1.645=0.84$，得 $\frac{1\cdot\sqrt n}{4}=2.485$，$\sqrt n=9.94$，$n\approx98.8$，取 $n=99$。或者 $n=\big((z_\alpha+z_\beta)\sigma/\Delta\big)^2=(2.485\times4)^2\approx98.8$。这再次说明：要抓住 $\Delta=1$（相对 $\sigma=4$ 仅为 $0.25\sigma$）的小效应，需约百个样本。

**10.5.5**　Bernstein–von Mises 定理：在正则条件下，后验分布渐近为以 MLE 为中心、方差 $[nI(\theta)]^{-1}$ 的正态分布，即 $\pi(\sqrt n(\theta-\hat\theta)\mid\mathbf X)\xrightarrow{d}N(0,I(\theta)^{-1})$。意义：大样本下贝叶斯后验与频率"$\hat\theta\approx N(\theta,I/n)$"趋于一致，二者不再冲突。启示：先验的影响随 $n$ 增大而逐渐消失，只要 $n$ 足够大，无论选何种合理先验，推理结论大体相同；先验真正起作用的是小样本或不规则情形。

**10.5.6**　Bootstrap 分位区间以经验分布替代总体，重复重抽样估计统计量抽样分布，几乎不假设分布形式，对**平滑统计量**（均值、回归系数）在 $n$ 足够大时稳健；但统计量非平滑（如极值、分位数边界）时收敛慢、区间可能偏窄。基于似然的区间（如 profile/LR）依赖"总体属于某参数族"的正则假设，拟合好时更精确、样本利用率高，但一旦族假设错误会失真。因此当总体非正态、统计量又非平滑时，**Bootstrap（配合 m-out-of-n 或 BCa 校正）通常更稳健**——它不锁定分布族，只是耗计算；若正则条件确凿可行，则基于似然的方法更经济高效。
