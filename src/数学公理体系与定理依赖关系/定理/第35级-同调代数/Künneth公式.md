...
> **一句话大白话**：两个空间的乘积（比如直积 $\leftrightarrow$ 平面）其洞可以从两个因子的洞做"外积"拼出来，还加上一个 $\mathrm{Tor}$ 修正项把缠绕误差补上——"乘积的洞=洞的外积+修正"。
>
> **小例子**：环面 $T^2=S^1\times S^1$，由 $H(S^1)=\mathbb{Z},\mathbb{Z}$ 卷出 $H_0=\mathbb{Z}$、$H_1=\mathbb{Z}^2$、$H_2=\mathbb{Z}$，无需另起锅灶。
