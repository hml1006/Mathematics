...
> **一句话大白话**：几乎任何三维空间都能这样"拧出来"：从球里挖掉一条绳子的管子，再按某个斜率把管子重新接回去，拧的松紧就决定了造出什么三维形状。
>
> **小例子**：沿 $S^3$ 中平凡结做斜率 $1/n$ 的 Dehn 手术得到透镜空间 $L(n,1)$；事实上每个闭可定向三维流形都可沿某链环用手术得到（Lickorish-Wallace 定理）。

## 一、定理介绍

> **前置依赖**：纽结管状邻域与子午线-经线基、实心环面的粘合同胚、Heegaard 分解、映射类群的 Dehn 扭转生成定理。
