...
> 微积分教材里，"这件事有多罕见"从来不是"极小极小"一句带过，而是**从真实问题出发 → 建模 → 一步步算 → 再翻译回现实**。本节用 Stewart 式的"策略 → 例题 → 分步解 → 回读现实"四步法，把本级大偏差理论的工具放进排队、破产、风控这些真实场景里跑一遍。每道例题先给**解题策略**，让你看到"这道题该往哪个方向想"，再给出**每步都解释'为什么'**的分步解答。

### 🎯 本级解题策略（Principles of Problem Solving）

Stewart 解决问题讲究"先问对问题，再动笔"，大偏差题尤其如此。做一道大偏差题，请按这四步走：
...
大偏差理论研究"罕见事件"的概率以指数速度衰减的速度。恒稳对数的先声是 **Boltzmann 与 Gibbs** 的统计力学观点——熵与概率的对数成正比，以及 **Laplace** 关于积分渐近展开的思想（拉普拉斯方法），它们都把"主贡献集中在使指数最大的点"作为核心近似。1930 年代 **Cramér** 在研究保险破产问题时提出**大偏差**的基本思想：令 $S_n$ 为独立同分布随机变量和，则 $P(S_n \ge n\alpha)$ 以 $e^{-n I(\alpha)}$ 为主的指数速率衰减，其中 $I(\alpha)$ 为**速率函数**（rate function）。此即 **Cramér 定理**，被视为大偏差理论的奠基性结果（1938 年在保险数学会议上提出，后成为标准结论）。

### 7.2 速率函数与 Legendre–Fenchel 变换

Cramér 定理的核心引入了**指数生成函数（cumulant）**的 Legendre–Fenchel 变换作为速率函数。其内容可视为从测度论与凸分析（Legendre 变换、共轭函数）发展而来，并勾勒出与自由能变分、熵泛函的深刻联系。这一层面后来被 **S. R. S. Varadhan** 系统化为 **Varadhan 引理**与"重对数率"的变分刻画，使大偏差思想能与拉普拉斯方法、热核估计等结合。

### 7.3 Sanov 定理与经验测度

大偏差并不局限于标量求和。**Sanov 定理**给出经验分布（经验测度）偏离真实分布的大偏差速率，其速率函数正是**相对熵（KL 散度）**。这不仅揭示经验测度集中于熵小球的典型性行为，也把大偏差与信息论、随机矩阵、统计物理中的自由熵联系起来。结合 Varadhan（控制随机过程、McKean–Vlasov）与 Boltzmann 熵的关系，Sanov 类型的结论成为统计与物理交叉的枢纽。

### 7.4 现代大偏差理论与应用

现代大偏差理论覆盖马尔可夫链、随机图、随机矩阵与动力系统的罕见事件（如与大偏差原理、Donsker–Varadhan 理论相关的遍历速率）。在实际中广泛应用于通信网络中的队列溢出概率、保险公司的破产概率、排队系统与风险测度估计。拉普拉斯方法、切比雪夫与指数切比雪夫不等式、以及 Legendre–Fenchel 变换等工具，构成分析这类指数速率的常规武器。本文件关于 Cramér 定理、大偏差速率函数、拉普拉斯方法、Sanov 定理的内容，正把握了这门"把小概率事件研究成指数速率问题"的现代学科脉络。
