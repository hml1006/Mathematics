...
### 15.1 流形的现代定义：从黎曼到惠特尼

"流形"思想由**黎曼（Bernhard Riemann, 1826–1866）**在 1854 年的《论作为几何学基础的假设》中提出多维度量流形，但其严格定义需待现代拓扑与可微分析成熟。**魏尔斯特拉斯（Weierstrass）**、**庞加莱**对此过程有所推进；真正给出流形（图册、坐标卡、光滑结构、最终可嵌入欧氏空间）的现代定义的是**惠特尼（Hassler Whitney, 1907–1989）**在 1936–1944 年的工作，他还证明了**Whitney 嵌入定理**（光滑流形可嵌入到欧氏空间，维数翻倍即可），使抽象流形获得"具体"实现。**光滑函数与光滑映射**的概念、切空间与切丛也由 Whitney 及其同代人（如 Steenrod 的纤维丛理论）系统化。本章的光滑流形的定义、光滑函数与光滑映射、切空间与切丛，正是这一现代奠基的直接成果。

### 15.2 张量、外代数与微分形式：从 Ricci 到 Cartan

张量的现代形式来自**里奇（Gregorio Ricci-Curbastro, 1853–1925）**与**列维-奇维塔（Tullio Levi-Civita, 1873–1941）**创立的**张量分析**（"绝对微分学"），**克里斯托费尔（Christoffel, 1829–1900）**引入的联络系数（Christoffel 符号）为平行移动与协变导数提供基础。这一切是广义相对论（爱因斯坦 1915）所需的数学语言。**外代数**（Grassmann 提出外楔积）与**外微分**、**微分形式**则由**嘉当（Élie Cartan, 1869–1951）**发展为流形上"自然积分对象"的完整理论，**de Rham** 据此定义 de Rham 上同调。**Cartan 结构方程**与活动标架法是嘉当对微分几何的机械再表述。本章的张量积、张量代数、张量场、外代数、外微分、微分形式、拉回、Cartan 结构方程，正是里奇—嘉当谱系。

### 15.3 联络、协变导数与平行移动：Levi-Civita 与 Cartan

把"沿方向求导"推广到切丛，需要**仿射联络**的概念。**列维-奇维塔**在 1917 年给出黎曼度量下无挠且度量的联络（Levi-Civita 联络），**嘉当**把它一般化为任意**仿射联络**并研究挠率与曲率；**平行移动**与**测地线**由此获得严格意义。**李导数（Lie derivative）**（来自 Lie 群作用）与**李括号**则刻画向量场的"几何流动"。本章的仿射联络、平行移动与测地线、挠率与曲率、活动标架法、李导数与李括号，正是联络论的发展主线。

### 15.4 流形上的积分与 Stokes 定理

微分形式在**定向流形**上的积分因具有坐标无关性而成为统一微积分的天然对象，**Stokes 定理**（嘉当、古典 Green/Gauss/Stokes 的高维推广）把定向流形上边界的积分与外导数相连，成为现代分析、几何与物理（如电磁学、流体）的标准工具。**庞加莱**与**de Rham** 的上同调则由闭形式与恰当形式商出 de Rham 上同调。本章的定向流形、流形上的积分、Stokes 定理正是这一大定理。

### 15.5 Frobenius 定理与张量分析的现代作用

张量分析与流形在现代以**刚体力学、广义相对论、规范场论与计算机图形学**为应用对象。**Frobenius 定理**（对合分布的可积性）由**弗罗贝尼乌斯（Ferdinand Georg Frobenius）**在 1877 年证明，源自 Clebsch–Pfaff 可分性理论而来，被归结为"对合分布有整流形叶"的经典结论，是几何与动力系统的支柱。**微分形式**、**协变导数**与**Stokes 定理**是相对论、杨-米尔斯理论与几何分析的主要工具；Cartan 的活动标架法更是现代"陈—Simons 理论"的温床。本章的 Frobenius 定理与 Cartan 结构方程所代表的方法，正预示了这些现代方向。
