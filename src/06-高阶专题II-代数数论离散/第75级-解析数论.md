...
### 1. Riemann Zeta 函数与 Riemann 猜想

解析数论的起点是研究素数与 ζ 函数。欧拉（Leonhard Euler，1707—1783）在 18 世纪已发现素数欧拉乘积公式，把 $\zeta(s)=\sum_{n=1}^{\infty}n^{-s}$ 与全体素数联系起来。而把 ζ 函数提升为整套理论核心的是 Bernhard Riemann（格奥尔格·黎曼，1826—1866）：他在 1859 年一篇关于素数个数问题的经典论文中，对 $\zeta(s)$ 作出解析延拓、证明了它的函数方程，并以其著名的 Riemann 猜想（ζ 的非平凡零点全部落在临界线 $\operatorname{Re}(s)=1/2$ 上）震撼后世。Riemann 猜想与素数分布的误差项、以及之后各种"黎曼假设的类比"相互交织，至今仍是数学中最著名的未解问题之一，也是现代解析数论最强大的统一假设。

### 2. 素数定理：从猜测到证明

素数定理 $\pi(x)\sim \frac{x}{\ln x}$ 的历史沿革漫长而曲折。勒让德（Legendre）与高斯（Gauss）在 18 世纪末—19 世纪初分别猜测了素数分布的主项；切比雪夫（P.L. Chebyshev，1821—1894）在 1850 年前后用初等方法给出了 $\pi(x)$ 上下界与素数在 $x$ 与 $2x$ 之间存在的证明（即 Bertrand 假设等结果），为素数定理奠定了坚实基础。真正的证明则要等到 1896 年，由哈达玛（Hadamard）与 de la Vallée Poussin（德·拉瓦莱-普桑）利用解析方法（复数零点无位于 $\operatorname{Re}(s)=1$ 上的事实）几乎同时独立完成——这正是"解析数论"方法战胜初等思路的标志性时刻，此后在 1949 年 Selberg 与 Erdős 给出初等证明又是一次重大进展。

### 3. Dirichlet 特征与 L 函数

狄利克雷（Peter Gustav Dirichlet，1805—1859）在 1837 年创立了解析数论的另一支柱：为证明"存在无穷多个落在给定的与模互素的算术级数中的素数"，他引入 Dirichlet 特征 $\chi$ 与 Dirichlet $L$-函数 $L(s,\chi)$，并证明其关键性质（如 $L(1,\chi)\neq 0$）。这一著名的 Dirichlet 算术级数定理，不仅开创了用解析函数研究算术分发的传统，也直接启发了类域论中的 Hecke 等后续发展。L 函数方法与 ζ 函数方法从此成为解析数论的两大解析工具。

### 4. 筛法与圆法

20 世纪解析数论开展出两大经典方法。一是筛法（sieve methods）：从 Brun（布容）的筛法到 Selberg 筛法，再到大筛法（large sieve，由 Linnik、Gallagher 与 Montgomery 等发展），用于逼近哥德巴赫猜想、孪生素数及三素数等问题。二是圆法（circle method）：由 Hardy（哈代）与 Littlewood（李特尔伍德）在 20 世纪初创始、后经 Vinogradov（维诺格拉多夫）等改进，利用指数的展开分析整数方程的解数，成功地处理了哥德巴赫的弱形式（三素数定理）等加性问题。这两类方法后来都被推广并精细化，是解析数论与加性数论的重要工具。

### 5. 现代发展与影响

对素数定理误差项的研究（如 Riemann 假设给出误差的上界 $\pi(x)=\operatorname{Li}(x)+O(\sqrt{x}\,\log x)$）以及大尺度数值计算（计算机辅助验证零点、以及近年 Riemann 猜想数值与理论工作），把经典解析数论推向当代。解析数论的方法与 L 函数的视角深刻影响了模形式、自守形式与算术几何中的 Langlands 纲领等现代研究。
