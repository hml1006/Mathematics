...
> **一句话大白话**：抓那些"没有边界噪点、处处好脾气"的解析函数——它们在上半平面/圆盘上充分正则，其边界行为受控，这类函数的空间记为 $H^p$，用在信号与复分析的结合处特别顺手。
>
> **小例子**：单位圆盘上解析函数 $f$ 在 $H^p$ 指 $\sup_{0<r<1}(\int_0^{2\pi}|f(re^{i\theta})|^p d\theta)^{1/p}<\infty$；$H^p$ 函数通过"边界函数"可唯一还原，边界上几乎处处有非切向极限——无尖刺的"好信号"。

## 一、定理介绍

> **前置依赖**：Poisson 积分与调和函数、非切向极大函数、Calderón-Zygmund 分解与 Whitney 分解、奇异积分算子、BMO 空间
