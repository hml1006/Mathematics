...
回归分析的核心工具——最小二乘法（least squares）——虽然真正得名"回归"要等到 Galton，但它的数学基础早在 19 世纪初就已奠定。1805 年法国数学家 **Adrien-Marie Legendre** 在《确定彗星轨道的新方法》中正式提出最小二乘法，用它处理天文观测数据。几乎同时，德国数学王子 **Carl Friedrich Gauss** 更早（约 1795 年）就已在测地工作中使用这一方法，并于 1809 年在《天体运动理论》中系统阐述，因而两人之间存在确立最小二乘优先权的著名争议。其核心思想是把"如何让一条拟合直线/曲线离数据点最近"转化为"让残差平方和最小"，这正是今天回归分析"最小化 SSE"的思想源头。Gauss 还在 1821 至 1823 年间证明了最小二乘估计的若干最优性质，为后来的 Gauss–Markov 定理埋下伏笔。

### 7.2 "回归"一词的由来与 Galton 的贡献

单词 **regression**（回归）出自英国科学家、统计学的先驱 **Francis Galton**（1885 年前后）。Galton 在研究中发现"向均值回归"现象：父母身高极端（很高或很矮）时，其子代的身高往往向人群平均水平靠拢。他用最小二乘法拟合父代与子代的关系，并把这种"向平庸回归"的趋势命名为"回归"。Galton 的学生与继承者 **Karl Pearson** 随后系统地发展了相关与回归理论，定义了皮尔逊相关系数 $r$，并推动建立了今天回归分析所使用的框架，包括拟合优度、决定系数 $R^2$ 等量化指标。因此，回归分析从 19 世纪末才真正脱离天文学，成为一门研究变量间统计依赖关系的统计学分支。

### 7.3 从回归到完整推断体系：Fisher 与 Gauss–Markov

20 世纪初，**Ronald Aylmer Fisher** 把推断统计注入回归分析：他建立方差分析（ANOVA），将 $SST = SSR + SSE$ 的平方和分解应用于回归，构造了用于检验整体显著性的 $F$ 检验，并与 Gosset 发展的 $t$ 检验一起构成对回归系数显著性的完整推断工具。同时代的 **Andrei Markov** 与 **C. F. Gauss** 的工作被整理为 **Gauss–Markov 定理**：在误差满足零均值、同方差、不相关的条件下，最小二乘估计是所有线性无偏估计中方差最小的（即 BLUE）。这一结果从理论上确认了 OLS 的相对最优性，也是本章诸多假设检验与区间估计成立的基石。

### 7.4 应对缺陷与现代拓展

经典 OLS 依赖若干理想假设（线性、同方差、无自相关、无多重共线性），现实数据往往偏离。为此发展出加权最小二乘（WLS）与广义最小二乘（GLS，Aitken 1935）处理异方差与自相关；Huber 等发展稳健回归以抵抗异常值；Box 与 Cox（1964）提出响应变量变换；而对高维数据，**Hoerl 与 Kennard（1970）**提出岭回归（$L_2$ 惩罚），**Tibshirani（1996）**提出 Lasso（$L_1$ 惩罚实现变量选择），经由 Efron 等的最小角回归（2004）与后来的 elastic net 形成完整的正则化族，使回归分析在现代数据科学、机器学习的浪潮中持续焕发生命力。
