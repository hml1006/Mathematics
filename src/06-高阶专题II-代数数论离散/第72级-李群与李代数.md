...
> 李群与李代数是一对"连续与无穷小"的学生兄弟：它们诞生于 19 世纪挪威数学家索福斯·李（Sophus Lie）对微分方程对称性的追问——为什么许多方程的解集在平移、旋转之下仍保持同一结构？把"连续变换群"在单位元附近线性化为"无穷小生成元"，就同时造出了李群与李代数两件工具。本章沿"对称性驱动"这条主线，追溯它从微分方程出发，经过结构分类（Killing、Cartan、Dynkin）、表示论（Weyl）的层层浇筑，最终成为现代数学物理共同语言的近一个半世纪历程。

### 7.1 微分方程的对称与有限的伽罗瓦

- **对称思想的前身**：19 世纪中叶以前，"对称"只是零散地出现在几何与代数里；伽罗瓦（Évariste Galois，约 1830 年）把多项式方程的解与"置换群"联系起来，第一次用群来刻画对称。但伽罗瓦处理的是有限、离散的对称——方程根的置换只能有限次进行，数学家还不知道"连续的对称"该怎样描述。
- **微分方程里的连续对称**：同时代里，阿贝尔、达布（Gaston Darboux）等人在求解微分方程时，已开始考察保持方程形式不变的坐标变换，达布对"接触变换"（接触几何的前身）尤有贡献。人们隐约感到：解方程的关键，往往藏在方程两侧同时变换而不改变形式的"对称"之中。
- **李的登场**：1869—1872 年间，年轻挪威数学家索福斯·李与朋友克莱因（Felix Klein）过从甚密，受代数学中群概念与几何学中变换群的启发，决定系统地研究"连续变换群"。与伽罗瓦相对照，李立刻发现：这是一个全新的、值得独立发展的数学对象。

### 7.2 无穷小方法与李代数的诞生

- **无穷小变换**：李的核心洞察是"取一小步"：一次连续变换总可以近似看作"按某个速度场走了一小段时间"。他把每一种连续变换看成 $x \mapsto x + \varepsilon X(x)$（$\varepsilon$ 为无穷小）的"积分"结果，其中 $X$ 是一个一阶微分算子，即"无穷小生成元"。
- **切空间即李代数**：这些无穷小生成元张成一个线性空间，但它还带一个天然的"括号"乘积：两个无穷小变换"先做甲的一小步、再做乙的一小步、再反向退回来"的净效果，由交换子 $[X,Y]=XY-YX$ 给出。李证明这个括号满足反对称与雅可比恒等式——这正是后来抽象的"李代数"。
- **概念的定型**：约 1870 年代李的讲义与论文逐步确立：连续变换群的局部结构完全由它的无穷小生成元代数决定。这个"群 → 无穷小代数"的对应，就是李代数概念的源起。

### 7.3 三大基本定理：群与代数的局部对应

- **李的三大定理**：李证明了连接群与代数的三条基本结果：第一定理说明单参数子群的无穷小生成元就是切向量；第二定理说明李代数决定群的局部结构；第三定理则断言（局部版本）每个实李代数都对应某个局部李群——"李第三定理"成为后世教材的支柱。
- **从局部到整体**：这些定理最初只保证"局部"的对应（在单位元附近）。人们随后意识到，一个完整的李群是多个局部块的贴身拼合，局部李群可以"全局化"为单连通覆盖上的整体李群——这是后来更精细的整体理论（见 7.11）的伏笔。
- **指数的角色**：把无穷小元素映射回群的关键工具是指数映射 $\exp(X)=\sum X^{k}/k!$，它把速度场"积分"成单参数子群。$X \mapsto e^{X}$ 在单位元处是局部微分同胚，这为"微分几何地"复原件群结构奠定了基础。

### 7.4 抽象化与严格化：从变换群到抽象李群

- **卡当/韦伯的整理**：李本人的论述依赖大量微分方程技术，叙述冗长。1888—1893 年他与恩格尔（Friedrich Engel）合著三卷本《变换群理论》，把变换群的构造系统化，但也暴露出基础不牢的问题。
- **抽象定义的建立**：20 世纪之交前后，希尔伯特（David Hilbert）等人的第五问题精神（不久将追问"何为李群的最少假设"）推动人们把李群从"具体变换群"提炼为"带群结构的流形"这一抽象对象。韦尔（见 7.8）在 1920 年代为其写出半严谨的复导读本。
- **后期严格化**：真正把"李群即流形 + 群同态"的现代定义夯实，要到 1950 年代（希尔伯特第五问题的解答）——此时李的直觉被完全纳入微分几何与拓扑学的语言，抽象李群才算最终定型。

### 7.5 结构理论的开端：Killing 的根

- **半单与可解的划分**：1880 年代末，德国数学家基灵（Wilhelm Killing）在为"为何李代数看似只有有限的'原型'"求解时，指出真正不可再分解的李代数是"单的"，并系统研究李代数的结构。他先立起"可解""单""半单"等关键分类范畴，明确分类的对象应当是复半单李代数。
- **Cartan 子群与根的提出**：Killing 引入"特征根"思想：取极大可交换子代数（后来称 Cartan 子代数），考察伴随作用在其上的特征值——这些特征值扫出的对象就是"根"。他还正确猜想：根只有少数几种有限的布局。
- **地基与bug**：基灵的工作极有洞见却常有疏漏与证明错误，其关键设想（根表只有有限几种）由后继者在严格证明中完成——这为卡当的收官之作作了铺垫。

### 7.6 Cartan 的收官：单李代数的完全分类

- **博士论文**：1894 年，法国数学家埃利·嘉当（Élie Cartan）发表博士论文《有限连续群的结构》，以严格复现基灵的思想为基础，正面证明了复单李代数的完全分类：四大经典族 $A_n,B_n,C_n,D_n$ 与五个例外代数 $G_2,F_4,E_6,E_7,E_8$。
- **重整根的方法**：卡当在基灵粗疏的根表之上，用"基本根"与 Jacobi 恒等式重新组织理论，把分类归结为配一种有限维组合结构，被后人称为卡当矩阵。这套"给定根 → 组合数据 → 枚举类型"的模板，成为结构理论的标准范式。
- **实李代数与对称空间**：卡当终其一生还把分类推广到实李代数、复域以外的情形，并转向"对称空间"的研究——为 20 世纪任几何学中的结构与分类提供了统一素材。

### 7.7 根系与 Dynkin 图：把分类画成一幅图

- **从矩阵到图**：分类虽由卡当完成，但"基本根"的内积数据（卡当矩阵盯住相邻根之间的夹角与长度比）仍不直观。20 世纪中期，苏联数学家邓金（Eugene Dynkin，1947 年前后）提出用"图"表示根系：每个顶点一支根，根之间按夹角画单边、双边或三边连线。
- **Dynkin 图与分类表**：邓金图让晦涩的分类一目了然：$A_n$ 是一串单点相连的链，$D_n$ 是分叉的树，$E_6,E_7,E_8$ 是带三叉的树，$G_2,F_4$ 则带特殊的多重边。对小型（>2 度）不可约根系的枚举由此彻底得到图解。
- **互补的成就**：几乎同时，科克斯特（Harold Coxeter）从对称多胞体与反射群角度独立得到等价的分类（科克瑟图）。两人殊途同归，使"结构分类 = 画出一幅 Dynkin 图"成为今天每一位学习者的常识。

### 7.8 表示论的成熟：Weyl 与特征标公式

- **从结构到表示**：分类告诉我们"一个半单李代数长什么样"，还须回答"它如何作用在向量空间上"。荷兰—德国数学家韦尔（Hermann Weyl，1920 年代）把李群表示论推向系统化，证明紧李群的表示完全可分解，并建立通常权与最高权的对应。
- **Peter–Weyl 定理**：1927 年韦尔与彼得（Fritz Peter）证明著名的 Peter–Weyl 定理：紧群上的平方可积函数可由其不可约表示矩阵系数逼近——这是傅里叶级数在紧群上的推广，也是调和分析在对称对象上的根基。
- **Weyl 特征标公式**：韦尔还给出最高权××不可约表示的"特征标"闭式公式：用权与根的对称组合，把不可约表示的维度、权重多重性一次性算清——这是表示论中最早也最深刻的"封口"结果之一。

### 7.9 紧群与调和分析：对称的谱理论

- **紧群作为"好对象"**：在紧李群上，一切表示都有有限维不可约分解、特征标与 Peter–Weyl 展开——紧性使分析问题具有"好性格"。韦尔与卡当对紧李群的分类、对最大环面的研究，把"有限维表示的分类"归结为"权格上的组合问题"。
- **与傅里叶的联系**：把圆周与球面看作旋转群的齐次空间，球谐函数、傅里叶级数等经典内容被重新解释为紧群表示的基函数——对称性分析由此回灌到古典分析（参见第 53 级调和分析、第 54 级傅里叶分析）。
- **对物理的溢出**：紧群（$SU(2),SU(3),\dots$）的表示语言后来直接成为粒子物理中"夸克颜色""同位旋"等书写的数学母语（见 7.10）。

### 7.10 物理的引擎：量子力学与对称性

- **角动量与旋转群**：1920—1930 年代，维格纳（Eugene Wigner）与韦尔把群表示引入量子力学：电子的角动量 $L$、自旋 $S$ 恰好落在 $\mathfrak{su}(2)$ 的可约表示上，其不可约分量对应不同的能级与量子数 $l,m$。对称性"为何能级如此分裂、为什么守恒律如此配对"的问题，有了统一回答。
- **表示即态的分类**：量子态的"量子数"被重新诠释为表示论的权；$SO(3)$ 与 $SU(2)$ 的表示分类直接对应原子光谱的标识。诺特定理（见 7.12）进一步把连续对称与守恒流联系起来，物理学家开始自觉地"以对称立方程"。
- **从数学到物理的对话**：$SU(2)$ 与 $\mathfrak{sl}(2)$ 的表示论成为标准物件——教材中反复出现的"角动量代数"本质上就是对 $\mathfrak{su}(2)$ 的抽象。对称性驱动物理分类的理念，使李群与李代数从纯数学走向现代物理的基础地位。

### 7.11 整体化与推广：从局部李群到 Kac-Moody

- **整体李群与齐性空间**：20 世纪中叶以后，李群被置于整体微分几何与拓扑的框架中：单连通覆盖、齐性空间 $G/H$、对称空间成为标准对象；伴随表示、Cartan 子群等概念被赋予几何直观
- **代数群的兴起**：切舍瓦莱（Claude Chevalley，1950 年代前后）把李代数"算数化"，研究定义在任意域上的代数群，使李理论脱离实数/复数，进入代数几何与代数数论领地（衔接第 77 级算术几何、第 141 级朗兰兹纲领）。
- **无穷维与 Kac-Moody**：1960 年代末，Kac 与穆迪（Igor Kac，Robert Moody，约 1968 年）独立把根系推广到一般化的（广义卡当）矩阵，构造出 Kac–Moody 代数——无穷维李代数首次获得与半单理论平行、由广义 Dynkin 图刻画的结构。这本是组合对象的意外馈赠，后来在物理学（顶点代数）中大放异彩。

### 7.12 现代图景：抽象、结构与物理的统一

- **从连续到结构**：至此，李理论形成一条清晰的进化链：对称问题（19 世纪李）→ 无穷小代数（李）→ 抽象李群（韦尔等）→ 结构分类（Killing 草创、CarTan 收官、Dynkin 图解）→ 表示论（Weyl）→ 物理实现（Wigner）→ 代数群与无穷维推广（Chevalley、Kac–Moody）。
- **理论基础的最后一块**：希尔伯特第五问题（李群 = 连续作用的局部队形）于 1952—1955 年间由格利森、蒙哥马利、齐平解答，确认"拓扑群的连续结构局部即是解析李群结构"，为整个理论补上了最严格的地基。
- **共同语言**：今天，李群与李代数已化身规范场论、标准模型、微分几何齐性空间、数论自守形式与代数几何旗流形背后的共用骨架——"用对称分析结构、以无穷小把握连续"成为贯穿多个学科的统一策略。

### 7.13 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 约 1830 年 | 伽罗瓦 | 以置换群刻画方程对称（有限群范本） |
| 1869—1872 | 索福斯·李、克莱因 | 提出系统研究连续变换群的计划 |
| 约 1870—1880 | 索福斯·李 | 建立无穷小方法与李代数；三大基本定理 |
| 1888—1893 | 李、恩格尔《变换群理论》 | 三卷本系统化连续变换群理论 |
| 1880 年代末 | 威廉·基灵 | 引入根与 Cartan 子代数，草创半单分类 |
| 1894 | 埃利·嘉当 | 博士论文严格完成复单李代数完全分类 |
| 1920 年代 | 赫尔曼·韦尔 | 紧李群表示论、Peter–Weyl 定理、特征标公式 |
| 1927 | 彼得、韦尔 | Peter–Weyl 定理：紧群上函数谱表示 |
| 1920—1930 | 维格纳、韦尔 | 群表示用于量子力学对称性 |
| 1947 前后 | 邓金、科克斯特 | Dynkin 图 / 图分类表示根系 |
| 1950 年代 | 切舍瓦莱 | 定义任意域上的代数群，李理论算术化 |
| 1952—1955 | 格利森等 | 希尔伯特第五问题解答，李群定义严格化 |
| 约 1968 年 | Kac、穆迪 | Kac–Moody 代数：广义根系的无穷维推广 |

### 7.14 小结

> 李群与李代数的历史，是一部"对称"从谜题到骨架的进化史：索福斯·李把微分方程的变换对称提炼为无穷小代数，莱布尼茨式的取小步思想为连续结构立下切空间模型；Killing 以根与 Cartan 子代数埋下分类的种子，卡当用博士论文收官完成复单分类，邓金与科克斯特再把分类画成一幅幅可读的 Dynkin 图；韦尔把表示论炼成特征标公式，维格纳让它在量子力学里开花结果；最终经希尔伯特第五问题严格化、切舍瓦莱普及化、Kac–Moody 推向无穷维。贯穿始终的，是"局部控制整体、无穷小把握连续"的信念——从微分方程到粒子物理，同一套对称密码在不同世纪反复破译。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对李群与李代数核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 8.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到李群与李代数语境的追问上。

| 步骤 | 行动 | 李群与李代数中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 面对的是李群还是李代数？矩阵的还是抽象的？要在李代数里算指数、括号，还是在李群里求单参数子群、还是分析根系？ |
| ② 计划 | 选择可行工具、预估方向 | 用 $\exp(tX)$ 回李群？用 $\operatorname{ad}(X)$ 与 Killing 形式判定半单与紧？用 Cartan 子代数 $+$ 根空间分解做分类？先想清楚哪条路最省力。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 每步写明依据（定义/定理编号），矩阵指数的项、括号对易号、根向量重数、Dynkin 图连线别漏。 |
| ④ 回看 | 核对结果、检验合理性 | 结果是否依赖基或表示的选取？行列式/迹/Trace 是否保持？能否用 $\operatorname{exp}$ 逆向回到 $G$ 再验一遍？ |

> 💡 **策略演示**：计算 $A=\begin{pmatrix}0&1\\0&0\end{pmatrix}$ 的矩阵指数。
> ① 理解：二阶幂零矩阵，求 $\exp(A)$。② 计划：$A^2=0$ 使级数从第三项起截断。③ 执行：$\exp(A)=I+A=\begin{pmatrix}1&1\\0&1\end{pmatrix}$。④ 回看：$\det\exp(A)=1$，与 $\exp(\operatorname{tr} A)=e^0=1$ 吻合，答案合理。
...
1. 李代数的括号运算既满足反对称性又满足雅可比恒等式。（　）
2. Ado 定理保证每个有限维李代数都有一个忠实有限维表示，从而可嵌入某个 $\mathfrak{gl}(n)$。（　）
3. 对任意李群，指数映射 $\exp:\mathfrak g\to G$ 都是一个群同态。（　）
4. 单参数子群是保群结构的同态 $\varphi:\mathbb{R}\to G$，且由 $\varphi'(0)\in\mathfrak g$ 唯一决定。（　）
5. 若 Killing 形式 $B$ 在 $\mathfrak g$ 上非退化，则 $\mathfrak g$ 半单。（　）
6. 对 $\mathfrak{sl}(2)$ 的 Cartan 子代数 $\mathrm{span}\{H\}$，$\operatorname{ad}(H)$ 在 $E,F$ 上的特征值分别为 $\pm2$。（　）

**选择**：

7. 紧连通半单李群对应李代数的 Killing 形式是（　）。
   - A. 正定　B. 负定　C. 退化　D. 恒为零
8. 指数映射 $\exp:\mathfrak g\to G$ 在单位元附近（　）。
   - A. 恒为满射　B. 是局部微分同胚　C. 恒为单射　D. 总把 $\mathfrak g$ 映满 $G$
9. 李代数 $\mathfrak{sl}(2,\mathbb{C})$ 的 Dynkin 图是（　）。
   - A. 一个顶点　B. 一条带两个顶点的边　C. 三个顶点　D. 四环图
...
> **问题**：求矩阵 $A=\begin{pmatrix}0&-1\\1&0\end{pmatrix}$ 的指数 $\exp(tA)$。

- ① 理解：$A$ 是 $90^\circ$ 旋转矩阵，求 $\exp(tA)\in\operatorname{SO}(2)$。
- ② 计划：观察到 $A^2=-I$，把指数级数按奇偶项重排，化成 $\cos$、$\sin$ 的组合。
- ③ 执行：

$$
\exp(tA)=I+\sum_{k\ge1}\frac{t^{2k}}{(2k)!}A^{2k}+\sum_{k\ge0}\frac{t^{2k+1}}{(2k+1)!}A^{2k+1}
=I\cos t+A\sin t=\begin{pmatrix}\cos t&-\sin t\\\sin t&\cos t\end{pmatrix}.
$$

- ④ 回看：所得矩阵行列式为 $1$、正交，确在 $\operatorname{SO}(2)$，且 $\exp((t+s)A)=\exp(tA)\exp(sA)$ 成立（$\mathbb{R}$ 到 $\operatorname{SO}(2)$ 的单参数子群）。**易错点**：忘掉 $A^{2k}=(-1)^k I$ 的奇偶分列导致符号错乱；应按 $k$ 奇偶分组。

**示例 S2：李括号的基计算**

> **问题**：在 $\mathfrak{sl}(2,\mathbb{C})$ 的基 $H=\begin{pmatrix}1&0\\0&-1\end{pmatrix},\ E=\begin{pmatrix}0&1\\0&0\end{pmatrix},\ F=\begin{pmatrix}0&0\\1&0\end{pmatrix}$ 下计算 $[H,E]$、$[H,F]$、$[E,F]$。

- ① 理解：李括号即矩阵乘法相减 $[X,Y]=XY-YX$，求出三个交换关系。
- ② 计划：逐个做矩阵乘法并相减，写出结构常数。
- ③ 执行：

$$
[H,E]=HE-EH=2E,\qquad [H,F]=HF-FH=-2F,\qquad [E,F]=EF-FE=H .
$$

- ④ 回看：$[H,\cdot]$ 作用到 $E,F$ 的特征值为 $\pm2$，与「$\mathfrak{sl}(2)$ 根为 $\pm2$」一致；反交换 $[E,F]=-[F,E]$ 已由括号定义自动保证。**检验法**：把任意交换子组合代回雅可比恒等式 $\sum_{\text{cyc}}[X,[Y,Z]]=0$ 验证不自相矛盾。

**示例 S3：单参数子群的求解**

> **问题**：求 $\mathfrak{sl}(2,\mathbb{R})$ 中 $X=\begin{pmatrix}0&1\\1&0\end{pmatrix}$ 生成的单参数子群 $\exp(tX)$。

- ① 理解：单参数子群即同态 $\varphi(t)=\exp(tX)$，求其显式矩阵。
- ② 计划：$X^2=I$，把级数按 $I$ 与 $X$ 的系数重排成 $\cosh$、$\sinh$。
- ③ 执行：$\exp(tX)=I\cosh t+X\sinh t=\begin{pmatrix}\cosh t&\sinh t\\\sinh t&\cosh t\end{pmatrix}$，行列式 $\cosh^2 t-\sinh^2 t=1$，故落在 $\operatorname{SL}(2,\mathbb{R})$。
- ④ 回看：$\varphi(s+t)=\varphi(s)\varphi(t)$ 由指数性质自动成立；$\varphi'(0)=X$ 与生成元相符。**易错点**：把 $\cosh/\sinh$ 误写成 $\cos/\sin$；当 $X^2=I$ 时应走双曲路线而非圆路线。

**示例 S4：根系统的分类计算**

> **问题**：对 $\mathfrak{sl}(3,\mathbb{C})$，取 Cartan 子代数 $\mathfrak h=\{\operatorname{diag}(h_1,h_2,h_3):\sum h_i=0\}$，求 $\operatorname{ad}(\mathfrak h)$ 的可对角化特征空间及其根。

- ① 理解：根是把 $\mathfrak h\to\mathbb{C}$ 的线性泛函，$[H,E_\alpha]=\alpha(H)E_\alpha$，要枚举所有基矩阵 $E_{ij}$ 的特征值。
- ② 计划：对对角矩阵与 $E_{ij}$ 做括号，提取特征值函数 $\alpha_{ij}(H)=h_i-h_j$，收集成根集。
- ③ 执行：

$$
\operatorname{ad}(\operatorname{diag}(h_1,h_2,h_3))\,E_{ij}
=[\operatorname{diag}(h_1,h_2,h_3),\,E_{ij}]=(h_i-h_j)E_{ij},
$$

$$
\Phi=\{\,e_i-e_j: i\ne j\,\}=\{\,e_1-e_2,\ e_2-e_3,\ e_1-e_3,\ \text{及其相反元}\,\}.
$$

- ④ 回看：$\Phi$ 恰含 $6$ 个非零根、符合 $\operatorname{rk}=2$ 时的 $A_2$ 根系；Cartan 对角部分对应多重度 $\operatorname{rk}=2$（余秩）。**检验法**：取三个根画在平面，夹角 $60^\circ/120^\circ$，Cartan 矩阵 $\begin{pmatrix}2&-1\\-1&2\end{pmatrix}$ 与 Dynkin 图 $A_2$ 一致——与分类表核对自洽。

### 8.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：对 $\mathfrak{su}(2)$ 的基（Pauli 矩阵）取一个 $X$，用数值积分验证 $\exp(sX)\exp(tX)=\exp((s+t)X)$ 及 $\exp(tX)\exp(tY)\ne\exp(t(X+Y))$（非交换），观察误差随步长的下降。
- **聚焦 P2（跨章节联系）**：用 §2.13 的 Weyl 维数公式对 $\mathfrak{sl}(3)$ 的最高权标出各表示的维数，并与 §2.14 的旗流形上 $G/B$ 的 Borel 子代数做交叉印证。
- **聚焦 P3（反例构造）**：构造一个非半单李代数（如可解的上三角代数 $\mathfrak b_n$）与一个半单的，比较它们 Killing 形式的退化情况，指出 $B$ 零根/退化如何测出「可解尾巴」。
...
- **自测 1**：用一句话解释「为什么李群在单位元处的切空间天然带有李代数结构」，并说出括号来自哪里。
- **自测 2**：说明「指数映射为什么只在单位元附近是局部微分同胚」，并举反例说明它不必整体满射。

**$\bullet$ 技能题**

- **自测 3**：计算 $B=\begin{pmatrix}0&0&1\\0&0&0\\0&0&0\end{pmatrix}$ 的矩阵指数（提示：$B^2=0$）。
- **自测 4**：对 $\mathfrak{sl}(2)$ 验证雅可比恒等式 $[[H,E],F]+\text{循环}=0$。
- **自测 5**：求 $\mathfrak{so}(2)$ 中单个基生成元的单参数子群，并写出其与 $\cos\theta,\sin\theta$ 的关系。

**$\blacklozenge$ 综合题**

- **自测 6**：对 $\mathfrak{sl}(2,\mathbb{R})$ 求 Killing 形式 $B(X,Y)=\operatorname{tr}(\operatorname{ad}X\operatorname{ad}Y)$，并判定其半单性与紧性。
- **自测 7**：画出 $\mathfrak{sl}(3)$ 根系 $A_2$ 的 $\pm(e_i-e_j)$ 六根图，写出对应 Cartan 矩阵，并指出三对相反根分别位于哪两条连线上（给出易混判别法）。

### 8.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（李代数定义即反对称 $+$ 雅可比）。2. **正确**（Ado 定理给出忠实有限维表示）。3. **错误**（仅对交换李群，$\exp(X+Y)=\exp X\exp Y$；一般用 Baker–Campbell–Hausdorff 修正）。
> 4. **正确**（$\varphi'(0)\in\mathfrak g$ 唯一决定 $\varphi$）。5. **正确**（Killing 形非退化等价于半单）。6. **正确**（$[H,E]=2E,\ [H,F]=-2F$）。
>
> **选择**：7. B　8. B　9. A。

> **自测**：
> - 自测 1：切空间 $T_eG$ 左不变向量场的括号仍左不变，故闭合成李代数，括号来自左不变生成元的对易子。
> - 自测 2：指数映射在单位元的微分是恒同、故局部可逆（反函数定理保证局部逆存在），但如 $\exp:\mathbb{R}\to U(1)$ 整体单参数覆盖、又例 $\mathfrak{sl}(2)\to\operatorname{SL}(2)$ 并不覆盖到全部矩阵。
> - 自测 3：$B^2=0\Rightarrow\exp B=I+B=\begin{pmatrix}1&0&1\\0&1&0\\0&0&1\end{pmatrix}$。
> - 自测 4：$[[H,E],F]=[2E,F]=2[E,F]=2H$，循环得 $0$（其余两项互抵），雅可比成立。
> - 自测 5：$\exp(tX)=\begin{pmatrix}\cos t&-\sin t\\\sin t&\cos t\end{pmatrix}$，即旋转群 $U(1)\cong\operatorname{SO}(2)$ 上的标准旋转。
> - 自测 6：$\operatorname{ad}$ 在基 $H,E,F$ 上 $B(H,H)=8,\ B(E,F)=4$（其余 $0$），非退化故半单；但 $\mathfrak{sl}(2,\mathbb{R})$ 上 Killing 特征值符号不尽同，故不紧。
> - 自测 7：六根 $\pm(e_1-e_2),\ \pm(e_2-e_3),\ \pm(e_1-e_3)$ 张成 $A_2$，三对相反根各方向夹角 $60^\circ$；简并的是 $e_1-e_3=(e_1-e_2)+(e_2-e_3)$ 为另两根之和，Cartan 矩阵 $\begin{pmatrix}2&-1\\-1&2\end{pmatrix}$，Dynkin 图为两点单边。