...
> **一句话大白话**：任何测度都能被干净地切成两半——一半"看得到密度、能写进积分"的绝对连续部分 + 一半"专门住在零测集上"的奇异部分，两个部分互相不理不睬。
>
> **小例子**：直线上的任何 $\sigma$-有限测度 $\mu$ 都可写成 $\mu=\mu_a+\mu_s$，其中 $\mu_a$ 对勒贝格测度绝对连续（有密度函数），$\mu_s$ 奇异（完全被某零测集承载），且两部分彼此正交。
...
**前置依赖**：Radon-Nikodym 定理、测度的绝对连续与奇异、$\sigma$-有限测度、Lebesgue 积分表示
