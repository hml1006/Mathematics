...
> **一句话大白话**：把"因子"（中心只是单个复数的 von Neumann 代数）按"维度类型"分成 I、II、III 型，从有限维一路到无穷维再到极度破碎——像给物质分固态、液态、气态。
>
> **小例子**：矩阵环 $M_n(\mathbb{C})$ 是 I$_n$ 型因子；由无限张量积算子代数得到的超有限因子 $L(S_\infty)$ 是 II$_1$ 型；而许多群测度空间因子属于 III 型。

## 一、定理介绍

> **前置依赖**：von Neumann 代数与双交换子定理、投影算子与 Murray-von Neumann 等价、部分等距算子、正规迹与正泛函、投影比较定理
