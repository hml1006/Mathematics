...
黎曼几何的诞生来自**黎曼（Bernhard Riemann, 1826–1866）**在 1854 年 6 月的就职演讲《论作为几何学基础的假设》（*Über die Hypothesen, welche der Geometrie zu Grunde liegen*，1866 年正式发表）。黎曼把"空间"推广为任意维的流形，其上以**黎曼度量**（点积在每一点处的光滑变体）指定内积，从而赋予"长度""角度""弯度"以一般含义。他提出常曲率空间与空间曲率可能随位置变化的思想，奠定了"度量张量"作为基本结构。本章的黎曼度量、黎曼流形、度量的存在性，正是这一开端的抽象表述。黎曼的 1854 论著通常被引为 1866 年的正式出版文本。

### 14.2 Levi–Civita 联络与曲率张量

有了度量，下一步是定义"如何沿流形平行移动与求导"。**克里斯托费尔（Christoffel）**在 1869 年引入联络系数（Christoffel 符号），**列维-奇维塔（Tullio Levi-Civita, 1873–1941）**在 1917 年把"平行移动"推广到弯曲流形，给出与度量相容且无挠的唯一联络（**Levi-Civita 联络**），**李（Sophus Lie）**的括号运算与**嘉当**等把联络、挠率、曲率统一到"协变导数"的语言。**Riemann 曲率张量**、截面曲率、**Ricci 曲率**与**标量曲率**作为刻画弯曲的量在 19–20 世纪逐步确立。本章的 Christoffel 符号、Levi-Civita 联络的唯一性、协变导数与各类曲率，正是这些奠基工作的标准呈现。

### 14.3 测地线与指数映射：几何路径的极值性

弯曲流形上最自然的"直线"是**测地线**（局部最短线），其方程与**指数映射**给出从切空间到流形的"沿测地线"对应。**Gauss 引理**提供正交性与测地线的扇形结构，**Jacobi 场**与**共轭点**刻画了测地线的稳定范围（由第一、第二变分公式刻画）——这些是整体几何与变形理论的基石，其后由 **Hopf–Rinow**（完备黎曼流形的测地完备等价于有界闭集紧致）等完成。本章的测地线方程、指数映射、Gauss 引理、Jacobi 场、共轭点、第二变分公式与 Hopf–Rinow 定理，正是这本"测地线世界观"的表述。

### 14.4 常曲率空间与空间的分类

**常曲率空间**（空间形式）是曲率恒定的黎曼流形，代表性的有球面 $S^n$、双曲空间 $H^n$ 与平直空间（如平坦环面 $T^n$）。**黎曼**本人已讨论常曲率模型，**嘉当**、**Klein** 与 **Thurston** 等将之纳入几何化纲领；**Berger 球面**则提供了非典型的（非等曲率的）度量构造。常曲率空间的覆盖与商（覆叠、商流形与 Galois 对称）也联系到代数与拓扑。本章的常曲率流形、$S^n$、$H^n$、平坦环面与 Berger 球面，正是对"曲率为常数的几何"的经典处理。

### 14.5 比较定理、Gauss–Bonnet 与物理学支点

整体黎曼几何以比较曲率导出全局结论。**Cartan–Hadamard 定理**（非正曲率流形的高维版）、**Bonnet–Myers 定理**（正 Ricci 曲率导致紧致与测地完备性）、**Rauch 比较定理**与 **Synge 定理**、**Ricci 比较**和**体积比较**是整体几何的核心；**Gauss–Bonnet–Chern 定理**（高维推广由陈省身给出）把曲率积分与拓扑（Euler 示性数）相连。这些定理所共同服务的，是**广义相对论**——爱因斯坦用 Ricci 曲率（$R_{ij}=0$ 的自由空间场方程）把引力几何化。黎曼几何由此进入物理学与几何分析的核心，成为现代数学与理论物理交汇的基石。本章的一组重要整体定理，正体现了这段比较理论的丰饶果实。
