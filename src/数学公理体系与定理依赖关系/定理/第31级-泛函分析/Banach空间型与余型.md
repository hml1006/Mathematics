...
> **一句话大白话**：型与余型是Banach空间"在* $p$-平均意义上常不常往欧几里得靠"的两种指标——型描述"该空间像不像 $L_p$ 那样好相加"，余型描述它的对偶空间那边——用它们判断空间能否嵌入普通欧氏空间。
>
> **小例子**：$L_2$ 空间的型与余型都取正确的 $p$（型取2、余型取2）；而 $L_1$ 空间的型很差（型取1），$L_\infty$ 的余型很差（余型取 $\infty$），说明它们离"欧氏味道"较远。
