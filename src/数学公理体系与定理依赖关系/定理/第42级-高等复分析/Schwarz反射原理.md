...
> **一句话大白话**：一个在上半平面解析、在实轴上连续且取值仍是实数的函数，可以"照镜子"般翻到下半平面再解析延拓——镜像映射 $\overline{u(\bar z)}$ 会把上半平面的解跨过边界平滑地接到下半平面。
>
> **小例子**：$f$ 满足 $f(\bar z)$ 的镜像粘贴映射——若 $f$ 在 $\{z:\Im z>0\}$ 解析、在实数轴上取 $\mathbb{R}$ 且无穷远处温和，则延拓到全平面仍解析（如 $\log z$ 沿正实轴被延拓出镜像分支）。
