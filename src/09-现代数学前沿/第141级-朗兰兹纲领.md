...
### 七.1 从类体论到Langlands纲领的诞生

朗兰兹纲领的信念建立在"类域论"的基础上。类域论（由希尔伯特、高木贞治（1920）、Artin等人于20世纪初建立）已经揭示了一个惊人的对应：交换情形下，有理数域/数域的Abel扩张（Galois群）可以用其"理元群/平格理类群"的某些Abel群刻画，即"Galois群"与"自守对象"之间初步的对应。真正把这一对应推广到"非交换"情形的是加拿大数学家朗兰兹（Robert Langlands）。1967年，朗兰兹在写给安德烈·韦尔（André Weil）的一封信中系统提出了著名的"Langlands纲领"，给出一个宏大的猜想框架：把数域的Galois（伽罗瓦）表示与自守表示（自守形式）对应起来，并要求L-函数保持一致。这一"letter to Weil"成为20世纪最深刻的数学纲领的起点。

### 七.2 Langlands对偶群、自守形式与谷山-志村

Langlands纲领的核心概念之一是"Langlands对偶群"（李群的对偶构型，如 $\mathrm{GL}(n)$ 的对偶仍是 $\mathrm{GL}(n)$），它把自守表示与Galois表示编码到同一群的语言中。自守形式（椭圆模形式、自守表示，由Jacquet与Langlands在1960年代系统化为"谱分解与L-函数"）提供了"分析侧"，Galois表示提供了"算术侧"，两侧经L-函数连接。最深刻的特例是模性定理：谷山-志村-韦尔猜想断言每条椭圆曲线对应权2模形式，1994年被怀尔斯（Wiles）与泰勒（Taylor）在椭圆曲线半稳定情形证明，并随之终结了费马大定理——这被视为朗兰兹纲领"自守侧—Galois侧"对应在 $\mathrm{GL}(2)$ 上的里程碑，也正是本教程 $\mathrm{GL}(1)$、$\mathrm{GL}(2)$ 章节的直接背景。

### 七.3 函数域与全局Langlands对应：Drinfeld

在函数域（有限域上的代数曲线）情形，Langlands对应被彻底解决。1980年代，德林费尔德（Vladimir Drinfeld）引入"shtuka"（俄语"裤子"）构造，证明了 $\mathrm{GL}(2)$ 在函数域上的全局Langlands对应；此后Lafforgue、Scholze等扩展至更高秩，并因这些工作分别获菲尔兹奖（Drinfeld 1990、Lafforgue 2002）。本教程的"全局Langlands对应（Drinfeld的shtuka构造）"正是描述这一成果。而"局部Langlands对应"则在局部域上由Harris-Taylor、Henniart等完全证明，本教程亦作专门介绍。

### 七.4 L-函数与函子性猜想

L-函数的思想渊源于Riemann zeta函数（1859）与Dirichlet L函数、Hecke L-函数；Langlands纲领把L-函数推广到所有自守表示，并猜测其解析性质（解析延拓、函数方程）。"Langlands函子性"是纲领的核心：它宣称Galois侧（或有理函数侧）之间的同态应诱导自守表示族之间的"提升"，从而把不同自守形式统一起来。Ngô（吴宝珠）证明的"基本引理"（2008年前后，导致2010年菲尔兹奖）为函子性猜想的证明提供了关键工具，是Langlands纲领近年最重要的严格进展之一；而Arthur–Selberg迹公式则是证明对应的一线理论工具。本教程的L-函数、Langlands函子性、Langlands对偶群章节即为此脉络。

### 七.5 Langlands纲领的影响与开放性

今天，朗兰兹纲领被誉为"数学的大统一理论"之一，它把数论、表示论、代数几何与数学物理（如几何Langlands、p-adic Langlands、弦论中的电-磁对偶）紧密联结。尽管在一般数域情形仍远未解决（全局Langlands、函子性只获部分进展），但其概念已渗透进大量现代研究，且不断催生新的理论分支（如几何Langlands见下一级）。其历史意义在于：它把"数论如何与群表示言语互通"这一古老追问，发展为一个仍在不断开花结果的统一蓝图。
