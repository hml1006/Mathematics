...
算子代数研究希尔伯特空间上算子全体所构成的代数结构，是二十世纪泛函分析最深刻的分支之一。它同时孕育于数学分析（谱理论）、量子力学（不可交换观测量的表示）以及 Banach 代数的一般理论，其历史与"把线性算子当作代数元素"这一观念的逐渐深化同步。

### 7.1 从算子论到 Banach 代数与 Gelfand 理论

算子代数的发展首先源于无穷维算子的谱理论。二十世纪初 Hilbert 在研究积分算子时引入谱分解思想；Fredholm、Hilbert 与 Schmidt 对积分方程与核的研究孕育了紧算子与自伴算子理论。1920 年代，von Neumann 在研究量子力学的数学基础时，需要把 Hermite 算子（观测量的自伴算子）系统地当作"函数"处理，自伴算子的谱定理即是对"对易算子可同时对角化"的无穷维推广。

1930 年代末，Gelfand 建立了 Banach 代数的一般理论：对交换 Banach 代数，用极大理想空间（谱空间）上的函数重新实现元素（Gelfand 变换），并由此给出谱半径公式。这一思想同时为 C\*-代数的分类理论奠基。

### 7.2 C\*-代数与 Gelfand-Naimark 理论

C\*-代数是一类带对合（共轭）运算且满足 $\|a^*a\|=\|a\|^2$ 条件的 Banach 代数，抽象自希尔伯特空间上有界算子树。1943 年 Gelfand 与 Naimark 证明了著名定理：任何（交换的）C\*-代数都同构于某紧 Hausdorff 空间上连续函数代数，任何抽象 C\*-代数都同构于希尔伯特空间上有界算子代数（Gelfand-Naimark/Segal 表示定理，Segal 亦独立贡献）。GNS 构造（Gelfand-Naimark-Segal）从态出发给出 C\*-代数的忠实表示，是表示论的核心脚手架。谱理论、连续函数演算与正元平方根、自伴算子谱定理等由此在 C\*-代数框架内获得统一而干净的表述。

### 7.3 von Neumann 代数与双交换子定理

von Neumann 代数关注的是在弱算子拓扑（或在代数上等价的强拓扑）下闭的自伴算子代数。1930 年代初，von Neumann 与 Murray 通过 von Neumann 双交换子定理（$\mathcal M''=\mathcal M$，即二次交换子还原自身）得到其抽象刻画，证明可分 Hilbert 空间上的 von Neumann 代数都同构于某约化因子代数的直和。Kaplansky 稠密性定理保证了在强弱拓扑下可用有界元逼近。张量积与可分性、Commutant（交换子）与约化问题等把 von Neumann 代数的结构理论一步步推向纵深。

### 7.4 因子分类、III 型因子与现代物理

1936–1943 年间，von Neumann 与 Murray 引入了"因子"（中心平凡的 von Neumann 代数）概念，并按投影比较理论将其分为型 I、II、III。1940 年他们发现超限维因子（现称 III 型）不能用求迹谱的方式度量，揭示了 von Neumann 代数比可交换情形（面对测度论）复杂得多的现象；此后对 III 型因子在 Moore-迹、共轭类与张量分类上的探索不断深化，并与其在共形场论、量子场论等数学物理中的应用交织。一般认为，C\*-代数（Gelfand、Segal）与 von Neumann 代数（von Neumann、Murray）共同构成了算子代数的两大支柱，其影响从经典谱理论延伸到量子信息、K-理论与非交换几何。
