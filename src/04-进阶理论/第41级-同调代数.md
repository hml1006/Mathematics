...
> **🧠 思考历程：为什么会发明"链复形"这么抽象的装置？**
>
> 任何一个大主题都有"它为谁而生"的出处。同调代数最初并不是为了代数而代数，而是为了回答一个更古老、更直观的问题：**怎样用一串数字（或一种代数结构），把"空间有几个洞"变成一个可精确计算、可加减、可比大小的量？** 本大节的所有概念（复形、同调、正合序列、链映射、蛇形引理、长正合序列、映射锥）都在接力回答这一句话：**怎样用代数的"账本"忠实无失真地记录空间与结构的形状信息。**
>
> - **Riemann 的"连通数"**。1850 年代 Bernhard Riemann 在黎曼曲面研究中提出的"连通数"概念，直观地度量"一张曲面要切多少刀才分离成数片"，并且已经隐含地用"闭链"（把曲面沿闭曲线剖开）去做底层算术——这是 Betti 数与同调的雏形，但也还停留在"数几个"的层面。
> - **Betti 数与关联矩阵**。1871 年 Enrico Betti 把这些想法系统化，定义了第 $k$ 个 Betti 数 $\beta_k$（他称之为"共轭度/连接数"），把"洞"整理成逐维的一串数字，被 Poincaré 吸收为可计算工具。
> - **Poincaré 的分析学新术语**。1895 年 Henri Poincaré 在《Analysis Situs》里首次引入"复形"（把空间切成单纯形），并用关联（incidence）矩阵编码"边界关系"，由此把各维之间的搭接写成一串可运算的代数关系。这实际上就是边缘算子与链复形的雏形。
> - **Noether 的抽象化转折——"代数起源"的关键一步**。1925–1926 年 Emmy Noether 提出把"Betti 数"整个升级成"**同调群 $H_n=Z_n/B_n$**"，让组合拓扑变成地地道道的代数学：数字被群结构取代，"洞的个数"变成"洞的加法结构"。这一转手正是"同调代数"这个名称里"代数"二字的由来。
> - **一条主线**：数洞 → Betti 数 → 链复形 → 同调群。从"几何形状"到"代数结构"的这条转化链，就是整个大节的"来龙去脉"。
>
> 你会发现，本节真正要解决的"问题"随着历史不断精化：从不精确的"数几个洞"，到 Poincaré 的"用行列式般的关联数算洞"，再到 Noether 的"让洞承载群的加减法"。理解这条脉络，你就抓住了后面所有"正合序列、蛇形引理、长正合序列"为何存在的意义——**它们是把"洞"这个几何直觉翻译成代数硬语言的最短路径。**
...
**定义 2.15（分裂短正合序列）** 短正合序列 $0 \to A \xrightarrow{f} B \xrightarrow{g} C \to 0$ 称为**分裂**的，若下列等价条件之一成立：

1. 存在**截面** $s : C \to B$，使得 $g \circ s = \operatorname{id}_C$；
2. 存在**缩回** $r : B \to A$，使得 $r \circ f = \operatorname{id}_A$；
3. 存在同构 $\theta : B \cong A \oplus C$，使得 $\theta \circ f = \iota_A$（标准包含）且 $g \circ \theta^{-1} = \pi_C$（标准投影）。

> **💡 直观理解（类比）**：分裂的短正合序列就是"可以无损拆成两部分"的流水线：$B$ 由嵌进来的 $A$ 拼上一块 $C$ 组成，既没有缠绕也没有乘法——干净利落是直和 $B\cong A\oplus C$。类比一盒拼装积木：若能"原样收回两盒"，就是分裂。反之，非分裂序列（如 $0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\to0$）就像粘死的胶水，$B\not\cong A\oplus C$（这里 $\mathbb{Z}\not\cong\mathbb{Z}\oplus\mathbb{Z}/2$）。"分裂"正是测"两部分是井水不犯河水还是纠缠更紧密"的分水岭。

**命题 2.16** 设 $0 \to A \xrightarrow{f} B \xrightarrow{g} C \to 0$ 是 Abel 范畴中的短正合序列。

1. 若 $A$ 是内射对象，则序列分裂；
2. 若 $C$ 是投射对象，则序列分裂。

**证明**：若 $A$ 内射，对单射 $f:A\to B$ 与恒等 $\operatorname{id}_A:A\to A$，内射性给出 $r:B\to A$ 使 $r\circ f=\operatorname{id}_A$，得缩回；若 $C$ 投射，对满射 $g:B\to C$ 与恒等 $\operatorname{id}_C:C\to C$，投射性给出 $s:C\to B$ 使 $g\circ s=\operatorname{id}_C$，得截面。$\square$

> **💡 直观理解（类比）**：命题 2.16 给出分裂的"自动触发判据"：只要左边 $A$ 内射或右边 $C$ 投射，序列必定分裂——内射的"延展性"让你从 $A$ 出发补出缩回 $r$，投射的"抬升性"让你从 $C$ 出发补出截面 $s$。类比两块积木只要有一块是"自带万能卡扣的零件"，就总能无损掰开。这条判据在第 3 节的 Tor/Ext 清零中反复使用。

**例 2.17**
- $0\to\mathbb{Z}/2\xrightarrow{1\mapsto(1,0)}\mathbb{Z}/2\oplus\mathbb{Z}/2\xrightarrow{(a,b)\mapsto b}\mathbb{Z}/2\to0$ 分裂（取截面 $s:\bar b\mapsto(0,\bar b)$）。
- $0\to\mathbb{Z}\xrightarrow{\times p}\mathbb{Z}\to\mathbb{Z}/p\to0$ 对素数 $p$ **不**分裂，因为 $\mathbb{Z}\not\cong\mathbb{Z}\oplus\mathbb{Z}/p$；这正是 $\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/p,\mathbb{Z}/p)=\mathbb{Z}/p\neq0$ 的根源。

> **🧠 思考历程：为什么要单独讨论"分裂"？**
>
> 分裂正合序列回答一个极其实际的问题——**"短正合序列到底是不是把 $B$ 拆成 $A\oplus C$"**。它的意义在于：一旦序列分裂，张量积与 $\operatorname{Hom}$ 都会**保持正合**，于是 $\operatorname{Tor}_n=\operatorname{Ext}^n=0$（$n\ge1$）当场清零；反之，$\operatorname{Tor}_1\neq0$ 恰恰标记"这段短正合序列不分裂"。也就是说，**分裂是"正合性在遭遇函子后仍不丢失"的最强保障**，也是后面众多 Tor/Ext 归零情形的共性来源。一个漂亮的小结：在第 3 节你会看到"分裂 ⟺ $B\cong A\oplus C$"，而第 3.3 节的 Tor 正是用来测"一时拆不开"的纠缠度的。

### 2.6 映射锥与映射柱

**定义 2.18（映射柱）** 设 $f : C_\bullet \to D_\bullet$ 是链映射。$f$ 的**映射柱**是链复形
$$
\operatorname{Cyl}(f)_n = C_n \oplus C_{n-1} \oplus D_n,
$$
带有微分
$$
d(x,y,z) = \big(d^C_n x + (-1)^{n-1} y,\; -d^C_{n-1} y,\; d^D_n z + f_{n-1}(y)\big)
\qquad (x\in C_n,\ y\in C_{n-1},\ z\in D_n).
$$
典型事实：存在包含 $\alpha:C_\bullet\to\operatorname{Cyl}(f)$ 与投影 $\pi:\operatorname{Cyl}(f)\to D_\bullet$ 满足 $\pi\circ\alpha=f$，且 $\pi$ 是**链同伦等价**，故 $H_\bullet(\operatorname{Cyl}(f))\cong H_\bullet(D)$。

> **💡 直观理解（类比）**：映射柱把"从 $C$ 到 $D$ 的搬运动作 $f$"本身变成一个实体的"中空直筒"：第 $n$ 层的料由 $C_n$、夹层 $C_{n-1}$ 与 $D_n$ 三段拼成，微分把三者按符号交错地缝合。类比在两条传送带之间插入一段"可伸缩变形"的中转筒，使搬运动作有了一段实体可追寻的轨迹。因为映射柱在"同调意义"上仍等价于目标 $D$（$\pi$ 是同伦等价），它给了 $f$ 一个"规范的实体化身"。

**定义 2.19（映射锥）** 链映射 $f : C_\bullet \to D_\bullet$ 的**映射锥**是链复形
$$
\operatorname{Cone}(f)_n = C_{n-1} \oplus D_n,
\qquad
d(x,y) = \big(-d^C_{\,n-1}x,\; d^D_n y - f_{n-1}(x)\big),
$$
其中 $x\in C_{n-1}$，$y\in D_n$。它是包含自然的短正合序列
$$
0 \to D_\bullet \xrightarrow{j} \operatorname{Cone}(f) \xrightarrow{p} C_\bullet[1] \to 0
$$
之中间项（$j(y)=(0,y)$，$p(x,z)=x$），其中 $C_\bullet[1]$ 是移位复形。由 2.3 的推论 2.12，此序列诱导**长正合序列**：
$$
\cdots \to H_n(D)\xrightarrow{H_n(j)} H_n(\operatorname{Cone}(f)) \to H_{n-1}(C)\xrightarrow{H_{n-1}(f)}H_{n-1}(D)\to\cdots
$$

**定理 2.20（映射锥判别拟同构）** 链映射 $f:C_\bullet\to D_\bullet$ 是**拟同构**（即 $H_n(f)$ 对每个 $n$ 都是同构）当且仅当映射锥 $\operatorname{Cone}(f)$ 是**零调的**，即 $H_n(\operatorname{Cone}(f))=0$ 对所有 $n$ 成立。

**证明**：由上面的长正合序列与五引理（引理 2.13）即可。若 $\operatorname{Cone}(f)$ 零调，则长正合序列口径得 $H_n(j)$ 满、而 $H_{n-1}(f)$ 单（追一圈），逐一推出 $H_n(f)$ 同构；反之若 $f$ 拟同构，长正合序列立即给出 $H_n(\operatorname{Cone}(f))=0$。$\square$

> **💡 直观理解（类比）**：映射锥是"把 $f$ 的偏差铸成一个复形"的量化器：想象你在两条传送带之间架一座观测桥 $\operatorname{Cone}(f)$，桥上的"洞"（同调）恰好数目化地记录了 $f$ 离"同调满同构"差了多少。定理 2.20 说：**$f$ 在我们关心的同调层面无瑕疵 ⟺ 这座观测桥没有洞**。它的二阶用途是"纠偏"：即便 $f$ 不是拟同构，只要把映射锥的信息并进来，就能把 $f$ 修补成三角结构里严格的第三边。

> **🧠 思考历程：为什么要引入"映射锥"这么怪异的对象？**
>
> 链映射本身只能给出"正合性与否"的评判，却缺少一个能**把一条链映射变成一个"真正严格"的第三边**的实体——而在我们要处理"从短正合序列过渡到长正合序列、进而把拟同构当同构"这类问题时（第 7 节导出范畴与三角范畴最需要），这个实体必不可少。映射锥解决了三个递进的问题：
>
> 1. **量化偏差**：$H_\bullet(\operatorname{Cone}(f))$ 精确量出 $f$ 与"拟同构"的差距（定理 2.20）。
> 2. **制造长正合序列**：自然短正合 $0\to D\to\operatorname{Cone}(f)\to C[1]\to0$ 又生出一条长正合序列，把"蛇形引理→长正合"的剧本在"映射"层面重演一遍。
> 3. **生成三角**：$D_\bullet\to\operatorname{Cone}(f)\to C_\bullet[1]$ 正是第 7.2 节三角范畴里"三角"的样板——每条态射都能按此长出第三边，这是导出范畴 $D(\mathcal{A})$ 具有三角结构的直接原因。
>
> 一句话：**映射锥把一个"只给出二段式关系"的链映射，升级成"缺失的第三边恰好接管一切同调信息"的闭环。**
...
> **🧠 思考历程：一个明明"差一点就正合"的函子，到底丢掉了多少信息？**
>
> 上一节的同调只对"链复形"说话，还没回答一个更刁钻的问题：现实里经常遇到的函子（如 $\operatorname{Hom}$ 与张量积 $-\otimes-$）并不完全正合，它们只在一边保持正合（$\operatorname{Hom}$ 左正合、张量右正合）。那**另一端究竟差了多少？** 这一大节的全部内容（投射/内射/平坦对象、左右导出函子、Tor、Ext、Cartan–Eilenberg 消解）就是为精确回答"差多少"而生的。
>
> - **问题从代数里自己长出来**。1920–1940 年代，人们在研究群扩张、模的根与基座、内射分解时发现：给定短正合序列 $0\to A\to B\to C\to0$，$\operatorname{Hom}$ 和张量只能保一半，"断掉"的那一半长度惊人地相似——似乎总有一条用"高阶项"续接起来的长序列在等着被发现。
> - **Hopf 与群扩张**。1941–1942 年 Heinz Hopf 在研究群的"更高同调"时注意到二维上同调与群扩张有关，Hochschild、Eilenberg 与 Mac Lane 等随之把 $\operatorname{Ext}$、$\operatorname{Tor}$ 等"高阶"函子逐一从具体计算（交叉同态、扩张、挠积）提炼出来。
> - **Cartan–Eilenberg 的系统化**。1956 年 Henri Cartan 与 Samuel Eilenberg 的《Homological Algebra》是这一次"来龙去脉"的最关键一环：把散落的技巧统一为"函子的导出函子"，用投射分解/内射分解这一套"脚手架 + 取同调"的固定流程，证明导出函子良定义、长正合序列存在，Tor/Ext 由此成为标准工具。
> - **解决的核心问题**：**左/右正合函子在"坏端"丢掉的精确结构可以用同调补回来**——第 $0$ 层恢复原函子，$n\ge1$ 层记录"非正合度"。
>
> 一句话把握本节：**Tor 测"张量积差多少才正合"，Ext 测"$\operatorname{Hom}$ 差多少才正合"，而所有导出函子 $L_nF$/$R^nG$ 就是"给一台不完美的机器补上它丢失的层"。**
...
> **💡 直观理解（类比）**：平坦模是"对张量积完全無辜"的模——$M$ 平坦当且仅当 $M\otimes_R-$ 是正合函子。类比一块不会"卡齿轮"的原料：无论与谁咬合，都不制造额外的纠缠（高阶 $\operatorname{Tor}$ 全为零）。自由模一定平坦，但反过来不成立（$\mathbb{Q}$ 是平坦 $\mathbb{Z}$-模却非投射）。投射 ⊂ 平坦 这一串包含关系，刻画了"从最自由的到最宽容的"阶梯。

**定义 3.19（平坦对象/平坦模）** 设 $R$ 是环。左 $R$-模 $F$ 称为**平坦**的，若函子 $F\otimes_R-$ 是**正合**的（既左正合又右正合），等价地 $\operatorname{Tor}_n^R(F,N)=0$ 对所有 $n\ge1$ 与所有 $N$ 成立。投射模都是平坦的；自由模当然平坦。典型例子：$\mathbb{Q}$ 作为 $\mathbb{Z}$-模平坦但非投射。

**定义 3.20（平坦分解）** 对象 $A$ 的**平坦分解**是正合序列 $\cdots\to F_1\to F_0\to A\to 0$，其中每个 $F_i$ 平坦。由于投射模都平坦，投射分解必然是平坦分解。

**命题 3.21（平坦分解与 Tor）** 左导出函子 $L_nF(A)$ 可用 $A$ 的**平坦分解**计算：对右正合函子 $F$，$L_nF(A)=H_n(F(F_\bullet))$。特别地，$\operatorname{Tor}_n^R(M,N)$ 既可用 $N$ 的投射分解，也可用 $N$ 的**平坦分解**求得，结果一致。

**证明**：关键是在"可接受类的比较引理"下，任意平坦分解与某投射分解之间存在拟同构式的比较，而 $L_nF$ 只依赖同调类，故替换平坦分解不改变结果。直观地，只要取的是"足够好"的分解（投射 ⊂ 平坦 ⊂ 某可接受类），导出的同调都一样。$\square$

> **🧠 思考历程：既然有投射分解，为何还要"平坦分解"？**
>
> 平坦对象回答了投射对象回答不了的一类问题：**当对象没有足够的投射裕度、但张量性质仍好时**（如 $\mathbb{Q}$ 这样的"半自由"对象），投射分解可能不存在或难找，而平坦分解往往唾手可得。更重要的是，张量的导出本来"只认平坦性不认投射性"——推导 $\operatorname{Tor}$ 时你真正用到的只是"这一项在张量下零化高阶障碍"，平坦恰好是精确条件。所以平坦分解把"用投射去限制"放松为"用平坦去限制"：**凡是你想对张量做的事，平坦分解都够用，且常常更省**。

### 3.7 Cartan–Eilenberg 消解

> **🧠 思考历程：当"对象"升级为"整个链复形"时，消解该怎么做？**
>
> 3.1 的消解都是针对**单个对象** $A$ 架起脚手架。可同调代数常常要同时处理**整条链复形** $A_\bullet$（例如要同时对所有 $n$ 给出 $A_n$ 的投射分解，且让它们与链图的映射相容）。Cartan–Eilenberg 在他们的经典专著里首次处理这一情况——**能否把一个链复形整体"拿起"成一个双复形投射消解，使得每一维、每个循环/边缘子对象都同时被忠实覆盖？** 这就是 Cartan–Eilenberg 消解要解决的问题。

**定义 3.22（Cartan–Eilenberg 消解）** 设 $A_\bullet$ 是下有界链复形（$A_n=0$ 对 $n\ll0$）。其 **Cartan–Eilenberg 消解**（简称 **CE 消解**）是一个双复形 $P_{\bullet,\bullet}$ 配上增广 $\varepsilon:P_{\bullet,0}\to A_\bullet\to0$，使得对每个 $q$，竖列 $P_{\bullet,q} \to A_q \to 0$ 是 $A_q$ 的**投射分解**，且增广还诱导每一列"循环/边缘/同调子复形"的投射分解——具体地，对固定的 $q$，由 $\varepsilon$ 与微分构造的 $Z_m(P_{\bullet,q})\to Z_m(A)_q$、$B_m(P_{\bullet,q})\to B_m(A)_q$、$H_m(P_{\bullet,q})\to H_m(A)_q$（对每个 $m$）都是**分解**。

> **💡 直观理解（类比）**：CE 消解是"整个链复形的三维脚手架"：单个对象的投射分解是一条一维梯子，而 CE 消解把梯子沿"第 $q$ 层"排成一片墙（双复形），并且要求这面墙不仅盖住每一片砖 $A_q$，还盖住砖缝里的"循环 / 边缘 / 同调"痕迹。类比既要按楼层盖楼（每层 $A_q$ 有脚手架），又要保证每道楼板缝里的管线（$Z,B,H$）都有对应支撑，整栋楼才能精确无误地"搭同调"。

**定理 3.23（CE 消解与导出函子）** 设 $F$ 是右正合函子，$A_\bullet$ 下有界。取 $A_\bullet$ 的 Cartan–Eilenberg 消解 $P_{\bullet,\bullet}$，则：
$$
L_nF(A_\bullet) := H_n\big(\operatorname{Tot}(F(P_{\bullet,\bullet}))\big),
$$
其中 $\operatorname{Tot}$ 是全复形（见 5.2 定义 5.4）。它不依赖 CE 消解的选择，且满足 $L_nF(A_\bullet)$ 是 $L_nF$ 在链复形层次的自然推广：当 $A_\bullet$ 集中在第 $0$ 位时，回归经典 $L_nF(A)$。

**证明概要**：选取 CE 消解后用双复形的谱序列（定理 5.5）沿"先垂直后水平"方向取同调；CE 条件保证该谱序列立即退化到第 $0$ 页的 $H_n(F(A_\bullet))$，从而给出答案；比较引理保证任一 CE 消解都给同构。$\square$

> **🧠 思考历程：Cartan–Eilenberg 消解到底“解决了什么”？**
>
> 在 Cartan–Eilenberg 1956 年的《Homological Algebra》里，CE 消解是把"导出函子作用于单个对象"系统升级为"导出函子作用于整条链复形"的关键跳板。它的三大用处：其一，**让 $L_nF(A_\bullet)$ 作为"复形的导出"有良定义**，不再只能对一点一点的对象算；其二，**提供 Tor/Ext"平衡性"与长正合序列的整体证明工具**；其三，**构成日后的导出范畴（第 7 节）里 $RF:D^-(\mathcal{A})\to D^-(\mathcal{B})$ 的具体实现蓝图**——它本身就用"双复形 + 全复形 + 谱序列"完整展示了现代导出函子的标准长相。

---

## 4. 范畴语言

### 4.0 来龙去脉：为什么要让"范畴"出场？

> **🧠 思考历程：当同一种论证在模、群、层里反复出现时，该怎么办？**
>
> 前面的同调、导出函子、Tor/Ext 都写得很具体，但敏锐的读者会发现：同样的"追图""蛇形引理""分解取同调"在不同代数结构里几乎一字不差地重演。这一大节要回答的问题是——**能否把"同调代数能自动成立"的抽象前提（核/余核、正合、左/右正合）单独抽出来，让一套证明通吃所有情形？** 这个抽象化的需要正是 Abel 范畴与范畴语言的"来龙去脉"。
>
> - **Eilenberg–Mac Lane 的范畴诞生**。1942–1945 年 Samuel Eilenberg 与 Saunders Mac Lane 为统一代数拓扑与同调的计算，正式提出"范畴"与"函子""自然变换"的概念——最初是为了严谨描述"同调是空间到群的自然变换"。
> - **从"模范畴"到"足够好"的抽象范畴**。1950 年代 Grothendieck 提出 **Abel 范畴**，把"有核余核、态射可分解、正合性良好"的一类范畴公理化，并证明一大套同调机器（含足够内射对象、导出函子、$\delta$-函子）在任意 Abel 范畴里照常运转。这正是第 6 节层上同调、第 7 节导出范畴能在"没有元素"的对象（层、复形之复形）上工作的原因。
> - **解决的核心问题**：**把"逐例证明"升级为"一次性通用证明"**——只要验证 $\mathcal{A}$ 是 Abel 范畴、且具有足够投射/内射对象，整套导出函子理论即刻自动成立。
>
> 一句话把握本节：**Abel 范畴是"同调代数的宪法"——它规定哪些运算（核、余核、直和、正合）永远合法，而蛇形引理、导出函子、$\delta$-函子则是写在这部宪法之上的定律。**
...
### 5.0 来龙去脉：算不动时，怎么"层层逼近"？

> **🧠 思考历程：当目标上同调一步算不出来时，数学家如何下台阶？**
>
> 前面 Tor/Ext 只处理"一个函子"的导出，可现实里常遇到复合函子 $G\circ F$、纤维丛 $F\to E\to B$ 这类"多道工序、多层结构"的目标——直接算 $R^{p+q}(G\circ F)(A)$ 或 $H^{p+q}(E)$ 往往难如登天。谱序列要解决的问题是：**能不能设计一台"逐页取同调、步步收敛"的机器，把不可直接计算的最终目标拆成一页页可追踪的渐近逼近？**
>
> - **Leray 的诞生背景**。1946 年 Jean Leray 在被俘期间为研究微分方程（层与谱序列由此发明），把"谱"当作逐层取上同调的抽象过滤器，用之于层的同调。
> - **Koszul、Borel 与代数化**。Jean-Louis Koszul（1940s）在同伦论中发展谱序列的代数版本；Armand Borel、Henri Cartan 在 1950 年前后把谱序列整理成 $E_r,d_r$ 的标准数据（"一页页底片"），让它在纯代数里站住脚。
> - **Leray–Serre 的拓扑王牌**。1950 年代 Jean-Pierre Serre 娴熟地把 Leray–Serre 谱序列用于纤维丛与环路空间，一举算出一大批拓扑空间的上同调——谱序列从此从"应手"晋升为"大杀器"。这就是本节 5.4 的"背景"。
> - **Grothendieck 的母体学说**。1957 年 Grothendieck 谱序列把复合函子 $G\circ F$ 的导出拆成 $R^pG\circ R^qF$，给 Leray 谱序列、群上同调配对等无数具体谱序列提供了一个共同的"母体定理"。
> - **解决的核心问题**：**把"算最终同调"降维成"逐页取同调直到 $E_\infty$ 收敛"**——牺牲一点点"一步到位"的雄心，换取"原则上永远可推进、可追踪"的机器。
>
> 一句话把握本节：**谱序列是"同调的长除法"——当目标一步算不出时，就一页一页地除，直到商页稳定收敛为止。**
...
### 6.0 来龙去脉：当"上同调"长进结构内部

> **🧠 思考历程：同一个导出函子配方，为什么能同时描述群、代数与层？**
>
> 前面第 3、4 节建立了一套通用的"右导出函子"机器，它本身只是抽象的轮廓。本大节要回答的问题是：**把这台机器装进三个最具体、最重要的"舞台"——群（$G$-模）、（非交换）代数、拓扑空间上的层——分别能给出什么**，以及它们的发现者到底在解决哪些实际问题。这正是上同调理论"遍地开花"的来龙去脉。
>
> - **群上同调**。源自 1900 年代 Schreier 等人的群扩张分类，以及 Hopf 在 1941–1942 年对"群的高维链"的观察；1950 年代 Eilenberg–Mac Lane、Hochschild–Serre 将其整理为 $H^n(G,M)$，成为类域论、表示论的核心。它解决的实际问题：**给"一个群如何被另一个模扩张"记账**（$H^2$ 分类扩张）、度量"不变部分差了多少"。
> - **Hochschild 上同调**。1945–1946 年 Gerhard Hochschild 提出，刻画（非交换）代数的**导子、扩张与形变**。1950 年代被 Cartan–Eilenberg 纳入 $\operatorname{Ext}_{A\otimes A^{\text{op}}}$ 的框架。它解决的实际问题：**给"代数的中心、导子、扩张、形变"建立统一的障碍量**。
> - **层上同调**。层由 Jean Leray（1946，与谱序列同源）与 Henri Cartan、Roger Godement 发展；1957 年 Grothendieck 用抽象导出函子 $R^n\Gamma$ 重新奠基，Alexander Grothendieck 与超同调、Čech 上同调定理随之完善。它解决的实际问题：**"局部都能定义、全局能否黏合、黏不住差多少"**——是代数几何（线丛、凝聚层、Serre 对偶）的看家本领。
> - **共同点**：三者本质都是"取‘不变/截面’类左正合函子的右导出"，只是舞台不同——这正是第 4 节 Abel 范畴公理化的回报。
>
> 一句话把握本节：**群、代数、层，三个舞台，一套配方——上同调是"在结构内部逐层挖出不变量"的通用探测器。**
...
### 7.0 来龙去脉：把"同调相同"变成"就是同一个"

> **🧠 思考历程：当数学家想把拟同构"翻转过来"时，会发生什么？**
>
> 前面所有章节都把"同调"当作终极答案——两个链复形只要每层同调同构（称为**拟同构**），我们就当它们"本质同一"。可长期存在一个别扭：想要把拟同构真的当作等号（例如把导出函子定义在"复形的等价类"上）时，范畴论里没有自然的说法让"不是同构却想变同构"的映射可逆。这一大节要回答的问题正是：**能不能构造一个范畴，让每条拟同构都真正成为同构、让"定义在同调上的概念"不再受限于具体复形？** → 答案就是**导出范畴**。
>
> - **Grothendieck 的愿景与 Verdier 的博士论文**。1960 年前后 Alexander Grothendieck 需要一种能容纳层上同调整体运算的框架；他的学生 Jean-Louis Verdier（1967 年博士论文）系统地引进了**导出范畴 $D(\mathcal{A})$** 与**三角范畴**的八面体公理，为同调对象在范畴层次上的"幻方运算"奠基。
> - **为什么非它不可**：经典导出函子只能对"单个对象 $A$"算 $R^nF(A)$，而对"需要保留整体链复形信息"的场景（如 $R\Gamma$、$R\operatorname{Hom}$、导出张量 $-\otimes^{\mathbb{L}}-$）力不从心。导出范畴把拟同构反转后，这些运算获得"只依赖同调类"的天然定义。
> - **解决的核心问题**：**让拟同构成为真正的同构，从而把无数次"取同调"浓缩成一次范畴层面的合法构造**——同调代数的"终极工作台"由此诞生。
> - **关键预备**："同伦范畴 $K(\mathcal{A})$"（态射取链同伦类）与"映射锥"（第 2.6 节）正是通往这里的直梯段——$D(\mathcal{A})$ 正是在 $K(\mathcal{A})$ 上反转拟同构所得，而三角的结构来自映射锥。
>
> 一句话把握本节：**先在同伦范畴里"忽略链同伦"，再反转拟同构——于是"算同调"被提升为一个范畴本身的首次构造性工作。**
...
**习题 301** 判断下述短正合序列是否分裂，并说明理由：
$$
0 \to \mathbb{Z} \xrightarrow{\times 2} \mathbb{Z} \to \mathbb{Z}/2 \to 0.
$$
若存在截面 $s:\mathbb{Z}/2\to\mathbb{Z}$，会导出什么矛盾？

**习题 302** 设 $0\to A\xrightarrow{f}B\xrightarrow{g}C\to 0$ 是分裂的。证明：对任意右正合函子 $F$，序列 $0\to F(A)\to F(B)\to F(C)\to 0$ 仍正合（提示：用缩回 $r$ 构造左逆，或用同构 $B\cong A\oplus C$）。由此解释分裂短正合序列为何蕴含 $\operatorname{Tor}_n=\operatorname{Ext}^n=0$（$n\ge1$）。

**习题 303** 设 $f:\mathbb{Z}\to\mathbb{Z}/2$ 是唯一满同态（把 $1$ 映到 $\bar 1$），把 $\mathbb{Z}$、$\mathbb{Z}/2$ 视为集中在第 $0$ 位的链复形（其余位为零）。写出映射锥 $\operatorname{Cone}(f)_n$ 的 $n=0,1$ 分量，并计算全同调 $H_\bullet(\operatorname{Cone}(f))$。

**习题 304** 设 $g:\mathbb{Z}\xrightarrow{\times 2}\mathbb{Z}$ 是集中于第 $0$ 位的链映射 $\mathbb{Z}\to\mathbb{Z}$（把 $\mathbb{Z}$ 当第 $0$ 位复形）。根据映射锥判别法（定理 2.20），判断 $g$ 是否是拟同构，并验证 $H_\bullet(\operatorname{Cone}(g))$ 是否为零调。

**习题 305** 把映射锥的长正合序列（2.6 节）应用到一条**断开的**短正合序列 $0\to C_\bullet\to 0\to\cdots$ 之前的自然投影 $p:\operatorname{Cone}(f)\to C_\bullet[1]$，说明它如何"吃进映射锥的同调"来补回 $f$ 的偏差。

**习题 306** 证明：对 Abel 范畴中的短正合序列 $0\to A\xrightarrow{f}B\xrightarrow{g}C\to0$，$C$ 投射（或 $A$ 内射）一定蕴含分裂；并举一个 $C$ 不投射、$A$ 不内射但序列恰好在 $\operatorname{Mod}_{\mathbb{Z}}$ 中分裂的反例。
...
**答案 301** 不分裂。若存在截面 $s:\mathbb{Z}/2\to\mathbb{Z}$，则 $g\circ s=\mathrm{id}_{\mathbb Z/2}$，而 $g$ 即满同态 $\mathbb{Z}\to\mathbb{Z}/2$；任取 $\bar1\in\mathbb{Z}/2$，必 $s(\bar1)=k\in\mathbb{Z}$，于是 $\bar g(k)=\bar1$。但 $\bar g(k)=g(s(\bar1))=\bar1$ 可满足；真正的矛盾在于：由收缩公理该有 $s$ 使 $g(s(\bar1))\neq0$ 且 $\mathbb{Z}$ 中 $2s(\bar1)=s(\bar0)=0\Rightarrow s(\bar1)=0$，从而 $\bar1=g(s(\bar1))=g(0)=0$，矛盾。简言之，$\mathbb{Z}$ 中唯一的"同态提升 $\mathbb{Z}/2\to\mathbb{Z}$"只能是零映射，无法绕过 $2$-挠。

**答案 302** 若序列分裂，则存在同构 $B\cong A\oplus C$，其中 $f$ 为含 $A$、$g$ 为投到 $C$。此同构在任意加性函子 $F$ 下保持为 $F(B)\cong F(A)\oplus F(C)$，且 $F(f)$、$F(g)$ 分别是其单入/投出、$F(g\circ s)=F(g)\circ F(s)=\mathrm{id}_{F(C)}$ 等皆由函子性保持。故 $0\to F(A)\to F(B)\to F(C)\to0$ 仍正合（$F$ 不必右正合也能保存这种"分裂+直和"结构）。由此，分裂短正合在 Tor/Ext 长正合中给出 $H_n(F(B))=H_n(F(A))\oplus H_n(F(C))$，而高阶项 $\operatorname{Tor}_n=\operatorname{Ext}^n=0$（$n\ge1$），因为分解可取在 $B\cong A\oplus C$ 上接"零延拓"。

**答案 303** 视 $\mathbb{Z}$ 为 $C_0=\mathbb Z$、$\mathbb{Z}/2$ 为 $D_0=\mathbb Z/2$，其余为零。$\operatorname{Cone}(f)_n=C_{n-1}\oplus D_n$：$n=0$ 时 $C_{-1}=0$、$D_0=\mathbb Z/2\Rightarrow\operatorname{Cone}_0=\mathbb Z/2$；$n=1$ 时 $C_0=\mathbb Z$、$D_1=0\Rightarrow\operatorname{Cone}_1=\mathbb Z$；其余为零。微分 $d_1:\mathbb Z\to\mathbb Z/2$ 由 $d_1(x)=d^D_0(0)-f_0(x)=-\bar x$。于是 $H_1=\ker d_1=0$，$H_0=\operatorname{coker}(d_1)=\mathbb Z/2$。即 $\operatorname{Cone}(f)$ 同调恰为 $\mathbb Z/2$（非零调），反映 $f$ 不是拟同构（$H(f):\mathbb Z\to\mathbb Z/2$ 非满）。这正好对应长正合 $H_1=H_1(D)=0$、$H_0(C)=\mathbb Z$、$H_0(D)=\mathbb Z/2$：$H_0(D)\to H_0(\operatorname{Cone})\to H_0(C[1])=0$ 中 $H_0(\operatorname{Cone})$ 满单等推得 $\mathbb Z/2$。

**答案 304** $g=\times2:\mathbb Z\to\mathbb Z$ 是第 $0$ 位链映射。映射锥 $\operatorname{Cone}(g)_0=\mathbb Z$（由 $D_0$）、$\operatorname{Cone}(g)_1=\mathbb Z$（由 $C_0$），微分 $d_1(x)=-g(x)$ 即 $-\times2$。故 $H_1=\ker(\times2)=0$，$H_0=\operatorname{coker}(\times2)=\mathbb Z/2\neq0$，映射锥**不**零调。由长正合 $H_1(g)$：$\mathbb Z\xrightarrow{\times2}\mathbb Z$ 不是同构（$2\mathbb Z\neq\mathbb Z$），故 $g$ 不是拟同构，与"零调⟺拟同构"一致。

**答案 305** 对自然投影 $p:\operatorname{Cone}(f)\to C_\bullet[1]$（它是短正合 $0\to D\to\operatorname{Cone}(f)\to C[1]\to0$ 的后一段），长正合给出
$$
\cdots\to H_n(D)\xrightarrow{H_n(j)}H_n(\operatorname{Cone}(f))\xrightarrow{H_n(p)}H_n(C)[1]\xrightarrow{H_{n-1}(f)}H_{n-1}(D)\to\cdots
$$
（末一项来自 $\partial$ 连接到 $f$）。口诀：映射锥的同调"夹在" $D$ 与 $C[1]$ 之间，而连接映射正是 $f$ 的移位——这正好补上 $f$ 在同调上丢的偏差，也是"锥是用来把不拟同构的映射嵌入严格三角"的机制来源。

**答案 306** 前半由命题 2.16 得出（$A$ 内射给出缩回 $r$，$C$ 投射给出截面 $s$）。反例：取 $A=C=\mathbb{Z}/2$、$B=\mathbb{Z}/2\oplus\mathbb{Z}/2$，$f:1\mapsto(1,0)$，$g:(a,b)\mapsto b$——序列分裂（截面 $s:b\mapsto(0,b)$），但 $A=C=\mathbb{Z}/2$ 作为 $\mathbb{Z}$-模既非投射也非内射。可见投射/内射判据只是"分裂"的**充分不必要**条件，分裂并不强制两端具备投射/内射性质。

---

## 11. 数学史（History of Mathematics）

> 同调代数不是凭空造出的"范畴游戏"，它诞生于一个非常实际的提问：**怎样用代数对象捕捉空间的"洞"与"缺失"？**庞加莱在 1895 年前后把拓扑空间化归为**链复形**并定义**同调群**，让"洞"有了可计算的代数化身；到了二十世纪中叶，嘉当与埃伦伯格在《同调代数》中把"如何追回被正合序列丢失的信息"提炼为**导出函子**，麦克莱恩与埃伦伯格又把对象装进**范畴与 Abel 范畴**，Leray 用**谱序列**、Grothendieck 与 Verdier 用**导出范畴**把它推向顶层。本章沿着"链复形→导出函子→谱序列→导出范畴"的线索，追溯同调群、Tor/Ext、长正合序列、蛇形引理等每个概念的前因后果。

### 11.1 从一个"洞"开始：链复形与同调群的由来

- **洞的代数化**：要问"这个空间有没有洞、洞在哪"，最聪明的做法是把它切成一块块"链"，再用"边界"记录相邻块的衔接——这便得到**链复形**。
- **同调群**：链复形上"闭链（边界为零）"与"边界"的商，被定义为一个向量空间/群的序列——即**同调群**。它把几何问"洞"翻译为代数算"商"。
- **从几何到代数**：**庞加莱（Henri Poincaré, 1854–1912）**在《位置分析》（*Analysis Situs*，1895）及其续篇中正是用这种"单纯剖分＋链边界"构造出**单纯同调**，从而奠定了代数拓扑，也意外地给后来的同调代数提供了第一个来源。
- **本章的承接**：本章开篇的"链复形与同调群""正合序列"正是把庞加莱的几何直觉剥离成纯代数的第一步——链的作用、边界的作用、商的作用在此各司其职。

### 11.2 挠与更精细的信息：同调的两个层次

- **Betti 数与挠系数**：同调群既含维数信息（**Betti 数**，来自贝蒂），又含周期/扭转信息（**挠系数**）——它比单纯"洞的个数"丰富得多。
- **为什么需要两大内容**：$H_0$ 管连通分量的个数，$H_1,H_2,\dots$ 逐级"高维洞"；而挠部分反映出不能在整数层面铺开的扭结，是理解环与模结构所必需的。
- **从精致直观到公理**：后人（Veblen、Alexander 等）把同调从"单纯剖分"修正为不受剖分影响的**奇异同调**，使同调具有不变性；**Eilenberg–Steenrod 公理化**则给出同调应满足的一整套公理，令"同调是完全称职的不变量"有了依据。
- **本章的承接**：本章的**自由 Abelian 群**、**商群**与**秩/挠**概念，正是这套"既要维数、又要扭结"的双层信息在现代教科书中的形态。

### 11.3 同调 vs 上同调：方向与配对的切换

- **上同调的方向**：把"链映射的诱导方向"反过来（从目标空间指向源空间），就得到**上同调**——它不是简单的对偶，而是可与乘积、层级语言自然兼容的构造。
- **杯积与配对**：上同调允许把两个上同调类"乘"在一起（**杯积**），因而具备**环结构**；上同调还通过**对偶**（如 Poincaré 对偶）与同调呼应，描述更丰富的代数结构。
- **Alexander、Kolmogorov、Čech、Steenrod 的推动**：1930 年代，**Alexander、Kolmogorov、Čech、Steenrod、Veblen** 等彼此独立地发展了上同调，逐步确立其"代数雏形"。
- **本章的承接**：本章"上同调与对偶""杯积"等认识，正是从"S 与 S\*"两个视角看同一空间的价值的体现，也是导出函子构造**Ext** 前的演练。

### 11.4 从切分到缝合：正合序列与蛇形引理

- **正合序列**：一列同态"像＝核"的情形称为**正合**序列；它精确记录了"某一层信息被同态吞掉或吐出"的位置，是"缺失信息"的记账本。
- **长正合序列**：给一段短正合序列配上"连接同态"，就能拉出——**长正合序列**，从而追踪洞如何在各阶同调之间流动。
- **蛇形引理**：把两个短正合序列用一列映射连接，会沿出一个**蛇形引理**的交换图，精确给出连接同态的存在与自然性——它是同调代数里最著名的"拼图"元定理。
- **本章的承接**：本章"正合序列、蛇形引理、正合性保持（左/右正合涵子）"正是把"哪一层信息被恢复"这一直觉变成可反复使用的机器。

### 11.5 导出函子：追问"丢失的信息"

- **右正合与左正合**：张量积、$\operatorname{Hom}$ 等许多构造只"一半保持正合"，正是这"丢失的半个正合条约"逼出了导出函子。
- **投射分解与内射分解**：用一个对象的**投射分解**（或**内射分解**）把问题的可解性提升，再对分解应用函子并取同调，便是**导出函子**的构造。
- **Tor 与 Ext**：**Tor** 探测张量积丢失的扭结，**Ext** 探测 $\operatorname{Hom}$ 丢失的扩张——它们正是本章标题里的主角。
- **本章的承接**：本章"投射分解""内射分解""导出函子""Tor""Ext""平坦分解"皆是对"用分解追回信息"思路的逐块展开，是导出函子作为同调代数轴心地位的实证。

### 11.6 Cartan–Eilenberg《同调代数》：学科的诞生

- **1956 年的那本书**：**亨利·嘉当（Henri Cartan, 1904–2008）**与**塞缪尔·埃伦伯格（Samuel Eilenberg, 1913–1998）**合著的《同调代数》（*Homological Algebra*，1956）系统整理了投射分解、导出函子、Tor/Ext 与谱序列，标志同调代数成为一门独立学科。
- **不变量与函子思维**：该书把拓扑、群论、迭代逼近中的同调现象归纳为**导出函子**的统一语言，确立"函子思维"在代数学中的地位。
- **Mac Lane 与 Eilenberg 的早期铺垫**：更早，Mac Lane 与 Eilenberg 在与分类空间、上同调维数相关的工作里已埋下投射/内射分解与函子观点的种子。
- **本章的承接**：本章"Tor/Ext 与平坦分解"的主体源于这本书；它把"同调代数"从一批孤立技巧，升格为有对象、有结构、有原理的成熟理论。

### 11.7 范畴与 Abel 范畴：环境公理化

- **1945 年的范畴论**：**麦克莱恩（Saunders Mac Lane）**与**埃伦伯格**在 1945 年引入**范畴**、**函子**与**自然变换**，为"数学结构间的地图"提供语言。
- **Abel 范畴**：**格罗滕迪克**定义**Abel 范畴**（具备核、余核与正合性良好行为），使同调方法可脱离"模范畴"这唯一产地，在任意 Abel 范畴（层、表示等）中运转。
- **动机：统一**：基本群是函子、同调是函子，范畴语言让它们被同一种"结构保持"语法挂接，是现代数学"去重"与"迁移"的引擎。
- **本章的承接**：本章"范畴语言""Abel 范畴""正合序列与函子"正是以格罗滕迪克—麦克莱恩—埃伦伯格的公理环境为地基，让导出函子不再限制在模上。

### 11.8 谱序列：算不动时的层层逼近

- **动机**：很多同调（如两层复合的纤维空间的同调）无法一次算清，只能在面积上逐页前进——**谱序列**应运而生。
- **Leray 的开创**：**让·勒雷（Jean Leray）**在二战期间为研究纤维空间同调——**Leray 谱序列**——奠定这一"层层过滤、逐步求极限"的框架，战后被 Koszul、Serre、Borel 等广泛使用与推广。
- **代数版本**：Cartan–Eilenberg、Eilenberg–Moore 把谱序列代数化（双复形、格上谱序列），Grothendieck 谱序列统一了复合函子；Lyndon–Hochschild–Serre 谱序列服务于群上同调，Hochschild 上同调服务形变与 D 模。
- **本章的承接**：本章"双复形的谱序列""Grothendieck 谱序列""Leray–Serre 谱序列"正是"算不动时逐层逼近、最后取极限"这门手艺的教科书形式。

### 11.9 层上同调与 Serre 对偶：从代数到几何

- **层上同调**：把代数拓扑的同调搬到复/代数流形上的**层**（sheaf）时，得到**层上同调**，成为代数几何与复几何分析局部与整体的标准语言。
- **Serre 对偶**：**让-皮埃尔·塞里（Jean-Pierre Serre）**用层上同调给出 **Serre 对偶**，把代数簇上的换位、消没与对偶漂亮地统一，并广泛传播了"层视角"。
- **后果**：同调代数因而成为代数几何的引擎（系列上同调、线丛与除子、直象与滤……），反使同调代数获得最强的"消费端"。
- **本章的承接**：本章"层上同调（引出）""Serre 对偶（引出）"虽只点到，但它们是激发导出范畴与许多现代工具的直接源头。

### 11.10 导出范畴与三角范畴：把同调变成对象

- **动机**：短正合序列经导出函子后引发无穷信息，仅靠同调群不够用；**格罗滕迪克**与**韦尔迪耶（Jean-Louis Verdier）**引入**导出范畴**，把"复形及其拟同构"作为对象，令"同调等价"升格为"导出等价"。
- **三角范畴**：为在导出范畴上做加性运算，需要**三角范畴**（三角公理、卷积与交换），把"补全成三角"变成合法操作。
- **分解定理与 D 模**：**Beilinson–Bernstein–Deligne–Gabber** 的分解定理与 **D 模**理论将导出方法用于表示论、代数几何与现代物理（Fukaya 范畴、稳定条件），令同调代数成为现代前沿的公共语言。
- **本章的承接**：本章"导出范畴""三角范畴"正是通往这条"把同调当作对象本身"之路的最后一站，也为读者指向 Künneth、t-结构等现代工具。

### 11.11 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1895 | 庞加莱《位置分析》 | 单纯同调：用链与边界捕捉空间的洞 |
| 1900 年代 | Veblen、Alexander | 修正单纯剖分，逼近奇异同调与不变性 |
| 1930 年代 | Alexander、Kolmogorov、Čech | 独立发展上同调 |
| 1940 年代初 | 勒雷 | 创立谱序列（Leray 谱序列） |
| 1945 | 麦克莱恩、埃伦伯格 | 范畴、函子与自然变换的语言 |
| 1956 | 嘉当、埃伦伯格《同调代数》 | 导出函子、Tor/Ext 系统化，学科成型 |
| 1950 年代 | 格罗滕迪克 | Abel 范畴；层上同调与 Serre 对偶的推广环境 |
| 1954—1958 | Serre | 用层上同调给出 Serre 对偶 |
| 1960 年代 | Verdier、Grothendieck | 导出范畴与三角范畴 |
| 1980 年代 | Beilinson、Bernstein、Deligne、Gabber | 分解定理；D 模将导出方法推向表示论与守恒律 |
| 1990s— | 现代同调代数 | Fukaya 范畴、稳定性条件、Hochschild 上同调广泛应用 |

### 11.12 小结

> 同调代数的全部历史可以用一句话概括：**为了让"丢失的信息"可以被代数地追问、补偿并人手相传**。庞加莱先让"洞"变成链复形的同调群；奇异性、挠与上同调让"洞"有了更细的面相；正合序列与蛇形引理给出信息流动的记账术；为追回"半个正合条约"被丢的信息，嘉当与埃伦伯格确立了导出函子（Tor、Ext）；Abel 范畴公理化使任何范畴里都能用这套语言；谱序列与导出范畴则让"算不动"与"同调本身"都成为受控对象。从拓扑出发、以代数落地、再反哺几何，同调代数由此成为横贯代数拓扑、代数几何、表示论与数论的一条中枢语言——这正是本章每个概念（同调、正合序列、Tor/Ext、谱序列、导出范畴）共同的来龙去脉。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对同调核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 12.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到同调语境的追问上。

| 步骤 | 行动 | 同调代数中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 对象盘踞在哪个 Abel 范畴？要求同调群、导出函子还是正合性？箭头方向与维数 $n$ 各是什么？ |
| ② 计划 | 选择可行工具、预估方向 | 用蛇形引理连两端？用投射/内射分解算 Tor/Ext？用谱序列层层逼近？用长正合序列把两类同调直接接起来？先选最省力一条路。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 边界算子 $\partial_n$ 具体作用在哪些生成元上；$\partial\partial=0$ 与 $\sum(-1)^i$ 的正负都别漏。 |
| ④ 回看 | 核对结果、检验合理性 | 得到的是 Abel 群还是自由模？Betti/挠结构对得上吗？换一条短正合序列能否重算核对？ |

> 💡 **策略演示**：判断 $0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\mathbb{Z}\to0$ 是否为短正合序列。
> ① 理解：三步同问「像 $=$ 核」。 ② 计划：按定义逐一核对 $\ker$ 与 $\operatorname{im}$。 ③ 执行：$\ker(\times2)=0$，$\ker(\mathbb{Z}\twoheadrightarrow\mathbb{Z}/2\mathbb{Z})=2\mathbb{Z}=\operatorname{im}(\times2)$，末端满射。 ④ 回看：两端项平凡、中段对合，三步全精确，故短正合成立。

### 12.2 概念检查（Concept Check）

> 先用一组判断题与选择题快速自检核心概念。每道题的判断均应落到书面理由（用一句话说明即可）。

**判断正误**（说明理由）：

1. 每个短正合序列 $0\to A\to B\to C\to0$ 都自动分裂。（　）
2. 链映射 $f:A_\bullet\to B_\bullet$ 总能诱导良定义的映射 $f_*:H_n(A)\to H_n(B)$。（　）
3. $\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/m,\mathbb{Z}/n)\cong\mathbb{Z}/d$，其中 $d=\gcd(m,n)$。（　）
4. 投射对象可在「满态下被提升」，内射对象可在「单态下被延拓」。（　）
5. 蛇形引理只给出两段正合关系，中间无法来自「核与余核」之外的连接。（　）
6. 谱序列可视作一层层逼近目标同调的化简装置，常用于不可直接分拆的复形。（　）
7. 若两条链映射链同伦，则它们诱导的同调映射完全相同。（　）

**选择**：

8. $\operatorname{Ext}_{\mathbb{Z}}^1(\mathbb{Z}/m,\mathbb{Z})$ 同构于下列哪个对象？（　）
   - A. $\mathbb{Z}$　B. $\mathbb{Z}/m$　C. $0$　D. $\mathbb{Z}_m\oplus\mathbb{Z}$
9. 蛇形引理的计算输入是下列哪组数据？（　）
   - A. 任一交换图　B. 一个满态及其核的对应　C. 两个行正合且竖向映射处处交换的三行图（核-引理图表）　D. 只需一个链复形
10. 左正合函子一般「保持核」但不保持满态，要获得长正合序列必须动用（　）。
    - A. 谱序列的 $E_1$ 项　B. 投射/内射分解导出的导出函子　C. 正向极限　D. Künneth 公式
...
> **问题**：计算链复形 $0\to\mathbb{Z}\xrightarrow{\times m}\mathbb{Z}\xrightarrow{0}\mathbb{Z}\to0$（$m\ge1$）的同调群 $H_0,H_1,H_2$。

- ① 理解：$\mathbf{Ab}$ 上一幅已知边界的图，目标是三处商 $\ker/\operatorname{im}$。
- ② 计划：各维直线核与像，再约简。
- ③ 执行：$\ker(\times m)=0$，$\operatorname{im}(\times m)=m\mathbb{Z}$，$\operatorname{im}(0)=0$：

$$
H_2=\ker(\times m)=0,\qquad
H_1=\frac{\ker(0)}{\operatorname{im}(\times m)}=\frac{\mathbb{Z}}{m\mathbb{Z}}\cong\mathbb{Z}/m,\qquad
H_0=\frac{\mathbb{Z}}{\operatorname{im}(0)}=\mathbb{Z}.
$$

- ④ 回看：出现挠 $H_1\cong\mathbb{Z}/m$。**检验**：欧拉示性数按链群 $\chi=1-1+1=1$，按同调秩 $H_0-H_1+H_2=1-0+0=1$，两者相符。**易错点**：商中 $\ker\partial_1$ 必须保留整体，只数自由秩会吞掉挠子群。

**示例 S2：用蛇形引理求连接同态 $\delta$**

> **问题**：对所给蛇形图表（两行短正合、竖向 $\alpha=0$，$\beta=\times2$，$\gamma=0$），计算连接同态 $\delta:\ker\gamma\to\operatorname{coker}\alpha$。
...
- ① 理解：目标是蛇形长正合中间的连接同态，先锁定四个核与四个余核。
- ② 计划：逐一计算 $\ker$、$\operatorname{coker}$，再按定义「顶行提升 $b$ → $\beta(b)$ → 沿底行 $A'\to B'$ 拉回成 $A'$ 的类」实现 $\delta$。
- ③ 执行：

$$
\ker\alpha=\ker\beta=\ker\gamma=\mathbb{Z}/2,\qquad
\operatorname{coker}\alpha=\operatorname{coker}\beta=\operatorname{coker}\gamma=\mathbb{Z}/2 .
$$

  取 $c=[1]\in\ker\gamma$：用顶行第二箭头（$b\bmod2$）以 $b=1$ 提升，$\beta(b)=\beta(1)=2$；因 $2\in\ker(\bmod2)=\operatorname{im}(\mathbb{Z}/2\to\mathbb{Z}/4)$，拉回得 $a'=1$，故 $\delta(c)=[1]\in\operatorname{coker}\alpha$。于是 $\delta$ 为恒等 $\mathbb{Z}/2\to\mathbb{Z}/2$。
- ④ 回看：蛇形序列为 $0\to\mathbb{Z}/2\to\mathbb{Z}/2\to\mathbb{Z}/2\xrightarrow{\ \delta=\mathrm{id}\ }\mathbb{Z}/2\to\mathbb{Z}/2\to\mathbb{Z}/2\to0$，逐格精确。**易错点**：$\delta$ 必须把 $\beta(b)$ 沿 $A'\to B'$ 拉回成 $A'$ 的类再取余核；若直接对 $\beta(b)=2$ 取 $\operatorname{coker}\alpha$ 会得到 $[0]$ 而非 $[1]$。

**示例 S3：用自由分解计算导出函子 Tor**

> **问题**：计算 $\operatorname{Tor}_n^{\mathbb{Z}}(\mathbb{Z}/m,\mathbb{Z}/n)$ 的非零项并给出以最少数量的项的类型。

- ① 理解：$\mathbf{Ab}$ 上张积的左导出，取第一变量的自由（投射）分解再张上 $\mathbb{Z}/n$。
- ② 计划：用 $0\to\mathbb{Z}\xrightarrow{\times m}\mathbb{Z}\to\mathbb{Z}/m\to0$ 作为 $\mathbb{Z}/m$ 的自由解，张上 $\mathbb{Z}/n$ 后取 $\ker/\operatorname{im}$。
- ③ 执行：张积后复形化为 $0\to\mathbb{Z}/n\xrightarrow{\ \times m\ }\mathbb{Z}/n\to0$，故取同调：

$$
\operatorname{Tor}_0^{\mathbb{Z}}(\mathbb{Z}/m,\mathbb{Z}/n)\cong\mathbb{Z}/d,\qquad
\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/m,\mathbb{Z}/n)=\ker(\times m)\cong\mathbb{Z}/d,\quad d=\gcd(m,n).
$$

- ④ 回看：$m,n$ 互素时 $d=1$，两次 $\operatorname{Tor}$ 平凡，符合直觉。**检验**：特例 $m=2,n=2$ 得 $\operatorname{Tor}_1=\mathbb{Z}/2$。**易错点**：$\times m$ 作用在 $\mathbb{Z}/n$ 上其核非零，须把 $\ker(\times m)$ 整体算成 $\mathbb{Z}/d$，不可武断为零。

**示例 S4：用长正合序列求 Ext**

> **问题**：由 $0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\to0$ 求 $\operatorname{Ext}_{\mathbb{Z}}^1(\mathbb{Z}/2,\mathbb{Z})$。

- ① 理解：$\operatorname{Hom}(-,\mathbb{Z})$ 对第一变量逆变且只保核（左正合），需用长正合补尾巴。
- ② 计划：作用 $\operatorname{Hom}(-,\mathbb{Z})$ 得长正合，利用 $\operatorname{Ext}_{\mathbb{Z}}^n(\mathbb{Z},\mathbb{Z})=0$（$\mathbb{Z}$ 投射）定位 $\operatorname{Ext}^1$。
- ③ 执行：得 $0\to\operatorname{Hom}(\mathbb{Z}/2,\mathbb{Z})\to\operatorname{Hom}(\mathbb{Z},\mathbb{Z})\to\operatorname{Hom}(\mathbb{Z},\mathbb{Z})\to\operatorname{Ext}^1_{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z})\to0$。因 $\operatorname{Hom}(\mathbb{Z}/2,\mathbb{Z})=0$、$\operatorname{Hom}(\mathbb{Z},\mathbb{Z})=\mathbb{Z}$ 且中间映射是 $\times2$，故：

$$
\operatorname{Ext}_{\mathbb{Z}}^1(\mathbb{Z}/2,\mathbb{Z})=\operatorname{coker}(\mathbb{Z}\xrightarrow{\times2}\mathbb{Z})\cong\mathbb{Z}/2 .
$$

- ④ 回看：$\mathbb{Z}/2$ 非投射，其 $\operatorname{Ext}^1$ 非零。**易错点**：$\operatorname{Hom}(-,\cdot)$ 对第一变量使所有箭头反向，且只在余核处冒出 $\operatorname{Ext}$；把它当正合处理会整段漏掉 $\operatorname{Ext}^1$。

### 12.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（代数/机器计算）**：用 Python（sympy 或自写矩阵秩）实现一个链复形的同调例程，输入关联矩阵输出各维 $\ker/\operatorname{im}$ 与 Betti/挠，并在 §8 示例 4 的复形上验证。
- **聚焦 P2（跨章节联系）**：把 §7 的谱序列 $E_2$ 与第 43 级《代数拓扑》的 Mayer–Vietoris 长正合对照，指出何时谱序列退化回长正合。
- **聚焦 P3（反例构造）**：在 $\mathbf{Ab}$ 中证明 $0\to\mathbb{Z}\to\mathbb{Z}\to\mathbb{Z}/2\to0$（$\times2$）不可分裂，并说明「右端自由」为何分裂、右端是挠群时却未必。
...
- **自测 1**：用一句话解释「短正合序列」与「分裂短正合序列」的差别。
- **自测 2**：说明为什么规定 $\partial\circ\partial=0$（链条件）是同调定义中不可省的前提。

**$\bullet$ 技能题**

- **自测 3**：计算复形 $0\to\mathbb{Z}^2\xrightarrow{\ d_1\ }\mathbb{Z}\to0$ 的同调，其中 $d_1=\begin{pmatrix}2&3\end{pmatrix}$。
- **自测 4**：计算 $\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z}/3)$（基 $m,n$ 互素情形）。
- **自测 5**：沿用示例 S4 的同一手法，求 $\operatorname{Ext}_{\mathbb{Z}}^1(\mathbb{Z}/p,\mathbb{Z})$，$p$ 素数。
- **自测 6**：写出球面 $S^2$ 的胞腔复形链 $0\to\mathbb{Z}\xrightarrow{\ \partial_2\ }0\xleftarrow{\ }\mathbb{Z}\to0$，算出 $H_0,H_1,H_2$ 并用 Euler 示性数检验。

**$\blacklozenge$ 综合题**

- **自测 7**：写出 $0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\to0$ 在 $\operatorname{Hom}(\cdot,K)$ 作用下的整条长正合（对 $K=\mathbb{Z}$ 与 $K=\mathbb{Z}/2$ 各写一次），并说明 Ext 项如何如实「记录」$\operatorname{Hom}$ 之不可正合。

### 12.6 自测答案与简要解析

> **概念判断**：
> 1. **错误**（短正合未必分裂，如 $0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\to0$）。2. **正确**（链条件保证复形与同态在同调上良定义）。3. **正确**（$\mathbb{Z}$ 模上 $\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/m,\mathbb{Z}/n)\cong\mathbb{Z}/d$）。4. **正确**（投射「充满下」、内射「溢出上」）。5. **错误**（蛇形引理恰给出 $\ker/\operatorname{coker}$ 之外的整条长正合，中间就是连接同态 $\delta$）。6. **正确**（谱序列即逐层逼近的装置）。7. **正确**（链同伦的映射在同调上恒等）。

> **选择**：8. B　9. C　10. B。

> **自测**：
> - 自测 1：短正合只要求三段「像 $=$ 核」；分裂还要求 $B\cong A\oplus C$，且对应的提升/收缩存在，右端自由时自动分裂，否则未必。
> - 自测 2：$\partial\partial=0$ 保证每个 $\operatorname{im}\partial_{n+1}\subseteq\ker\partial_n$，这样商 $H_n=\ker/\operatorname{im}$ 才良定义；没有它「同调」无从谈起。
> - 自测 3：$d_1(a,b)=2a+3b$，$\ker d_1=\langle(3,-2)\rangle\cong\mathbb{Z}$，$\operatorname{im}d_1=\gcd(2,3)\mathbb{Z}=\mathbb{Z}$，故 $H_1\cong\mathbb{Z}$、$H_0=0$。
> - 自测 4：$d=\gcd(2,3)=1$，故 $\operatorname{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z}/3)=0$（互素时无挠扩张）。
> - 自测 5：与 S4 同理，$\operatorname{Ext}_{\mathbb{Z}}^1(\mathbb{Z}/p,\mathbb{Z})=\mathbb{Z}/p$。
> - 自测 6：$S^2$ 胞腔复形为 $0\to\mathbb{Z}\xrightarrow{0}0\to\mathbb{Z}\to0$；$H_2=\mathbb{Z}$，$H_1=0$，$H_0=\mathbb{Z}$；$\chi=1-0+1=2$ 与链群 $1-0+1=2$ 相符。
> - 自测 7：对 $K=\mathbb{Z}$：$0\to 0\to\mathbb{Z}\xrightarrow{\times2}\mathbb{Z}\to\mathbb{Z}/2\to\cdots$，$\operatorname{Ext}^1_{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z})=\mathbb{Z}/2$；对 $K=\mathbb{Z}/2$ 亦可得 $\operatorname{Ext}^1=\mathbb{Z}/2$，即 $\operatorname{Hom}(\cdot,K)$ 失去正合之处以长正合补回。