...
> **一句话大白话**：不停重复同一个保测变换然后取平均，这些"时间平均"会在 $L^2$（均方）的意义下收敛到一个固定函数——而且那个极限正是"空间平均"按不变子空间投影后的结果。
>
> **小例子**：对单位圆上旋转/混合映射，$\dfrac1n\sum_{k=0}^{n-1}f(T^k x)$ 在 $L^2$ 中收敛到全体 $T$ 不变向量张成的投影 $\mathbb{P}f$。
