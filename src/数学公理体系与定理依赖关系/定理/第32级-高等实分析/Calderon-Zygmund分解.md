...
> **一句话大白话**：任意非负可积函数都能被拆成两半——一份"个头小、整体好掌控"的良态部分 + 一份"只集中在可数小积木里"的坏态部分，坏的部分即使分摊在小方块上也累计不起来。
>
> **小例子**：把 $f\in L^1$ 按高度阈值 $\lambda$ 切成 $g$（小值）与 $b$（集中在大值处）两部分，使 $|g|\le\lambda$ 且 $b$ 仅支撑在互相分离的小区间上，且 "坏块"总测量几乎可控——这是奇异积分估计的看家本领。
