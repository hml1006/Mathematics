...
> **一句话大白话**：在两个"就近"的辛结构之间，总能找到一个点对点的一一光滑变换把其中一个变成另一个（同时让自己逐点小变形）——"局部均一样则可由小斜率变形搬来"，所以辛表在不炸的地方都一样，是分类问题的"平滑"锚点。
>
> **小例子**：两件辛结构 $\omega_0,\omega_1$ 若满足同伦并保"接近"（闭且共上同调），Moser 保证存在保点同胚 $\phi$ 使 $\phi^*\omega_1=\omega_0$——同胚由线性的依赖连续变形而来，频用于把"规范形"抓在手上。

## 一、定理介绍

> **前置依赖**：de Rham 上同调、Cartan 微分公式（魔法公式）、Poincaré 引理、辛形式的非退化性、常微分方程流的存在性。
