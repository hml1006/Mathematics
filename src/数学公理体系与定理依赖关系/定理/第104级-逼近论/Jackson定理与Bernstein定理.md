...
> **前置依赖**：Weierstrass 逼近定理、一致范数与最佳逼近误差 $E_n(f)$、磨光算子（卷积核）、插值误差定理、Lipschitz 空间与有限差分。
...
- [Jackson 定理与 Bernstein 定理（第67级-逼近论与样条理论）](../第67级-逼近论与样条理论/Jackson定理与Bernstein定理.md)：与本条目为同一定理，另收录于第67级-逼近论与样条理论，可交叉参考。
