...
### 7.1 鞅（Martingale）理论的建立

随机分析的核心概念之一是**鞅**（martingale）。"martingale"一词原指一种赌博下注策略，后在 1930 年代由 **Jean Ville** 从概率角度引入，把数学意义上的"公平博弈"加以形式化：在已知过去与现在的全部信息后，对未来取的期望仍等于当前值，即没有系统性的获利机会。1940 年代 **Paul Lévy** 系统研究鞅并建立了鞅的基本性质；而真正把鞅置于随机分析中心地位的是 **Joseph L. Doob**，其著作《随机过程》（1953）系统建立了鞅理论、停止时间与可选停时定理，奠定了收敛定理与停时理论的基础。

### 7.2 停时与可选停时定理

**停时**（stopping time）刻画了"何时停止观察"这类依赖历史信息的随机时刻。它与随机过程中"信息流（filteration，滤流）"的概念紧密相连，而滤流概念由 **André Kolmogorov** 在 1933 年《概率论基础》中以"σ-代数族"的形式给出。Doob 发展的可选停时定理（optional stopping theorem）说明了在何种条件下，公平博弈的期望收益在随机停止后仍为初始值，这一结果成为随机分析中触发创新（如鞅表示、局部化技术）的钥匙。

### 7.3 伊藤演算与随机积分

日本数学家**伊藤清（Kiyosi Itō）**在 1940 年代创建的**伊藤积分**与**伊藤引理**是随机分析的直接工具：伊藤的公式给出了随机微分法则，其中布朗运动的二次变差属 $dt$ 阶，使被积函数需取当前时刻值（Itô 一侧）；与此同时 **Stratonovich** 提出对称取值的另一积分约定。马尔可夫性与测度变换由 **K. Itō, Girsanov（Girsanov 定理）**等完善，为随机积分提供了严密的分析框架。这些工具正是本文件随机积分、伊藤引理与鞅表示等结论的数学地基。

### 7.4 Malliavin 分析与现代随机分析

1970 年代，**Paul Malliavin** 引入 **Malliavin 微积分**，把变分导数与分部积分引入随机变量空间，用以研究随机微分方程解的光滑性与密度存在性，开辟了随机分析的解析方向。此后随机分析融合了平方可积鞅、局部鞅、随机微分与滤波理论（如 Kalman 滤波工程应用），并在数学金融、随机偏微分方程与算法概率领域持续演进。本文件鞅、停时、伊藤引理、随机积分与 Malliavin 微积分等概念，正共同构筑了这一现代数学分支的完整图景。
