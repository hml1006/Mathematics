...
**马尔可夫链**以俄国数学家**安德烈·马尔可夫（Andrey Markov, 1856–1922）**命名。马尔可夫在 1906–1907 年研究大数定律与词频统计时，提出"下一状态只依赖当前状态"的**马尔可夫性（无后效性）**，并证明了相关的极限定理（马尔可夫定理）。尽管类似思想在伯努利试验与随机游走中萌芽，但马尔可夫系统明确了状态转移与转移矩阵的分析框架。本章的马尔可夫链定义、转移概率、平稳分布、常返/瞬态/周期性与遍历定理，正是马尔可夫开创的链理论的现代发展。

### 7.2 布朗运动：从物理观察到数学构造

**布朗运动**（Brownian motion）先被植物学家布朗（Robert Brown, 1827）在显微镜下观察花粉的随机运动，其后**爱因斯坦**在 1905 年用统计力学给出解释（扩散方程、均方位移 $\sim t$），**斯莫卢霍夫斯基（Smoluchowski）**等也独立研究；**维纳（Norbert Wiener, 1894–1964）**在 1923 年给出布朗运动作为连续随机过程的严格数学构造（**维纳过程**），**Wiener 测度**成为概率论与泛函分析交汇的里程碑。**Levy** 之后（Lévy 过程、Levy 型布朗分解）把布朗运动推广为更一般的具有独立平稳增量的过程。本章的布朗运动的定义、Wiener 的构造方法与性质（连续、不可微、马尔可夫性、鞅性），正是维纳—列维的精确化工程。

### 7.3 鞅、停时与 Doob 定理

**鞅（martingale）**源于赌场对局的语言，1940 年前由 **勒维（Paul Lévy）**引入（他给出鞅的收敛性质），而**杜布（Joseph Doob, 1910–2004）**在 1940 年代系统发展鞅论：**Doob 停止定理（optional stopping）**、**Doob 分解定理**与**鞅收敛定理**成为随机分析的支柱；**停时（stopping time）**与强马尔可夫性则把"何时停止观察"形式化。本章的鞅、停时、Doob 停止定理、鞅收敛定理与 Doob 分解，正是鞅论的经典内容。**迹的收敛**（如 $L^1$ 收敛）也在统一框架下。

### 7.4 Poisson 过程、更新过程与随机积分的出现

**Poisson 过程**是计数随机过程，由一个参数（强度）决定，源于**泊松（Simeon Poisson）**的稀有事件定律（泊松分布，1837）在时间维度的推广，用于到达、排队与可靠性。**更新过程**与**Kolmogorov 相容性定理**（Kolmogorov, 1933 给出"无限维分布族"确定随机过程）成为随机过程存在性的基础。**Itô 公式（伊藤引理）**由**伊藤清（Kiyosi Itô, 1915–2008）**在 1940 年代为随机积分与随机微分方程奠定基础，加上**Ornstein–Uhlenbeck 过程**（Uhlenbeck–Ornstein, 1930）与**平稳过程/功率谱**（Wiener–Khinchin），随机分析最终成为现代概率论的核心。本章的 Poisson 过程、更新过程、随机积分初步、Itô 引理、OU 过程与平稳过程功率谱，正是这一"随机演算"的蓝图。

### 7.5 条件期望、强马尔可夫性与随机过程的现代影响

**条件期望**的严格概念由 **Kolmogorov（柯尔莫哥洛夫）**在 1933 年《概率论基础》中给予测度论定义，使概率论完全公理化；**强马尔可夫性**把马尔可夫性的适用范围从固定时间推广到一切停时。随机过程此时已渗透到金融数学（Black–Scholes）、统计物理、滤波（Kalman）、生物与网络科学。本章以条件期望与 Kolmogorov 相容性定理贯穿全章，正是对从柯尔莫哥洛夫建立可测结构到伊藤的现代随机分析这一整个历史脉络的忠实呈现。
