...
> **一句话大白话**：只要团的规模不能被特征数整除（最经典是复系数），任何表示都能"拆干净成营垒块不打架"——每个不变子空间都拉得出一块补空间，表示必是完全可分解的，像把一栋楼安全分成独立的房间。
>
> **小例子**：对称群 $S_3$ 在复数上的表示完全可约：任何表示都能分解为若干不可约子表示的直和（如标准二维与平凡一维），因为 $|S_3|=6$ 不整除余数特征下的特征。
