...
> **一句话大白话**：一个数域里能"乘出 1"的可逆数（单位），虽然往往有无限多个，却总能由有限个"原子型生成元"拼出来，而且结构是"有限个格子方向"——也就是秩有限的格子。
>
> **小例子**：$\mathbb{Q}(\sqrt5)$ 的全部单位都由 $\frac{1+\sqrt5}{2}$ 的幂（乘 $\pm1$）张成，秩为 $1$；纯虚二次域 $\mathbb{Q}(i)$ 的单位只有 $\{\pm1,\pm i\}$，秩为 $0$。
