...
> **一句话大白话**：在一堆"没有洞挡路"（区域缺口已被绕过）的开集上，任何解析函数都能用有理函数甚至多项式一并逼近——逼近的误差可压小，前提是对要逼近的解析函数所在区域开洞不碍事。
>
> **小例子**：若补集 $\mathbb{C}\setminus K$ 连通，则每个在 $K$ 的邻域内解析的函数 $f$ 可被多项式一致逼近；比如整函数在任一紧圆盘上能用多项式逼近（Taylor 截断即满足）。
