...
> **🧠 来龙去脉：随机过程是怎么从"一个个随机变量"长成"一整套关于时间的有组织随机性"的？**
>
> 随机过程这门学科回答一个朴素而深刻的问题：**当不确定性不仅出现在某一个时刻，而是爬满了整条时间线，我们该如何统一地描述它？** 它的历史，正是数学家一步步给"随时间变化的随机性"寻找合适词汇的过程。
>
> - **掷出的骰子"多了"就是过程**。古典概率时代的主角是单次试验（掷币、抽牌），Bernoulli 1713 年的大数定律让"大量重复"进入数学。1880 年代 Galton 与 Pearson 等人研究"醉汉随机游走"（1905 年 Pearson 正式命名 drunkard's walk），第一次把"一步步随机"与"位置随时间的演化"连在一起——**这是随机过程的胚胎**。
> - **从"大量重复"到"逐年演化"——Poisson 与 Markov**。Poisson 1837 年给出以他命名的分布，但把它铸成"随时间发生的计数过程"要归功于后来对电话话务、放射性衰变的建模；Markov 则在 1906 年为了证明大数定律并不需要独立同分布，发明了"下一时刻只依赖当前状态"的链——**"无记忆"成为第一个真正意义上的随机过程模型**。
> - **让随机性"连续流动"——Einstein, Wiener, Kolmogorov, Lévy**。物理世界强烈要求"连续时间里的随机性"：Einstein 1905 解释 Brown 运动，Kolmogorov 1933 给概率论公理化并用相容性定理回答"给定一堆有限维分布能否拼出整个过程"，Wiener 1923 构造出连续轨道的 Brown 运动，Lévy 系统化了独立增量的分类。**随机过程终于有了严格的身份**。
> - **为"公平之赌"立法——鞅与 Doob**。1930–40 年代 Doob 吸收赌博术语 "martingale"，把它做成衡量"公平博弈"的数学对象，完成了停止定理、收敛定理等大部分鞅论框架；之后 Itô 又在 Brown 运动的"湍急河床"上建起随机微积分。**从此随机过程既有骨架（Markov 性、鞅性）又有分析工具（随机积分）**。
>
> 本节的每一个概念——定义、有限维分布、条件期望、鞅、停时、Markov 链、Poisson 过程、Brown 运动——都不是凭空出现的，而是为了解决上面某一个具体困惑而"生长"出来的工具。跟着"谁、哪年、解决了什么"这条线索走，你会看到整门学科是如何环环相扣地成形的。

### 2.1 随机过程的定义 (Stochastic Processes)

> **🧠 来龙去脉：为什么要"把一堆随机变量放在一起"，还专门给它个名字？**
>
> 单独一个随机变量只能描述"某一时刻/某一状态"的不确定性；但自然界与工程里真正难缠的是**多个时刻、彼此相关**的不确定性——明天的温度与后天的温度、这一步股价与下一步股价，都无法各自孤立地描述。随机过程定义的本质，就是把"随时间索引的随机变量族"$X=\{X_t:t\in T\}$ 当作整体来研究。它要解决的首要问题是：**如何把无穷多个相关随机变量装进同一个概率空间，使"样本轨道"（一次完整演化）这个概念有意义**。这正是它与有限维统计学的最大分野——随机过程研究的不是"一个点"，而是"一根随时间展开的曲线"。简单随机游走 $S_n=\sum_k \xi_k$ 就是最朴素的起点：每一步独立，但位置之间强相关，整条轨道由"掷硬币"这一个随机源决定。
...
> **直观理解（现实类比）**：随机过程就是"一堆随时间变化的骰子"——每个时刻 $t$ 对应一个随机变量 $X_t$，把所有时刻拼在一起就是一条"不确定的河流"。固定的 $\omega$ 相当于"掷出一次完整的命运"，画出一条具体的样本轨道；换个 $\omega$，河流就换了走向。简单随机游走 $S_n$ 就像"醉汉每步掷硬币决定向左还是向右"，状态空间是所有整数 $\mathbb{Z}$，指标集是可数的自然数——随机过程与普通函数的最大区别，就是"每次重播都不尽相同，却有统计规律"。
...
> **🧠 来龙去脉：无穷多个随机变量，怎么用"有限的快照"说清？**
>
> 一个过程有不可数多个彼此相关的 $X_t$，直接定义其联合分布是不可能的。**Kolmogorov（1933）**的妙招是：只需指定所有 **有限维分布** $\mu_{t_1,\ldots,t_n}$，再用 **相容性条件** 保证它们互不矛盾，就能唯一确定整个过程（相容性定理）。这相当于用"任意有限帧截图"拼出整部电影，是把无穷维对象还原为可操作的有限数据的关键一步。
...
> **直观理解（现实类比）**：有限维分布像"给一部庞大电影按其中几帧截图"——$n$ 个时刻 $t_1,\ldots,t_n$ 的联合分布就是一张多帧合影，随机过程的所有性质都藏在无穷多张这样的合影里。相容性条件则保证这些截图"不互相矛盾"：任意调换相机机位顺序（置换不变性）、或只保留部分画面（边缘相容性），都能得到一致的视图。Kolmogorov 相容性定理说：只要这些截图彼此相容，就一定能"拼出"一部连续的电影（一个真实存在的随机过程）——就像从一套自洽的分镜脚本可以拍出完整影片。

### 2.3 条件期望 (Conditional Expectation)

> **🧠 来龙去脉：只有部分信息时，该怎样"最佳预测"一个随机量？**
>
> 条件期望回答"在已知线索 $\mathcal{G}$ 下对随机量 $X$ 的最优无偏预测"。它的历史与 **Doob（1940 年代）**把旧式"特定条件的平均"重铸为 $\sigma$-代数上的严格对象有关，其存在唯一性由 Radon-Nikodym 定理保证。它既是鞅的定义基础（用 $\mathbb{E}[X\mid\mathcal{G}]$ 而非直觉），也是一切"信息分层"分析（塔性质、方差分解）的支点——没有它，鞅、停时与随机积分全都无从谈起。
...
> **直观理解（现实类比）**：条件期望是"在只掌握部分信息时的最佳预测"。$\mathcal{G}$ 代表"你目前能看到的全部线索"，$\mathbb{E}[X\mid\mathcal{G}]$ 就是基于这些线索对 $X$ 做出的最优无偏估计——它只依赖已知线索（$\mathcal{G}$-可测），而且在任何线索集合上的平均恰到好处（两条定义性质）。就像天气预报：看不见全宇宙，只能根据现在能观测的气象数据，对未来温度做出最合理的猜测。塔性质则说"先粗预测再细预测，等于直接按最新信息细预测"——信息只会越看越准，不会越看越差。
...
> **🧠 来龙去脉：鞅是怎么从"赌场词汇"变成随机过程脊梁骨的？**
>
> "Martingale"本是 18 世纪法国赌场里"输了加倍下注、务求回本"的激进策略——它理想化地想靠"迟早赢一局"战胜庄家。数学上真正把它扶正的是 **Ville 与 Doob**：Ville 1939 年把鞅作为"无利可图的赌局"的形式化，**Doob 在 1940–50 年代**将其发展为系统的鞅论（停止定理、收敛定理）。鞅要解决的问题非常本质：**当信息随 $\mathcal{F}_n$ 一层层展开时，什么样的"家底"才是数学上斩钉截铁的公平？** 答案就是"现在已知，未来期望仍等于现在"。看似朴素，它却统一刻画了随机游走、Poisson 过程与 Brown 运动（三者都能造出鞅），并把"能否靠择时或策略系统性赢钱"这类问题变成严格定理。它是随机过程里与 Markov 性并列的"第二根梁"。
...
> **直观理解（现实类比）**：鞅是"公平赌博"的数学模型——无论之前赚了亏了多少，下一局的家底期望恰是"保持现状"：$\mathbb{E}[M_{n+1}\mid\mathcal{F}_n]=M_n$。好比零和赌场里的玩家，掌握全部历史后，对未来家底的期望仍等于现在的口袋。随机游走就是最典型的例子（每步期望回来）。鞅之所以重要，是因为它既是"公平"的精确度量，又配有一整套剖析长期行为的强大工具（停止定理、收敛定理）——看似不起眼的"期望不变"，却隐藏着极深的结构。
...
> **🧠 来龙去脉：随机实验里"随时喊停"也要有规矩。**
>
> 停时的思想源于 **Wald（1940 年代）的序贯分析**——允许样本量随机决定的统计检验（不合格就继续抽检）。把"何时停止"形式化为只依赖过去信息（$\{\tau=n\}\in\mathcal{F}_n$）的随机时间，是连接赌博策略、首达时分析、最优停止与 Doob 停止定理的关键枢纽：没有停时的严格概念，后续的停止定理、鞅收敛与选择性检验全都无从谈起。
...
> **直观理解（现实类比）**：停时是"合法的停止时刻"——是否停在这一刻，只能依据已经发生的信息（$\{\tau=n\}\in\mathcal{F}_n$），不能依赖未来或"偷看天机"。像赌场里的规则"输了 $K$ 元就收手"是合法停时，而"等到我确定能赢的那一局"因依赖未来而非法。首达时 $\tau_a=\inf\{n:S_n=a\}$ 是停时，因为它纯粹由"已经到过 $a$"这一过去事实决定。停时 $\sigma$-代数 $\mathcal{F}_\tau$ 则是"停下来的那一刻所积累的全部信息"——把随机时刻定格下来，再读此刻的信息库。
...
> **🧠 来龙去脉：Markov 为什么偏要在"没有独立性"的地方硬造出一条链？**
>
> 20 世纪初，"独立同分布"被当作大数定律的前提。**俄国数学家 Markov（1906 年）**提出质疑：现实中的天气、选词、社会状态几乎从不独立——今天会影响明天。为了在"只依赖当前"这种最小限度的相关下仍能证明大数定律，他引入"给定现在、将来与过去独立"的 **Markov 性**，并证明了常返链的时间平均收敛。这等于把概率论从"独立时代"推进到"相关时代"，且用最省的假设（无记忆）换取最强的结论。后来由 Markov 链催生的 MCMC 采样算法，让这套理论在 21 世纪大量数据的时代依然是最重要的计算工具之一。
...
> **直观理解（现实类比）**：Markov 性就是"无记忆"——系统怎么走到现在不重要，只凭"当下在哪"就能决定下一步的概率。像一只蚂蚁在每个节点只看眼前路标选择方向，对来路毫不在意；也像发直球的手气，下一球与前面输赢无关，只看现在的状态。这让复杂的全部历史被压缩成一个数——当前状态，从而使分析大大简化。
...
> **直观理解（现实类比）**：Chapman-Kolmogorov 方程像"传话接力"——先走 $m$ 步到某个中间站 $k$，再走 $n$ 步到目标 $j$，把所有可能的中间站接到一起求和，就等于直接走 $m+n$ 步。矩阵形式 $P^{m+n}=P^mP^n$ 正是矩阵乘法的本质：$P$ 的幂一步步刻画"接力概率"的叠加。它告诉我们，只要知道一步转移规则，任何多步转移概率都能通过简单的矩阵乘累积出来。

### 2.7 平稳分布与遍历性 (Stationary Distribution and Ergodicity)

> **🧠 来龙去脉：链跑得再久，会不会"忘了起点"而住进一座固定的分布？**
>
> 平稳分布问：是否存在一个概率 $\pi$，使得 $X_n$ 的分布一旦等于 $\pi$ 就永远等于 $\pi$？遍历性进一步问：长期的时间平均是否等于按 $\pi$ 的空间平均？这两个问题的数学源头在 **Birkhoff（1931）的遍历定理**（动力系统）与 Kolmogorov 等人对 Markov 过程不变测度的研究，而把它们落到 Markov 链上的经典表述来自 **Doob 与 Feller（1940–50 年代）**。它是 MCMC 的命脉：只要链不可约且正常返，初始分布就被"洗掉"，样本最终按 $\pi$ 分布。
...
> **直观理解（现实类比）**：平稳分布 $\pi=\pi P$ 像"水温泄光还要保持的稳定水位"——从 $\pi$ 出发走一步还是 $\pi$，说明系统长期运行的"驻扎比例"不再漂移。细致平衡 $\pi_i p_{ij}=\pi_j p_{ji}$ 像"进出一家一户的人流恰好相等"——每对状态间双向流动抵消，整体自然稳。不可约像"整张地图全部打通"：从任意一处都能连到任意另一处，没有孤岛；正常返像"走散了总归回得来且平均回来还不慢"。三者凑齐，系统才有唯一可靠的长期统计，这正是 MCMC 采样能收敛到目标分布的底气。

### 2.8 Poisson 过程 (Poisson Process)

> **🧠 来龙去脉：Poisson 分布如何长成"排队电话、放射性衰变"的万能计数器？**
>
> **Poisson（1837）**在研究陪审团裁决等"稀有事件"时给出 $\frac{(\lambda t)^k}{k!}e^{-\lambda t}$——二项分布在"试验极多、单搏概率极小"下的极限，即"稀有事件定律"。但把它铸成**随时间演化的计数过程**是 20 世纪初的事：电话话务员要估计呼叫涌入，物理学家要刻画放射性粒子看似"无规律"的稀疏衰变。关键洞察是 **独立增量 + 平稳增量** 这两条公理——它们精确抓住"过去不影响将来、单位时间平均到达率恒定"的现象，并推出指数到达间隔（无记忆）这一极反直觉的性质。Poisson 过程因此成为"完全随机的稀疏到达流"的黄金标准，也是更新过程的特例。
...
> **🧠 来龙去脉：把 Poisson"不需记忆的指数寿命"换成任意寿命，故事照样转得动。**
>
> 现实中更换灯泡、维修机器、病人到诊的间隔并不是指数分布。**更新过程**把 Poisson 的假设放宽为"独立同分布的任意间隔"，研究计数 $N(t)$ 的增长。其核心是 **更新定理**（Blackwell, 1948）与更新回报定理：在一般间隔下仍成立 $N(t)/t\to1/\mu$ a.s.。它把 Poisson 的直截了当，推广成一副更普适的"寿命账本"。
...
> **直观理解（现实类比）**：更新过程像"换灯泡或修机器的寿命账本"——每个 $\tau_n$ 是一盏新灯泡的使用寿命，$T_n$ 是第 $n$ 次更换的时刻，$N(t)$ 是截至 $t$ 时刻共换了多少盏。它把 Poisson 过程的"指数寿命"换成了任意分布 $F$ 的寿命：寿命不必无记忆，可以是固定的、重尾的、双峰的……只要独立同分布即可。更新函数 $m(t)$ 则是"到 $t$ 时刻平均换了几个零件"。之所以叫"更新"，正因为它刻画"每一段寿命结束后系统满血复活、重新计时"的过程。Poisson 过程只是其中"寿命服从指数分布"的特例——顾客电话到访、故障更替、基因突变都可用它建模。
...
> **🧠 思考历程：花粉在显微镜下的颤抖，怎么会成为整个金融与物理的"随机底层"？**
>
> Brown 运动是随机微积分的原点。它最初只是植物学家的观察，却被数学、物理、金融先后轮番收编，最终成为描述一切"连续又要随机"现象的万能模板。
>
> - **第一步：生物学家看到"花粉在动"**。1827 年植物学家布朗（Brown）用显微镜观察花粉，发现它们在水中永不停止地乱跳。起初以为是花粉"活着"在动，后来即使是无生命微粒也不会停下——**一个"永远不停"的随机游走首次现身实验室。**
> - **爱因斯坦用"碰撞"给出理论（1905）**。爱因斯坦天才地解释：**花粉被不计其数的水分子从四面八方随机撞击，结果它的位置做随机游走，波动尺度随时间 $\sqrt{t}$ 增长**。他还顺带衍生了著名的"扩散方程 + 布朗运动"关系，促成了阿伏伽德罗常数的测量——**"看懂"一次显微镜抖动，竟连上原子论的依据。**
> - **巴舍利耶早一步押注股市**。数学家巴舍利耶（Bachelier）1900 年就把"股票价格"建模成布朗运动来给期权定价——**比爱因斯坦还早了几年，只在数学上被冷落**。半个世纪后它被布莱克、斯科尔斯与默顿（Black-Scholes-Merton）重新拾起，成为金融期权定价理论的地基，让交易所与随机过程正式握手。
> - **维纳把它做成严格数学**。布朗运动之所以俗称"维纳过程"，正是因为**维纳（Wiener）在 1920 年代用构造方法（见 3.5 节）给这种"猜出来的运动"建立了严密的对象**：独立增量、高斯分布、轨道连续——三条公理，一个连相加减也需要专门的 Itô 积分才能伺候的"连续但处处不平滑"对象。
> - **心路**：Brown 运动告诉你：**"看似杂乱无章"的运动，一旦找到合适的公理与工具（扩散、随机积分），反而能成为无数领域共同的语言**。从花粉的抖动到期权的价格，再到粒子扩散——一个由随机撞击写成的"处处震荡"过程，恰恰是确定性模型统治不了的那些世界真正的底层语法。

### 2.11 随机积分初步 (Stochastic Calculus Introduction)

> **🧠 来龙去脉：为什么 Brown 运动带来的积分，连"切分求和"都要重新发明？**
>
> 普通微积分处理 $x(t)\,dx(t)$ 没有问题，但把被积函数换成 $B_t$、把积分微元换成 $dB_t$ 时，经典 Riemann-Stieltjes 积分立刻失效——因为 $B_t$ 的轨道处处不可微、总变差无穷。**Itô（1940–50 年代）**为解决 Brown 驱动系统的随机微分方程，发明了取**左端点**（可料）的积分：每一小段的取值只依赖过去而不"偷看"未来。这一选择换来最强的数学性质——**积分结果仍是鞅**——并催生了 Itô 引理。后来 Stratonovich 提出满足普通链式法则的另一种积分，两者只差一个修正常数。随机微积分由此成为金融（Black-Scholes）、物理（扩散）、工程（SDE）的共同语言。
...
> **直观理解（现实类比）**：Itô 积分像"赌场里只能照当下的牌下注，不能看底牌再改"——在第 $t_k$ 格只能依据当时（左端点 $f_{t_k}$）已知道的信息把赌注压在下一小段的随机涨落 $B_{t_{k+1}}-B_{t_k}$ 上，而不是事后用中点的信息"作弊"。左端点的选择正是它具备鞅性的关键：每一段的条件期望为 $0$，整场"累积押注"的期望也就保持中立、公平。Itô 等距 $\mathbb{E}[(\int f\,dB)^2]=\mathbb{E}[\int f^2 dt]$ 像"波动能量的守恒账本"——赌注的能量全部转成被积函数的平方面积，没有白赚也没有泄漏。反观普通积分（中点取法）会改变二阶项，导致随机微积分与经典微积分分道扬镳（$d(B^2)\ne 2B\,dB$）。
...
### 2.12 强 Markov 性 (Strong Markov Property)

> **🧠 来龙去脉：普通 Markov 性只管"确定的现在"，而强 Markov 性把"现在"本身也变成了随机的。**
>
> 普通 Markov 性只说在**确定的时刻** $n$，未来只依赖当前状态。但应用中我们常在**随机时刻**（停时）问同样的问题：赌博破产时、股票首达某价位时，未来会怎样？**Doob 与 Hunt（1950 年代）**把 Markov 性推广到"停在任何一个合法停时 $\tau$（甚至可能取 $\infty$）时仍然成立"，这就是强 Markov 性。它是更新定理、Markov 链遍历（3.4 节）与 Brown 反射原理（4.4 节）背后的通用引擎。

**定义 2.28**（强 Markov 性）设 $\{X_n\}_{n\ge0}$ 是可数状态空间 $S$ 上的齐次 Markov 链，$\tau$ 是关于其自然滤子的停时，且在 $\{\tau<\infty\}$ 上 $X_\tau=i$。称链具有 **强 Markov 性**（strong Markov property），若对任意 $j\in S$ 与任意 $n\ge0$，

$$
\mathbb{P}(X_{\tau+n}=j \mid \mathcal{F}_\tau,\; X_\tau=i) = \mathbb{P}(X_n=j \mid X_0=i) = P^n_{ij}.
$$

换言之，在停时 $\tau$ 处"重置时钟"，未来从 $X_\tau$ 出发按原转移矩阵独立地重新开始，如同一次全新启动。

> **直观理解（现实类比）**：Markov 性好比"无论路上走了多久，一到此刻就看路标选下一步"；强 Markov 性则说"连'此刻'本身都不是固定的——哪怕这次是破产时刻、是利润翻倍的时刻，只要这个时刻完全由已发生的事决定（是停时），一到那刻照样只看眼前的路标"。这大大扩展了"无记忆"的适用范围。

**例 2.29**（首次返回后的重置）设链从 $i$ 出发，$\tau_i=\inf\{n\ge1:X_n=i\}$ 是首次返回时间。则由强 Markov 性，在 $\{\tau_i<\infty\}$ 上，链在 $\tau_i$ 之后与从 $i$ 独立重新出发的副本同分布。这正是推导常返性质、更新论证（3.4 节）与"循环分解"的根基。

### 2.13 Markov 链的常返、瞬态与周期 (Recurrence, Transience and Periodicity)

> **🧠 来龙去脉：链会不会"回得来"，决定了它有没有长期统计。**
>
> 在判断"这个链能否用平稳分布描述它的长期行为"之前，必须先回答更基本的问题：**从一个状态出发，它还回来吗？回来得频繁吗？** **Pólya（1921）**回答了最著名的一例——对称随机游走在 1、2 维几乎必然回到原点（常返），在 3 维及以上的高维却几乎必然一去不返（瞬态）。常返/瞬态的分类是 Markov 链分析的地基，也是平稳分布存在性与遍历性（2.7、3.4 节）的前提。

**定义 2.30**（常返与瞬态）设 $\{X_n\}$ 是齐次 Markov 链，状态 $i$，令 $\tau_i=\inf\{n\ge1:X_n=i\}$。若 $\mathbb{P}_i(\tau_i<\infty)=1$，称 $i$ 为 **常返态**（recurrent）；若 $\mathbb{P}_i(\tau_i<\infty)<1$，称 $i$ 为 **瞬态**（transient）。记 $f_{ii}^{(n)}=\mathbb{P}_i(X_n=i,\;X_k\ne i,\;1\le k<n)$ 为首次返回概率，则常返性等价于

$$
\mathbb{P}_i(\tau_i<\infty)=\sum_{n\ge1}f_{ii}^{(n)}=1.
$$

**定理 2.31**（常返性判据）对状态 $i$，以下三条相互等价：

1. $i$ 是常返态。
2. $\sum_{n\ge0} p_{ii}^{(n)}=\infty$。
3. 从 $i$ 出发访问 $i$ 的次数期望无限：$\mathbb{E}_i\!\left[\sum_{n\ge0}\mathbf 1_{\{X_n=i\}}\right]=\infty$。

> **直观理解（现实类比）**：常返像"总有办法回到老地方"——即使可能跑得很远，也几乎必然回来且回来次数期望是无穷的；瞬态像"一去不复返"——一旦离开，回来的机会不足，访问次数期望有限。常返态又分两种：**正常返**（$\mathbb{E}_i[\tau_i]<\infty$，定义 2.20，长时间后能坐稳平稳分布）与 **零常返**（回来几乎必然，但平均要等无穷久）。

**例 2.32**（Pólya 定理）设 $d$ 维对称随机游走每一步以相等概率沿坐标轴移动一格。则：

- $d=1$ 与 $d=2$：常返（$\mathbb{P}_0(\text{回 }0)=1$）。
- $d\ge3$：瞬态（$\mathbb{P}_0(\text{回 }0)<1$）。

**周期（period）**：状态 $i$ 的 **周期** 定义为 $d(i)=\gcd\{n\ge1: p_{ii}^{(n)}>0\}$。若 $d(i)=1$，称 $i$ 为 **非周期**（aperiodic）。周期决定 $p_{ii}^{(n)}$ 沿哪条子序列收敛，进而影响 $p_{ij}^{(n)}\to\pi_j$ 是否对每个 $n$ 成立（见习题 181）。

### 2.14 Ornstein-Uhlenbeck 过程 (Ornstein-Uhlenbeck Process)

> **🧠 来龙去脉：纯 Brown 运动会无限漂走，现实却需要"有向心回拉的波动"。**
>
> Brown 运动 $\sigma B_t$ 的方差 $\sigma^2 t$ 随 $t$ 无界增长，而价格、速度、温度却往往在某个均衡值附近波动而不至于失控。**物理学家 Ornstein 与 Uhlenbeck（1930）**为描述"有向心力的随机游走"（如同时受黏滞阻力与热随机碰撞作用的粒子速度）提出了 OU 过程——它把 Brown 运动的"杂乱驱动"与一个线性回归的"向 0 回拉"相结合：

$$
dX_t = -\theta X_t\,dt + \sigma\,dB_t.
$$

其解为 $X_t = e^{-\theta t}X_0 + \sigma e^{-\theta t}\int_0^t e^{\theta s}\,dB_s$，均值向 $0$ 指数衰减，方差稳定于 $\sigma^2/(2\theta)$。它是第一个被严格研究的连续时间 Markov 高斯过程，也是金融利率模型（Vasicek）、统计学习与生理信号的标准模板。

> **直观理解（现实类比）**：想象一根弹簧系着的小球浸在随机水流里——弹簧一直把它拉回原点（$- \theta X_t$ 项），水流则不断踢它（$\sigma dB_t$ 项）。两者平衡后，小球不会像自由 Brown 那样无限飘散，而是在原点附近呈稳定的高斯抖动，相关性按 $e^{-\theta \Delta t}$ 随时间衰减。

**性质 2.33**（OU 过程的基本性质）设 $\{X_t\}$ 满足上式，$X_0$ 独立于 $\{B_t\}$。

1. $X_t \sim N\!\left(e^{-\theta t}X_0,\; \dfrac{\sigma^2}{2\theta}(1-e^{-2\theta t})\right)$。
2. 若取 $X_0\sim N(0,\sigma^2/(2\theta))$，则 $\{X_t\}$ 是 **平稳高斯过程**：$\mathbb{E}[X_t]=0$，$\text{Cov}(X_s,X_t)=\dfrac{\sigma^2}{2\theta}e^{-\theta|t-s|}$，有限维分布不随时间平移而改变。

验证只需把 Itô 引理作用于 $e^{\theta t}X_t$ 再取期望。OU 是连接 Markov 过程与平稳谱分析（下节）的桥梁。

### 2.15 平稳过程与功率谱 (Stationary Processes and Power Spectrum)

> **🧠 来龙去脉：一个过程"看上去不随时间漂移"，能否用正弦波把它拆开看？**
>
> 现实信号（噪声、振动、价格、语音）往往在统计上"稳态"：平移时间后均值与相关不变。**平稳性**（stationarity）把这种直觉严格化，分两种：**严平稳**（所有有限维分布随平移不变）与 **宽平稳**（仅一、二阶矩随平移不变）。宽平稳更易实测也更常用。**Wiener 与 Khinchine（1930 年代）**证明了深刻结论——宽平稳过程的自协方差与一个非负测度（谱测度）互为 Fourier 关系（Wiener-Khinchin 定理）：平稳性说明自相关可以按频率"摊开"看，功率分散在谱上。这让随机过程与信号处理、谱分析接通。

**定义 2.34**（宽平稳）实值过程 $\{X_t:t\in\mathbb{R}\}$ 称为 **宽平稳**（wide-sense stationary）或 **二阶平稳**，若均值 $\mathbb{E}[X_t]=\mu$（常数）且自协方差仅依赖于滞后：$\gamma(h)=\text{Cov}(X_{t+h},X_t)$ 与 $t$ 无关。

**定理 2.35**（Wiener–Khinchin 定理）设 $\{X_t\}$ 为宽平稳过程，自协方差 $\gamma$ 连续。则存在唯一有限非负测度 $\mu$（**谱测度**，spectral measure）使得

$$
\gamma(h)=\int_{-\infty}^{\infty} e^{i h \lambda}\,d\mu(\lambda).
$$

若 $\mu$ 关于 Lebesgue 测度绝对连续，则 $\gamma(h)=\int_{-\infty}^{\infty} e^{ih\lambda}f(\lambda)\,d\lambda$，其中 $f(\lambda)\ge0$ 称为 **谱密度**（spectral density）。

> **直观理解（现实类比）**：$\gamma(h)$ 刻画"隔 $h$ 时刻还残留多少相关"，Wiener-Khinchin 定理却说"这一串衰减的相关，恰好等于把信号按不同频率的正弦波拆开、看每个频段塞了多少'功率'"。就像一台均衡器：总相关是"混合声卡里所有频段"的和谱，谱密度则是每个频段的音量。白噪声（相关集中在 $\gamma(0)$、谱平直）与 OU 过程（自协方差 $e^{-\theta|h|}$、谱为柯西型）是两端最重要的实例。

**例 2.36**（OU 过程的谱）由性质 2.33，OU 过程自协方差 $\gamma(h)=\frac{\sigma^2}{2\theta}e^{-\theta|h|}$，其谱密度为柯西/洛伦兹型

$$
f(\lambda)=\frac{\sigma^2}{2\pi}\cdot\frac{1}{\lambda^2+\theta^2},
$$

即 $|\lambda|$ 越小功率越大、低频主导的平滑噪声——这也是它在滤波器设计与生理建模中被广泛使用的原因。

---

## 3. 定理与证明

> **🧠 来龙去脉：有了概念，为什么还要花整节去"较真"证明？**
>
> 概念回答"对象是什么"，定理回答"对象凭什么存在、凭什么收敛、凭什么可以被安全地停止"。随机过程的深层力量恰恰藏在证明里：Kolmogorov 相容性定理用柱集与紧正则性回答"存在性"，Doob 停止定理用划分论证回答"公平终止"，鞅收敛定理用上穿不等式回答"必有归宿"，Wiener 构造用 Schauder 级数回答"连续轨道确凿存在"，遍历定理回答"时间平均 = 空间平均"。这些证明不只是严谨性点缀——它们给出了可操作的方法与构造（耦合、上穿、内正则），是理解"为什么结论成立"的关键钥匙。
...
> **直观理解（现实类比）**：Doob 停止定理是"公平赌局在任何合法收手时刻仍公平"的定心丸——只要 $M$ 是鞅、$\tau$ 是合法停时且被某个上限困住，那么"走到 $\tau$ 时的家底期望"依然等于"开场时的家底"：$\mathbb{E}[M_\tau]=\mathbb{E}[M_0]$。这告诉赌徒"输 $K$ 元就收手"这样的策略无法靠"择时离场"来系统性盈利——因为停时判断只依赖已知信息，未来赢不了期望。它也是推导首达时概率和期望时间的利器：像在 $4.1$ 里，把 $\mathbb{E}[S_\tau]=0$ 与 $\mathbb{E}[S_\tau^2]=\mathbb{E}[\tau]$ 联立，就轻松解出 $\mathbb{P}(S_\tau=a)=\frac{b}{a+b}$ 和 $\mathbb{E}[\tau]=ab$。有界停时的假设防止了"无限期等待极端行情的止损陷阱"。
...
> **直观理解（现实类比）**：鞅收敛定理说"只要一个公平赌局的期望始终兜底（$\sup_n\mathbb{E}|M_n|<\infty$），那它几乎必然水落石出、尘埃落定"——尽管途中 $M_n$ 可能上下颠簸，它不会一直无穷震荡下去，最终会稳定逼近一个极限家底 $M_\infty$。形象点：一个人只赢不亏或只亏不赢地无限赌下去，长期必然滑向某个终端状态。证明使用的**上穿不等式**更是点睛之笔：若 $M_n$ 在某个区间 $[a,b]$ 之间永远来回钻（上穿无穷多次），那必然需要"无底洞"式的能量；而期望有界就封死了这种无限上穿。这正是"期望管住，波动不乱奔"的精确版本。金融里，一个价格若期望无界，长期就不会稳定；一旦可积性成立，就注定有个"结局"。
...
> **直观理解（现实类比）**：Markov 链遍历定理是"强大数定律在无记忆场景的翻版"——只要链不可约又正常返，那么把 $f$ 的值沿时间平均（$\frac1n\sum f(X_k)$）和沿空间平均（$\sum \pi_i f(i)$）最终一致，即"时间平均 = 空间平均"。好比长期随机逛一座城：无论从哪条街口出发，逛得足够久后，在每条街区流连的时间占比都会收敛到同一个固定比例 $\pi$，与起点无关。它同时保证极限 $\lim p^{(n)}_{ij}=\pi_j$：多步后，从任意初态 $i$ 走到目标 $j$ 的概率都趋同于 $\pi_j$，"记忆"被彻底洗掉。这正是 MCMC 的核心保证——样本只依赖初始迭代之后很快就按平稳分布分布，不必再管初值。
...
> **直观理解（现实类比）**：Brown 运动听起来像"猜出来的对象"，而 Wiener 的构造相当于"用乐高积木把它真的搭出来、盖章验真"。思路是把 $[0,1]$ 上的路径写成一系列互相独立的"三角形小坡"的无穷和：每个 i.i.d. 高斯系数 $Z_n$ 控制一块小三角 $s_n(t)$ 的高度。层叠的小三角（Haar/Schauder 小波）一块块加密，越叠越细，最终拼出"连续却处处抖动"的一条曲线。为什么要如此大费周章？因为给出四条公理是一回事，真正确认"存在一个概率空间承载它们"则是另一回事；Wiener 用这一可量化的级数既保证了样本轨道连续（小三角合力收敛），又验证了有限维分布正是高斯的 $\text{Cov}=\min(s,t)$。这好比"光有设计图，还要一把螺丝刀把它拧出来"——构造即证明，严谨地基由此落下。
...
> **直观理解（现实类比）**：Itô 引理是随机世界的"链式法则 + 二阶项修正"。普通微积分里 $df=f_x\,dx$，但在 Brown 跑动下，平方项不再可以忽略：因为 $(dB_t)^2=dt$ 这个非随机的"二次变差"贡献了额外的 $\frac12 f_{xx}dt$ 项。打个比方：普通微分的增量像骑车的斜率，而随机微分的增量还包含"路有多颠（$d B$）所导致的额外二阶抖动"——$f$ 越弯（$f_{xx}\ne0$），这段随机颠簸就越要还回去一个确定性修正。所以 $d(B_t^2)=2B_t\,dB_t+dt$ 而非 $2B_t\,dB_t$。正是这一点，让随机微积分与经典微积分分道扬镳。金融上 $S_t=e^{\sigma B_t-\frac12\sigma^2 t}$（几何 Brown 运动）就是 Itô 引理的对数应用产物，得出著名的 $dS_t=\mu S_t\,dt+\sigma S_t\,dB_t$，是整个 Black-Scholes 定价公式的引擎。
...
### 3.7 Doob 分解定理 (Doob Decomposition)

> **🧠 来龙去脉：下鞅"只升不亏"的能量，从哪里来？**
>
> 鞅是"公平"，下鞅是"平均偏向上"。Doob 分解说：**任何下鞅都是"一个公平的部分（鞅 $M_n$）+ 一个必然累积的趋势（可料增过程 $A_n$）"**。这好比把一支"总体走高"的基金拆成"随机波动（鞅）+ 确定的累计漂移（$A_n$）"。因为 $A_n$ 可料（由到 $k-1$ 为止的信息决定）、"预先知道"，它正是平方鞅的增过程 $\langle M\rangle_n$（习题 155、224）、二次变差与金融中"漂移项与鞅项分离"乃至测度变换（把漂移吸收进测度）的代数根源。

**定理 3.10**（Doob 分解定理）设 $\{X_n\}_{n\ge0}$ 是关于滤子 $\{\mathcal{F}_n\}$ 的下鞅（submartingale）。则存在唯一分解

$$
X_n = M_n + A_n,
$$

其中 $\{M_n\}$ 是鞅，$\{A_n\}$ 是**可料增过程**（predictable nondecreasing process，即 $A_0=0$，$A_n$ 是 $\mathcal{F}_{n-1}$-可测，且 $A_n\le A_{n+1}$ a.s.）。

**证明（构造）**：令

$$
A_n=\sum_{k=1}^n \mathbb{E}[X_k-X_{k-1}\mid \mathcal{F}_{k-1}], \qquad M_n=X_n-A_n.
$$

由于 $X$ 是下鞅，$A_n$ 非负且可料；直接验证 $\mathbb{E}[M_{n+1}\mid\mathcal{F}_n]=M_n$，故 $M$ 为鞅。唯一性由 $A$ 的可料性与 $M$ 的鞅性逐项递归确定。$\square$

**例 3.11**（Doob 分解示例）设 $X_n=S_n^2$（$S_n$ 为简单随机游走）。由例 2.11，$M_n=S_n^2-n$ 是鞅，故取 $A_n=n$ 即得 $X_n=M_n+A_n$ 的 Doob 分解——趋势项 $A_n=n$ 恰是逐块的漂移积累 $\mathbb{E}[\Delta X_k\mid\mathcal{F}_{k-1}]=\mathbb{E}[\xi_k^2]=1$ 之和。这印证了"增过程 = 逐块漂移的累计"这一直观。
...
### 新增主题习题（对应 2.12–2.15 与 3.7）

> 以下习题配套 2.12（强 Markov 性）、2.13（常返、瞬态与周期）、2.14（Ornstein-Uhlenbeck 过程）、2.15（平稳过程与功率谱）与 3.7（Doob 分解定理）等新增小节。编号自 301 起延续全篇，答案见第 6 节"新增主题习题答案"。

**习题 301**（强 Markov 性定义）写出强 Markov 性相对于普通 Markov 性的差别：前者把"现在"换成什么随机对象？其成立的关键前提是什么？

**习题 302**（强 Markov 性-首次返回）解释为何在首次返回时刻 $\tau_i$ 之后，链独立于过去并以 $X_{\tau_i}=i$ 重新开始。据此说明 $\sum_{n\ge1}f_{ii}^{(n)}$ 在常返性与"循环计数"上的含义。

**习题 303**（常返与瞬态）给出常返态与瞬态的定义，并写出常返性等价于 $\sum_{n\ge0}p_{ii}^{(n)}=\infty$ 的判据表述。

**习题 304**（Pólya 定理）陈述 Pólya 定理：$d$ 维对称随机游走分别在哪几维常返、哪几维瞬态？给出判断依据。

**习题 305**（正常返与零常返）常返态分为正常返与零常返两种。试说明两者都以概率 1 返回，区别究竟在哪？哪种才有唯一的平稳分布？

**习题 306**（周期定义）状态 $i$ 的周期 $d(i)$ 的定义是什么？什么情况下称 $i$ 非周期？周期对 $p_{ii}^{(n)}$ 的收敛有何影响？

**习题 307**（OU 过程定义）写出 Ornstein-Uhlenbeck 过程的随机微分方程，并写出其显式解 $X_t$ 的表达式。

**习题 308**（OU 平稳性）取 $X_0\sim N(0,\sigma^2/(2\theta))$ 时 OU 过程是平稳高斯过程。写出 $\mathbb{E}[X_t]$ 与 $\text{Cov}(X_s,X_t)$。

**习题 309**（宽平稳）什么是宽平稳（二阶平稳）过程？写出其均值与自协方差的条件。

**习题 310**（Wiener-Khinchin）陈述 Wiener-Khinchin 定理：自协方差 $\gamma(h)$ 与谱密度 $f(\lambda)$ 之间满足什么 Fourier 关系？

**习题 311**（OU 谱密度）由 OU 过程自协方差 $e^{-\theta|h|}$ 写出其谱密度 $f(\lambda)$，并说明为何它体现"低频主导"。

**习题 312**（Doob 分解）对下鞅 $X_n$ 写出 Doob 分解 $X_n=M_n+A_n$，说明 $A_n$ 的可料单调性并写出其构造公式。
...
1. 强 Markov 性把"现在"换成随机停时 $\tau$：给定 $\mathcal{F}_\tau$ 与 $X_\tau=i$，未来以原转移矩阵从 $i$ 重新开始；关键是 $\tau$ 必须是合法停时（$\{\tau=n\}\in\mathcal{F}_n$）。普通 Markov 性只对确定时刻 $n$ 成立。
2. 由强 Markov 性，在 $\{\tau_i<\infty\}$ 上链在 $\tau_i$ 后与从 $i$ 独立出发的副本同分布。于是 $\sum_{n\ge1}f_{ii}^{(n)}=\mathbb{P}_i(\tau_i<\infty)$；它等于 1 即常返，小于 1 即瞬态，并给出"循环次数"的分解结构。
3. 常返：$\mathbb{P}_i(\tau_i<\infty)=1$；瞬态：$\mathbb{P}_i(\tau_i<\infty)<1$。等价判据：常返 $\iff \sum_{n\ge0}p_{ii}^{(n)}=\infty$（访问次数期望无限）。
4. Pólya 定理：$d=1,2$ 对称随机游走常返（$\mathbb{P}_0(\text{回 }0)=1$）；$d\ge3$ 瞬态。依据是返回概率 $\mathbb{P}_0(X_{2n}=0)\sim C_d n^{-d/2}$，其和当且仅当 $d\le2$ 发散。
5. 正常返与零常返都以概率 1 返回，区别在平均返回时间：正常返 $\mathbb{E}_i[\tau_i]<\infty$，零常返 $\mathbb{E}_i[\tau_i]=\infty$。仅正常返（不可约）链存在唯一的平稳分布 $\pi$，且 $\pi_j=1/\mathbb{E}_j[\tau_j]$。
6. $d(i)=\gcd\{n\ge1:p_{ii}^{(n)}>0\}$。$d(i)=1$ 时称非周期。周期决定 $p_{ii}^{(n)}$ 只能在周期子序列（如 $d,2d,3d,\ldots$）上非零，从而对 $\lim p_{ij}^{(n)}=\pi_j$ 只在整链非周期时对每个 $n$ 成立。
7. $dX_t=-\theta X_t\,dt+\sigma\,dB_t$，解为 $X_t=e^{-\theta t}X_0+\sigma e^{-\theta t}\int_0^t e^{\theta s}\,dB_s$。
8. 平稳时 $\mathbb{E}[X_t]=0$，$\text{Cov}(X_s,X_t)=\frac{\sigma^2}{2\theta}e^{-\theta|t-s|}$（仅依赖 $|t-s|$），有限维分布随平移不变。
9. 宽平稳：$\mathbb{E}[X_t]=\mu$（与 $t$ 无关）且 $\gamma(h)=\text{Cov}(X_{t+h},X_t)$ 只依赖滞后 $h$、与 $t$ 无关。
10. Wiener-Khinchin：$\gamma(h)=\int_{-\infty}^{\infty} e^{ih\lambda}f(\lambda)\,d\lambda$，$f(\lambda)\ge0$ 为谱密度（自协方差与谱互为 Fourier 变换对）。
11. $f(\lambda)=\frac{\sigma^2}{2\pi}\frac{1}{\lambda^2+\theta^2}$，在 $\lambda=0$ 处取峰值并随 $|\lambda|$ 增大而衰减，故功率集中在低频——"低频主导"的平滑噪声。
12. $A_n=\sum_{k=1}^n\mathbb{E}[X_k-X_{k-1}\mid\mathcal{F}_{k-1}]$（可料且非降，$A_0=0$），$M_n=X_n-A_n$；$M$ 为鞅、$A$ 为可料增过程，分解唯一。

---

## 7. 数学史（History of Mathematics）

> 随机过程研究的是**一族随时间（或空间）演化的随机变量**——从赌场里反复下注的输赢、通信线路上的噪声，到花粉颗粒在水中的漫步、股市价格的无序跳动。它的历史并非从抽象定义开始，而是从真实的随机现象一点一点"生长"出来的：马尔可夫面对词频统计与相依试验提出链，物理学家琢磨布朗运动的本质，赌徒与精算师思考"何时收手"，而伊藤则要为随机世界发明一套微积分。本节沿着"**现象→模型→严格化→工具化**"这条主线，还原随机过程从十九世纪末的散点观察到二十世纪成为分析核心的完整历程。

### 7.1 马尔可夫链与"无后效性"的诞生（1906 前后）

- **一个法律问题带来的数学冲动**：** 1906 年前后，**安德烈·马尔可夫（Andrey Markov, 1856–1922）**研究俄国《普希金传》中元音与辅音是否相互独立时，发现简单大数定律不足以刻画"相依"序列，于是提出"**下一状态只由当前状态决定**"的马尔可夫性（无后效性），给出转移概率与相关的极限定理——**马尔可夫链**由此诞生（§2.6）。
- **与伯努利试验的关系**：** 马尔可夫的思想把"独立同分布"的伯努利试验推广到"一阶相依"，链条既保留可分析的转移矩阵结构，又纳入了真实的依赖性，这是"从独立到相依"的第一次系统化（§2.2、§2.6）。
- **为何重要**：** 马尔可夫链给出了一种可计算、可迭代的动态相依模型，其转移矩阵的谱分析成为随机过程理论的基石，近代 **MCMC**（马尔可夫链蒙特卡洛）更是滥觞于此。
- **本章对应**：** §2.6 的转移概率、§2.7 的平稳分布与遍历性、§2.13 的常返/瞬态/周期，正是这条"链"的理论全貌。

### 7.2 布朗运动：从花粉的观察到数学的构造（1827–1923）

- **物理观察的起点**：** 植物学家**布朗（Robert Brown）**在 1827 年用显微镜看到花粉（实为尘埃）在水中不规则的跳动——**布朗运动**的名称即源于此，但其随机本质当时并不清楚。
- **爱因斯坦的定量解释（1905）**：** **爱因斯坦**用统计力学解释布朗运动：通过扩散方程给出**均方位移与时间成正比**（$\langle x^2\rangle\sim t$），为"分子存在"提供了可观测检验，**斯莫卢霍夫斯基**等也独立给出类似结果。
- **维纳的严格数学构造（1923）**：** **诺伯特·维纳（Norbert Wiener）**用正交级数（Schauder 函数）构造出连续的随机过程，证明布朗运动作为**连续但几乎处处不可微**的随机过程**存在**——**Wiener 过程**与 **Wiener 测度**由此成为概率与泛函分析的交汇点（§3.5、§2.10）。
- **从特例到一般：Lévy 过程**：** **列维（Paul Lévy）**把布朗运动推广为具有**独立平稳增量**的一类更广过程（Lévy 过程），并把布朗运动的样本轨道性质研究推入新阶段，这是随机过程理论的又一次深化。

### 7.3 鞅、停时与 Doob 的理论大厦（1940 年代）

- **从"公平赌局"到数学对象**：** "鞅"一词源于法语的公平赌博策略——在公平赌局中，给定过去，未来收益的期望等于当前收益。这类对象在 1940 年前由**列维（Lévy）**从分析上掌握，而**杜布（Joseph Doob）**则把它体系化为**鞅论**（§2.4）。
- **停时与可选停止**：** **停时（stopping time）**刻画"何时根据观测停止"这类随机时刻；**Doob 可选停止定理（optional stopping）**断言在适当条件下，鞅在停时处的期望等于初值——这是鞅论最漂亮也最实用的结果之一（§3.2）。
- **收敛与分解**：** **鞅收敛定理**（§3.3，$L^1$ 有界鞅几乎必然收敛）与 **Doob 分解定理**（§3.7：下鞅可唯一分解为鞅＋递增可料过程）构成了鞅"极限行为"与"结构"的双支柱，也引出二次变差与随机积分的代数根源。
- **对整章的意义**：** 鞅与停时不仅是 §2.4–2.5 的核心内容，其收敛定理与可选停止还在排队、金融与首达时问题（§4.3）中反复登场。

### 7.4 条件期望的严格化与 Kolmogorov 的公理化（1933）

- **为什么条件期望需要重定义**：** 粗糙的"条件平""要面对连续情形下的条件概率不唯一、条件期望不可直接相除等问题。**柯尔莫哥洛夫（Kolmogorov）**在 1933 年《概率论基础》中把概率论建立在测度论之上（§2.3），以"基于 $\sigma$–代数的条件期望"给出正则而唯一的定义。
- **公理化带来的统一**：** 测度论化让"概率为测度""随机变量为可测函数""期望为 Lebesgue 积分"成为常规，条件期望被内化为一类正交投影（在 $L^2$ 意义）。
- **与布朗运动的呼应**：** 在公理化框架下，**Kolmogorov 相容性定理（扩展定理）**（§3.1）保证了无穷维随机过程的**存在性**，使"靠有限维分布确定整个过程"有据可依——这是本章 §2.2 的基础，也是把随机过程从形式推向严格的关键一步。
- **与本章的衔接**：** §2.3 的条件期望、§2.2 的相容性定理与 §3.1 的存在性证明，正是柯尔莫哥洛夫精神的直接体现。

### 7.5 Poisson 过程与更新过程：稀疏事件的计数时间模型（19–20 世纪）

- **从稀有事件定律到泊松过程**：** 1837 年**泊松（Simeon Poisson）**提出"泊松分布"描述稀有事件；把"单位时间事件数为泊松分布"细化到"事件发生的时刻"，便得到**Poisson 过程**（§2.8）——从电话交换、放射性衰变、疾病爆发都能看到它。
- **更新过程的一般化**：** **更新过程**（§2.9）把 Poisson 过程的"指数到达间隔"放宽为一般分布，用于可靠性、保险与库存等场景，构成寿命/刷新理论的基石。
- **可加性与可算性**：** Poisson 过程最重要的性质是**独立增量**与**平稳性**，可加性（两过程之和仍是 Poisson 过程）与**合并/细分**使它在排队论（§4.2）与随机服务系统中极为方便。
- **从计数到连续时间**：** 这类计数过程与泊松分布、指数分布群一起，构成了"随机时刻序列"的一整套建模框架。

### 7.6 随机积分与 Itô 公式：为随机世界发明微积分（1940 年代）

- **为什么普通微积分会失效**：** 布朗运动的轨道几乎处处不可微，二次变差非零，导致 $dB$ 不能像 $dx$ 那样被经典微积分处理，必须另立随机积分的语义（§2.11）。
- **伊藤积分的定义**：** **伊藤清（Kiyosi Itô, 1915–2008）**在 1940 年代定义关于布朗运动的**Itô 积分**，并给出作为随机微积分基本公式的 **Itô 引理**（§3.6）：对 $X_t=f(B_t)$，
  $$
  dX_t=f'(B_t)\,dB_t+\frac12 f''(B_t)\,dt,
  $$
  额外出现的 $\frac12 dt$ 项正来自布朗运动非零的二次变差。
- **随机微分方程的确立**：** Itô 积分与引理使一类**随机微分方程（SDE）**可以由信号+噪声的驱动来定义与求解，为金融（Black–Scholes）、控制与物理打开了大门。
- **与奥恩斯坦–乌伦贝克过程的联系**：** **Ornstein–Uhlenbeck 过程**（§2.14）是"线性回拉＋布朗驱动"的典型 SDE 解，也是第一个平稳高斯马尔可夫过程，常作为布朗运动更自然的物理替代。

### 7.7 强马尔可夫性与首达时：随机时刻下的"无记忆"（20 世纪）

- **马尔可夫性的推广**：** 把"给定现在，未来独立于过去"从"固定的现在时刻"推广到"**停时**这一随机时刻"，得到的性质就是**强马尔可夫性**（§2.12），它保证在随机停止的瞬间推断未来仍保持无后效性。
- **首达时与返回理论**：** 强马尔可夫性是研究**首达时**（首次到达某集合的时刻，§4.3）与返回行为的利器，也是推导"遍历性与常返瞬态"之间关系的桥梁（§2.13）。
- **与鞅理论的结合**：** 停时 + 强马尔可夫性 + 鞅的可选停止三者联动，把"何时看"与"结果如何"统一在同一套形式语言中。
- **本章的覆盖**：** §2.5 停时、§2.12 强马尔可夫性、§3.2 可选停止，正构成这套"随机时刻"理论的小全。

### 7.8 平稳过程与功率谱：从 Wiener–Khinchin 到信号处理（1930 年代）

- **为何需要平稳性**：** 对振动、噪声、气候等信号，往往只有"统计特性随时间平移不变"（**弱平稳**）可用，这引出**自协方差函数**与**功率谱密度**（§2.15）的概念。
- **Wiener–Khinchin 定理**：** 该定理把平稳过程的自协方差与其**谱密度**（经 Fourier 变换）联系，使"时域平稳性"与"频域频谱"互相翻译，为通信与信号处理提供了数学基础。
- **与布朗运动、白噪声的关系**：** 白噪声（谱为常数）与布朗运动（谱 $\sim1/f^2$）等作为理想的平稳示例，常被用来检验滤波与估计的理论（§2.15）。
- **跨学科的印记**：** 从维纳（滤波）、辛钦（谱分析）、柯尔莫哥洛夫（预测）到现代 DSP，这一脉相承的谱分析理论贯穿随机过程的工程应用。

### 7.9 随机游走与极限定理的回响：从马尔可夫到中心极限

- **随机游走的原型地位**：** **随机游走**（喝酒的醉汉、股票的股价）是最简单的马尔可夫链之一，其鞅性质、首返时与常返性（二维游走常返、三维游走瞬态）成为概率论最生动的模型（§4.1）。
- **大数定律与中心极限的随机过程版本**：** 马尔可夫链的遍历定理、鞅的大数律与可交换中心极限定理，把独立情形的结论扩展到相依序列，为统计与动力系统的长期平均提供了理论支撑。
- **从独立到相依的综述**：** 随机游走既可用鞅的收敛理论处理，也可用 Markov 过渡概率刻画，是"马尔可夫链—鞅—遍历"三大阵地交汇的绝佳实例。
- **本章示例的落点**：** §4.1 的随机游走鞅分析、§4.3 的布朗运动首达时，正是这些极限与返回思想的典型应用。

### 7.10 多文明的接力与现代渗入：从高斯、泊松到当代

- **奠基人物的多点开花**：** 高斯（最小二乘/随机误差）、伯努利（大数定律）、泊松（分布）、马尔可夫（链）、爱因斯坦/斯莫卢霍夫斯基（布朗）、维纳（构造）、柯尔莫哥洛夫（公理）、伊藤（随机积分）、Doob（鞅论）……随机过程几乎是在欧洲多国与俄国之间接力完成。
- **物理背景的驱动**：** 布朗运动本身源自对分子运动的物理疑惑，统计物理（玻尔兹曼、吉布斯）对平衡分布与系综的思考也持续供随机过程的测度论工具以活水。
- **当代的广度**：** 金融数学（Black–Scholes）、工程滤波（Kalman）、生物统计、量子概率与机器学习（扩散模型）皆以随机过程为骨架，它是二十世纪以来最"接地气"又最深刻的数学分支之一（§4.2 排队、§2.15 谱分析皆指于此）。

| 时间 | 人物／文献 | 事件与贡献 |
|:---:|:---|:---|
| 1827 | 布朗 | 观察花粉微粒的无规则运动，命名布朗运动 |
| 1906–1913 | 马尔可夫 | 提出马尔可夫链与无后效性 |
| 1905 | 爱因斯坦 | 用扩散方程解释布朗运动，均方位移∼时间 |
| 1923 | 维纳 | 布朗运动的严格数学构造，Wiener 过程与测度 |
| 1933 | 柯尔莫哥洛夫《概率论基础》 | 概率论公理化，条件期望与扩展定理 |
| 1930s–1940s | 列维、Doob | 鞅、停时与鞅收敛/可选停止理论的系统化 |
| 1940s | 伊藤清 | 随机积分与 Itô 引理，随机微分方程基础 |
| 1930s | 维纳、辛钦 | 平稳过程谱理论（Wiener–Khinchin） |
| 1950s–1970s | Black–Scholes（1973） | 随机分析在期权定价的经典应用 |

> **小结**：随机过程的历史，是"**从散珠的随机现象，到一整座可计算的数学大厦**"的历程。马尔可夫回答"相依怎样建模"，维纳把布朗运动从观测变成严格的连续不可微过程，柯尔莫哥洛夫以测度论给出公理与存在性，Doob 以鞅论提供收敛与结构支柱，伊藤则用随机积分和 Itô 引理为随机世界发明了微积分。本章以"有限维分布→条件期望→鞅→马尔可夫链→Poisson/更新过程→布朗运动→随机积分"的次序组织，正是沿着这条"现象→模型→严格化→工具化"的主线展开；理解这段谱系，就能理解为何随机过程的各个定义与定理会以今天的面貌出现——它们都是在回答现实世界一个接一个提出的问题。
...
**Markov 链**：Markov 链是"无记忆"的随机过程——给定现在，将来与过去独立。Chapman-Kolmogorov 方程刻画了转移概率的演化规律。对于不可约正常返链，遍历定理保证了时间平均收敛到平稳分布，这是 Markov 链 Monte Carlo (MCMC) 方法的理论基础。进一步，**强 Markov 性**（2.12）把"无记忆"推广到随机停时时刻，而**常返、瞬态与周期**（2.13）则回答了"链是否回得来、沿哪种子序列收敛"这一更基本的问题，为平稳存在性提供了前提。

**扩展结构：Doob 分解、OU 过程与平稳谱**：**Doob 分解定理**（3.7）把任何下鞅唯一拆成"鞅 + 可料增过程"，是二次变差、$\langle M\rangle_n$ 与测度变换的代数根源。**Ornstein-Uhlenbeck 过程**（2.14）以"线性回拉 + Brown 驱动"给出第一个平稳高斯 Markov 过程，是利率（Vasicek）与生理信号的标准模板。**平稳过程与功率谱**（2.15）通过 Wiener-Khinchin 定理把自协方差与谱密度相连，让随机过程与信号处理/谱分析接轨。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对随机过程核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 12.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到随机过程语境的追问上。

| 步骤 | 行动 | 随机过程中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 过程是离散还是连续时间？状态空间离散还是连续？目标是算期望、求平稳分布、两步转移概率，还是随机积分？ |
| ② 计划 | 选择可行工具、预估方向 | 用条件期望 / 塔形律，特征函数，Chapman-Kolmogorov，Doob 停止定理，还是 Itô 引理？先想清楚哪条路最省力。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 记清 $\mathbb{E},\mathbb{P}$ 作用的对象，写对转移矩阵 / 生成元，二次变差项别漏，停时是否合法。 |
| ④ 回看 | 核对结果、检验合理性 | 结果是否满足鞅性、塔形律、停时条件？方差非负、量纲合理？能否用蒙特卡洛模拟再验一遍？ |

> 💡 **策略演示**：计算对称简单随机游走 $S_n$ 的 $\mathbb{E}[S_n^2]$。
> ① 理解：对一步步 $\xi_k$ 独立的平方和求期望。② 计划：用方差分解 $\mathbb{E}[S_n^2]=\mathrm{Var}(S_n)+(\mathbb{E}S_n)^2$。③ 执行：$\mathrm{Var}(\xi_k)=1$，故 $\mathrm{Var}(S_n)=n$，且 $\mathbb{E}S_n=0$，得 $\mathbb{E}[S_n^2]=n+0=n$。④ 回看：$\mathbb{E}[S_n^2]=n\ge0$ 且随 $n$ 增长，与「平方距离期望等于步数」的直觉一致，答案合理。
...
1. 鞅满足 $\mathbb{E}[M_{n+1}\mid\mathcal{F}_n]=M_n$，从而 $\mathbb{E}[M_n]$ 与 $n$ 无关。（　）
2. 不可约正常返 Markov 链存在唯一平稳分布 $\pi$，且 $\pi_j=1/\mathbb{E}_j[\tau_j]$。（　）
3. 布朗运动具有独立增量，且增量 $B_{t+h}-B_t\sim\mathcal{N}(0,h)$。（　）
4. Itô 引理里 $d(B_t^2)=2B_t\,dB_t+dt$，那多出的 $dt$ 项源自 Brown 运动的非零二次变差。（　）
5. Poisson 过程是一个连续时间、可数状态空间的 Markov 链。（　）
6. 只要状态空间有限，时间离散 Markov 链的平稳分布就必存在且唯一。（　）

**选择**：

7. 一维对称简单随机游走在 $\mathbb{Z}$ 上是（　）。
   - A. 常返　B. 瞬态　C. 视初值而定　D. 只在偶数次时常返
8. Brown 运动 $B_t$ 的二次变差 $\langle B\rangle_t$ 等于（　）。
   - A. $0$　B. $t$　C. $t^2$　D. $1$
9. 对鞅 $M_n$ 与合法停时 $\tau$（满足可积性条件），必有 $\mathbb{E}[M_\tau]=$（　）。
   - A. $\mathbb{E}[M_0]$　B. $M_0$　C. $0$　D. 无穷
...
> **问题**：设 $X\sim U(0,1)$，$Y\sim\mathrm{Bin}(n,X)$（成功概率随 $X$ 随机），求 $\mathbb{E}[Y]$ 与 $\mathrm{Var}(Y)$。

- ① 理解：$Y$ 是成功概率为随机变量 $X$ 的二项变量，需做条件期望。
- ② 计划：塔形律 $\mathbb{E}[Y]=\mathbb{E}[\mathbb{E}[Y\mid X]]$，全方差律 $\mathrm{Var}(Y)=\mathbb{E}[\mathrm{Var}(Y\mid X)]+\mathrm{Var}(\mathbb{E}[Y\mid X])$。
- ③ 执行：已知给定 $X$ 时 $\mathbb{E}[Y\mid X]=nX$、$\mathrm{Var}(Y\mid X)=nX(1-X)$，于是

$$
\mathbb{E}[Y]=\mathbb{E}[nX]=\frac{n}{2},\qquad
\mathrm{Var}(Y)=\mathbb{E}[nX(1-X)]+\mathrm{Var}(nX)=\frac{n}{6}+\frac{n^2}{12}.
$$

- ④ 回看：$\mathrm{Var}(Y)$ 由「组内方差」与「组间方差」两段合成，缺一不可。**易错点**：直接用 $\mathrm{Var}(Y)=n\,p(1-p)$ 而漏掉随机 $p$ 的组间方差。**检验**：取 $n=1$，$Y$ 是随机系数 Bernoulli，得 $\mathbb{E}[Y]=\tfrac12,\ \mathrm{Var}(Y)=\tfrac16+\tfrac1{12}=\tfrac14$，与直接算相符。

**示例 S2：有限状态 Markov 链的平稳分布**

> **问题**：状态 $\{1,2\}$，转移矩阵 $P=\begin{pmatrix}0.6&0.4\\0.2&0.8\end{pmatrix}$，求平稳分布 $\pi$。

- ① 理解：平稳分布满足 $\pi P=\pi$ 且 $\pi_1+\pi_2=1$。
- ② 计划：列方程组分步消元，再用归一化条件定出唯一解。
- ③ 执行：展开 $\pi_1=0.6\pi_1+0.2\pi_2$、$\pi_2=0.4\pi_1+0.8\pi_2$：

$$
0.4\pi_1=0.2\pi_2\ \Rightarrow\ \pi_2=2\pi_1,\qquad \pi_1+\pi_2=1\ \Rightarrow\ \pi_1=\tfrac13,\ \pi_2=\tfrac23 .
$$

- ④ 回看：$\pi_1+\pi_2=1$ 且 $\pi P=\pi$ 自洽。**易错点**：方程写错方向（用 $P\pi=\pi$ 而非 $\pi P=\pi$，转置会改变结果）。**检验**：让链在某态长期运行统计频率，应趋近 $\tfrac13,\tfrac23$。

**示例 S3：Itô 引理求 Brown 平方过程的微分**

> **问题**：用 Itô 引理求 $X_t=B_t^2$ 的随机微分，并求 $\mathbb{E}[B_t^2]$ 与 $\mathrm{Var}(B_t^2)$。

- ① 理解：$B_t^2$ 是 Brown 的二次泛函，须用 Itô 而非普通链式法则。
- ② 计划：对 $f(x)=x^2$ 用 Itô 引理，再取期望利用 Itô 积分的零均值鞅性质。
- ③ 执行：由 Itô 引理
...
取期望得 $d\mathbb{E}[B_t^2]=dt$，故 $\mathbb{E}[B_t^2]=t$（Itô 积分 $\mathbb{E}\int 2B_t\,dB_t=0$）。再由高斯四阶矩 $\mathbb{E}[B_t^4]=3t^2$ 得 $\mathrm{Var}(B_t^2)=3t^2-t^2=2t^2$。
- ④ 回看：关键在 $dt$ 项由非零二次变差 $\langle B\rangle_t=t$ 引起，若无它会错算成 $\mathbb{E}[B_t^2]\equiv0$。**易错点**：把 $dB_t^2=2B_t\,dB_t$ 当成普通微分漏掉 $dt$。**检验**：数值模拟轨道平方取平均，应随 $t$ 线性增长。

**示例 S4：用停止定理求随机游走首达时期望**

> **问题**：对称简单随机游走 $S_n$，$\tau=\inf\{n:S_n=a\}$（$a>0$），用鞅法求 $\mathbb{E}[\tau]$。

- ① 理解：$\tau$ 是首达正水平 $a$ 的首达时，属随机时间。
- ② 计划：构造鞅 $M_n=S_n^2-n$，对其在 $\tau\wedge k$ 停止并用可选停止定理，再令 $k\to\infty$。
- ③ 执行：$M_n$ 为鞅，$S_{\tau\wedge k}$ 有界可停，故

$$
\mathbb{E}[S^2_{\tau}- \tau]=\mathbb{E}[S_0^2-0]=0\ \Rightarrow\ \mathbb{E}[\tau]=\mathbb{E}[S_\tau^2]=a^2 .
$$

- ④ 回看：$\mathbb{E}[\tau]=a^2$ 与「离开原点越远越耗时」的直觉一致。**易错点**：忽略停时的合法性验证，直接对无界 $\tau$ 套停止定理导致无穷期望矛盾。**检验**：作数值模拟对多条轨道计时取平均，逼近 $a^2$。

### 12.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：模拟 $d=1,2,3$ 维对称随机游走，统计回到原点（或原点邻域）的样本比例，直观验证 Pólya 定理「$d\le2$ 常返、$d\ge3$ 瞬态」。
- **聚焦 P2（跨章节联系）**：利用 Itô 引理推导 Black-Scholes 方程中 $d(e^{-\lambda t}B_t)$ 等变化公式，并与第 46 级「流形与张量分析」的微分结构就「二次变差」做交叉对照。
- **聚焦 P3（建模）**：用 Poisson 过程建立 $M/M/1$ 排队系统，推导并验证其稳态队长与等待时间的平稳分布，仿真对比理论值。
...
- **自测 1**：用一句话说明「鞅」与「公平赌博」的联系，并指出它的期望守恒性质。
- **自测 2**：比较 Markov 链的「无记忆性」与 Poisson 过程的「无记忆性」在表述与严格形式上的异同。

**$\bullet$ 技能题**

- **自测 3**：链的转移矩阵 $P=\begin{pmatrix}0.9&0.1\\0.5&0.5\end{pmatrix}$，求两步转移概率矩阵 $P^{(2)}=P^2$。
- **自测 4**：对 $P=\begin{pmatrix}0.9&0.1\\0.5&0.5\end{pmatrix}$ 求平稳分布并验证。
- **自测 5**：用 Itô 引理求 $d(e^{B_t})$，进而求 $\mathbb{E}[e^{B_t}]$。

**$\blacklozenge$ 综合题**

- **自测 6**：结合 Pólya 定理，说明为何判别 $\sum_n \mathbb{P}_0(X_{2n}=0)\sim C_d n^{-d/2}$ 的敛散即可定常返/瞬态，并给出结论。
- **自测 7**：写一段含「表格 + 一句话结论」的分析：比较 Poisson 过程与一般更新过程在到达间隔分布、无记忆性、应用场景上的差异。

### 12.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（鞅的条件期望等于当前值，取无条期望后为常数）。2. **正确**（正常返且不可约时平稳分布唯一且 $\pi_j=1/\mathbb{E}_j[\tau_j]$）。
> 3. **正确**（Brown 运动的独立平稳高斯增量，方差 $h$）。4. **正确**（$d(B_t^2)$ 的 $dt$ 项来自 $\langle B\rangle_t=t$）。
> 5. **正确**（Poisson 是计数过程的连续时间可数状态 Markov 链）。6. **错误**（平稳分布唯一需「不可约」；有限但可约时可能不唯一）。
>
> **选择**：7. A　8. B　9. A。

> **自测**：
> - 自测 1：鞅给当前信息后未来期望等于当前值，正如公平赌局中的下注期望不增不减，故 $\mathbb{E}[M_n]$ 恒等于 $\mathbb{E}[M_0]$。
> - 自测 2：Markov 无记忆指「给定现在，将来独立于过去」，用条件期望 $\mathbb{P}(X_{n+1}\in A\mid X_n,\dots)= \mathbb{P}(X_{n+1}\in A\mid X_n)$ 形式化；Poisson 的无记忆指数到达间隔 $\mathbb{P}(T>s+t\mid T>s)=\mathbb{P}(T>t)$，是连续的指数型无记忆。
> - 自测 3：$P^2=\begin{pmatrix}0.86&0.14\\0.70&0.30\end{pmatrix}$，其中 $P^{(2)}_{01}=0.14$ 等即两步转移概率。
> - 自测 4：$0.1\pi_1=0.5\pi_2\Rightarrow\pi_1=5\pi_2$，归一化得 $\pi_1=\tfrac56,\ \pi_2=\tfrac16$，代入 $\pi P=\pi$ 成立。
> - 自测 5：$d(e^{B_t})=e^{B_t}\,dB_t+\tfrac12 e^{B_t}\,dt$；取期望得 $d\mathbb{E}[e^{B_t}]=\tfrac12\mathbb{E}[e^{B_t}]\,dt$，解得 $\mathbb{E}[e^{B_t}]=e^{t/2}$。
> - 自测 6：返回概率 $\mathbb{P}_0(X_{2n}=0)\sim C_d n^{-d/2}$，其级数当且仅当 $d\le2$ 发散；由「常返 $\iff\sum p_{00}^{(n)}=\infty$」得 $d=1,2$ 常返、$d\ge3$ 瞬态。
> - 自测 7：表格列 Poisson/更新的到达间隔（指数 vs 一般）、无记忆性（有 vs 无）、典型用途（独立齐次 vs 可变间隔）；结论——Poisson 是更新过程的指数特例，「无记忆」仅是 Poisson 独有的性质。