...
> **一句话大白话**：空间拆成两块，各自的基本群已知，公共重叠带的基本群负责"粘合约定"，Van Kampen 定理就把两块的基础群按这条约定"焊接"成整个空间的基本群——像拼积木时在交接处对齐。
>
> **小例子**：两个圆沿一点粘成"8字形"，每片基本群都是 $\mathbb{Z}$，交集缩成一点无额外约束，故整体 $\pi_1=\mathbb{Z}*\mathbb{Z}$（自由群）。
