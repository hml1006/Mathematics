...
> **一句话大白话**：实向量丛"怎么拧在一起"的拓扑信息，能用一组叫 Pontryagin 类的整数记下来，且连续拉扯不改变——像用标签记录布料纤维整体卷了几圈。
>
> **小例子**：$n$ 维实向量丛第 $k$ 个 Pontryagin 类 $p_k\in H^{4k}(M;\mathbb{Z})$；对光滑流形 $M$，$p_1$ 落在 $H^4(M)$ 中，编码流形的实际扭转方式。

## 一、定理介绍

> **前置依赖**：实向量丛与复化、Chern 类、上同调环与杯积、分类空间 $BO(n)$ 与 $BU(n)$、配边理论与定向基本类
