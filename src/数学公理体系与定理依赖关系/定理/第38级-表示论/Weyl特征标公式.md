...
> **一句话大白话**：李群每个不可约表示该长什么样？不必逐个直算——特征标（一步到位统计痕迹的指纹）能由"分子带符号数权、分母魏尔分母"这一统一分式直接算出来，如同给每个表示发一张定型脸。
>
> **小例子**：对 $SU(2)$，最高权 $m$ 的不可约表示特征标为 $\chi_m=\frac{\sin((m+1)\theta)}{\sin\theta}$（几何级数式公式），由 Weyl 特征标公式的木刻刀一毛不差地给出。
...
- [Weyl 特征标公式（第72级-李群与李代数）](../第72级-李群与李代数/Weyl特征标公式.md)：与本条目为同一定理，另收录于第72级-李群与李代数，可交叉参考。
