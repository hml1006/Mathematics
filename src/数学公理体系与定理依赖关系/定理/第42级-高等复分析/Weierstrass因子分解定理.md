...
> **一句话大白话**：整函数（处处可导的复函数）就像多项式一样能按"零点"拆成乘积——只要告诉它零点的位置与重数，就能造出一个整函数，把这些无穷多个因子（配上收敛因子 $E_p$）乘起来拼成，不分先后、清清楚楚。
>
> **小例子**：$\sin \pi z$ 的全体零点在 $n\in\mathbb{Z}$，其 Weierstrass 积为 $\sin\pi z=\pi z\prod_{n\ne0}\left(1-\frac{z}{n}\right)e^{z/n}$ 这种无穷乘积展开。
