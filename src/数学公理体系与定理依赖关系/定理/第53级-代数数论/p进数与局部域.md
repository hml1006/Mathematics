...
> **一句话大白话**：换一种"距离"来看有理数：越能被某素数 $p$ 的幂整除就越"近"。这样补齐出来的数叫 $p$-进数，是和实数平行的另一个数字世界，拥有自己独特的分析。
>
> **小例子**：在 $p$-进距离下 $p$ 的高次幂 $p^n\to0$（$n\to\infty$）；幂级数 $1+p+p^2+\cdots=1/(1-p)$ 在 $\mathbb{Q}_p$ 中收敛，而它在实数里却发散。

## 一、定理介绍

> **前置依赖**：非 Archimedean 绝对值与强三角不等式、度量完备化（Cauchy 序列）、Newton 迭代法（Hensel 引理的证明）、离散赋值环与剩余域、Ostrowski 定理的素理想论证
