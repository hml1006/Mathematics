...
导出代数几何（derived algebraic geometry, DAG）源于经典代数几何长期未能妥善处理的"退化"情形。两条曲线"擦边相切"的相交，在经典环论中只留下一个带有重数的"厚点"，但这未必完整反映真实几何（例如横截性被破坏时的交点厚度、形变空间的非光滑障碍）。1950年代，塞尔（Jean-Pierre Serre）在相交理论中首先指出：要正确计算重数，应当考虑"导出"（higher Tor/homology）的资料而非仅取 $\pi_0$。1970-80年代的三角范畴（Verdier）、导出范畴与谱序列（Grothendieck）为"导出"提供了代数工具，而 Quillen 的模型范畴论（1967）则为"导出的空间"预备了严格框架——直觉上，"取 $\pi_0$ 会丢掉的高阶信息"正是导出代数几何要找回的部分。

### 七.2 格罗滕迪克、奎伦与"导出的形变"

20世纪60-70年代，格罗滕迪克建立了概形论，也发展出同调代数与形变理论中"余切复形"（cotangent complex）等导出工具，让"形变的空间是否光滑、有何障碍"得以用导出后期资料刻画。与此同时，奎伦（Daniel Quillen，1967）提出模型范畴的公理框架，为"在模数意义下做代数"（同伦交换代数）提供了基础。这些工作即使并非以"导出代数几何"命名，其精神正是DAG的核心：把代数几何中的"环"推广为"可以携带多层同调信息的链复形/环谱"。

### 七.3 $E_\infty$-环与导出概形

导出代数几何的关键一步是把结构层从"交换环"替换为"携带高阶同伦信息的$E_\infty$-环"（或单纯交换环的层）。$E_\infty$-环来自同伦论中的"交换环谱"概念，其$\pi_0$是普通交换环，而$\pi_k$（$k\ge1$）携带同伦信息。这一替换使我们可以定义"导出概形"：一个空间$X$配上$E_\infty$-环的层，其同调逐层提供$\pi_0,\pi_1,\ldots$信息，从而可以刻画导出交点$X\times_Y^{\mathbf{R}}Z$、导出模空间与导出Hilbert概形、以及导出形变论中的障碍。本教程的导出概形、$E_\infty$-环、Cotangent/Tangent复形、导出交点、导出模空间等概念即由此而来。

### 七.4 Toën-Vezzosi与Lurie的现代导出代数几何

2000年代，两条现代体系几乎同时建立：托恩与韦佐西（Bertrand Toën & Gabriele Vezzosi，基于HAG模型结构，2000s）建立了"同伦/导出代数几何"的完整基础设施；卢里（Jacob Lurie，2009-2010s，《高等代数》、《导出代数几何》）在∞-范畴的框架下系统发展了导出代数几何，把结构层、$E_\infty$-环、谱代数与导出栈/导出模空间纳入统一语言。这两个体系互为印证，使DAG成为当代代数几何（尤其在模空间、相交理论、表示论与数学物理交叉处）的标准工具。本教程的Toën-Vezzosi导出几何基本定理与Lurie框架章节即由此而来。

### 七.5 导出代数几何的意义与影响

导出代数几何弥合了"经典概形层只看第0层、遗失高阶同调信息"的缺憾，为交点厚度、形变障碍、模空间的结构与光滑性提供了精确的"导出"答案。它在导出相交理论、导出Hilbert概形、导出模空间、以及几何朗兰兹、镜对称与代数K-理论等现代研究中发挥关键作用。该领域的成长，是"把同调代数的全部力量带回几何"的历程：从Serre的重数、Grothendieck的余切复形、Quillen的模型范畴，到今天以$E_\infty$-环与∞-范畴为材料建成的现代大系统。
