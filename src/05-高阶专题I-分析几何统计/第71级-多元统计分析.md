...
多元统计分析 (multivariate statistics) 研究多个随机变量同时被观测时数据结构的推断与约简。它的历史主线有二：一是把单变量统计（正态、卡方、$t$、$F$）系统推广到向量情形而得到的多元分布理论；二是"降维/探索"流派——从高尔顿、皮尔逊到费希尔，致力于从高维数据中提炼出少数的方向与结构。这两条线索在二十世纪交汇成一套既有严密分布理论、又有广泛应用的现代方法体系。

### 1. 多元正态分布与 Wishart 分布

多元分析的理论基础是多变量正态分布，其密度中出现的协方差矩阵把"变量之间如何联动"编码为对称正定阵。十九世纪末高尔顿 (Galton) 在研究遗传学时已触及双变量正态与回归，皮尔逊 (Karl Pearson) 与费希尔 (R. A. Fisher) 在二十世纪初完成了相关、回归与 $t$/$F$ 单变量分析的奠基。Wishart 分布在 1928 年由约翰·威沙特 (John Wishart) 提出：它刻画了样本协方差矩阵（多元"散布"）的分布，是卡方分布在矩阵情形的推广，由此可构造多元正态的似然比检验。协方差矩阵的逆（精度矩阵）与多元正态的极大似然估计、偏相关系数等理论在 1930–40 年代被系统化。

### 2. Hotelling 检验、费希尔判别与 MANOVA

把单变量显著性检验推广到向量是费希尔—Hotelling 时代的重要工作。霍特林 (Harold Hotelling) 在 1930–31 年首先提出主成分分析（PCA，参见下节），并构造了 Hotelling $T^2$ 统计量（把 $t$ 检验推广到均值向量）。Wilks 于 1932 年前后提出 Wilks $\Lambda$（多元 ANOVA/MANOVA 的似然比检验统计量），把 $F$ 检验推广到整组均值向量。费希尔 (Fisher) 在 1936 年提出线性判别分析（Fisher 判别），把"组间/组内方差比"最大化以找到最利于分类的投影，这把判别问题转化为一个优良的最优化问题。典型相关分析（CCA，Hotelling 1936）则研究两组变量之间的最大相关方向，是相关分析的多元推广。

### 3. 主成分、因子分析与多维标度：降维的三条路径

探索型降维方法在二十世纪的演化由三条主线构成。主成分分析 (PCA) 由皮尔逊 (Pearson) 于 1901 年在《关于“以最小二乘法求主平面系”》中最先提出几何意义上的"主直线"思想，并由霍特林 (Hotelling) 于 1933 年以统计/代数形式独立发展，其现代形式为对协方差矩阵作特征分解，找到使投影方差最大的正交方向。因子分析则由斯皮尔曼 (Spearman) 于 1904 年在智力测量的背景下提出，用少数"共同因子"解释显式变量间的相关结构。多维标度 (MDS) 源于心理测验（"由对象间的相似/距离反推其空间位置"），由 Torgerson（1952）、Kruskal（1964）等发展为"已知对象间距离、重构低维嵌入"的方法。这几条线索共同构成数据降维的经典谱系。

### 4. 聚类、ICA、SEM 与现代数据分析

无监督分群（聚类分析）与监督分类相对：它无须标签，仅依相似度把对象"自动抱团"，可追溯到数值分类学（Sneath 与 Sokal 的数值分类）与统计聚类的经典文献，配合 Ward 层次法、k-均值等算法。独立成分分析 (ICA) 源于盲信号分离的"鸡尾酒会问题"，由 Jutten 与 Hérault（1988）、Comon（1994）等确立为目标：从混合观测中恢复统计上独立的源信号。协方差结构模型 (SEM，也称"路径分析/结构方程模型") 由 Wright（1921 路径分析）与 Jöreskog、Keesling、Wiley、Bollen 等发展为把潜变量、测量模型与因果结构统一估计的框架。多元线性回归与样本主成分的渐近分布理论（Anderson 等的经典工作）为推断提供了密度/极限依据。一般认为，多元统计分析已从"检验与降维的古典方法"演化为连接数据科学、机器学习的现代体系，其数学内核（分布理论、矩阵分析、最优投影与对偶）至今仍是数据方法的共同语言。
