...
### 六.1 Minkowski 与几何数论的创立

几何数论（geometry of numbers）由德国数学家 Hermann Minkowski（赫尔曼·闵可夫斯基，1864—1909）在 1890 年代至 1900 年代创立。其出发点是把数论中的问题翻译成高维空间中的格（lattice）与凸体（convex body）问题。Minkowski 在 1896 年的《Geometrie der Zahlen》及后续著作中证明了一系列基本定理，其中最核心的是 1889—1890 年证明的"凸体定理"：一个中心对称且体积足够大的凸体必含有一个非零格点。这一仅凭体积与对称性就能"保证存在"被非零格点的原理，是整套学科的基石，并可等价表述为 Minkowski 线性型定理。它直接给出丢番图逼近（如 $\|q\alpha-p\|<1/q^n$ 型结果）、二次型最小值的统一证明，同时为 Dedekind 单位定理与类数有限性提供了漂亮的几何证明。

### 六.2 临界行列式、Hermite 常数与约化

伴随凸体定理出现的是一系列把"格与凸体"量化的不变量：临界行列式（格子中无（相对）内部格点的最大容许体积）、连续最小值，以及描述正定二次型最值比的 Hermite 常数（以 Charles Hermite，查尔斯·埃尔米特命名，他本人曾研究二次型的约化问题）。Minkowski 约化理论（把每个正定二次型归于根具优良性质的标准型）以及 Blichfeldt 定理、Minkowski–Hlawka 定理（下界）进一步刻画了这些不变量，建立了用几何工具研究二次型理论的完整框架。这套理论在晶体学、格基约化（如 LLL 算法）与密码学中至今仍有直接应用。

### 六.3 格点计数与高斯圆问题

几何数论的计数分支源于一个著名的误差估计问题——高斯圆问题（Gauss circle problem）：数一数半径为 $R$ 的圆内（整数格）格点个数 $N(R)$，其主项为 $\pi R^2$，而相差的"误差项" $E(R)=N(R)-\pi R^2$ 到底有多大。高斯本人在 19 世纪初就给出 $E(R)=O(R)$，此后 G.H. Hardy、Sierpiński 乃至 Lander、沃尔什等不断改进指数，圆问题的精确下界至今仍是开放问题。这一类"格点计数"与丢番图逼近、解析数论紧密关联，是几何数论与解析数论之间的重要交汇点。

### 六.4 后续发展与影响

Minkowski 的思想此后被 Minkowski 的学生与后继者（如 Hlawka、C. L. Siegel、Cassels）系统化。Siegel 将其推广用于格上求和公式与二次型质量公式，几何数论也开始与代数数论、表示论及解析数论深度结合，并在现代算法（格基约化、格点计数）与密码学（格密码）中重新焕发生机。一般认为，几何数论从一个"用体积保证存在"的思想，成长为连接数论、分析、几何与算法的成熟分支。
