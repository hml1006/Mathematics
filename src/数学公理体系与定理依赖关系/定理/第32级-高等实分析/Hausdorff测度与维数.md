...
> **一句话大白话**：用"够小的毛毯能盖住集合多便宜"来给集合定尺码——不断缩小毛毯尺寸看覆盖总花费怎么变，拐点处的"尺码指数"就是集合的分形维数。
>
> **小例子**：康托尔三分集用单条长度 $3^{-n}$ 的小段、约 $2^n$ 条就能覆盖，覆盖总花费按 $2^n(3^{-n})^s$ 变化，取临界 $s=\log2/\log3$ 对应其Hausdorff维数——一个介于0与1之间的分形维数。

## 一、定理介绍

> **前置依赖**：外测度与 Carathéodory 条件、度量空间与直径、Lebesgue 测度、Vitali 覆盖引理
