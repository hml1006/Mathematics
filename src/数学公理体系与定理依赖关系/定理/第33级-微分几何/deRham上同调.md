...
> **一句话大白话**：用"微积分算子（外微分）能认得几个独立的闭合函数（闭微分形式）"来数一数流形上有几个洞——几何的形状信息被翻译成了线性代数里的维数。
>
> **小例子**：在球面 $S^2$ 上所有"闭"且"恰有面积"的2-形式彼此相差一个精确形式，故第2个de Rham上同调维数 $b_2=1$；紧流形公开故事：$b_k$ 正比于"第 $k$ 维孔洞"的数量。

## 一、定理介绍

> **前置依赖**：微分形式与外微分（$d^2 = 0$）、光滑同伦与可缩空间、单位分解、复形与上同调群的代数结构
