...
> **一句话大白话**：任何无偏估计的精度都有"天花板"——它的方差不可能小于 Fisher 信息量的倒数 $\frac{1}{nI(\theta)}$，样本再多也只能逼近这个极限，不可能突破。
>
> **小例子**：设 $X_1,\dots,X_n\sim N(\theta,1)$，$\bar X$ 是无偏估计且 $\text{Var}(\bar X)=\frac{1}{n}=I(\theta)$，恰达到 CRLB，故 $\bar X$ 是有效估计。
