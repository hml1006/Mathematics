...
> **🧠 思考历程：为什么光滑流形要配一把"逐点变动的尺子"？**
>
> 拓扑流形只告诉你哪些点"邻近"，却无法度量彼此"有多远"。1854 年，Bernhard Riemann 在哥廷根就职演讲中把这作为几何学的真正起点：他拒绝固守全局坐标，主张在每一点配备一把光滑依赖位置的"本地内积"——这正是黎曼度量 $g=g_{ij}\,dx^i\,dx^j$。它要解决的根本问题是：**给欧氏以外的弯曲空间（如球面）一个合法的长度、角度、体积的定义**。本大节将先给出度量的精确语言，再说明子流形如何天然"继承"母空间的度量，并回答一个基础性问题：度量是不是总存在（见新增的 2.3）。这样，"换一把尺子就改变整个几何"的广义相对论语言也就有了严格的落脚点。
...
> **💡 直观理解（类比）**：欧氏度量像"整张桌面上每处都用一模一样的刚性直尺"——不管你把尺子挪到哪个点、朝哪个方向摆，刻度完全统一，测得的长短角度就是最普通的平面直觉。它是所有其它黎曼度量拿来对比的"基准尺"，因为 $\delta_{ij}$ 意味着坐标之间互不纠缠、长度就是坐标差的平方和。
...
> **💡 直观理解（类比）**：乘积度量像"给仓库做一张无缝的矩形货单"——两个独立货架（因子 $M_1$、$M_2$）的面积规则互不干扰地叠加，一横一纵两把尺子在同一块拼接面上丈量。正是由于两把尺子互不相干，乘积流形上"横着走多长"与"竖着走多长"互不纠缠，长度按勾股式累加，这也是"两独立系统放一起各自测长短"的几何语言。
...
### 2.3 黎曼度量的存在 (Existence of Riemannian Metrics)【新增】

**来龙去脉**：既然黎曼度量在流形上如此重要，度量是否总存在就成了第一道基础门槛。答案是肯定的：哪怕流形千奇百怪，总能用局部铺平、整体缝合的单位分解办法拼出一把全局尺。这一问题的解决扫清了把几何施加到任意光滑流形上的障碍。

**命题 2.7**（黎曼度量的存在）每个光滑流形 $M$（第二可数、Hausdorff）上都存在黎曼度量。

> **💡 直观理解（类比）**：任何一张能局部铺平的地形图，都能用无缝拼接的本地直尺拼出一把全局尺——每个坐标小馆内用标准尺，再用光滑权重把它们加权缝合，得到一把处处正定的全球尺。

**证明（用单位分解）**：取 $M$ 的一个局部有限开覆盖 $\{U_\alpha\}$，每个 $U_\alpha$ 有坐标卡 $\varphi_\alpha: U_\alpha \to \mathbb R^n$。在 $U_\alpha$ 上取拉回度量 $g_\alpha=\varphi_\alpha^* g_{\text{eucl}}$。设 $\{\rho_\alpha\}$ 是 $M$ 上的单位分解（$\rho_\alpha\ge 0$、$\operatorname{supp}\rho_\alpha\subset U_\alpha$、$\sum_\alpha \rho_\alpha=1$）。定义
$$
g = \sum_\alpha \rho_\alpha\, g_\alpha.
$$
每一点处至少一个 $\rho_\alpha>0$，故 $g$ 正定；它是光滑对称 $(0,2)$-张量，因而是黎曼度量。$\square$

> （子流形的诱导度量已在定义 2.3 给出，此处不重复——它说明子流形自动继承母空间度量。）
...
> **🧠 思考历程：有了尺子之后，怎么搬运方向才不会搬歪？**
>
> 度量给了长度，但要在弯曲空间里比较、求导不同点的向量，还需要平行移动这把刷子。早年的 Christoffel（1869）为 Riemann 度量引入了一组"修正系数"（Christoffel 符号），1917 年前后 Tullio Levi-Civita 在这基础上提出平行移动与协变导数，从而回答：**如何在曲面上把切线方向从一点平移到另一点，既不打转（无挠）、又不伸缩（度量相容）**。Levi-Civita 证明了这样的规则可以唯一选定，把搬运方向从含糊直觉变成严格可计算的操作。本大节由此建立仿射联络、Levi-Civita 联络的唯一性与协变导数。
...
> **💡 直观理解（类比）**：欧氏空间里所有坐标方向都"老老实实不动"——度量阵是常数 $\delta_{ij}$，所以坐标地基完全不走样，搬运方向时无需任何补偿。这正说明欧氏空间是"零曲率"的样板：灯塔式的直尺指点，指标在欧氏几何里压根不需要"纠偏"，代价是它无法描述弯曲的曲面。
...
> **🧠 思考历程：平行移动绕一圈方向偏了——这个偏差要如何精确量化？**
>
> 一旦有了平行移动，就能用绕小平行四边形搬运向量一圈再比对来诊断空间的弯曲。Riemann（1854）与 Christoffel 定义了曲率张量，Bianchi 等人梳理了它的对称性与恒等式，回答：**怎样用一个张量把空间在每个方向到底弯多少完整地、且不随坐标选取而变地记录下来**。Riemann 张量太庞杂，于是又缩并出截面曲率、Ricci 曲率与标量曲率这三级摘要，分别服务于几何本身、广义相对论与几何分析。本大节建立整套曲率语言。
...
## 5. 测地线 (Geodesics)
> **🧠 思考历程：弯曲面上最直的线与最短的线到底是什么？**
>
> 把以恒定速率、方向完全交给空间自己搬运的曲线解出来，就得到测地线——它是直线在弯曲空间的推广，是欧氏几何里两点之间直线最短在一般流形上的化身。从测地线方程（二阶 ODE），到指数映射 $\exp_p$（把切向量发射成曲线上的一点），再到 Gauss 引理（径向测地线与切球面正交），本大节回答的问题是：**如何从一点和初速度出发，系统地生成所有最直的路径，并判明它们何时真正最短**。测地线是后续 Jacobi 场、完备性、比较定理的公共舞台。
...
> **🧠 思考历程：相邻的两条测地线，会越贴越近、越分越开，还是刚好平行？**
>
> 19 世纪变分法指出：一族测地线中相邻两条的间距满足一个由曲率驱动的线性方程。这个间距变化（雅可比场）直接刻画了测地线会不会聚焦——聚焦点就是共轭点，一旦中途出现，测地线便不再是局部最短的。本大节回答：**如何用雅可比方程量出相邻最短线之间的拉力或斥力、从而判断指数映射何时退化、测地线何时失去极小性**，并以第二变分公式作为判定工具。
...
## 7. 完备性 (Completeness)
> **🧠 思考历程：什么样的空间没有尽头、去哪都到得了？**
>
> 直到 1930 年代，Hopf 与 Rinow 才彻底厘清完备的多重等价刻画：度量完备、测地可无限延伸、某点的指数映射全域定义、有界闭集紧——它们竟彼此等价。本大节的动机是：**有了完备性，才敢保证任意两点之间总有一条最短测地线，也才能放心地谈极限、直径与紧性**。完备性是把局部几何推向整体定理的通行证。
...
## 8. 常曲率空间 (Space Forms)
> **🧠 思考历程：处处弯得一样的空间，一共有几种？**
>
> 球面、平面、双曲空间是最早被系统研究的三种几何（Gauss、Lobachevsky、Bolyai 奠定了双曲几何；Klein 在 1872 年用群论统摄之，并引入空间形式的观念）。给定常截面曲率 $K$，曲率张量由这一个数完全决定，故这类空间形式成为一切比较定理的基准标尺。本大节回答：**如何在 $S^n$、$\mathbb R^n$、$H^n$ 三种模型上显式计算度量、联络、曲率与测地线，并以此衡量一般流形**。
...
> **💡 直观理解（类比）**：球面是"处处同样鼓起的标准气球"——每一个点、每一条大圆测地线一圈都刚好走 $2\pi$，曲率恒为 $1$，是正曲率的完美样板。指数映射则把切空间看成一团"到对径点 $\pi$ 距离就到头"的布：向任一方向铺到半径 $\pi$ 时地图折叠回原点对面（对径点），那正是测地线失去最短性的"聚焦点"（共轭点兼割点）。
...
> **💡 直观理解（类比）**：双曲空间是"处处同样凹陷的标准马鞍/薯片"——恒定负曲率 $K=-1$，三角形内角和恒小于 $\pi$、平行线越铺越散。上半平面/圆盘两种模型异曲同工：都以共形因子把欧氏"拉松"出无穷边界的感受。正因负曲率让测地线永不聚焦，指数映射成为整张双曲空间到切空间的"一对一放大镜"（全局微分同胚），没有共轭点、也从不塌缩。
...
> **💡 直观理解（类比）**：平坦环面像"一卷卷轴纸头尾相接成甜甜圈"——$\mathbb{R}^2$ 的方格图案在 $\mathbb{Z}^2$ 平移识别下卷曲闭合，但每处依然平整（$K\equiv0$）。斜率为有理数的测地线像"恰好绕卷轴若干圈闭合的线段"；无理的则像"永远绕圈却从不停驻、把环面铺得密密麻麻"的螺旋线。换一个复参数 $\tau$ 相当于用不同长宽比的卷轴，闭合的方格错落不同，于是得到形形色色的（非等距）平坦环面。

### 8.5 Berger 球面 (Berger Spheres)【新增】

**来龙去脉**：除了常曲率空间，还有一族特别重要的齐性空间——Berger 球面。M. Berger 在 1960 年代研究截面曲率受限却与常曲率空间不同胚的空间时，给出 $S^3$ 上一族椭球型齐性度量（沿纤维方向把球压扁拉宽）。它常被用作球定理（sphere theorem，见探究题 280）的测试例，也是理解正曲率紧流形分类（最小曲率、直径极值）的关键样本。

**构造**：设 $\{\omega_1,\omega_2,\omega_3\}$ 为 $S^3\subset\mathbb R^4$ 上由左不变 1-形式构成的基（满足 $d\omega_i=\omega_j\wedge\omega_k$ 循环）。对参数 $a,b,c>0$ 定义
$$
g = a^2\,\omega_1^2 + b^2\,\omega_2^2 + c^2\,\omega_3^2.
$$
当 $a=b=c$ 时为常曲率 $S^3$（$K=4/a^2>0$）；一般情况下它是齐性但**非各向同性**的度量。最简单一族取 $a=b=1$、$c\ne1$，即把所有纤维方向 $\omega_3$ 缩放。

**截面曲率**：对一族 $(a,b,c)$ 可显式写出两两方向的截面曲率。当三者足够接近时所有截面曲率 $>0$；而当差别过大时，会出现负截面曲率——这是正曲率度量并非只有常曲率一种的生动例证。

> **💡 直观理解（类比）**：把单位球 $S^3$ 想成一团橡皮泥，Berger 度量就是沿一个方向捏扁、另两个方向保持的雪球形结构——虽然整体仍是一个球（拓扑不变），但各处弯曲强弱不再均匀，正曲率却依然处处成立。它提醒我们：$K>0$ 允许远比常曲率丰富得多的几何。
...
> **🧠 思考历程：一个空间的对称究竟有多少，又如何刻画？**
>
> 大约在 1890 年前后，W. Killing 而今人在研究把几何整体搬动而不变形的变换时，把对称落到无穷小层面：那些保度量、沿之流动不改变形状的向量场正是 Killing 向量场，它们生成等距群。Noether 原理进一步表明：对称（Killing 场）自动给出沿测地线的守恒量。本大节回答：**如何用 Killing 方程识别对称方向、如何刻画等距群，以及对称如何导出物理意义上的守恒律**。
...
> **💡 直观理解（类比）**：这三个等距群例是"每种对称世界能做的整体动作清单"——球面能"随便转和翻"（正交群 $O(n+1)$，维数 $\frac{n(n+1)}2$），双曲平面用 $3$ 维的 $PSL(2,\mathbb{R})$ 做到"所有保定向的搬动"，欧氏空间除旋转外还能"整体平推"（加上 $\mathbb{R}^n$ 平移，维数同样是 $\frac{n(n+1)}2$）。维数越高，往往意味着这个空间对称空间越"灵活"。

### 9.3 覆叠、商流形与 Galois 对称 (Coverings, Quotients and Galois Symmetry)【新增】

**来龙去脉**：把流形除以一个离散等距群得到商流形，再把万有覆叠一层层铺回原空间，构成一对相互对应的整体对称操作。它们与 Galois 理论中域扩张的对称群高度相似：覆叠的**覆盖变换群**（deck transformations）恰似 Galois 群里保持基底不变的自同构，而基本群则类比 Galois 群。这为黎曼几何提供了一种对称解读的通用语言。

**定义**：设 $\Gamma\subset\operatorname{Iso}(\tilde M)$ 是等距群的自由离散子群（作用无固定点）。则商 $M=\tilde M/\Gamma$ 自然带有黎曼度量，投影 $\pi:\tilde M\to M$ 是黎曼覆叠映射。**覆盖变换群**
$$
\operatorname{Deck}(\pi)=\{D\in\operatorname{Iso}(\tilde M)\mid \pi\circ D=\pi\}
$$
中每个 $D$ 都保持 $\tilde M$ 的度量，把一层同构图搬运到另一层。

> **💡 直观理解（类比）**：$\mathbb R^2$ 是 $T^2=\mathbb R^2/\mathbb Z^2$ 的万有覆叠，$\pi_1(T^2)=\mathbb Z^2$ 就像平移对称的 Galois 群——每平移一格，$\mathbb R^2$ 叠成一层对应点，几何完全相同而层号不同。整体对称常常藏在离散作用与覆叠结构里，正是所谓 Galois 型对称。

**应用**：它统一了大量例子中来自对称的性质——$S^n/\{\pm1\}$ 得到实射影空间 $\mathbb{RP}^n$；常曲率空间的更多成员正是其离散等距群的商。Galois 式的观点把覆盖、对称群与基本群编织成一条主线。
...
> **🧠 思考历程：曲率的符号，能倒逼出空间的整体拓扑吗？**
>
> 20 世纪中叶，几何迎来局部到整体的飞跃。Cartan–Hadamard 说明非正曲率空间微分同胚于 $\mathbb R^n$；Bonnet–Myers 用正 Ricci 曲率封顶直径与基本群；Synge 与 Rauch 发展出比较思想（见新增的 10.5、10.6），把曲率上下界转成对测地线与体积的控制；Gauss–Bonnet 把总曲率与 Euler 示性数焊在一起，后被 Chern 推广到高维（见新增的 10.7）。本大节回答：**如何仅凭曲率的正负与大小，读出流形的拓扑与整体度量结构**——这是黎曼几何对拓扑学最深刻的回赠。
...
> **💡 直观理解（类比）**：这三个计算把"总弯曲量"按曲面开洞数摆成一张查表：球面（无洞、$\chi=2$）总曲率是 $4\pi$，环面（一个洞、$\chi=0$）总曲率归零，而亏格 $g$ 的多孔曲面（$\chi=2-2g$）总曲率逐洞往下跌成负数。正曲率被"洞"吃光，洞越多负得越狠——Gauss-Bonnet 用最简单直白的三个算例，把"洞数铁定总弯"这句全局规律落到实处。
...
### 10.5 Synge 定理 (Synge's Theorem)【新增】

**来龙去脉**：1936 年 J. L. Synge 证明了两个正曲率下的整体拓扑结论，合称 Synge 定理。它回答：**正截面曲率的完备紧致流形，其绕圈的世界有多受限制？** 结果异常漂亮——$n$ 为偶数时拓扑单层（基本群平凡），$n$ 为奇数时可定向。它通过对 Jacobi 场与沿闭测地线的平行移动的分析得出。

**定理 10.10**（Synge 定理）设 $(M,g)$ 是完备紧致、截面曲率 $K>0$ 的 $n$ 维黎曼流形。
1. 若 $n$ 偶，则 $\pi_1(M)=0$，$M$ 单连通。
2. 若 $n$ 奇，则 $M$ 可定向。

> **💡 直观理解（类比）**：正曲率像鼓得不能再鼓的球，容不下分叉的圈与镜像错位。偶维度连一个环绕的洞都不许有；奇维度虽可绕圈，但绝不允许翻个面再回来的不可定向缠绕。它是最早把曲率符号转译成基本群/可定向性的定理之一。

**证明思路**：取 $M$ 中长度极小的非平凡闭测地线 $\gamma$。沿 $\gamma$ 平行移动法向标架；由 $K>0$，法向 Jacobi 场的第二变分为正，若 $\gamma$ 平凡地绕成非简单圈则能量可降，矛盾于极小性。偶维时这迫使 $\gamma$ 决定单连通的环路结构，从而 $\pi_1=0$；奇维的可定向性由闭测地线上的平行移动公式给出。$\square$

**配套习题 304**（见 12.5）。

### 10.6 Ricci 比较定理 (Ricci Comparison for Volume)【新增】

**来龙去脉**：Bonnet-Myers 已用 Ricci 曲率下界控制直径。Ricci 比较定理把这一思想推进到体积层面：它回答 **当 Ricci 曲率不小于某常数时，空间中的测地线球及其体积要素如何被常曲率空间限制**。这个逐点比较体积要素、再积分的方法，正是现代几何分析中曲率到体积再到拓扑链路的基石（也支撑上文推论 10.6）。

**定理 10.11**（Ricci 比较，体积要素形式）设 $M$ 满足 $\operatorname{Ric}\ge(n-1)\kappa$，$p\in M$，$\exp_p(r\theta)$ 为测地极坐标。则径向 Jacobi 场的行列式（体积要素）满足
$$
\frac{\mathrm d}{\mathrm dr}\frac{\sqrt{\det g(r,\theta)}}{\mathrm{sn}_\kappa(r)^{n-1}}\le 0,
$$
其中 $\mathrm{sn}_\kappa(r)=\dfrac{\sin(\sqrt\kappa\,r)}{\sqrt\kappa}$（$\kappa>0$）、$=r$（$\kappa=0$）、$=\dfrac{\sinh(\sqrt{-\kappa}\,r)}{\sqrt{-\kappa}}$（$\kappa<0$）。从而 $p$ 处半径 $r$ 的测地球体积 $V(r)$ 不超过常曲率空间 $M(\kappa)$ 中半径 $r$ 球的体积 $V_\kappa(r)$。

> **💡 直观理解（类比）**：这推广了正 Ricci 让空间像被攥紧的直觉——不只盯直径，而是看每个同心层的皮能包住多少空间：曲率越大，同样半径装的东西越少。它把 Rauch 对 Jacobi 场长度的比较，升级为对整个体积要素（$n-1$ 维）的比较，是可积形式的涨缩定理。

**配套习题 305**（见 12.5）。

### 10.7 Gauss-Bonnet-Chern 定理 (Gauss-Bonnet-Chern Theorem)【新增】

**来龙去脉**：二维 Gauss-Bonnet 已把总曲率与 Euler 示性数联系（10.4）。能否推广到任意偶维紧流形？S.-S. Chern（陈省身，1944）给出高维的内蕴证明，用曲率 2-形式拼出的 Pfaffian 把 $\int \mathrm{Pf}$ 与 Euler 示性数 $\chi$ 挂钩，回答：**偶维紧致可定向流形上，总曲率 $=2\pi\chi$ 这句二维神谕的天然推广是什么**。它开创了示性类（Chern–Weil 理论），并为 Atiyah–Singer 指标定理铺路。

**定理 10.12**（Gauss-Bonnet-Chern）设 $(M^{2m},g)$ 是紧致无边的可定向黎曼流形，$\Omega$ 为其 Levi-Civita 联络的曲率 2-形式矩阵（$\Omega^i{}_j=\tfrac12 R^i{}_{jkl}\,dx^k\wedge dx^l$）。则
$$
\int_M \operatorname{Pf}\!\left(\tfrac{\Omega}{2\pi}\right)=\chi(M).
$$
当 $m=1$（二维）时退化为经典 Gauss-Bonnet：$\int_M K\,dA=2\pi\chi(M)$。

> **💡 直观理解（类比）**：把曲率加起来等于洞数升级到任何偶维：用曲率拼成的 Pfaffian 仪表盘对整个流形积分，依旧只读出 Euler 数——几何的千姿百态仍被拓扑一锤定音。它说明 Euler 示性数本质上是个曲率积分（示性类），而不是凭空数洞的约定。
...
## 11. 示例：具体计算 (Examples)
> **🧠 思考历程：把抽象理论落到球面、双曲、环面，需要真刀真枪的手算**
>
> 概念再漂亮，也要靠具体计算站稳。前文已约定三种模型（$S^2$、$H^2$、$T^2$），本节用手把 Christoffel 符号、Riemann 分量与 Gauss 曲率逐项算出，既检验前面公式，也示范从度量到曲率的完整流水线。这正符合美版教材先抽象、再算例巩固的教法。
...
> **💡 直观理解（类比）**：这段球面计算像"用纯算术亲手称出一只标准气球的总弯度"——先由度量逐步配出 Christoffel 符号，再凑出更复杂的 Riemann 曲率，最终化简回一个常数 $K=1$，从而总曲率 $4\pi$ 恰好对上 Euler 数 $2$。一遍走完就明白：曲率这类"轮廓感"虽由大量局部符号堆成，但整体结果如此干净利落，正是 Gauss-Bonnet 把局部到整体焊死的证据。
...
> **💡 直观理解（类比）**：双曲计算与球面如出一辙却"符号相反"——同样从度量开始配 Christoffel 符号，得到的 Riemann 曲率带负号，最终 Gauss 曲率收敛到 $K=-1$。两场算账一正一负地摆在一起，正好把"球面鼓起取正、马鞍凹陷取负"这句直觉凝固成一个经得起推导的数字：负曲率空间在代数层面同样"越算越凹"。
...
> **💡 直观理解（类比）**：平坦环面的 Jacobi 场简单得就像"匀速离开的汽车"——无曲率时 Jacobi 方程为二阶导为零，相邻测地线的间距 $J(t)=J(0)+t\,\dot J(0)$ 呈直线发散，除非初始方向和速度都给零否则绝不回零。于是环面上不会出现"测地线聚焦撞车"的共轭点，正印证了 $\mathbb{R}^2$ 商卷成环后"处处平平、永不汇聚冷场"的朴素直觉。
...
### 15.5 新增概念配套习题 (Additional Exercises for New Concepts)【新增】

**习题 301** 用单位分解证明：每个光滑流形上至少存在一个黎曼度量。

**习题 302** 写出 Berger 球面度量 $g=a^2\omega_1^2+b^2\omega_2^2+c^2\omega_3^2$ 中 $a=b=c$ 时对应的几何类型，并说明此时截面曲率为何。

**习题 303** 证明 $\mathbb R^2/\mathbb Z^2$ 的覆盖变换群 $\operatorname{Deck}(\pi)$ 同构于 $\mathbb Z^2$；说明它如何扮演平移对称的 Galois 群。

**习题 304** 陈述 Synge 定理在 $n$ 偶时的结论，并用它判断实射影平面 $\mathbb{RP}^2$ 上能否有处处 $K>0$ 的度量。

**习题 305** 用 Ricci 比较定理说明：$\operatorname{Ric}\ge(n-1)\kappa>0$ 时单位球的体积不超过 $S^n(\kappa)$ 中相同半径球的体积。

**习题 306** 写出 Gauss-Bonnet-Chern 定理，并说明当 $m=1$ 时它如何退化为经典二维 Gauss-Bonnet。
...
### 13.5 新增习题答案 (Additional Solutions for New Concepts)【新增】

**答案 301** 见 2.3：取局部有限覆盖与单位分解 $\{\rho_\alpha\}$，令 $g=\sum_\alpha\rho_\alpha\,\varphi_\alpha^*g_{\mathrm{eucl}}$；每点至少一个权重 $>0$，光滑正定，故为黎曼度量。$\square$

**答案 302** $a=b=c$ 时 $g=a^2(\omega_1^2+\omega_2^2+\omega_3^2)$ 是半径 $a$ 的常曲率球，截面曲率 $K=1/a^2$（等距于标准球缩放）。$\square$

**答案 303** $\pi:\mathbb R^2\to\mathbb R^2/\mathbb Z^2$ 的覆盖变换是整格平移 $x\mapsto x+(m,n)$，故 $\operatorname{Deck}\cong\mathbb Z^2=\pi_1(T^2)$；每一平移层几何全同，恰如 Galois 群保持基底作用。$\square$

**答案 304** 偶维紧 $K>0$ 时 $\pi_1=0$。$\mathbb{RP}^2$ 的 $\pi_1=\mathbb Z_2\ne0$，故其上不存在处处 $K>0$ 的度量（否则与 Synge 冲突）。$\square$

**答案 305** 由定理 10.11，体积要素 $\sqrt{\det g}\,\mathrm dr\,\mathrm d\theta\le \mathrm{sn}_\kappa(r)^{n-1}\mathrm dr\,\mathrm d\theta$，积分即 $V(r)\le V_\kappa(r)$。$\square$

**答案 306** $\int_M\operatorname{Pf}(\Omega/2\pi)=\chi(M)$；$m=1$ 时 $\Omega=\begin{pmatrix}0&K\,dA\\-K\,dA&0\end{pmatrix}$ 的 Pfaffian $\operatorname{Pf}(\Omega/2\pi)=K\,dA/(2\pi)$，代入即 $\int_MK\,dA=2\pi\chi(M)$。$\square$

---

## 14. 数学史（History of Mathematics）

> 黎曼几何把"空间弯不弯、怎么弯"变成一门精确科学。它始于黎曼 1854 年的就职演讲：把"度量"逐点化为任意高维流形上的**黎曼度量**，暗示空间曲率可随位置而变；随后 Christoffel 引入联络系数、列维-奇维塔用平行移动与协变导数立起"沿弯曲流形求导"的规则，Riemann 曲率张量、Ricci 曲率与标量曲率逐一成型；测地线、指数映射、Jacobi 场与共轭点把"最直之路"发展成整体理论，Hopf–Rinow、Cartan–Hadamard、Bonnet–Myers 与比较定理让局部曲率讲出全局故事。爱因斯坦用量符号与 Ricci 曲率写下广义相对论，凸黎曼几何的油门。本章沿着"度量→联络→曲率→测地线→整体定理→物理"的线索，追溯黎曼度量、Levi-Civita 联络、曲率张量、测地线、比较定理等每个概念的前因后果。

### 14.1 黎曼的就职演讲：几何学历的开创

- **1854 年的宣言**：**黎曼（Bernhard Riemann, 1826–1866）**在《论作为几何学基础的假设》中提出"空间"不必拘于欧几里得，可由任意维的**流形**与其上的**度量张量**描述，曲率可以逐点不同。
- **三大几何的舞台**：他的框架同时容纳**球面**（正曲率）、**双曲空间**（负曲率）与**平直空间**——非欧几何由此被纳入统一度量语言。
- **从"外部的弯"到"内部的弯"**：黎曼继承高斯内蕴几何思想，让"弯"成为流形自身的性质，而非依赖嵌入的外空间。
- **本章的承接**：本章"黎曼度量、黎曼流形、度量的存在性"正是这一开端的抽象表述，也是全部后续（联络、曲率、测地线）的出发之土。

### 14.2 黎曼度量：把"点积"逐点搬到流形

- **度量张量**：在每个点的切空间上给一个正定内积 $g_{ij}$，并让它光滑变化，就得到**黎曼度量**；它把"长度、角度、面积"逐点定义出来。
- **为什么是重心**：测量靠它、测地线靠它、曲率靠它——度量决定"这是什么样的空间"，是黎曼几何一切几何量的母语。
- **可视化的手感**：同一流形上不同的度量给出不同的几何，正如同一张地图被不同的经纬线拉出不同的"距离感"。
- **本章的承接**：本章"黎曼流量、度量与积分"等内容正以黎曼度量为起点，推断全部后续工具的"为何而来"。

### 14.3 Christoffel 符号与曲率张量的伏笔

- **联络的辨雏**：要在流形上对张量求导必须先把"相邻点如何对齐"讲清楚，**Christoffel 符号**（1869）正是这套系数。
- **从联络到曲率**：两次求导次序不可交换便产生**Riemann 曲率张量** $R^i_{\,jkl}$，它量化"协变导数不可交换"的程度。
- **它们是"弯"的算术**：Christoffel 符号与曲率张量把"空间怎么弯"翻译成可算的系数，为黎曼几何奠定分析骨架。
- **本章的承接**：本章"Christoffel 符号、Levi-Civita 联络、协变导数"正是从这组系数出发把"曲率"写出来。

### 14.4 Levi-Civita 联络与平行移动

- **平行移动的直觉**：沿一条曲线"把向量平移但仍不转、此刻避开"，需要**无挠且度量相容**的唯一联络——**Levi-Civita 联络**（1917）。
- **为何唯一**：在黎曼流形上有且只有一个这样的联络，它由度量完全决定，使"平移"成为几何的稳定概念。
- **协变导数**：沿切向量做协变导数 $\nabla_X Y$ 时把 Levi-Civita 联络纳入，才让"沿曲线求导"不依赖坐标。
- **本章的承接**：本章"Levi-Civita 联络的唯一性、协变导数、平行移动"正是"如何在弯流形上做内蕴求导"的标准答案。

### 14.5 Riemann 曲率张量、Ricci 与标量曲率

- **从不可交换到曲率**：**Riemann 曲率张量** $R(X,Y)Z$ 度量"求导次序换来换去"造成的差，是整个空间弯曲度的完整档案。
- **压痕成 Ricci** ：收缩掉曲率张量的两个指标得到 **Ricci 曲率** $R_{ij}$，再取迹得**标量曲率** $R$——它们概括"体积怎么变""气球缩不缩"。
- **物理的钥匙**：Ricci 曲率正是广义相对论场方程的左毂（$R_{ij}=0$ 的真空），让"曲率—物质"达成平衡。
- **本章的承接**：本章"各类曲率（截面、Ricci、标量）"正是把黎曼曲率的完整档案做"降维浏览"，让它们各有物理与几何的读数。

### 14.6 测地线与指数映射：最直之路的整体化

- **测地线方程**：使长度（或能量）泛函取极值且扭率为零的曲线，满足**测地线方程** $\frac{d^2x}{dt^2}+\Gamma^{i}_{jk}\dot x^j\dot x^k=0$，即"弯曲空间里的直线"。
- **指数映射**：给定初值与速度，沿测地线"飞出去"就得到**指数映射**耗尽每个切向量对应于目标点。
- **从局部到全局**：测地线把"极小化"与"最短距离"连起来，指数映射则把切空间的扇形直觉带到流形上，是整体黎曼几何的枢纽。
- **本章的承接**：本章"测地线方程、指数映射、测地完备性"正是"谁是最直路线、它能延伸多远"这一古老问题的现代答案。

### 14.7 Gauss 引理、Jacobi 场与共轭点

- **正交扇形**：**Gauss 引理**说明沿测地线的径向扰动与周向扰动正交，揭示"扇形"与距离函数梯度的关系。
- **Jacobi 场**：沿测地线的变分给出**Jacobi 场**，它度量"相邻测地线怎么散开或聚拢"，也定义**共轭点**。
- **共轭点的意义**：测地线越过共轭点后不再是最短的（极小化之路到此"退休"），这标记极小性失效的界限。
- **本章的承接**：本章"Gauss 引理、Jacobi 场、共轭点"正是"测地线为何时最短、何时失效"的精确判据。

### 14.8 第二变分公式与极值性的一生

- **变分分析**：对能量泛函做一阶变分得到测地线，二阶变分（第二变分公式）则判断测地线是极小、极大还是鞍点。
- **指数负曲率的说服力**：在非正曲率空间里第二变分足够大、共轭点可避免，因而测地线"处处最短"——这是**非正截面曲率**的强力推论。
- **极值性的闭环**：第一/第二变分公式把"最短线"完整地变成可判定的局部—整体性质，是几何分析的标准配齐。
- **本章的承接**：本章"第二变分公式"正是对测地线极小性的深化，也为后面比较定理与 Myers 之类的结论提供工具。

### 14.9 Hopf–Rinow 定理：什么时候"空间是完整而紧致"的

- **完备性等价**：**Hopf–Rinow 定理**说：达到"每两条测地线连接两点"（测地完备）与"有界闭集必紧致"（长度完备）在黎曼流形上等价，且可延到"任意两点可被极小测地线连接"。
- **完整黎曼流形的肖像**：完备保证了指数映射全局满射，让"切空间 → 流形"成为整体构图。
- **为什么关键**：它是"局部性质推向整体"的第一个坚实垫脚石，也是非紧情形里最难逾越的门槛之一。
- **本章的承接**：本章"测地完备、Hopf–Rinow"正回答了"这个空间直觉上的完整欠不欠缺"，是整体黎曼几何起步的第一门登记。

### 14.10 常曲率空间与空间的分类

- **空间形式**：截面曲率恒为 $K$ 的完备连通黎曼流形称为**空间形式**，经典例子即球面 $S^n$（$K>0$）、双曲空间 $H^n$（$K<0$）与平直空间（含平坦环面 $T^n$，$K=0$）。
- **从李到几何化**：赖黎曼与嘉当、Klein、Thurston 的叙述，常曲率空间与**几何化纲领**相连——把流形剖成"受常曲率几何支配"的块。
- **Berger 球面**：**Berger 球面**提供非等度量却仍具对称性的构造，提醒人们"曲率的简单"与"形状的复杂"并不总一致。
- **本章的承接**：本章"常曲率流形、$S^n$、$H^n$、平坦环面、Berger 球面"正是空间分类里最基本的一等床铺。

### 14.11 比较定理与整体结构：局部曲率逼出的全局结论

- **Cartan–Hadamard** ：非正曲率的完备单连通流形与欧氏空间微分同胚，让指数映射成为全局同胚——悲观而宁静的几何。
- **Bonnet–Myers**：正 Ricci 曲率下限迫使流形紧致且在有限直径内——"曲率太弯，空间撑不大"。
- **Rauch 与各种比较**：**Rauch 比较定理**、体积比较与 **Synge 定理**让"曲率如何逼出（或禁止）测地线的汇聚"凝成定理，构成整体黎曼几何的主干。
- **本章的承接**：本章"Cartan–Hadamard、Bonnet–Myers、Rauch、Synge、比较定理"正是"局部曲率到全局形状"这张地图。

### 14.12 Gauss–Bonnet–Chern 与物理学支点

- **Gauss–Bonnet**：曲率积分等于 $2\pi$ 乘欧拉示性数，把局部曲率与整体拓扑焊在一起（微分几何 42 级已有）。
- **Chern 的高维**：**Chern–Gauss–Bonnet 定理**（陈省身）用示性类给出高维推广，打开了不变量表。
- **广义相对论**：爱因斯坦用**Ricci 曲率**（真空 $R_{ij}=0$）把引力解释为时空弯曲——黎曼几何从此成为现代物理的母语，也是 Ricci 流、几何分析（Perelman）的温床。
- **本章的承接**：本章"比较定理、Gauss–Bonnet–Chern 与物理"正体现出黎曼几何既是纯数学的高地，也是广义相对论与几何分析的生力军。

### 14.13 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1854 | 黎曼就职演讲 | 提出高维度量流形与黎曼度量 |
| 1869 | 克里斯托费尔 | Christoffel 符号（联络系数） |
| 1917 | 列维-奇维塔 | Levi-Civita 联络与平行移动 |
| 19—20 世纪 | 里奇 | Ricci 曲率与标量曲率的定型 |
| 1930 年代 | Hopf、Rinow | Hopf–Rinow 定理（测地完备性） |
| 20 世纪中 | Cartan、Hadamard、Myers、Rauch | 比较定理与整体黎曼几何 |
| 1944 | 陈省身 | Chern–Gauss–Bonnet 定理与示性类 |
| 1915 | 爱因斯坦 | 广义相对论：Ricci 曲率几何化引力 |
| 20 世纪末 | Perelman | Ricci 流与三维庞加莱猜想 |

### 14.14 小结

> 黎曼几何的历史是"把'弯'做得越来越精确"的历史：黎曼先断言空间可以是任意维度量流形、曲率可随位置而变；Christoffel 与列维-奇维塔用联络与平行移动把"沿弯流形求导"做成可执行技术；Riemann 曲率张量、Ricci 与标量曲率把"弯成什么样子"编成档案；测地线与指数映射、Jacobi 场与共轭点、Hopf–Rinow 与比较定理则把局部的"弯"推向全局的形状；最终 Gauss–Bonnet–Chern 把曲率与拓扑焊连，爱因斯坦用 Ricci 曲率把引力几何化。从黎曼的 1854 到 Ricci 流的当代，黎曼几何始终在回答同一个问题——**空间如何弯、又如何由弯决定形状**——这正是本章每个概念（度量、联络、曲率、测地线、比较定理）共同的前因后果。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对黎曼几何核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 15.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到黎曼几何语境的追问上。

| 步骤 | 行动 | 黎曼几何中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 流形是 $S^n$、$H^n$、$T^2$ 还是通用 $M$？给的是度量 $g_{ij}$、联络还是曲率？要算曲率、平行移动、测地线还是整体不变量？ |
| ② 计划 | 选择可行工具、预估方向 | 用 Christoffel 符号 $\Gamma^k_{ij}$ 代 Levi-Civita 公式？用 $R(X,Y)Z$ 求截面曲率 $K$？用 Gauss-Bonnet $\int_M K\,dA=2\pi\chi(M)$ 推整体结论？先想清哪条路最省力。 |
| ③ 执行 | 按计划逐步推导、保留过程 | 每步写明依据（公式/定理编号），指标上下标、$(-1)$ 符号、缩并哑指标别漏。 |
| ④ 回看 | 核对结果、检验合理性 | 常曲率空间结果是否符合 $K=1,0,-1$？量纲与维数对不对？能否用对称性（Killing 场）或极限（$r\to0$）再验一遍？ |

> 💡 **策略演示**：判断单位球 $S^2$ 上的截面曲率是否为常数。
> ① 理解：$S^2\subset\mathbb R^3$ 配诱导度量，目标是一个二维截面曲率。② 计划：用曲率张量定义直接算，或借助球面的最大对称性。③ 执行：$S^2$ 的等距群 $O(3)$ 作用可迁于切平面丛，各截面曲率彼此相等，取 $K\equiv1$（单位球）。④ 回看：与「$K=1$ 的空间形式」对照一致，答案合理。

### 15.2 概念检查（Concept Check）

> 先用一组判断题与选择题快速自检核心概念。每道题的判断均应落到书面理由（用一句话即可）。

**判断正误**（说明理由）：

1. Levi-Civita 联络是黎曼流形上唯一同时满足无挠性与度量相容性的仿射联络。（　）
2. 截面曲率 $K$ 的值完全决定 Riemann 曲率张量 $R$。（　）
3. 每一条测地线都一定是连接其两端点的最短路径。（　）
4. 正 Ricci 曲率（$\operatorname{Ric}\ge k>0$）的完备黎曼流形必具有有限基本群。（　）
5. 曲率张量 $R$ 处处为 $0$ 的流形一定是平直的（局部等距于 $\mathbb R^n$）。（　）
6. Bonnet-Myers 定理给出正 Ricci 流形的直径上界 $\mathrm{diam}\,M\le\pi/\sqrt{k}$。（　）
7. 紧无边曲面的 Gauss-Bonnet 定理把总曲率与 Euler 示性数联系在一起。（　）

**选择**：

8. 常曲率 $1$、完备且单连通的二维黎曼流形一定等距于（　）。
   - A. 环面 $T^2$　B. 球面 $S^2$　C. 双曲平面 $H^2$　D. 射影平面 $\mathbb{RP}^2$
9. Hopf-Rinow 定理隐含：在完备黎曼流形中，任意两点之间（　）。
   - A. 必可由一条极小测地线连接　B. 必无测地线　C. 曲率必为正　D. 直径必有限
10. 在单位球 $S^n$ 上沿一条闭合测地线（大圆）做平行移动，一个切向量绕回起点后（　）。
    - A. 必与原来完全相同　B. 一般发生绕过封闭区域的回转（holonomy 旋转）　C. 必恰好反向　D. 结果与路径无关

### 15.3 分步骤工作示例（Worked Examples）

> 每道示例严格套用四步流程，并在「回看」中给出一条易错点或检验法。

**示例 S1：用 Levi-Civita 公式计算球面的 Christoffel 符号**

> **问题**：在 $S^2\subset\mathbb R^3$ 用经纬坐标 $(\theta,\phi)$，度量 $g=d\theta^2+\sin^2\theta\,d\phi^2$，求非零 Christoffel 符号。

- ① 理解：$g_{\theta\theta}=1,\ g_{\phi\phi}=\sin^2\theta,\ g_{\theta\phi}=0$，其余组合见 $g^{ij}$。
- ② 计划：代入连通度量（由 $g^{rk}$ 与度量导数的乘积）的标准公式，只看三个非平凡分量。
- ③ 执行：

$$
\Gamma^r_{ij}=\frac12 g^{rk}\left(g_{ik,\,j}+g_{jk,\,i}-g_{ij,\,k}\right),\qquad
\Gamma^\phi_{\theta\phi}=\frac{\cos\theta}{\sin\theta},\quad
\Gamma^\theta_{\phi\phi}=-\sin\theta\cos\theta .
$$

- ④ 回看：$\Gamma^\phi_{\theta\phi}=\cot\theta$ 在极点发散，源于球面坐标的几何奇性而非流形奇点。**易错点**：$S^2$ 不是 $\mathbb R^2$，$\Gamma$ 依赖度量导数，不能照搬平面极坐标。据此缩并即可得 $K=1$。

**示例 S2：闭合曲线平行移动的 holonomy 回转角**

> **问题**：在二维常曲率 $K$ 曲面上一闭曲线围出面积 $A$，求沿该闭路平行移动的回转角。

- ① 理解：所求是 holonomy（回转角），曲面维数 $2$，可用面积分联系。
- ② 计划：用同 Gauss-Bonnet 同源的 holonomy 公式——绕闭合曲线的回转角等于被围区域上 $K\,dA$ 的积分。
- ③ 执行：

$$
\Delta\alpha=\int_{D}K\,dA,\ \Rightarrow\ \Delta\alpha=KA\ (\text{当 } K \text{ 恒定、} A \text{ 充分小}).
$$

- ④ 回看：回代 $K=1,\ A=2\pi$（赤道围半球）得 $\Delta\alpha=2\pi$，与「向量绕赤道一圈方向恢复」一致。**易错点**：回转角依赖被围区域（路径依赖），不是测地线本身固定的量。

**示例 S3：Gauss-Bonnet 计算曲面总曲率**

> **问题**：计算单位球 $S^2$ 与亏格 $g=1$ 环面 $T^2$ 上的 $\int_M K\,dA$。

- ① 理解：$\int_MK\,dA$ 是整体拓扑不变量，正比于 Euler 示性数。
- ② 计划：直接用 Gauss-Bonnet：$\int_MK\,dA=2\pi\chi(M)$，取 $\chi(S^2)=2,\ \chi(T^2)=0$。
- ③ 执行：

$$
\int_{S^2} K\,dA=2\pi\cdot2=4\pi,\qquad
\int_{T^2} K\,dA=2\pi\cdot0=0 .
$$

- ④ 回看：环面的 $K$ 不必处处为零，但总积分恒为 $0$（平坦环面 $K\equiv0$ 即一例）。**易错点**：总曲率只看 $\chi$，不取决于局部 $K$ 的正负——环面上正负曲率可以并存但总和为 $0$。

**示例 S4：平面极坐标下的测地线与角动量守恒**

> **问题**：在 $\mathbb R^2$ 配极坐标 $ds^2=dr^2+r^2\,d\theta^2$，求从原点出发的测地线并识别第一积分。

- ① 理解：平直度量，测地线在欧氏意义下应是直线。
- ② 计划：列出测地方程 $\nabla_{\dot\gamma}\dot\gamma=0$，导出守恒量再积分。
- ③ 执行：由 Christoffel 得

$$
\ddot r=r\dot\theta^2,\qquad \ddot\theta+\frac{2}{r}\dot r\dot\theta=0\ \Rightarrow\ r^2\dot\theta=\text{常量（角动量）}.
$$

- ④ 回看：$\theta$ 平移是等距（Killing 场 $\partial_\theta$），故 $r^2\dot\theta$ 守恒，自洽。**检验**：$\exp_0$ 在原点处保持径向测地线，满足 Gauss 引理，解为直线 $r=\rho/\cos(\theta-\theta_0)$。

### 15.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：在球面坐标上数值积分测地线方程并把一个切向量沿固定纬圈平行移动一圈，实测回转角与纬圈围成面积的比值是否恒为 $1$（单位球 $K$）。
- **聚焦 P2（跨章节联系）**：用单位分解与局部 Euclidean 拉回把任意流形配出全局黎曼度量（见 §2.3），并与第 45 级「流形与张量分析」的联络与张量做交叉引用，验证正定性不依赖局部坐标。
- **聚焦 P3（反例构造）**：在一个 $g=1$ 环面上构造局部 $K>0$ 与 $K<0$ 并存的光滑度量，并用 Gauss-Bonnet 解释总曲率为何仍为 $0$。

### 15.5 梯度化自测（Graded Exercises）

> 难度记号：$\circ$ 概念 / $\bullet$ 技能 / $\blacklozenge$ 综合应用，均要求在四步流程中写出「回看」一句。

**$\circ$ 概念题**

- **自测 1**：用一句话解释「为什么 Levi-Civita 联络是唯一的」，并指出是哪两个条件把其锁定。
- **自测 2**：分开说清 Riemann 曲率张量、Ricci 曲率与标量曲率三者靠什么「缩并」彼此联系。

**$\bullet$ 技能题**

- **自测 3**：对共形度量 $g=e^{2f}(dx^2+dy^2)$，写出 Christoffel 符号并用 Liouville 公式求曲率 $K$。
- **自测 4**：在单位球 $S^2$ 上沿赤道平行移动一个向量一圈，求回转角（结合 holonomy 公式）。

**$\blacklozenge$ 综合题**

- **自测 5**：用 Gauss-Bonnet 论证闭曲面的总曲率必为 $2\pi\chi(M)$，并给出 $g=2$ 曲面上一个正负曲率并存的总曲率分配。
- **自测 6**：写一段含「表格 + 一句话结论」的分析：为什么正 Ricci 完备流形既直径有界又基本群有限（Bonnet-Myers）。

### 15.6 自测答案与简要解析

> **概念判断**：
> 1. **正确**（无挠 + 度量相容在仿射联络中唯一确定，Koszul 公式显式给出）。
> 2. **正确**（度量相容约束下，各二维截面曲率的值集合完全决定 $R$）。
> 3. **错误**（测地线只局部最短；跨过共轭点后往往不再全局最短）。
> 4. **正确**（Bonnet-Myers 保证基本群有限）。
> 5. **正确**（$R=0$ 局部等距 $\mathbb R^n$，即平直）。
> 6. **正确**（$\mathrm{diam}\,M\le\pi/\sqrt{k}$）。
> 7. **正确**（$\int_MK\,dA=2\pi\chi(M)$）。
>
> **选择**：8. B　9. A　10. B。

> **自测**：
> - 自测 1：在「无挠」与「度量相容」两个条件下，Levi-Civita（Koszul）联络被唯一确定，两条约束互锁得出唯一 $\nabla$。
> - 自测 2：Ricci 是 $R$ 对第一与第三指标缩并（$R^j_{ijk}$），标量曲率再对 Ricci 取全迹 $g^{ij}\operatorname{Ric}_{ij}$；截面曲率则张起全部二维切平面。
> - 自测 3：共形平坦时 $K=e^{-2f}\big(f_{xx}+f_{yy}\big)$；取 $f\equiv0$ 得 $K=0$，退化为平面。
> - 自测 4：赤道围成半球面积 $A=2\pi$，回转角 $KA=2\pi$，向量方向完整转回自身，判断自洽。
> - 自测 5：依 $\int_MK\,dA=2\pi(2-2g)$，当 $g=2$ 时总曲率为 $-4\pi$，需局部正负曲率并存，总和被 Euler 示性数锁定。
> - 自测 6：$\mathrm{diam}\,M\le\pi/\sqrt{k}$，配合有限基本群把「强正曲率」翻译成「小而近完备」的整体约束。