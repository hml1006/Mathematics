...
> **一句话大白话**：把空间拆成点、线、三角形、四面体等小砖块并按规则粘起来，再用代数数出它有多少"洞"——$H_0$ 数几块连通，$H_1$ 数几个环，$H_2$ 数几个空腔。
>
> **小例子**：圆（三角化成多边形）有 $\dim H_1=1$；环面有 $\dim H_1=2,\dim H_2=1$。同调群 $H_n$ 只依赖空间的连通拓扑，不依赖切分方式。

## 一、定理介绍

> **前置依赖**：抽象单纯复形与几何实现、自由交换群与线性代数、链复形与同调群定义、单纯逼近定理、同伦等价不变性。
