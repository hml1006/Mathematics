...
> **一句话大白话**：给一条紧黎曼曲面上"只能有这些极点的函数"数一数，把"够格又带极点的函数个数"和"补余的函数空间大小"关联起来，通过亏格 $g$（曲面的洞数）和除子的度数一算便知。
>
> **小例子**：对亏格为 $g$ 的曲线，公式 $\ell(D)-\ell(K-D)=\deg D+1-g$；比如黎曼球面（$g=0$）上对零除子 $D=0$，有 $\ell(0)-\ell(K)=1$，即整体以外的有理函数维数可逐笔结算。
