...
渐近分析 (asymptotic analysis) 研究函数在某个变量趋向极端（如 $\to\infty$ 或 $\to 0$）时的行为。它起源于对特殊函数与积分的近似计算，在严密的极限语言建立之前就已为数值函数与物理学提供了思想基础，并在二十世纪成为研究偏微分方程、概率论与数论的重要工具。

### 6.1 渐近级数的起源与 Poincaré 的形式化

渐近展开的思想可以追溯到欧勒 (Euler)、斯特林 (Stirling) 与拉普拉斯 (Laplace) 等对阶乘、Gamma 函数与拉普拉斯积分的近似。人们很早注意到，有些函数有"形式上收敛但实际发散"的展开——例如 Euler 在研究级数 $\sum n!x^n$ 时发现它对任何 $x>0$ 都发散，却依然能对较大的项做出极好的局部近似。这种"发散却有用"的级数，在 1886 年被庞加莱 (Henri Poincaré) 在《天体力学的分析方法》中给出严格定义：$f(x)\sim\sum_n a_nx^{-n}$ 指对每个固定的 $N$ 都有 $f(x)-\sum_{n< N}a_nx^{-n}=o(x^{-N})$。Poincaré 由此把"渐近展开"从被质疑的对象变为可计算的严格工具，并把 $O$/$o$ 符号的用法规范化。

### 6.2 复合渐近方法：Laplace 方法、驻相法与最速下降

渐近分析的核心技术是把积分化为"主项来自积分区间内一个点"的形式。Laplace 方法源于拉普拉斯对拉普拉斯型积分 $\int \mathrm{e}^{-x\phi(t)}g(t)\,dt$ 的研究：当 $x\to+\infty$ 时，贡献集中在 $\phi$ 取最小值的点、由局部二次近似给出主项。驻相法（又译平稳相位法）处理振荡积分 $\int \mathrm{e}^{\mathrm{i}x\phi(t)}\,dt$，把贡献集中在相位 $\phi$ 的驻点（$\phi'=0$ 处），由黎曼-勒贝格式的相消给出主项。最速下降法（鞍点法）由 Debye 在 1909 年前后为研究物理中的衍射与光学积分而提出，方法是在复平面内将路径变形为沿"最速下降"的路径，从而兼得 Laplace 与驻相的优势。Watson 引理把这些程序的局部论证归结为对拉普拉斯型积分主项的严格估计，如今是渐近分析的常见起点。

### 6.3 Stirling 公式与 Euler–Maclaurin 求和

阶乘的渐近——Stirling 公式 $n!\sim\sqrt{2\pi n}\,\left(\tfrac ne\right)^{n}$ ——是渐近分析的招牌例子，它于 1730 年由斯特林给出，欧勒独立得到并作了改进；Stieltjes 等人在十九世纪完成严格化。把求和化为积分再加无穷级数修正的 Euler–Maclaurin 求和公式，由欧勒与麦克劳林在 1740 年代独立发现，在数值分析、数论（如 $\zeta$ 函数）与渐近级数运算中至关重要。渐近级数的乘法、除法与反演，以及用 Padé 逼近（1880 年代 Padé 提出，以有理函数近似幂级数以改善"太紧张"的发散区域）来捕捉级数的极点与奇点，都是渐近工具包里常见的技巧。

### 6.4 摄动论与 WKB

渐近分析的强大之处在于处理"含有小参数"的问题，即摄动论。正则摄动（解在参数 $\to 0$ 时平滑变化）与奇异摄动（边界层、转向点等导致解在小尺度上急剧变化）在二十世纪早期随流体力学（边界层理论，Prandtl 1904）、天文学与量子力学而重要起来；匹配渐近展开把在不同尺度上成立的局部渐近解拼接起来，是处理边界层的标准方法。WKB 近似（Wentzel-Kramers-Brillouin 方法，亦常追溯到 Liouville 与 Green）把 Schrödinger 方程的振荡/指数解表示为 $\exp\bigl(\tfrac{i}{\varepsilon}S(x)\bigr)$ 的展开，其相位函数 $S$ 满足 Hamilton-Jacobi 方程，是量子力学半经典分析与几何光学交汇的核心，其衔接公式处理穿过转向点时的指数衰减与振荡解之间的过渡。一般认为，渐近分析在现代数学中既是计算的工具箱，也是连接可积系统、动力系统与调和分析的观念枢纽。
