...
> **一句话大白话**：在一个光滑曲面上给每个点配上一把"局部尺子"，使能尝到角度、弧长与垂直的滋味——把欧几里得量距的本领"点对点"地移植到任意弯曲的流形上。
>
> **小例子**：在球面上，标准Riemann度量给出赤道上一点沿纬线走一小段的弧长微元 $ds^2=R^2d\theta^2$ 附近的形式；不同度量对应不同"internal几何"，例如平面用 $dx^2+dy^2$，球面则是 $R^2(d\theta^2+\sin^2\theta\,d\varphi^2)$。
...
**前置依赖**：光滑流形与切空间、对称正定双线性形式、$(0,2)$-型张量场、单位分解（存在性构造）
