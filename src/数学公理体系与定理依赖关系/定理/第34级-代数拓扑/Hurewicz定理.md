...
> **一句话大白话**：同伦群（看"一维洞有多少圈"）和同调群（用代数加法数洞）本是两套语言，Hurewicz 定理说它们其实握手言和——最低非零同伦群底下它们长得一模一样，且第一个同调群就是把基本群"变成阿贝尔群"后得到的结果。
>
> **小例子**：圆周 $S^1$ 的基本群 $\pi_1=\mathbb{Z}$ 已经阿贝尔化，故 $H_1(S^1)\cong\mathbb{Z}$；而两个圆粘成"8字形"时 $\pi_1$ 是自由群 $\mathbb{Z}*\mathbb{Z}$，阿贝尔化后得 $H_1=\mathbb{Z}\oplus\mathbb{Z}$。
...
**前置依赖**：基本群与道路同伦、奇异同调群与闭链、群的 Abel 化与换位子子群、相对同伦群与长正合序列
