...
## 14. 补充阅读：《普林斯顿微积分读本》第 30 章精选

> 以下内容整理自《普林斯顿微积分读本》第 30 章（微分方程），侧重实践视角的例题与求解策略，与以上各节的理论内容互为补充。

### 14.1 基本概念与初值问题

微分方程的阶数是其所包含的最高阶导数的阶数。例如：

![微分方程阶数示例](images/03582.gif)

**初值问题（IVP）** 的核心思想是：已知一个初始条件（如 $y(0)=5$）和微分方程，可确定解中的不定常数。对于一阶方程，需一个初始条件；对于二阶方程，需两个初始条件（通常为 $y(0)$ 和 $y'(0)$ 的值）。

![IVP 示例](images/03583.gif)

对于 $\frac{dy}{dx} = -2y$，$y(0)=5$，其通解为 $y = Ae^{-2x}$，代入 $x=0$、$y=5$ 得 $A=5$，故特解为 $y=5e^{-2x}$。

### 14.2 可分离变量的一阶微分方程

若能将一阶微分方程中所有关于 $y$ 的部分（包括 $dy$）放在一边，所有关于 $x$ 的部分（包括 $dx$）放在另一边，则该方程称为**可分离变量**的。

**标准解法**：分离变量后两边积分。

![可分离变量方程示例](images/03585.gif)

化为：

![分离变量后的形式](images/03586.gif)

两边积分：

![积分过程](images/03590.gif)

得到 $\tan(y) = \sin(x) + C$。

**注意反函数的定义域问题**：当使用 $\tan^{-1}$ 表示解时，需根据初始条件选择正确的分支。例如，若 $y(0)=\pi/4$，则解为 $y = \tan^{-1}(\sin(x)+1)$；但若 $y(0)=5\pi/4$，则需加上 $\pi$，即 $y = \tan^{-1}(\sin(x)+1) + \pi$。

![分支选择说明](images/03601.gif)

### 14.3 一阶线性方程与积分因子

一阶线性微分方程的标准形式为：

![标准形式](images/03602.gif)

其中 $p$ 和 $q$ 是关于 $x$ 的函数。注意，方程中 $y$ 和 $\frac{dy}{dx}$ 的幂次必须都是 1。

**求解步骤**：

1. 将方程化为标准形式（$\frac{dy}{dx}$ 的系数为 1）

2. 计算积分因子 $f(x) = e^{\int p(x)\,dx}$（不必在指数上的积分中加 $C$）

![积分因子公式](images/03636.jpeg)

3. 两边乘以积分因子，左边可重写为 $\frac{d}{dx}(y \cdot f(x))$

![因式分解形式](images/03615.jpeg)

4. 两边积分（右边需加 $C$），除以积分因子解出 $y$

**例**：求解 $y' + 6x^2 y = x^2$。

积分因子 $f(x) = e^{\int 6x^2\,dx} = e^{2x^3}$。乘以方程两边后：

![积分因子应用](images/03609.gif)

积分得：

![积分结果](images/03610.gif)

解得：

![最终解](images/03611.gif)

**为什么积分因子有效？** 关键在于乘积法则的逆向运用：

![乘积法则验证](images/03640.gif)

由链式求导法则，$\frac{d}{dx}e^{\int p(x)\,dx} = p(x)e^{\int p(x)\,dx}$，因此：

![推导过程](images/03644.gif)
...
方程 $y' + ay = 0$ 的解为 $y = Ae^{-ax}$。

![一阶齐次方程示例](images/03653.gif)

解为 $y = Ae^{3x}$。

#### 二阶齐次方程

对于 $ay'' + by' + cy = 0$，求解步骤如下：

1. 写出特征二次方程 $at^2 + bt + c = 0$ 并求解
2. 根据根的类型写出解的形式：

| 特征根类型 | 解的形式 |
|:---|:---|
| 两个不同实根 $\alpha, \beta$ | $y = Ae^{\alpha x} + Be^{\beta x}$ |
| 一个重根 $\alpha$ | $y = Ae^{\alpha x} + Bxe^{\alpha x}$ |
| 共轭复根 $\alpha \pm i\beta$ | $y = e^{\alpha x}(A\cos(\beta x) + B\sin(\beta x))$ |

![解的三种情况](images/03656.gif)

![重根情况](images/03657.gif)

![复根情况](images/03658.gif)

**例**：
- $y'' - y' - 20y = 0$：特征方程 $t^2 - t - 20 = 0$，根 $t = -4, 5$，解为 $y = Ae^{-4x} + Be^{5x}$
- $y'' + 6y' + 9y = 0$：特征方程 $(t+3)^2 = 0$，重根 $t = -3$，解为 $y = Ae^{-3x} + Bxe^{-3x}$
- $y'' - 2y' + 5y = 0$：特征方程 $t^2 - 2t + 5 = 0$，复根 $1 \pm 2i$，解为 $y = e^{x}(A\cos(2x) + B\sin(2x))$

**为什么特征方程适用？** 将 $y = e^{\alpha x}$ 代入方程：

![代入验证](images/03662.gif)

若 $\alpha$ 是特征方程的根，则 $a\alpha^2 + b\alpha + c = 0$，从而 $y = e^{\alpha x}$ 满足微分方程。

对于重根情况，$y = xe^{\alpha x}$ 也是解，因为此时不仅有 $a\alpha^2 + b\alpha + c = 0$，还有 $2a\alpha + b = 0$。

![重根验证](images/03665.gif)

### 14.5 非齐次方程与特解

对于非齐次方程，全解为 $y = y_H + y_P$，其中 $y_H$ 是齐次解，$y_P$ 是一个特解。

**求特解的思路**：根据方程右边 $f(x)$ 的形式猜测特解的形式，代入原方程确定待定常数。

| $f(x)$ 的形式 | 特解 $y_P$ 的形式 |
|:---|:---|
| $n$ 次多项式 | $n$ 次一般多项式（$ax^n + bx^{n-1} + \cdots$） |
| $e^{kx}$ 的倍数 | $Ce^{kx}$ |
| $\cos(kx)$ 或 $\sin(kx)$ 的倍数 | $C\cos(kx) + D\sin(kx)$ |
| 上述形式的和或积 | 对应形式的和或积（合并常数） |

**冲突处理**：若猜测的特解形式已包含在齐次解中，则应乘以 $x$（或 $x^2$）以消除冲突。

**例**：$y'' - 3y' + 2y = 7e^{2x}$。
- 齐次解 $y_H = Ae^{x} + Be^{2x}$
- 特解形式 $Ce^{2x}$ 与 $y_H$ 冲突（$B$ 项），故改为 $y_P = Cxe^{2x}$

![冲突解决示例](images/03711.gif)

代入得 $C = 7$，全解为 $y = Ae^{x} + Be^{2x} + 7xe^{2x}$。

若 $y_H$ 中同时包含 $e^{\alpha x}$ 和 $xe^{\alpha x}$（重根情形），则需乘以 $x^2$。

### 14.6 初值问题的完整求解

求解 IVP 的完整流程：

1. 解齐次方程，得到 $y_H$（含待定常数）
2. 求特解 $y_P$（不含待定常数）
3. 写出通解 $y = y_H + y_P$
4. 利用初始条件确定待定常数

**例**：$y'' - 5y' + 6y = 2x^2 e^x$，$y(0) = 0$，$y'(0) = 0$。

齐次解 $y_H = Ae^{2x} + Be^{3x}$，特解 $y_P = (x^2 + 3x + 7/2)e^x$，全解为：

![IVP 示例](images/03720.gif)

代入初始条件得 $A = -4$，$B = 2$：

![IVP 最终解](images/03724.gif)

### 14.7 微分方程建模

现实世界的许多问题可以用微分方程来建模。以下是一个经典的种群增长与抗生素作用的例子。

**问题**：某细菌培养以指数方式增长，每小时瞬时增长率等于细菌数量的两倍。抗生素以每小时 8 盎司的恒定速率连续注入培养皿，每盎司抗生素每小时杀死 25000 个细菌。为保证细菌不灭绝，初始数量至少需为多少？

**建模过程**：
- 无抗生素时：$\frac{dP}{dt} = 2P$
- 考虑抗生素：每小时注入 8 盎司，$t$ 小时共有 $8t$ 盎司，死亡率为 $8t \times 25000 = 200000t$
- 完整的微分方程：$\frac{dP}{dt} = 2P - 200000t$

![微分方程建模](images/03737.gif)

化为标准形式：

![标准形式](images/03738.gif)

积分因子为 $e^{-2t}$，乘以方程两边：

![乘以积分因子](images/03739.gif)

左边可写为：

![左边化简](images/03741.gif)

分部积分后解得：

![通解](images/03743.gif)

代入初始条件 $P(0) = P_0$ 得 $C = P_0 - 50000$：

![代入初始条件](images/03745.gif)

**分析**：
- 若 $P_0 = 50000$，则 $P = 100000t + 50000$，细菌以恒定速率增长，永不灭绝
- 若 $P_0 > 50000$，指数项为正，细菌增长更快
- 若 $P_0 < 50000$，指数项为负，最终细菌会灭绝

![细菌种群变化图](images/03747.jpeg)

**图 30-1**：$P_0 = 49999$ 时细菌数量随时间的变化。前 5 小时近乎线性增长，随后快速逆转，在约 6.5-7 小时间降为零。

这个例子生动展示了微分方程如何将实际问题转化为数学语言，并通过求解获得定量结论的完整过程。