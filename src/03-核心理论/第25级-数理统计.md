...
数理统计研究"如何从数据中获取信息、做出推断"。它与概率论相伴而生，却在 19—20 世纪才真正形成自己的方法论体系。以下按误差理论、推断逻辑、现代奠基三方面梳理。

### 12.1 从概率到统计：描述与分析统计的分野

早期的统计更多是"政治算术"式的描述记录。17—18 世纪，格拉特（Graunt）与哈雷（Halley）等人开始用数据刻画人口与死亡率，为统计学积累了经验基础。与此同时，概率论（由帕斯卡、伯努利、拉普拉斯发展）为从部分推断整体提供了理论工具。真正把统计提升为一门"用样本推断总体"的学科，离不开大数定律与误差理论的支撑。

### 12.2 误差分析与最小二乘法：勒让德与高斯

"如何处理观测误差"是天文学与测地学在 18—19 世纪提出的核心实际问题。勒让德（Legendre）在 1805 年正式发表最小二乘法；高斯（Gauss）声称并通常被认为更早就使用了这一方法（1795 年前后），并在 1809 年把它与"误差服从正态分布、均值即最小二乘估计"的假设联系起来，同时确立了正态分布（高斯分布）在误差理论中的地位。最小二乘法由此成为回归与分析的核心估计工具，也是现代统计推断的重要先声。

### 12.3 分布理论的形成：χ²、t 与 F 分布

为提高推断的可靠性，19 世纪末至 20 世纪初的英国统计学家系统研究抽样分布。皮尔逊（K. Pearson）提出了 χ² 分布与拟合优度检验；戈塞特（Gosset，笔名 Student）在 1908 年给出小样本下的 t 分布及其检验；费希尔（Fisher）则在其后引入 F 分布、方差分析，并系统发展了极大似然估计。这三类分布（本教程的"三大基石"）把统计推断从"依赖大样本正态近似"推进到"精确抽样分布"时代。

### 12.4 现代数理统计的确立

20 世纪，数理统计的哲学与方法论趋于成熟。费希尔（Fisher）确立了"用似然与充分统计量做统计推断"的频率学派纲领，区分了参数估计、假设检验与实验设计；内曼（Neyman）与皮尔逊（E. Pearson）则将假设检验与置信区间建立在严格的概率语言之上。到中叶，数理统计已从"数据分析技法"成长为拥有公理论证与系统推断框架的成熟数学学科，这也是本教程所覆盖内容的历史定位。
