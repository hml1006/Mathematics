...
> **一句话大白话**：只需要这个算子在两个端点"不太强"地控制住函数（弱型），它就在中间的任意"强弱尺度"上都规规矩矩有界——"两头凑合够用，中间必扎实"。
>
> **小例子**：奇异积分算子 $\mathcal{T}$ 若在 $L^1$ 上弱$(1,1)$、在 $L^\infty$ 上强$(1,\infty)$，则由Marcinkiewicz插值得知它在任意 $L^p(1<p<\infty)$ 上都有界——把端点数据"内插"到中间所有温和指数。
...
**前置依赖**：分布函数与层蛋糕表示公式、弱 $L^p$ 空间、次线性算子、Fubini-Tonelli 定理
