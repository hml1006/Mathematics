...
概率论的萌芽源于 17 世纪赌博中的计数问题，**Pascal 与 Fermat（1654）**通过通信解决了"分注问题"，Bernoulli 家族将之推进。**Jacob Bernoulli** 的《猜度术》（1713）证明了大数定理的特殊形式——伯努利大数定律，回答了"频率为何随试验增多而趋于稳定概率"。随后 **de Moivre（1730s）**与 **Laplace** 逐步发展正态逼近，使概率论从纯计数走向连续分析。

### 7.2 概率的公理化与测度论奠基

19 世纪概率论缺乏严格逻辑基础。真正的公理化归功于 **Andrey Kolmogorov** 1933 年的《概率论基础》，他以 **测度论**（Lebesgue 测度、σ-代数、可测集）为框架，用三条公理把"概率"定义为满足可数可加性的测度。这一革命把零测集、几乎处处、鞅、条件期望等概念纳入同一严格体系。在此之前，**Émile Borel（1898）**与 **Henri Lebesgue（1902）**已发展出测度与积分理论（Lebesgue 积分），为概率的公理化提供了必要工具。

### 7.3 收敛概念与强大数定律

Kolmogorov 不仅给出了公理，还证明了**强大数定律**，回答了"几乎必然地"频率趋于概率这一更精细的问题。与之相伴，概率论中出现了按点收敛、依概率收敛、依分布收敛、$L^p$ 收敛等不同收敛概念，以及中心极限定理（Lindeberg–Lévy）与特征函数方法。这些结果由 **Markov、Lévy、Cramér、Feller** 等系统推进，使"随机变量列的极限行为"成为一门严密理论。

### 7.4 现代测度论概率及其影响

以测度论为基础的概率论不仅支撑了随机过程、随机积分与统计推断，也深刻影响了泛函分析、调和分析与动力系统。**条件期望**在测度论框架下被严格定义为对 σ-代数的正交投影，成为鞅论与随机分析的支柱。如今测度论概率论已是整个随机数学共同的语言基础，本文件关于测度、可测函数、勒贝格积分、依概率收敛与两种大数定律的内容，正完整地承袭并呈现了这一由 Kolmogorov 等人奠定的体系。
