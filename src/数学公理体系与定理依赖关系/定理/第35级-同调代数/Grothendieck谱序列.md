...
> **一句话大白话**：两个"线性工具" $F,G$ 不能一蹴而就地先 $G$ 再 $F$ 颠倒次序，Grothendieck 谱序列就是一张账本，把这"次序颠倒"多出来的误差一层层算清楚，最终拼出正确结果。
>
> **小例子**：取 $F$=全局截面、$G$=某个局部运算，Grothendieck 谱序列就把"先局部取、再全局取"与"直接全局取"之间的差异用 $R^pF\,R^qG$ 的层化账目压缩成一个谱序列。
