...
金融数学的根可追溯到15-17世纪欧洲商业复兴与利息计算的实践。复利的基本思想在意大利银行家和商人那里早已运用，其数学整理出现在"利息表"与海事保险的精算中。17世纪的朗姆伯特、18世纪的比伯等学者系统研究年金与终身年金的现值，荷兰数学家赫伊更斯（Christiaan Huygens）亦涉足寿险期望寿命的计算。1725年，英国数学家多德森（James Dodson）拟定了一种按年龄缴付保费的寿险方案，被称为现代人寿保险的奠基人。19世纪精算科学的成形（如英国精算师学会的建立）使"利率、贴现、年金、投资组合"这些贯穿本教程的概念获得了规范表达：它们本质上都是"把未来现金流折现到今天"的数学问题。

### 七.2 随机游走、鞅与随机微积分

现代金融数学的理论基础来自20世纪的概率论。1900年，巴舍利耶（Louis Bachelier）在他的博士论文中首次用随机游走与布朗运动为巴黎证券交易所的期权定价，堪称金融数学的滥觞，虽然因当时缺乏严谨的理论框架而一度被忽视。20世纪40年代，伊藤清（Kiyosi Itô）建立了伊藤积分与伊藤引理，为描述股价的几何布朗运动提供了严格的随机微分方程语言。同一时期，维纳（Norbert Wiener）奠定了布朗运动的数学基础。1960年代，杜布与多位概率学家系统发展"鞅"理论，而鞅（martingale）恰好刻画了"公平博弈中不能被系统性地占便宜"的性质——这正是后来"无套利与风险中性测度"概念的核心。

### 七.3 马科维茨与有效前沿

1952年，马科维茨（Harry Markowitz）发表《投资组合选择》，开创现代投资组合理论：他把收益率的均值与方差分别视为"报酬"与"风险"，证明分散化可以降低非系统风险，并据此构造"均值—方差有效前沿"。这一工作把金融从经验判断提升为可计算的优化问题，使马科维茨获得1990年诺贝尔经济学奖，其有效前沿思想贯穿本教程的投资组合章节。

### 七.4 布莱克-斯科尔斯与期权定价

期权定价是金融数学的里程碑。1960年代，萨缪尔森（Paul Samuelson）等尝试为认股权证定价；1973年，布莱克（Fischer Black）与斯科尔斯（Myron Scholes）在松永（Robert C. Merton）的完善下发表了著名的布莱克-斯科尔斯期权定价公式，其要害在于"无套利"与"动态对冲"思想——通过持续调整一个由股票与无风险债券构成的复制组合消除风险，从而在风险中性世界下给期权定价。这一定价方法深刻依赖伊藤随机微积分与鞅测度（Girsanov变换），并催生了"希腊字母"风险敏感度分析。三人因这一贡献分获1997年诺贝尔经济学奖（布莱克已于此前去世）。同一年代，考克斯、罗斯与鲁宾斯坦（Cox, Ross & Rubinstein, 1979）提出二叉树模型，以离散树上的风险中性概率逼近连续模型，使得期权定价可被初等概率方法理解、也便于数值实现，成为教学中理想的进入口。

### 七.5 从定价到风险管理与衍生品多元化

1970年代以后，金融数学迅速向利率、信用与风险测度扩展。Black-Scholes公式被推广到更复杂的衍生品与利率期限结构（如HJM框架）；风险价值（VaR）与条件风险价值（CVaR）等量化风险工具逐渐成为金融监管与机构风控的通用语言；Copula理论为多维相依结构建模提供了便利；2008年国际金融危机则进一步凸显了这些数学模型在极端情形下的局限，推动了对随机波动率、跳跃扩散与模型风险的反思。金融市场本身也从工具性模型扩展为以Copula、随机模拟（蒙特卡洛）与数理经济为支柱的交叉学科。
