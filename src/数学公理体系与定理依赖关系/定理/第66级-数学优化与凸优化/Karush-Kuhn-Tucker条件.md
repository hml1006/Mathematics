...
> **一句话大白话**：带约束寻优时，最优点必须满足一套"会计等式"——梯度方向被多项式个约束梯度张成的锥形框住，互补松弛表明没用到的约束就"不花钱"。
>
> **小例子**：最小化 $f(x,y)$ 受 $g\le0$ 约束，KT 条件 $\nabla f=\lambda\nabla g,\;\lambda\ge0,\;\lambda g=0$ 在约束起作用处（$g=0$）成立，是判断最优点是否到位的必要（凸时充分）判据。

## 一、定理介绍

> **前置依赖**：Lagrange 乘数法、Farkas 引理（线性不等式组的择一定理）、凸函数的一阶条件、约束规范（LICQ、MFCQ 或 Slater 条件）、可行方向锥与线性化锥。
