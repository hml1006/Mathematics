...
> **一句话大白话**：曲面上某个小区域的"弯曲量"、它边界曲线的"拐弯量"与顶点处的"角度亏空"三者加加减减正好凑满 $2\pi$——局部几何被一个守恒式妥妥地框住。
>
> **小例子**：对曲面上一块由光滑测地线段围成的三角形区域，$\int_{区域}K\,dA+\sum(\text{外角})=2\pi$；特别地，常高斯曲率 $K$ 时三角形"内角和亏空"正比于 $K$ 乘面积。
...
**前置依赖**：Gauss 曲率与面积元、测地曲率、切向量场的旋转指数、Levi-Civita 联络与协变导数
