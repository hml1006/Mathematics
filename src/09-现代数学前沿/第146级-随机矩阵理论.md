...
### 七.1 随机矩阵的起源：核物理与Wigner半圆律

随机矩阵理论的诞生与20世纪中叶的核物理密切相关。1950年代，英国物理学家威格纳（Eugene Wigner）研究重原子核的能级统计时发现：对于复杂核系统，精确求解薛定谔方程几乎不可能，但能级的统计分布却呈现普适规律。为建模这一"能级排斥"现象，他引入随机的实对称矩阵（其元素独立同分布）作为哈密顿量的集合平均，并惊人地发现当矩阵维数趋于无穷时，特征值的全局密度收敛到一个半圆形状——即"Wigner半圆律"。这一"随机矩阵取代哈密顿量、用统计平均而非单次求解"的思路，是随机矩阵理论与现代核物理的奠基。威格纳1963年获诺贝尔物理学奖，其半圆律至今是随机矩阵理论的基石。

### 七.2 Dyson系综与特征值排斥

1962年，戴森（Freeman Dyson）把随机矩阵系统化为"三大经典系综"。他按时间反演对称性分出三类——正交系综（GOE，$\beta=1$，实对称）、酉系综（GUE，$\beta=2$，Hermite）与辛系综（GSE，$\beta=4$，四元数自对偶），并以Dyson指数$\beta$统一描述。戴森还发现特征值之间存在"排斥"（level repulsion）：距离很近的两个特征值出现的概率随其距离幂次递减，Wigner猜测最近邻间距分布由此近似为Rayleigh分布（$n=2$时精确）。这些"普适性"预言（梅塔-戴森猜想）此后大量被数值与解析证实，成为随机矩阵谱统计的核心内容。本教程的三大系综与Wigner猜测章节正源于此。

### 七.3 Tracy-Widom分布、行列式点过程与成果谱

20世纪末是随机矩阵精细分析的黄金期。约1994年，特雷西与维德姆（Craig Tracy & Harold Widom）证明GUE最大特征值的涨落在标度极限下服从Tracy-Widom分布（以Painlevé II超越函数表述），此分布后来在排序统计、统计学与数值分析中广泛出现。2000年代，通过Riemann-Hilbert问题与非线性速降法（Deift-Zhou），随机矩阵的核与普适性被严格证明；GUE特征值构成"行列式点过程"，其核由Christoffel-Darboux公式给出。这些使随机矩阵理论拥有一套精细的极限理论，本教程的Tracy-Widom、关联函数、核极限等章节即由此而来。

### 七.4 Marčenko-Pastur定律与样本协方差

对于高维样本协方差（如$p$个变量、$n$个样本的数据矩阵），1967年Marčenko与Pastur确定了其经验谱分布的极限（Marčenko-Pastur定律），它刻画了高维数据中特征值的聚集与阈值行为。这一结果在介观物理、高维统计、主成分分析与机器学习（近年）中极为重要，构成随机矩阵应用于数据的桥梁。本教程的Marčenko-Pastur定理正是对它的介绍。

### 七.5 从量子混沌到数论与深度学习的跨领域扩散

随机矩阵的"普适性"使它在多个学科开花：量子混沌领域，Bohigas-Giannoni-Schmit猜想（1984前后）断言不可积分量子系统能谱统计同于随机矩阵系综（与经典混沌对应）；数论领域，蒙哥马利（1973前后）发现Riemann zeta函数零点间距统计与GUE一致，这一惊人的"零点—随机矩阵"联系沟通了两大领域，甚至成为理解黎曼假设的新视角；自由概率论（Voiculescu的渐近自由性）则为随机矩阵统计提供了代数框架。近年来，自由概率与非Hermite矩阵（环形律）以及深度学习中的神经网络谱分析，进一步拓展了随机矩阵理论的应用边界。这一学科的历史，是"把庞杂数据/能谱的统计规律统一到普适极限"的历程。
