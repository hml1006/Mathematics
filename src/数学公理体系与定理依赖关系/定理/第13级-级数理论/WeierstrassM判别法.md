...
> **一句话大白话**：每个函数项都被固定常数"罩住"（$|u_n(x)|\le M_n$），只要这些常数拼成的级数 $\sum M_n$ 收敛，整串函数项级数就必定处处一致收敛——箱子摞稳了，里面的东西也就跟着整齐。
>
> **小例子**：$\sum_{n=1}^\infty\frac{x^n}{n^2}$ 在 $[0,1]$ 上取 $M_n=\frac{1}{n^2}$，而 $\sum\frac{1}{n^2}$ 收敛，所以它在该区间上一致收敛。
