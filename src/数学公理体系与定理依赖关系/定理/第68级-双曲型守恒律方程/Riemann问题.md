...
> **一句话大白话**：Riemann问题研究"左右两侧初值不同、中间是个台阶突变"的守恒律方程，看这条台阶会自动裂解成哪些基本波——激波、稀疏波、接触间断。
>
> **小例子**：左右气体密度一大一小突然相接，Riemann解通常把它拆成：向左传播的稀疏波 + 中间的接触间断 + 向右传播的激波。

## 一、定理介绍

> **前置依赖**：严格双曲系统与特征值、特征向量、自相似解与尺度不变性、Rankine–Hugoniot 跳跃条件、Hugoniot 曲线与积分曲线、隐函数定理、Lax 熵条件。
