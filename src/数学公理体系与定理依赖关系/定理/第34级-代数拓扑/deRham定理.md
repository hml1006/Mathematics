...
> **一句话大白话**：用"微分形式能不能被积分"定义的 de Rham 上同调，和用纯拓扑方法定义的奇异上同调（实系数）其实是一回事，连乘法配方都对得上——分析穿上拓扑的衣服。
>
> **小例子**：圆周 $S^1$ 上的 1-形式 $\frac{d\theta}{2\pi}$ 围一圈积分得 1，恰好对应 de Rham 上同调的非零类，与奇异上同调 $H^1(S^1)\cong\mathbb{R}$ 里的生成元吻合。

## 一、定理介绍

> **前置依赖**：de Rham 上同调（微分形式与外微分）、实系数奇异上同调、Stokes 定理、Poincaré 引理、Mayer-Vietoris 序列与五引理、楔积与杯积
