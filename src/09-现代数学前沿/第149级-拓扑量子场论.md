...
拓扑量子场论（TQFT）的土壤是量子场论中的"路径积分"，由费曼（Richard Feynman）于1940年代提出：量子论中的作用量被赋予一个由 $\mathrm{e}^{iS}$ 加权的路径积分，用于计算各种关联量，也为拓扑效应提供工具。然而无限维路径积分难以严格构造，于是物理学家与数学家思考：能否只保留"不依赖背景度量、从而对拓扑敏感"的那部分量子场论——即在流形上取值只与其拓扑（配边类）有关、而与局域度规无关的理论？1990年代前后，这一"拓扑量子场论"经由阿蒂亚（Michael Atiyah）与西格尔（Graeme Segal）的公理化在数学中落地。路径积分思想（配分函数、期望值、局部—整体原理）渗透进TQFT的每一个概念，本教程的配边范畴、不变量构造等皆受其启发。

### 七.2 Atiyah公理与TQFT的定义

1988年前后，阿蒂亚（Michael Atiyah）将TQFT公理化：一个$d+1$维TQFT是"$d$维配边范畴"到"有限维向量空间范畴"的对称幺半、张量函子，把$d$维流形$M$映射到态空间$Z(M)$，把配边$M\to N$映射到算子$Z(M\to N)$，并满足张量性、复合与单位性质。这一"配边范畴上的函子"想法（植根于西格尔的二维共形场论公理化）使TQFT成为可严格计算的数学对象，并使其只依赖于流形的拓扑而不依赖度量信息。本教程的Atiyah公理即是TQFT的严格定义。

### 七.3 Witten的Chern-Simons理论与Jones多项式

石破天惊的一步来自爱德华·威滕（Edward Witten，1989）：他提出把三维的Chern-Simons理论（一种不依赖度量的拓扑场论）中的Wilson圈期望值，与琼斯多项式（Vaughan Jones, 1984）纽结不变量对应起来，从而在同一框架里重新导出并推广了Jones多项式，并由此发展出与拓扑量子计算相关的量子不变量。Witten的"用物理Chern-Simons理论计算拓扑量"的方法，把纽结论、低维拓扑与量子场论接通，引发了量子拓扑学的爆发。它经由Reshetikhin-Turaev的构造（从量子群在单位根的表示论出发）得到严格数学实现。本教程的三维Chern-Simons与Jones多项式、Reshetikhin-Turaev、量子群与TQFT等章节正源于此。

### 七.4 量子群、Frobenius代数与2维TQFT分类

TQFT强烈依赖代数结构。在2维情形，由于分/曲面结构简单，可以完全分类：一个2维TQFT精确相当于一个（可交换）Frobenius代数，其代数数据（积、余积、单位、对合）正好由配分函数确定。Frobenius代数的概念源自十九世纪的德国数学家费罗贝尼乌斯（Ferdinand Georg Frobenius），现代经Kock等系统阐述后发现其与2维拓扑的深刻对应，此对应在弦图/图示微积分中得到可视化呈现。同时，量子群（Kulish-Reshetikhin、Drinfeld、Jimbo，1980s，源于量子杨-巴克斯特方程与可解模型）为"模块化张量范畴"提供构造源，进而用于三维TQFT与拓扑量子计算。本教程的弦图与图示微积分、2维TQFT分类、量子群等章节即属此类。

### 七.5 扩展TQFT、Bordism假设与数学物理的当代交汇

在现代数学中，TQFT被推广到"扩展TQFT"与"高阶/$\infty$-范畴"框架：卢里（Jacob Lurie）提出并部分证明了坍缩公设（Bordism假设/Cobordism Hypothesis），断言完全扩展TQFT由其在"点"上的值——一个对称单性$\infty$-范畴中的对象——完全决定，从而为TQFT的分类提供统一纲领。TQFT与Donaldson不变量等4维不变量（经Floer同调、瞬子模空间、以及Khovanov的同调不变量）联系，刻画光滑4维流形的结构。物理侧，拓扑序、量子纠缠与高维场论继续为TQFT注入新素材。拓扑量子场论的历史，是把"路径积分/量子场论"这一物理直觉，通过公理化与范畴工具落地为可计算的拓扑不变量的历程。
