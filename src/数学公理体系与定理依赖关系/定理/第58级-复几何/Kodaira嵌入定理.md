...
> **一句话大白话**：只要紧复流形上有一个"足够正"的线丛（正线丛），它就能整体放进高阶射影空间，复结构像图画一样"嵌入"——"正性线丛 ⇒ 射影性"这一再造，是把射影几何命运钥匙递给复流形分类的关键句。
>
> **小例子**：紧复流形 $M$ 上有正线丛 $\mathcal L$（第一陈类可被 Kähler 类表示）时，$M$ 可全纯嵌入到某个 $\mathbb P^N$，且 $H^0(M,\mathcal L^{\otimes k})$ 足够大给出坐标——嵌入与坐标全来自 $L$ 的高冪截面，像线丛升高冪造出"坐标图册"。
