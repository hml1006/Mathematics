...
> **一句话大白话**：Ext 和 Tor 是"衡量搞不定之事的两个量杯"——Tor 量模块间的纠缠/扭曲程度（像计算两个几何体的乘积时多出的扭曲重叠部分），Ext 量从模块到对偶空间的延伸次数（衡量同态的分歧）。
>
> **小例子**：$\mathrm{Tor}_1^{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z}/2)\cong\mathbb{Z}/2$ 记录阿贝尔群的纠缠；$\mathrm{Ext}^1_{\mathbb{Z}}(\mathbb{Z}/2,\mathbb{Z}/2)\cong\mathbb{Z}/2$ 记录扩张的种类，它们都能从分解算出。
