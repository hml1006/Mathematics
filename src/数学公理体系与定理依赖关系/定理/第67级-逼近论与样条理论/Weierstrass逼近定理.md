...
> **一句话大白话**：闭区间上随便一尊连续曲线，都能被一堆多项式"包到任意贴身"——想多贴就有多贴，多项式虽简单却能以任意精度复刻连续函数。
>
> **小例子**：$f\in C[a,b]$ 对任意 $\varepsilon>0$ 都存在多项式 $p$ 使 $\|f-p\|_\infty<\varepsilon$；构造上可取 Bernstein 多项式 $B_n(f)(x)=\sum_{k=0}^n f(k/n)\binom{n}{k}x^k(1-x)^{n-k}$ 均匀收敛于 $f$。

## 一、定理介绍

> **前置依赖**：Bernstein 多项式的定义与恒等式、闭区间上连续函数的一致连续性、二项分布的均值与方差（大数定律思想）、上确界范数与一致收敛。
...
- [Weierstrass 逼近定理（第104级-逼近论）](../第104级-逼近论/Weierstrass逼近定理.md)：与本条目为同一定理，另收录于第104级-逼近论，可交叉参考。
