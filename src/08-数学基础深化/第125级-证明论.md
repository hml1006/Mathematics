...
> 证明论的历史主线，是追问"一条证明到底有效到什么程度"。它从希尔伯特为整个数学奠基的乐观纲领出发，被哥德尔的不完全性定理当头浇醒，又由根岑用自然演绎、序贯演算与切消定理重建，最终发展成一门用超穷序数量度"证明能强到什么地步"的学科。整部历史可概括为"证明从不可揣摩的黑箱，变成了可被剖析、可被比较、可被赋大小的结构"。

### 形式主义的前夜：从欧几里得到希尔伯特

- **证明的传统形象**：从《几何原本》至今，证明被理解为"从公理经逻辑推演得出结论的序列"，但"怎样的推演步骤才算合法"长期靠直觉把握，不同教师的演绎风格五花八门，无人给它文法。
- **逻辑语言的精确化**：19 世纪末，Gottlob Frege（戈特洛布·弗雷格）、Giuseppe Peano（朱塞佩·佩亚诺）与后来 Russell 的《数学原理》把证明落成符号推演，使"证明=依规则对符号串操作"第一次成为可能，也让"证明可以被当作数学对象来研究"不再是空想。
- **希尔伯特的元数学构想**：Hilbert 在 1900 年前后力主把算术乃至全部数学写成可机械检查的形式系统，再用有限的元数学手段证明它不会导出矛盾；"形式系统（公理＋推理规则）"与"元数学"两个概念由此成形，证明论的设问方式随之登台。

### 希尔伯特计划：为数学而建的"防火环"

- **计划的核心**：David Hilbert（大卫·希尔伯特）设想，用一条**有限主义**（finitism）的元数学之路证明皮亚诺算术乃至全部经典数学的一致性，让数学成为不怕"集合论式的悖论"攻打的坚固堡垒。
- **判定问题与可证性**：希尔伯特同时把"判定一个公式是否可证"（Entscheidungsproblem）列为重要问题，它与"证明可否机械化"缠在一起，为证明论和递归论（第 124 级）都提了问。
- **公理化纲领的收成**：即使计划后来受挫，其副产品仍然丰厚：形式系统、可证性谓词、元数学语言这些概念被固定下来，为认识"证明"提供了前所未有的清晰镜头。

### 哥德尔不完全性定理：第一声惊雷

- **第一不完全性定理**：1931 年 Kurt Gödel（库尔特·哥德尔）用**哥德尔数**把"公式""证明"编码为自然数，并构造出自指的**说谎者式语句** $G$：它"说"自己不可证。结果证明：任何足够强的一致可公理化系统（如皮亚诺算术 PA），都存在既不可证也不可否证的语句——这就是第一不完全性定理。
- **第二不完全性定理**：定理的延展进一步断言：这样的系统无法在自身内部证明自身的一致性（即 $G$ 与一致性语句 $\mathrm{Con}$ 都成为"不可证可断"的边界语句）。Rosser 后来把证明改良为只需普通一致性即可，使结论更犀利。
- **对希尔伯特计划的冲击**：哥德尔的工作并未推翻形式化的全部用处，却击碎了"用有限元数学证明全部数学一致"这一最大目标；它同时证明形式系统足以承载全部能行算术的语法——不完备与强大并存。

### 根岑的重建：自然演绎与序贯演算

- **自然演绎**：Gerhard Gentzen（格哈德·根岑）1935 年创立**自然演绎**（Natural Deduction）与**序贯演算**（Sequent Calculus，LK 经典／LJ 直觉主义），让每个连接词都有引入/消去规则，证明成为一棵可逐拍检验的推导树或推导序列，证明的通路第一次被可视化。
- **对称的序贯演算**：序贯演算以"$\Gamma\Rightarrow\Delta$"（由前提序列到结论序列）为对象，其切割（cut）规则对应逻辑规则的对偶，为切消等元定理提供了舞台，也奠定了现代证明论记挑战的核心语法。
- **证明即对象**：由此"证明可以化简、可以比较、可以做结构归纳"变成可操作的事实——证明不再只是一长串符号，而是有形状、有规模、可被解剖的数学实体。

### 切消定理与证明的规范形

- **切消定理（Hauptsatz）**：根岑 1936 年证明**切消定理**：任何 LK 证明都能在不改变结论的前提下消去所有 cut——即证明可被规范化、化简为不含冗余中间步骤的正规形。
- **子公式性质**：切消定理的推论是**子公式性质**：正规形证明中每个出现的公式都是结论或前提的子公式，这直接给出了一致性证明的钥匙，也让"证明能否变干净"成为可判定的问题。
- **证明论谱系的开端**：切消定理同时引入了"用小证明证明大语句"的结构思想，与后来"把证明映射到序数"的序数分析一脉相连，是证明论成为一门"证明可以被赋予大小"的学问的重要一步。

### 序数分析：度量证明的强度

- **ε₀ 与 PA**：在皮亚诺算术之外允许超穷归纳到序数 $\varepsilon_0=\omega^{\omega^{\omega^{\cdots}}}$，根岑就能给出 **PA 的一致性证明**，并把超出可证明最强的可计算函数的增长水平与 $\varepsilon_0$ 联系起来——这就是序数分析的肇始。
- **更大的序数**：随后，Takeuti、Schütte、Feferman 等把序数分析推进到 $\Gamma_0$（Gamma_0，首个 $\phi$-函数不动点）乃至更大的可计算序数，用来横向比较由扩展公理、扩充语言、建更强记法所构成的系统谁更强、谁更能证明收敛。
- **快增长谱系**：可计算序数与**快增长函数谱系**（如 Ackermann 函数相对 H 谱系、ε<sub>0</sub> 对应的 Hardy 函数等）被联结起来，使"这个系统一致性能证明多强的总函数"变成可精确标度的学科。

### 不完备之后：记法、可证性逻辑与弱算术系统

- **可证性逻辑**：Arithmetical 可证性谓词（满足 G1−G3 条件）引出**模态可证性逻辑 GL**，用 $\Box\varphi$ 表示"$\varphi$ 可证"，系统性研究"什么关于可证性的公式恒真"（如 Löb 定理），这是不完备性在元理论层的精致化。
- **弱算术与代换引理**：一阶算术的子理论（如 $I\Sigma_1$、$I\Delta_0$）、代换引理与不可判定性的逐层分析，让证明论可以研究"到底需要多强的归纳才够用"，也让"可证性与增长"的图景更精细。
- **反向数学**：Friedman、Simpson 等把"整条定理究竟需要哪些集合存在公理"反过来追问（反向数学），以弱算术层次给数学定理排序，成为证明论与集合论之间的桥。

### 现代证明论与技术验证：从纸面到机器

- **证明助手**：切消、正规化与类型论（第 127 级）的结合催生了 Coq、Lean、Agda 等**证明助手**，让"机械核验证明"从希尔伯特的可想变为可做的现实；数学结构被编码为类型正确的程序，证毕与否交给机器判定。
- **一阶与高阶的连接**：现代证明论继续探索 $\Pi^1_2$ 以上系统的序数（尚无干净记法），并借助重写系统、规范对象的计算内容，把理论推进到可控且可实现的边界。
- **继承的意义**：从希尔伯特"整个数学一致性"的宏大愿景，到根岑的切消，再到证明助手的百万行证明库，"把证明本身交给精确的计算"始终是贯穿证明论的一条主线——即使目标被一再缩小，形式化的愿望却从未熄灭。

### 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1879 年 | 弗雷格《概念文字》 | 谓词逻辑形式化，证明可符号化 |
| 1889 年 | 佩亚诺 | 算术公理体系 |
| 1900 年代 | 希尔伯特 | 元数学与形式系统构想 |
| 1920 年代 | 希尔伯特 | 希尔伯特计划与有限主义纲领 |
| 1931 年 | 哥德尔 | 第一、第二不完全性定理 |
| 1935 年 | 根岑 | 自然演绎、序贯演算（LK/LJ） |
| 1936 年 | 根岑 | 切消定理（Hauptsatz）及 $\varepsilon_0$ 序数分析 |
| 1950—1970 年代 | Takeuti、Schütte、Feferman | 序数分析推进到 $\Gamma_0$ 等更大记法 |
| 1970—1980 年代 | Friedman、Simpson | 反向数学：按证明所需公理给定理排序 |
| 1970 年代 | 索洛韦 | 可证性逻辑 GL 的完全性与 Löb 定理 |
| 1984— | Coq、Lean、Agda | 证明助手：类型论与切消相结合 |
| 现代 | Friedman 等 | 反向数学的推进与证明强度的精细谱系 |

### 小结

> 证明论的历史，是一部把"证明"从直觉的黑箱逐步锻造成精确对象的过程。希尔伯特提出形式化与有限主义的元数学纲领，让"证明可否机械化、数学可否一致"成为可严格讨论的问题；哥德尔用不完全性定理宣告"有限手段证全部一致"此路不通，却同时确立了形式系统足以承载算术这一面；根岑以自然演绎、序贯演算与其切消定理重建这座大厦，用 $\varepsilon_0$ 的序数分析让"证明所能达到的强度"首次被精确度量。此后可证性逻辑、反向数学、弱算术系统与证明助手不断接力，使证明论既能在元理论层面做精密分类，又能服务于数学机械核验的工程实践。希尔伯特的愿景虽被缩小，形式化的火光却穿越一个世纪，仍在前行。
