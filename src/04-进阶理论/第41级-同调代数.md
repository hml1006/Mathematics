...
### 11.1 从拓扑到代数：同调与上同调的诞生

同调论的思想起源于**庞加莱（Henri Poincaré, 1854–1912）**在 1895 年《位置分析》（*Analysis Situs*）中开辟的代数拓扑：他把空间的"洞"与"亏空"代数化为**同调群**（当时的 Betti 数与挠系数）。**上同调（cohomology）**作为对偶构造，则稍后由 **Alexander、Kolmogorov、Čech、Steenrod、Veblen** 等在 1930 年代系统发展。本章"来龙去脉：复形与同调从哪里来"正是对这段谱系的回顾：链复形、链映射与同伦、蛇形引理等，均从拓扑空间上链单位同调群的计算规律抽象出来。**正合序列**与**同调代数基本引理**（蛇形引理）则是把各分支缝合成统一工具的第一步。

### 11.2 导出函子与 Tor/Ext

从正合序列到"探测缺失部分"是导出函子的核心动机。**嘉当（Henri Cartan, 1904–2008）**与**埃伦伯格（Samuel Eilenberg, 1913–1998）**在 1956 年合著的《同调代数》（*Homological Algebra*），系统建立了投射分解、内射分解与**导出函子（derived functor）**、**Tor** 与 **Ext** 函子，把同调方法推广到 Abel 范畴的抽象层面。导出函子与合取、张量积的联系早期由 **Mac Lane** 与 **Eilenberg–Mac Lane** 在分类空间与上同调维数的研究中铺垫。本章的投射/内射分解、导出函子、Tor、Ext 与平坦分解，正是"同调代数"作为独立学科的核心。

### 11.3 范畴语言与 Abel 范畴

把同调代数"语言化"到范畴论，是现代数学革命的一个关键节点。**麦克莱恩（Saunders Mac Lane）**与**埃伦伯格（Samuel Eilenberg）**在 1945 年引入**范畴**、**函子**与自然变换等概念，旨在统一拓扑与代数的构造（如基本群、同调函子）；**格罗滕迪克（Grothendieck）**在其关于 Abel 范畴的论著中定义了**Abel 范畴**，使同调代数可以在任意 Abel 范畴（而不是仅模范畴）中通行，并由此发展出层上同调与 **Serre 对偶**。本章"范畴语言、Abel 范畴、正合序列与函子"正出自这一确立性的思潮。

### 11.4 谱序列与同调计算

当同调难以直接计算时，**谱序列（spectral sequence）**提供"层层逼近"。**让·勒雷（Jean Leray）**在二战期间（1940 年代初）为研究纤维空间的同调创立了谱序列，其后被**Koszul、Serre、Borel**等广泛使用并推广为**Grothendieck 谱序列**与 **Leray–Serre 谱序列**；**Eilenberg–Moore**与 **Cartan–Eilenberg**（消解上的格）完善了代数版本。本章"双复形的谱序列、Grothendieck 谱序列、Leray–Serre 谱序列"正是这一"算不动时逐层逼近"思想的产物。**Lyndon–Hochschild–Serre 谱序列**同时服务于**群上同调**与**Hochschild 上同调**（量子形变与环的 Hochschild–Kostant–Rosenberg 定理），**层上同调**则直接孕育了代数几何中 Serre 对偶与贯穿的现代工具。

### 11.5 导出范畴与现代发展

1960 年代后，**格罗滕迪克**与**韦尔迪耶（Jean-Louis Verdier）**提出**导出范畴（derived category）**与**三角范畴（triangulated category）**，把"同调相同"提升为"导出等价"，使同调代数作为自然对象的研究范畴运作；**Beilinson–Bernstein–Deligne–Gabber** 的分解定理与 D 模理论进一步将导出方法运用于表示论与代数几何。今日，同调代数的语言（导出范畴、$t$-结构、Künneth）已贯通代数拓扑、代数几何、表示论与数论（Fukaya 范畴、稳定性条件）——这正是本章从链复形走到导出范畴再指向现代方向的脉络。
