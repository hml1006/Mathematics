...
> **一句话大白话**：BMO说的是函数"在任何一块小范围里的数值，和它在该范围的平均值都差不太多"——允许整体上下乱跳，但拒绝在某处突然"高昂"地偏出，是衡量函数"局部乖不乖"的指标。
>
> **小例子**：$\log|x|$ 在 $\mathbb{R}$ 上无界却能落入BMO，因为它的每次局部平均值都稳稳盖住自身的震荡；相比之下 $e^{x}$ 之类"振幅失控"的函数就进不了BMO。
