...
几何分析 (geometric analysis) 是"用偏微分方程研究几何"的学科：几何结构（度量、曲率、子流形）被编码成微分方程的解，而方程的深刻性质反过来揭示几何与拓扑。它虽在二十世纪才作为一个领域成形，其源流却可一直追溯到位势论、变分法与黎曼几何的早期。

### 7.1 从调和函数到谱几何

几何分析的第一支柱是分析工具与几何的结合。十九世纪的位势论（Laplace 方程、调和函数）提供了最朴素例子；黎曼 (Riemann) 提出黎曼度量后，高斯曲率与 Laplace-Beltrami 算子的关系被逐步揭示，Christoffel 等为该算子奠定了张量分析框架；1960 年代起 Kac 的著名问题"能否听到鼓的形状"又把谱几何推向前台。Hodge 于 1930–40 年代建立 Hodge 理论，把黎曼流形上的微分形式分解为调和形式与精确形式之直和，从而把拓扑（Betti 数）纳入分析框架，这是几何分析与代数拓扑合流的关键一步。

### 7.2 几何中的变分法与极小子流形

几何分析的第二个支柱是变分法。把"面积"视为子流形的能量泛函，其驻点即极小/极小化子流形，这一思想来自柏拉图式的肥皂膜问题，并因道格拉斯 (Douglas) 与拉多 (Radó) 在 1930 年代利用变分法解决 Plateau 问题而成为严格理论。极小子流形、调和映射与能量泛函的临界点由此成为研究曲率、拓扑与刚性的重要工具，并与正质量定理深刻地交织在一起。

### 7.3 曲率比较、Bochner 技巧与刚性

黎曼几何早期最重要的定量工具是曲率比较定理：Bonnet-Myers 定理（正 Ricci 曲率 → 直径有界 → 紧致）与 Bishop-Gromov 体积比较，把曲率下界翻译成尺度与体积的约束。Lichnerowicz 用 Bochner 技巧给出紧致正 Ricci 流形上第一非零本征值的下界，并由此推出消失/刚性结论（正 Ricci 与第一 Betti 数零等）。Bochner 技巧——对微分形式/张量做 "$L^2$ 模加上曲率项"的恒等式——成为几何分析的通用杠杆。Sobolev 不等式与等周不等式在流形上的推广（源于 Sobolev 的嵌入理论、等周问题的变分-对称化之旅）则提供了分析所需的范数工具。

### 7.4 Ricci 流、标量曲率与现代突破

几何分析最富传奇的篇章是 Ricci 流。Hamilton 于 1982 年引入 Ricci 流 $\frac{\partial}{\partial t}g=-2\,\mathrm{Ric}$，希望通过流把任意黎曼度量"光滑化"成常曲率度量；Perelman 于 2002–2003 年给出熵单调公式与不塌缩，并完成 Poincaré 猜想与更一般的几何化猜想的证明。Yamabe 问题（每一共形类里是否存在常标量曲率度量）由 Yamabe 于 1960 年提出，经 Trudinger 的修正与 Schoen-随后的解，最终在 1980 年代由 Schoen 借助正质量定理解决；正质量定理（Schoen-Yau 用极小曲面对割空间方法、Witten 用旋量方法）本身就把"标量曲率非负"与引力/能量联系起来。一般认为，Ricci 流、极小曲面与比较定理的汇合，使几何分析成为二十一世纪拓扑与几何变革的核心工具。
