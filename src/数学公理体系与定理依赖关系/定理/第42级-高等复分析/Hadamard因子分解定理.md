...
> **一句话大白话**：一个有"有限个子几级"的整函数，可以拆成一个"温和的多项式"乘以一串"以零点为主部的标准因子"（外加一次正确的收敛兜底指数）——把整函数按零点"拆积"成可驾驭的因子块。
>
> **小例子**：$\sin z$ 是整函数，Hadamard 分解成 $z\prod_{n\neq0}(1-\frac{z}{\pi n})e^{z/(\pi n)}$——用全部零点读出函数骨架，成长速度决定兜底指数。
