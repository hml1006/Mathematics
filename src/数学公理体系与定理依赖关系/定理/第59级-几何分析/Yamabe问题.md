...
> **一句话大白话**：在任意封闭曲面或高维流形上，总能找到一种"拉伸但保持角度关系"的变形（共形变换），让曲率到处都一样——处处等于同一个常数。
>
> **小例子**：对流形上一度量 $g$，寻找函数 $u>0$ 使 $\tilde g=u^{\frac{4}{n-2}}g$ 具有常数标量曲率，等价于求一个半线性椭圆方程的解。

## 一、定理介绍

> **前置依赖**：标量曲率的共形变换公式、临界 Sobolev 嵌入、变分法直接法与椭圆正则性、正质量定理、Weyl 张量与局部共形平坦。
