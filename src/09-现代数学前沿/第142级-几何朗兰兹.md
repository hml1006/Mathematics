...
几何Langlands是Langlands纲领在一个"几何化"语境下的类比。经典的Langlands纲领在数域上将Galois表示与自守表示对应；1980年代末到1990年代初，朗兰兹本人以及他的学生、合作者（如Frenkel、Gattsgory、Kapustin等）意识到，如果把"数域"换成"有限域上的曲线"或"复曲线"（代数曲线），把"Galois群"换成"基本群"，把"自守形式"换成"D-模"（整层的模），就能得到一个几何版的对应：把曲线$C$的$G$-丛模空间$Bun_G$上的D-模与$C$上的$\check{G}$-局部系统对应。这样，"数论—表示"的两侧都被翻译成代数几何语言，从而可以用导出范畴、D-模与模空间等成熟工具研究。这一"几何化"并非只是为了换个视角，而是使原先深奥的对应变得可以严格构造、可以计算。

### 七.2 Bun_G、Hecke特征值与模空间

几何Langlands的两大舞台是$Bun_G$（$G$-丛的模空间）与$\mathrm{LocSys}$（局部系统的模空间）。$Bun_G$上的Hecke算子（其在点$x$处把$G$-丛沿某一分化修改）生成"Hecke代数"作用；一个D-模被称为"Hecke本征D-模"，若它对Hecke算子按某个局部系统特征值作用。这一"Hecke特征值"正是自守侧（分析侧）要求"像特征函数"的几何化身。模空间$Bun_G$虽然是无限维、非紧的一般空间，但在Beilinson-Drinfeld、Gaitsgory等的理论中仍可用代数几何的（稳定、导出）语言处理。本教程的Bun_G、Affine Grassmannian、Hecke算子与Hecke特征值章节即围绕这些模空间展开。

### 七.3 Beilinson-Drinfeld定理与GL(1)情形

几何Langlands最简洁的完整结果来自$\mathrm{GL}(1)$情形：通过Fourier-Mukai变换与Poincaré线丛，$Bun_{\mathrm{GL}(1)}$上的D-模与$C$上的$\mathrm{GL}(1)$-局部系统一一对应，这正是"阿贝尔几何Langlands"。对一般$\mathrm{GL}(n)$或一般约化群$G$，核心由Beilinson与Drinfeld在1990年代深入推进：他们把"Hecke本征D-模"与"$\check{G}$-局部系统上的（拟）凝聚层"之间的等价（即几何Langlands对应）建立为猜想与部分定理，并以导出范畴化为最终目标。这一工作（Drinfeld的shtuka与Beilinson-Drinfeld的体系）使几何Langlands成为当代代数几何与表示论最活跃的领域之一。

### 七.4 Kapustin-Witten与物理对偶

几何Langlands在21世纪初获得物理视角的"加持"。2006年前后，Kapustin与Witten等人从$\mathcal{N}=4$超Yang-Mills理论的S-对偶（电-磁对偶）出发，把几何Langlands对应重新解释为与"朗兰兹对偶群"相关的对偶现象，从而把几何Langlands与弦论、量子场论中的对偶性联系起来。这种物理联系不仅加深了对几何Langlands的理解，也反哺了数学（如对$\mathcal{N}=4$理论、边界条件与模空间的研究）。本教程的Kapustin-Witten对偶与S-duality章节正是这一进展。

### 七.5 几何化朗兰兹的范畴化与前沿

几何Langlands的一个重要方向是"范畴化"：把对应的两侧都视为$\infty$-范畴/导出范畴，甚至把谱、稳定条件纳入，从而得到"范畴化的几何Langlands"。这一高阶框架（Gaitsgory、Arinkin、Lafforgue等）与"几何Satake对应"（把表示论与周遍层、仿射Grassmannian上的层联系起来）紧密相关。当前，几何Langlands仍有多条主线（导出范畴等价、Arinkin-Gaitsgory猜想、与p-adic Langlands的联系）处于活跃研究之中。其历史意义在于：它为经典Langlands纲领提供了可构造、可范畴化的几何模型，并深刻影响量子场论中的对偶性与现代代数几何。
