...
复几何 (complex geometry) 研究复流形及其上的几何结构，是代数几何、微分几何与分析（偏微分方程）交汇的中间地带。它从黎曼对"单值化/共形结构"的洞察出发，经嘉当、小平邦彦、Kodaira、Chern 与 Yau 等人的工作，成长为以里奇流、Calabi-Yau、Kähler 几何为主题的现代方向，甚至深刻影响了物理中的弦论。

### 1. 从黎曼曲面到复流形

复几何的源头之一是黎曼 (Riemann) 对"黎曼曲面"的研究：他意识到一个拓扑曲面可以携带复结构，复分析（全纯函数）由此被赋予几何意义。二十世纪初，嘉当 (Élie Cartan) 等把"复流形"作为独立的现代对象定义：它是在每点附近与 $\mathbb{C}^n$ 的一个开集全纯等价、且坐标变换为全纯的一类流形。全纯函数、Hodge 分解与 de Rham/Dolbeault 上同调分别在黎曼曲面与高维复流形上被逐步推进，黎曼曲面上的单值化与紧黎曼曲面分类成为复几何的经典先例。

### 2. Kähler 几何、Chern 类与热论

Kähler 流形是携带"辛 + 复 + 黎曼"三者兼容度量的一类复流形，其度量由 Kähler 势定义；Kähler 度量自 1930 年代（Kähler）起成为复几何最自然的框架。陈省身 (S. S. Chern) 于 1940 年代中期建立陈类（Chern 类）理论：用曲率构造代表上同调类的多项式不变量，把复几何的"扭转"编码为拓扑不变量（第一陈类等），并随后发展出 Chern-Weil 理论的几何—拓扑桥梁。Hermite 度量、Chern 联络与 Levi-Civita 联络的关系，以及 Kähler 流形上 Hodge 理论的特例（一系列 Hodge 数与 Lefschetz 分解）在这一时期被系统建立。

### 3. 小平邦彦、Kodaira 与代数几何序列

复几何与现代代数几何交替推进的标志性成果来自小平邦彦 (Kunihiko Kodaira) 与 Spencer 等的分析—代数方法。Kodaira 的消灭定理与嵌入定理：具有正线丛的复流形能嵌入复射影空间（"每个 Kähler 流形都在某个意义下代数化"），配合小平（Kodaira–Nakano）消灭定理、Lefschetz 超平面定理，以及形变理论（Kodaira-Spencer），把复流形与代数簇的疆界打通。Riemann-Roch 定理的 Hirzebruch 泛化（全纯线丛的 Euler 特征数可由陈类与 Todd 类的积分给出）是这一分析—代数汇流的里程碑。

### 4. Calabi 猜想、Kähler-Einstein 与 Calabi-Yau

复几何最著名的现代篇章是 Calabi 猜想与 Yau 定理。卡拉比 (E. Calabi) 在 1950 年代猜想：给定 Kähler 流形上符合相容性条件的一个"体势"，可唯一解出相应 Ricci 曲率；丘成桐 (S.-T. Yau) 于 1976–1978 年证明这一猜测，即著名的 Calabi-Yau 定理，由此得到第一陈类为零的 Kähler-Einstein 度量（Calabi-Yau 流形）。Kahler-Einstein 度量的存在性——在正/零/负第一陈类情形——把复几何与几何流、非线性 Monge-Ampère 方程深刻联系起来；人们一般认为，Yau 定理与随后在弦论中 Calabi-Yau 紧化扮演的角色，使复几何成为连接纯数学与理论物理的最活跃分支之一。
