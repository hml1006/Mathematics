...
> 渐近分析的主线是"当变量趋于极端（$\to\infty$ 或 $\to0$）时，函数如何行为"：它起源于欧拉、斯特林、拉普拉斯对特殊函数与积分的局部近似，因"形式上发散却实际上有用的级数"而一度名声狼藉；1886 年庞加莱给出严格定义，把它从被质疑的对象变为可计算工具；随后 Laplace 方法、驻相法、最速下降法与 WKB 等复合技术把"主项来自一个点"的直觉系统化，使渐近分析成为量子力学、流体力学与数论的常备武器。本章沿"疑难级数→形式化→复合方法→物理应用"的线索，追述这门"近似却严谨"的学问如何定型。

### 6.1 渐近级数的混沌起源：发散却有用

- **欧拉的悖论级数**：欧拉研究级数 $\sum n!x^n$ 时发现：它对任何 $x>0$ 都发散，却仍能对较大的项做出极好的局部近似——"发散却有用"的级数由此登上台面，却也让严谨数学家困惑。
- **斯特林与阶乘**：1730 年斯特林给出阶乘的渐近公式，欧拉独立得到并改进——n! 这类"离散对象"与 $\sqrt{2\pi n}\,(n/e)^n$ 这类"连续公式"之间的渐近等价，是渐近分析最经典的出生证。
- **形式运算的信赖**：18—19 世纪，Euler、Laplace 在近似天文与物理问题时大量使用形式上不收敛的展开，取得惊人数值精度；但"为什么不在乎发散"在当时并无解释，只能靠经验信任。

### 6.2 庞加莱的形式化：1886 年的严正定义

- **庞加莱的定义**：1886 年，庞加莱在《天体力学的分析方法》中给出严格定义：$f(x)\sim\sum_n a_nx^{-n}$ 指对每个固定 $N$ 都有
$$f(x)-\sum_{n<N}a_nx^{-n}=o(x^{-N}).$$
他把"渐近展开"从不被信任的对象升格为严格意义下的收敛（逐项收敛于首项意义）之级数。
- **$O$/$o$ 符号**：庞加莱还规范化了 $O$/$o$ 记号的使用，使"量级估计"成为可操作的语法。
- **意义**：形式化让渐近级数从"魔术"变成"可验证的近似尺度"——能讨论其主项、控制误差、逐项运算而不至于误判，这正是它得以进入严格分析的原因。

### 6.3 Stirling 公式与 Euler-Maclaurin 求和

- **Stirling 公式**：$n!\sim\sqrt{2\pi n}\,\left(\frac{n}{e}\right)^{n}$ 是渐近分析的招牌例子；Stieltjes 等在十九世纪完成严格化，使其成为精确到任意阶的渐近级数。
- **Euler-Maclaurin 求和**：把求和化为积分再加无穷级数修正的 Euler-Maclaurin 求和公式，由欧拉与麦克劳林在 1740 年代独立发现——它把离散求和与连续积分连接起来，是数值分析、数论（如 $\zeta$ 函数）与渐近级数共用的枢纽。
- **Padé 逼近**：1880 年代 Padé 提出以有理函数逼近幂级数以捕捉发散级数的极点与奇点，为"发散却承载信息"提供了补救性工具。
- **意义**：这组工具使渐近分析拥有了可算可证的第一批"标准零件"。

### 6.4 Laplace 方法：主项来自最小值点

- **积分的单峰直觉**：Laplace 方法处理积分 $\int e^{-x\phi(t)}g(t)\,dt$：当 $x\to+\infty$ 时，被积函数集中到 $\phi$ 取最小值的点附近，主项由局部二次近似给出。
- **由拉普拉斯到 Watson 引理**：拉普拉斯本法启发下，Watson 引理把这类局部论证严格化为对拉普拉斯型积分主项的精确估计，成为渐近积分的标准出发点。
- **意义**：Laplace 方法提供了"主项来自一个点"的直觉范本，可以被系统复用在各种积分上（第 102 级大偏差理论将再次遇到它）。

### 6.5 驻相法与最速下降：振荡与复平面

- **驻相法**：处理振荡积分 $\int e^{\mathrm ix\phi(t)}\,dt$，把贡献集中在相位 $\phi$ 的驻点（$\phi'=0$）处，由黎曼-勒贝格式的相消给出主项——这是光学与波动现象里的主力工具。
- **最速下降（鞍点法）**：1909 年前后 Debye 为研究光学衍射引入复平面内的路径变形，沿"最速下降"路径走，从而兼得 Laplace 与驻相之长；配合鞍点的局部二次展开，主项由鞍点处的值决定。
- **意义**：这两法把"主项集中在某点"的直觉从实轴战场迁到复平面的鞍点战场，是量子力学半经典与积分渐近的核心引擎。

### 6.6 摄动论：小参数下的正则与奇异

- **正则摄动**：解在参数 $\to0$ 时平滑变化的摄动问题，可直接按小参数幂级数展开求解。
- **奇异摄动与边界层**：当小参数导致解在小尺度上急剧变化（边界层、转向点）时，摄动失效；Prandtl 1904 年的边界层理论在流体力学中确立了奇异摄动的地位，匹配渐近展开（在不同尺度上成立的局部渐近解拼接）成为处理边界层的标准方法。
- **意义**：摄动论让渐近分析从"看函数"升级为"解方程"，是它与偏微分方程、流体力学、天文学深度联姻的入口。

### 6.7 WKB 与转向点：量子力学即渐近分析

- **WKB 近似**：Wentzel-Kramers-Brillouin 方法把 Schrödinger 方程的振荡/指数解写为 $\exp\bigl(\tfrac{\mathrm i}{\varepsilon}S(x)\bigr)$ 的展开，相位 $S$ 满足 Hamilton-Jacobi 方程——这是量子力学半经典分析与几何光学的交汇。
- **Liouville-Green 先声**：WKB 的源头常追溯到 Liouville 与 Green 用于二阶线性方程的常系数型近似；它把"在缓变系数下的振荡"压缩进一个相位。
- **衔接公式**：穿过转向点时指数衰减解与振荡解之间的过渡由衔接公式处理——"近乎匹配"把不同尺度上的解缝合起来，是现代 WKB 卓有成效的实践。
- **意义**：WKB 是渐近分析用于量子物理的最深烙印，也是匹配渐近在"复特征"情形的延伸（第 50 级动力系统、第 135 级生物数学也会遇到它）。

### 6.8 渐近分析法：从"不精确"到"坚不可摧"

- **误差控制与整体渐近**：现代渐近分析对"哪一层给出主项、哪里必须逐项控制误差"有极其严格的规定——它既要知道首项，也要知道可证的多项级误差界。
- **与数值、概率、PDE 交织**：渐近展开在**大偏差理论**（第 102 级）、**随机矩阵**（第 146 级）与**偏微分方程的短时/长时极限**中反复登场，成为这些领域给出直觉与可算化结果的通用语言。
- **意义**：经过庞加莱之后的百年打磨，渐近分析完成了"从可疑级数到严格方法"的跃迁——它始终是"近似中求真、发散中取序"的典范学科。

### 6.9 历史年表

| 时间 | 人物／文献 | 事件与贡献 |
|:---|:---|:---|
| 1730 年前后 | 斯特林、欧拉 | Stirling 公式与阶乘的渐近 |
| 1740 年代 | 欧拉、麦克劳林 | Euler-Maclaurin 求和公式 |
| 18—19 世纪 | 欧拉、拉普拉斯 | 形式展开在天文学中的成功应用 |
| 1880 年代 | Padé | Padé 逼近：有理函数近似幂级数 |
| 1886 年 | 庞加莱 | 渐近展开的严格定义与 $O$/$o$ 规范化 |
| 1909 年前后 | Debye | 鞍点法（最速下降）用于光学积分 |
| 1904 年 | Prandtl | 边界层理论：奇异摄动的确立 |
| 20 世纪前期 | Watson、Skyrme 等 | Watson 引理与拉普拉斯型积分严格估计 |
| 20 世纪中 | Wentzel、Kramers、Brillouin | WKB 近似与衔接公式 |
| 20 世纪后 | Erdélyi、de Bruijn、Olver | 渐近方法系统化教科书与误差理论 |

### 6.10 小结

> 渐近分析的成长史，是一部"发散级数被驯服为严格近似"的历史。欧拉与斯特林在数值与天文问题中发现"发散却有用"的级数，庞加莱在 1886 年用 $o(x^{-N})$ 的精确定义把它制度化，使其能在明确的首项与可证的误差下被使用；Laplace 方法、驻相法与最速下降法把"主项来自一个点"的直觉系统化到实轴与复平面，Euler-Maclaurin 求和与 Padé 逼近则提供了求和与改造发散级数的标准工具。进入 20 世纪，奇异摄动、边界层与 WKB 把渐近分析从"看函数"推进到"解方程"，使其成为量子力学、流体力学、大偏差与随机矩阵理论的共同底座。这门学科始终保有一种独特的气质——在"近似"与"严谨"之间，它选择用严格的方法讲述关于极端的真相，这正是渐近分析历久弥新的原因。
...
> 本节以 James Stewart *Calculus* 的教学编排为蓝本：用「四步解题流程」贯穿概念检查、分步骤工作示例、小规模应用项目与梯度化自测习题，帮助你在动手计算中建立对渐近分析核心概念的肌肉记忆。小节间相互独立，可按需跳读。

### 7.1 问题求解四步流程（Problem-Solving Strategy）

> Stewart 力推的策略是把每个问题拆成四步：**理解 → 计划 → 执行 → 回看**。这里把每一环落到渐近分析语境的追问上。

| 步骤 | 行动 | 渐近分析中的具体追问 |
|:---|:---|:---|
| ① 理解 | 复述问题、圈出已知与目标 | 被研究的量是积分、求和还是数列？参数 $\lambda$ 或 $n$ 趋向什么极限？要估计阶数、首项系数还是完整展开？ |
| ② 计划 | 选择可行工具、预估方向 | 用 Laplace 方法找实指数最小值点，还是用驻相法找振荡相位驻点？要不要先做 Watson 引理化简？先给阶数 $O$ 的粗估再细算系数？ |
| ③ 执行 | 按计划逐步推导、保留过程 | 展开后逐项保留主项，注意符号、阶乘 $n!$、Gamma 值、分部积分的边界项别写漏。 |
| ④ 回看 | 核对结果、检验合理性 | 把 $\lambda\to\infty$（或 $n\to\infty$）代回看主项是否真的主导？误差项是否比主项低一阶？能否换一种方法复核首项系数？ |

> 💡 **策略演示**：估计 $\int_0^\infty e^{-\lambda x^2}dx$ 的主项。
> ① 理解：实指数最小值点在 $x=0$，属「Laplace 方法」题型。② 计划：把积分化成 Gaussian 借助换元。③ 执行：$\int_0^\infty e^{-\lambda x^2}dx=\frac12\sqrt{\frac{\pi}{\lambda}}$。④ 回看：结果随 $\lambda\to\infty$ 按 $\lambda^{-1/2}$ 衰减，指数扩散主导直观合理。
...
1. 一个发散的渐近级数完全不能用于数值近似。（　）
2. 若 $f(\lambda)\sim g(\lambda)$（$\lambda\to\infty$），则 $\lim_{\lambda\to\infty} f(\lambda)/g(\lambda)=1$。（　）
3. Laplace 方法中，主贡献总是来自实指数函数取极大值（约等于最小值点起主导）的邻域。（　）
4. Stirling 公式的主项是 $n!\sim\sqrt{2\pi n}(n/e)^n$。（　）
5. 驻相法中，积分主贡献来自相位函数 $f(x)$ 的驻点 $f'(x_0)=0$ 附近。（　）
6. 若 $a_n=o(b_n)$，则 $a_n+b_n\sim b_n$。（　）

**选择**：

7. 对于 $\int_0^1 e^{\lambda x}dx$（$\lambda\to\infty$），主贡献来自（　）。
   - A. 区间左端点　B. 区间右端点　C. 内点　D. 无主项
8. $\Gamma(n+1)=n!$ 的渐近主项是（　）。
   - A. $(n/e)^n$　B. $\sqrt{n}(n/e)^n$　C. $\sqrt{2\pi n}(n/e)^n$　D. $n^n$
9. Laplace 方法与驻相法的本质区别是（　）。
   - A. 指数实部 vs 虚部主导　B. 端点 vs 内点　C. 实变 vs 复变　D. 无区别

### 7.3 分步骤工作示例（Worked Examples）

> 每道示例严格套用四步流程，并在「回看」中给出一条易错点或检验法。

**示例 S1：阶的估计——用 $O$ 与 $o$ 比较增长速度**

> **问题**：比较 $n!$、$n^n$、$e^n$、$n\ln n$ 在 $n\to\infty$ 时的增长速度。

- ① 理解：要对四类典型量排定义增长阶的全序。

- ② 计划：写出对数增长阶，再用 $O$ / $o$ 记号比较。

- ③ 执行：

$$
\ln(n!)\sim n\ln n,\qquad \ln(n^n)=n\ln n,\qquad \ln(e^n)=n,\qquad \ln(n\ln n)\sim \ln n .
$$

  由对数阶排序，$n^n$ 与 $n!$ 远高于任何指数函数，而 $n\ln n$ 只以 $\ln n$ 增长；再精细地 $n^n\gg n!$。

- ④ 回看：确证 $n\ln n=o(e^n)$、$e^n=o(n!)$、$n!=o(n^n)$。**易错点**：直接比较底数而忽略指数增长速度，会误判 $n\ln n$ 与 $n^n$ 的差距。

**示例 S2：用 Laplace 方法 + Watson 引理推导 Stirling 公式首项**

> **问题**：从积分表示推导 $n!\sim\sqrt{2\pi n}(n/e)^n$。

- ① 理解：$\Gamma(n+1)=n!$ 有积分表示，其指数在内部取极大。

- ② 计划：改写为 Laplace 形式，在驻点 $y=1$ 附近做高斯展开，再套 Watson 引理补高阶。

- ③ 执行：

$$
n!=\Gamma(n+1)\sim e^{-n}n^{n+1}\sqrt{\frac{2\pi}{n}}=\sqrt{2\pi n}\left(\frac{n}{e}\right)^n .
$$

  这里用到 $g(y)=y-\ln y-1$ 在 $y=1$ 取最小值且 $g''(1)=1$。

- ④ 回看：$n=10$ 时 $10!\approx3.6288\times10^6$，近似 $\sqrt{20\pi}(10/e)^{10}\approx3.5987\times10^6$，相对误差约 $0.8\%$。**检验**：比值趋于 $1$，误差项具 $1+\frac1{12n}+\cdots$ 结构，与 Watson 引理一致。

**示例 S3：Laplace 方法直算边界主导积分**

> **问题**：求 $I(\lambda)=\int_0^1 e^{\lambda\sqrt{x}}dx$ 的渐近主项（$\lambda\to\infty$）。

- ① 理解：指数 $\lambda\sqrt x$ 在右端点 $x=1$ 最大，主贡献来自边界。

- ② 计划：在最大值点附近换元，把指数展开成 $e^{\lambda(1-\tfrac12 y+\cdots)}$ 做边界型展开。

- ③ 执行：令 $y=1-x$，$\sqrt x=1-\tfrac12y+O(y^2)$，故 $I(\lambda)\sim e^{\lambda}\int_0^\infty e^{-\lambda y/2}dy=\dfrac{2}{\lambda}e^{\lambda}$。

- ④ 回看：主项为 $e^\lambda/\lambda$ 级。**易错点**：误以为内点主导——这里最大值在边界，须做边界型展开而非完整高斯展开，否则会白白损失端点系数。

**示例 S4：驻相法计算振荡 Fourier 型积分**

> **问题**：求 $J(\lambda)=\int_0^1 e^{i\lambda x^2}dx$ 在 $\lambda\to+\infty$ 的主渐近。

- ① 理解：相位 $\phi(x)=x^2$ 的驻点 $x=0$ 恰落在积分端点。

- ② 计划：驻相法——在驻点小邻域用 $\phi(x)\approx x^2$ 近似，余项化为 Gauss 型；驻点在端点时主项系数折半。

- ③ 执行：在内点口径下 $\int e^{i\lambda x^2}dx=\sqrt{\pi/(2\lambda)}\,e^{i\pi/4}$；端点时幅度为内点之半。于是 $J(\lambda)\sim\dfrac12\sqrt{\dfrac{\pi}{2\lambda}}\,e^{i\pi/4}$。

- ④ 回看：幅值按 $\lambda^{-1/2}$ 衰减并带 $e^{i\pi/4}$ 相移。**检验**：若驻点为内点系数应为 $\sqrt{2\pi/(\lambda|\phi''|)}\,e^{\pm i\pi/4}$，端点情形多出因子 $\tfrac12$，两口径不可混用。

### 7.4 应用聚焦（Application & Exploration Projects）

> 小型开放任务：把本节抽象结论落到可操作的计算或仿真，任选其一完成。

- **聚焦 P1（数值仿真）**：对 $I(\lambda)=\int_0^1 e^{\lambda\sqrt x}dx$ 在 $\lambda=10,20,\dots,100$ 数值积分，作图并检验 $\lambda\, I(\lambda)\,e^{-\lambda}\to2$——观察误差项随 $\lambda$ 的衰减速度。
- **聚焦 P2（跨章节联系）**：用 Laplace 方法推导 Bessel 函数 $I_0(\lambda)=\frac1{2\pi}\int_{-\pi}^{\pi}e^{\lambda\cos\theta}d\theta$ 的主项 $\sim e^\lambda/\sqrt{2\pi\lambda}$，并与特殊函数章节的结论交叉引用。
- **聚焦 P3（边界情形）**：构造一个函数使其 Laplace 方法的最大值点与驻相法的驻点同落端点，比较两种口径给出的阶数差异，判定哪一方占主导。
...
- **自测 1**：用一句话解释「为何发散的渐近级数仍可用于截断近似」。
- **自测 2**：区分 $O$、$o$、$\sim$ 三种符号，并各举一个 $f=O(n^2)$ 但不 $\sim(n^2)$ 的反例说明。

**$\bullet$ 技能题**

- **自测 3**：写出 Stirling 公式并估计 $\ln(100!)$ 的前三位有效数字。
- **自测 4**：用 Laplace 方法求 $\int_0^\infty e^{-\lambda(x^2+x)}dx$ 的主项。
- **自测 5**：用驻相法求 $\int_{-\infty}^\infty e^{i\lambda\cos x}dx$ 的渐近主项（$x_1=0$、$x_2=\pi$ 为内点驻点）。

**$\blacklozenge$ 综合题**

- **自测 6**：对同一积分先用 Laplace 方法定主阶、再用 Watson 引理补误差项，写出完整四步并回看误差相对主项低一阶。
- **自测 7**：估计 $n!$ 相对 $\sqrt{2\pi n}(n/e)^n$ 的相对误差，并说明取到下一项 $1/(12n)$ 的收益。

### 7.6 自测答案与简要解析

> **概念判断**：
> 1. **错误**（渐近级数虽不收敛，但截断后误差先降后升，取最优截断可获极佳的近似）。2. **正确**（$f\sim g$ 定义即比值趋于 $1$）。3. **正确**（Laplace 主贡献来自实指数取极大值的最小值点邻域）。4. **正确**（正是 Stirling 主项）。5. **正确**（驻相主项来自相位驻点邻域）。6. **正确**（$a_n=o(b_n)$ 使和被 $b_n$ 主导）。
>
> **选择**：7. B　8. C　9. A。

> **自测**：
> - 自测 1：Poincaré 渐近级数各阶截断余项先减后增，选最优截断可远优于精确展开。
> - 自测 2：$O$ 有界、$o$ 严格更低、$\sim$ 比值趋于 $1$；如 $f=n$ 满足 $f=O(n^2)$ 却 $f=o(n^2)$，故不 $\sim n^2$，即 $O$ 只给上界、不一定达到同阶。
> - 自测 3：$n!\sim\sqrt{2\pi n}(n/e)^n$；由 $\ln n!\approx n\ln n-n+\tfrac12\ln(2\pi n)$ 得 $\ln(100!)\approx363.73$。
> - 自测 4：最小值在 $x=0$，扩为 $x^2+x\approx x$，$I(\lambda)\sim\int_0^\infty e^{-\lambda x}dx=\tfrac1\lambda$。
> - 自测 5：两内点驻点 $\phi''(0)=-1,\phi''(\pi)=1$，各贡献 $\sqrt{2\pi/\lambda}e^{\mp i\pi/4}$，总主项 $=\sqrt{2\pi/\lambda}(e^{-i\pi/4}+e^{i\pi/4})=2\sqrt{\pi/\lambda}$。
> - 自测 6：先定最小点取主阶（Laplace），再用 Watson 引理把邻域幂律展开逐项求出，回看下一项确比主阶低一档幂次。
> - 自测 7：相对误差约 $1/(12n)$；补上 $1+\tfrac1{12n}$ 后误差降至 $O(1/n^2)$，即取下一项收益显著。
