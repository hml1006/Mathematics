...
> **一句话大白话**：一族一起收敛的解析函数（局部一致有界），总藏着处处收敛的子列——"挤成一团不炸开"的正则函数族，准能从中挑出一串最终稳定下来的，是无穷维里的"追紧"保障。
>
> **小例子**：$\{f_n\}$ 全在圆盘里有界且解析，Montel 定理保证存在子列 $\{f_{n_k}\}$ 在圆盘上一致收敛；像一群被绳子拴住互不失控的曲线，总能抽出一根"定型"的。

## 一、定理介绍

> **前置依赖**：Cauchy 积分公式与 Cauchy 估计、Arzelà-Ascoli 定理、正规族的定义、Weierstrass 一致收敛定理、对角线方法。
