...
用最小二乘法拟合一条直线 $y = c_0 + c_1 x$ 到数据点 $(0, 1)$、$(1, 2)$、$(2, 2)$、$(3, 3)$。写出正规方程并求出系数 $c_0$、$c_1$。

**提示**：设 $A = \begin{pmatrix} 1 & 0 \\ 1 & 1 \\ 1 & 2 \\ 1 & 3 \end{pmatrix}$，$\boldsymbol{b} = (1, 2, 2, 3)^\top$。正规方程 $A^\top A \boldsymbol{c} = A^\top \boldsymbol{b}$ 为 $\begin{pmatrix} 4 & 6 \\ 6 & 14 \end{pmatrix}\begin{pmatrix} c_0 \\ c_1 \end{pmatrix} = \begin{pmatrix} 8 \\ 15 \end{pmatrix}$，解得 $c_1 = 3/5$，$c_0 = 11/5$。

### 习题 8

判断线性方程组 $A\boldsymbol{x} = \boldsymbol{b}$，$A = \begin{pmatrix} 4 & 1 & 1 \\ 1 & 4 & 1 \\ 1 & 1 & 4 \end{pmatrix}$ 用 Jacobi 与 Gauss-Seidel 迭代是否收敛，并说明理由。若已知 $A$ 的特征值为 $\lambda_1 = 5$，$\lambda_2 = \lambda_3 = 3$，请据此计算 Jacobi 迭代矩阵的谱半径。

**提示**：$A$ 严格对角占优（每行 $4 > 1 + 1$），故两种迭代均收敛。Jacobi 迭代矩阵 $D^{-1}(L+U)$ 的特征值与原矩阵可通过相似关系联系，其谱半径满足 $\rho(B_J) < 1$；对本题可求得 $\rho(B_J) = 1/2$。

### 习题 9

设矩阵 $A = \begin{pmatrix} 1 & 2 \\ 2 & 4.0001 \end{pmatrix}$。计算其条件数 $\kappa(A) = \sigma_{\max}/\sigma_{\min}$，并说明若右端项存在 $10^{-4}$ 量级的相对扰动，解的相对误差最多会被放大多少倍。

**提示**：$\sigma_{\max} \approx 5$，$\sigma_{\min} \approx 10^{-4}$，故 $\kappa(A) \approx 5 \times 10^4$。解的相对误差满足 $\frac{\|\delta\boldsymbol{x}\|}{\|\boldsymbol{x}\|} \le \kappa(A)\frac{\|\delta\boldsymbol{b}\|}{\|\boldsymbol{b}\|}$，即约放大 $5 \times 10^4$ 倍，属于病态问题。
