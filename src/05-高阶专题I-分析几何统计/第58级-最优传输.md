...
最优传输 (optimal transport) 研究"如何用最小的代价把一堆质量从一个分布搬运到另一个分布"。这一问题由数学家最早以搬土、运沙的形式提出，却在二十世纪被线性规划、概率论与偏微分方程显著重塑，最终成为一幅覆盖分析、几何、概率与机器学习的宏大图景。

### 7.1 Monge 的搬土问题（1781）

最优传输的起点是加斯帕尔·蒙日 (Gaspard Monge) 于 1781 年提出的经典问题：把一堆"土"搬到一条沟时，如果某点 $\mathbf{x}$ 处搬出的质量为 $\mu$，对应落到 $\mathbf{y}$，希望最小化总体搬动费用
$$\int c(\mathbf{x},T(\mathbf{x}))\,d\mu(\mathbf{x})$$
（经典情形取费用为欧氏距离）。Monge 问题要求一个确定的（确定性、一对一或"推送"式）传输映射 $T$，把这堆土确切地搬到指定位置。它本质上是约束很强的变分问题，因"映射不受脉冲分布白纸"而一度难以数学处理，在近两世纪里进展缓慢。

### 7.2 Kantorovich 的线性规划革新（1942）

直到 1942 年，坎托罗维奇 (Leonid Kantorovich) 在苏联研究战时资源最优配置时，把问题重新表述为一个关于联合分布（耦合测度）$\gamma$ 的线性规划：允许把质量"拆散"再重新分配，只需最小化
$$\int c(\mathbf{x},\mathbf{y})\,d\gamma(\mathbf{x},\mathbf{y}),$$
约束条件是 $\gamma$ 的边缘分布恰为 $\mu$ 与 $\nu$。这一"松弛"极大简化了问题，并凭借线性规划的对偶定理导出 Kantorovich 对偶性（最大下确界可写为势函数差之感），从而为最优传输提供了可计算与理论统一的框架。Kantorovich 的这一贡献对其后最优配给与经济学研究价值巨大，他也因此获得 1975 年诺贝尔经济学奖。

### 7.3 Wasserstein 距离、Brenier 定理与 Monge-Ampère 方程

在 Kantorovich 框架下，如果把费用取为距离的 $p$ 次幂，得到的最小代价恰成为度量空间上概率测度间的距离，即 Wasserstein 距离 $W_p$（该术语由 Vaserstein 提出，故又译 Vaserstein 距离）。Brenier 于 1980 年代证明：当费用为平方欧氏距离时，最优传输映射几乎处处存在且可写成凸函数的梯度（Brenier 定理），在原测度绝对连续且排除共同原子的情形下把松弛的耦合问题还原回（唯一）传输映射。由此，最优传输映射（若存在光滑）满足 Monge-Ampère 方程（$\det(D^2\varphi)=$ 密度之比），与偏微分方程理论深度联结。Benamou 与 Brenier（2000）发现可把 Wasserstein 距离视为动力系统上的最小作用量问题（密度 + 速度场），得到其流体力学表示。Kantorovich-Rubinstein 定理则把 $W_1$ 与有界 Lipschitz 函数的对偶刻画相联系。

### 7.4 熵正则化、离散化与现代复兴

近年来最优传输因两个原因而大规模复兴。一是对偶与熵正则化：通过给耦合分布加上熵惩罚并使用 Sinkhorn 迭代（1970 年代提出的矩阵重标定算法），把大规模离散最优传输转化为可高效并行计算的线性问题，成为机器学习、图像配准与数据同化的重要工具。二是与几何的深度融合：Wasserstein 距离在概率测度空间上诱导测地线，其凸性质（如凸性沿常光线）在梯度流中给出 Fokker-Planck / 热方程的解释。一般认为，最优传输已发展为连接 Monge 古典几何、Kantorovich 线性规划、Monge-Ampère 分析与现代计算的枢纽学科。
