...
> **一句话大白话**：遍历系统的"时间平均"几乎处处等于"空间平均"——沿着轨道取一长串观测再除以时间，结果（对几乎所有出发点）就等于在整个空间上按不变测度算的平均。
>
> **小例子**：对不可约的 Circle 旋转/移位，几乎所有点 $\mathrm{x}$ 都满足 $\lim_{n}\dfrac1n\sum_{k\lt n}f(T^k\mathrm{x})=\int f\,d\mu$；即"Irving 每个点的长期频度=全局比例"。
