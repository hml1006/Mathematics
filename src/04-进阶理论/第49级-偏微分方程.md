...
### 7.1 波动方程与抛物方程：物理驱动的起源

偏微分方程（PDE）的萌芽源于物理问题。**波动方程**由**达朗贝尔（Jean le Rond d'Alembert）**在 1746 年研究振动弦时提出，**欧拉**与**丹尼尔·伯努利**随后贡献了解法与展开；**傅里叶（Joseph Fourier, 1768–1830）**在 1807–1822 年的《热的解析理论》中研究**热方程**并引入**Fourier 级数与 Fourier 变换**，虽引发收敛性的争论（Dirichlet 等），却奠定了分析学的新工具。拉普拉斯方程与调和函数的研究（来自拉普拉斯、泊松与格林的位势理论）则与引力、静电等多年物理问题相连。本章的波动方程、热方程、Laplace/Poisson 方程、Fourier 变换与分离变量法，正是这段物理—数学交织的起源。

### 7.2 椭圆方程、调和函数与最大原理

**拉普拉斯方程**与**调和函数**是椭圆型方程的典范。**拉普拉斯（Pierre-Simon Laplace, 1749–1827）**给出方程与算子 $\Delta$，**泊松（Simeon Denis Poisson）**则加入非齐次项；**格林（George Green, 1793–1841）**在 1828 年以私人小册子引入格林函数与格林公式（Green 函数解含边界问题），**刘维尔（Liouville）**等研究调和函数的有界性，**Harnack 不等式**（Harnack, 1887）与**最大值原理**成为椭圆型方程的核心工具。对二阶椭圆型方程，**Schauder 估计**、**Lax–Milgram 定理**（1962）与变分法的抽象 Hilbert 空间框架（存在唯一性）在二十世纪成形。本章的 Laplace 方程、最大值原理、Green 函数方法、Harnack 不等式、Lax–Milgram 与椭圆正则性，正对应这一谱系。

### 7.3 一阶方程与存在唯一性：从特征线到柯西–柯瓦列夫

**一阶偏微分方程**用**特征线法**（characteristic curves）求解的思想源自**柯西（Augustin-Louis Cauchy）**与**拉格朗日**，并由**雅可比**等系统化。**存在唯一性**的经典表述为**柯西–柯瓦列夫（Cauchy–Kowalevski）定理**（1875）：解析初值问题的解析解局部存在唯一。**d'Alembert 公式**给出了一维波动方程的显式解（行波分解）。本章的一阶方程与特征线法、d'Alembert 公式、存在唯一性定理、Sobolev 空间与弱解，正是从经典到现代的过渡：**Sobolev 空间**（Sobolev, 1936–38）把微分方程的解空间扩容到分布意义，**弱解**与**能量估计**成为非线性与守恒律分析的利器。

### 7.4 弱解、Sobolev 空间与近代偏微分方程

现代 PDE 以弱解与变分法为核心。**Sobolev（谢尔盖·索伯列夫）**在 1930 年代引入含广义导数的 **Sobolev 空间**，使 Lax–Milgram、嵌入定理与存在唯一性可用于退化与低正则性问题；**Sobolev 嵌入定理**、**Lions–Lax–Milgram 引理**、**Duhamel 原理**与**热半群**把线性与准线性方程纳入统一框架。**Lions（雅克–路易·利翁，Jacques-Louis Lions）、Brezis**等人在 1950–70 年代发展单调算子与非线性分析。本章的 Sobolev 空间、弱解、Sobolev 嵌入、热半群、椭圆正则性与 Duhamel 原理，正是这段"从经典求解到现代弱分析"历程的体现。

### 7.5 双曲方程与整体理论的影响

**双曲型方程**（如波动方程）以其有限传播速度与因果关系显著区别于椭圆、抛物型。**柯西问题**（Cauchy problem）的可解性自柯西、柯瓦列夫以来一直是分析的核心课题；能量估计与弱解的建立（Friedrichs、Lax、Kato 等）推动了**非线性双曲理论**与守恒律。PDE 理论深刻影响流体力学（Navier–Stokes）、电磁学（Maxwell）、量子力学（Schrödinger）与几何分析（Ricci 流，Perelman 引入以证庞加莱猜想），成为现代科学计算与数学物理的核心引擎。本章虽是基础 PDE 教程，但足以为读者打开通往这些现代前沿的门径。
