...
> **一句话大白话**：想知道某个区域里每个点的"电位/温度"由电荷/热源怎么贡献，只需把每个点源的"扩散影响"（Green 函数）叠加起来——跟各处放个点然后加在一起一个道理。
>
> **小例子**：全平面 Poisson 方程 $-\Delta u=f$ 的 Green 函数是 $\Gamma(x)=-\frac{1}{2\pi}\log|x|$，于是 $u(x)=\int \Gamma(x-y)f(y)\,dy$；带边界条件时再修一个调和"镜像项"即可。
