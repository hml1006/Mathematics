...
统计学习理论研究"从观测数据中学习规则"的数学基础，其源头可追溯到大数定律与函数族的**一致收敛**思想。1960 年代 **V. N. Vapnik 与 A. Ya. Chervonenkis** 在研究模式识别与回归时，系统地提出了经验风险最小化（ERM）原则，并建立用于刻画函数类复杂度、从而保证"经验风险逼近真实风险"一致成立的 **VC 理论**。Vapnik–Chervonenkis 维数（VC 维）衡量一个假设类能打散样本集的程度，是理解泛化能力的关键量，这一框架后来由 Vapnik 多次系统整理（如 1998 年《统计学习理论的本质》）。

### 7.2 泛化界与偏差方差权衡

VC 理论的核心产物是一族**泛化界**（generalization bound）：它们用训练样本量、假设空间复杂度（VC 维或 Rademacher 复杂度）与置信度，给"真实误差超出经验误差的程度"穿上上界。这一"以复杂度换取界"的思路正是**偏差-方差权衡**的严格表达——模型过于简单（偏差大）难以拟合，过于复杂（方差大）难以泛化，最优处正是两者折中。Rademacher 复杂度（Bartlett、Mendelson）等现代度量进一步细化了对假设类容量的刻画。

### 7.3 正则化、核方法与结构风险最小化

为在有限样本下抑制过拟合，Vapnik 提出**结构风险最小化**（SRM）：在控制经验误差的同时，以推荐的复杂度次序约束假设对，构成"复杂度惩罚"的思想雏形，并与后来的**正则化**（regularization）技术呼应。1990 年代 **Boser、Guyon、Vapnik** 把核技巧与最大间隔思想结合形成**支持向量机（SVM）**，把线性方法推广到高维特征空间，成为统计学习理论从理论走向实用的标志。这些概念共同回答了"数据有限时如何保证学到的模型能泛化"这一根本问题。

### 7.4 现代学习理论的发展

此后 **PAC（Probably Approximately Correct）学习**框架由 Valiant（1984）提出，把"以大概率近乎正确"形式化；随后产生样本复杂度、对抗鲁棒性、深度学习泛化分析等大量研究。统计学与优化理论（风险与经验风险的差、贝叶斯误差、模型选择）与高维统计共同丰富了现代学习理论。本文件关于 VC 维、泛化界、经验风险最小化与偏差方差权衡的内容，正是这门学科自 1960 年代末创立以来不断发展的主线。
